...
class ApiUrl(splunk.rest.BaseRestHandler):
    ...
    def handle_GET(self):
        try:
            conf = splunk.bundle.getConf('application', self.sessionKey,namespace='Threatbook-TIP-v10',owner='nobody')['config']
    ...
    def handle_POST(self):
        ...
        # 保存配置
        try:
            bun = splunk.bundle.getConf('application', self.sessionKey,namespace='Threatbook-TIP-v10',owner='nobody')
            bun.beginBatch()
            conf = bun['config']
