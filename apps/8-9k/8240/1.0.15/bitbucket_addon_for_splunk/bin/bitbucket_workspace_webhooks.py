...
class BITBUCKET_WORKSPACE_WEBHOOKS(smi.Script):
    def __init__(self):
        super(BITBUCKET_WORKSPACE_WEBHOOKS, self).__init__()
    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_workspace_webhooks')
        scheme.description = 'Workspace Webhooks'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        urls = []
        for i in slug_list:
            url2 = f"https://api.bitbucket.org/2.0/workspaces/{i}/hooks?pagelen=50"
            urls.append(url2)
            ...
            while url2:
                ...
                for ids in data2["values"]:
                    uid = ids["uuid"]
                    url3 = f"https://api.bitbucket.org/2.0/workspaces/{i}/hooks/{uid}?pagelen=50"
                    ...
                    checkpoint = checkpointer.KVStoreCheckpointer(
                        "bitbucket_workspaces_webhooks_checkpoint",
                        session_key,
                        "bitbucket_addon_for_splunk"
                    )
                    unique_key = hashlib.md5(json.dumps(data3, sort_keys=True).encode()).hexdigest()
                    ...
                    if state is None:
                        event = smi.Event(
                            ...
                            sourcetype='bitbucket:workspace_webhooks',
                        )
...
if __name__ == '__main__':
    exit_code = BITBUCKET_WORKSPACE_WEBHOOKS().run(sys.argv)
    sys.exit(exit_code)
