...
class BITBUCKET_DEPLOYMENTS(smi.Script):
    def __init__(self):
        super(BITBUCKET_DEPLOYMENTS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_deployments')
        scheme.description = 'Deployments'
        ...
        for i in slug_list:
            ...
            for j in range(0,val2):
                ...
                for k in data2["values"]:
                    repo = str(k["slug"])
                    url3 = f"https://api.bitbucket.org/2.0/repositories/{i}/{repo}/deployments?pagelen=50"
                    urls.append(url3)
                    ...
                    while url3:
                        ...
                        for ids in d3["values"]:
                            checkpoint = checkpointer.KVStoreCheckpointer(
                                "bitbucket_deployment_checkpoint",
                                session_key,
                                "bitbucket_addon_for_splunk"
                            )

                    
                            state = checkpoint.get(str(ids['uuid'].strip('{}'))+str(ids['created_on'])+str(ids["deployable"]["commit"]["hash"])+str(ids["environment"]["uuid"].strip('{}'))+str(ids["step"]["uuid"].strip('{}')))
                            if state is None:
                                event = smi.Event(
                                    data=json.dumps(ids),
                                    index=input_item.get("index"),
                                    sourcetype='bitbucket:deployments',
                                )
                                ew.write_event(event)
                                count = count + 1
                                checkpoint.update(str(ids['uuid'].strip('{}'))+str(ids['created_on'])+str(ids["deployable"]["commit"]["hash"])+str(ids["environment"]["uuid"].strip('{}'))+str(ids["step"]["uuid"].strip('{}')), "Indexed")
                            #checkpoint.delete(str(ids['uuid'].strip('{}'))+str(ids['created_on'])+str(ids["deployable"]["commit"]["hash"])+str(ids["environment"]["uuid"].strip('{}'))+str(ids["step"]["uuid"].strip('{}')))
...
if __name__ == '__main__':
    exit_code = BITBUCKET_DEPLOYMENTS().run(sys.argv)
    sys.exit(exit_code)