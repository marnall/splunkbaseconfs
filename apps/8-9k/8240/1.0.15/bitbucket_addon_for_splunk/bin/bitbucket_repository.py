...
class BITBUCKET_REPOSITORY(smi.Script):
    def __init__(self):
        super(BITBUCKET_REPOSITORY, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_repository')
            ...
            while url2:
                ...
                for ids in data2["values"]:
                    checkpoint = checkpointer.KVStoreCheckpointer(
                        "bitbucket_repository__checkpoint",
                        session_key,
                        "bitbucket_addon_for_splunk"
                    )
            
                    state = checkpoint.get(str(ids["slug"]) + str(ids["updated_on"]))

                    if state is None:
                        event = smi.Event(
                            data=json.dumps(ids),
                            index=input_item.get("index"),
                            sourcetype='bitbucket:repository',
                        )
                        ew.write_event(event)
                        count = count + 1
                        checkpoint.update(str(ids["slug"]) + str(ids["updated_on"]), "Indexed")
                    #checkpoint.delete(str(ids["slug"]) + str(ids["updated_on"]))
...
if __name__ == '__main__':
    exit_code = BITBUCKET_REPOSITORY().run(sys.argv)
    sys.exit(exit_code)