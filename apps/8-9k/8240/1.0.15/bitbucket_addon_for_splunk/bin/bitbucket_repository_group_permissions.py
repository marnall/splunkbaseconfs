...
class BITBUCKET_REPOSITORY_GROUP_PERMISSIONS(smi.Script):
    def __init__(self):
        super(BITBUCKET_REPOSITORY_GROUP_PERMISSIONS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_repository_group_permissions')
        scheme.description = 'Repository Group Permissions'
        ...
        for i in slug_list:
            ...
            for j in range(0,val2):
                ...
                for k in data2["values"]:
                    repo = str(k["slug"])
                    url3 = f"https://api.bitbucket.org/2.0/repositories/{i}/{repo}/permissions-config/groups?pagelen=50"
                    urls.append(url3)
                    ...
                    while url3:
                        ...
                        for ids in data3["values"]:
                            checkpoint = checkpointer.KVStoreCheckpointer(
                                "bitbucket_repository_group_permissions_checkpoint",
                                session_key,
                                "bitbucket_addon_for_splunk"
                            )

                    
                            state = checkpoint.get(str(ids["group"]["full_slug"]) + str(ids["links"]["self"]["href"])+str(ids["permission"]))
                            ...
                            if state is None:
                                event = smi.Event(
                                    ...
                                    sourcetype='bitbucket:repository groups permissions',
                                )
                                ew.write_event(event)
                                count = count + 1
                                checkpoint.update(str(ids["group"]["full_slug"]) + str(ids["links"]["self"]["href"])+str(ids["permission"]), "Indexed")
                            #checkpoint.delete(str(ids["group"]["full_slug"]) + str(ids["links"]["self"]["href"])+str(ids["permission"]))
...
if __name__ == '__main__':
    exit_code = BITBUCKET_REPOSITORY_GROUP_PERMISSIONS().run(sys.argv)
    sys.exit(exit_code)