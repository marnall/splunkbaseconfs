...
class BITBUCKET_BRANCH(smi.Script):
    def __init__(self):
        super(BITBUCKET_BRANCH, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_branch')
        scheme.description = 'Branch'
        ...
        for i in slug_list:
            ...
            for j in range(0,val2):
                ...
                for k in data2["values"]:
                    repo = str(k["slug"])
                    url3 = f"https://api.bitbucket.org/2.0/repositories/{i}/{repo}/refs?pagelen=50"
                    urls.append(url3)
...
if __name__ == '__main__':
    exit_code = BITBUCKET_BRANCH().run(sys.argv)
    sys.exit(exit_code)