...
class BITBUCKET_COMMITS(smi.Script):
    def __init__(self):
        super(BITBUCKET_COMMITS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_commits')
        scheme.description = 'Commits'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        count = 0
        url4 =  "https://api.bitbucket.org/2.0/repositories/{workspaces}/{repositories}/commits?pagelen=50"
        ...
        for i in slug_list:
            ...
            for j in range(0,val2):
                ...
                for k in data2["values"]:
                    repo = str(k["slug"])
                    url3 = f"https://api.bitbucket.org/2.0/repositories/{i}/{repo}/commits?pagelen=50"
                    ...
                    while url3:
                        ...
                        for ids in d3["values"]:
                            ...
                            checkpoint = checkpointer.KVStoreCheckpointer(
                                "bitbucket_commit_checkpoint",
                                session_key,
                                "bitbucket_addon_for_splunk"
                            )

                    
                            state = checkpoint.get(str(ids["hash"]))
                            if state is None:
                                event = smi.Event(
                                    data=json.dumps(ids),
                                    index=input_item.get("index"),
                                    sourcetype='bitbucket:commits',
                                )
                                ew.write_event(event)
                                count = count + 1
                                checkpoint.update(str(ids["hash"]), "Indexed")
                            #checkpoint.delete(str(ids["hash"]))
        ...
        logsummary.log_summary(activation_key, meta, url4, status, count, source, host, version)
...
if __name__ == '__main__':
    exit_code = BITBUCKET_COMMITS().run(sys.argv)
    sys.exit(exit_code)