...
class BITBUCKET_WORKSPACE_USER_PERMISSIONS(smi.Script):
    def __init__(self):
        super(BITBUCKET_WORKSPACE_USER_PERMISSIONS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_workspace_user_permissions')
        scheme.description = 'Workspace User Permissions'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        for i in slug_list:
            ...
            url2 = f"https://api.bitbucket.org/2.0/workspaces/{i}/permissions?pagelen=50"
            urls.append(url2)
            ...
            while url2:
                ...
                for ids in data2["values"]:
                    checkpoint = checkpointer.KVStoreCheckpointer(
                        "bitbucket_workspace_users_permission_checkpoint",
                        session_key,
                    ...
                    state = checkpoint.get(str(ids["user"]["uuid"]) + str(ids["workspace"]["uuid"]) + str(ids["permission"]))

                    if state is None:
                        event = smi.Event(
                            data=json.dumps(ids),
                            index=input_item.get("index"),
                            sourcetype='bitbucket:workspace_user_permissions',
                        )
                        ew.write_event(event)
                        count = count + 1
                        checkpoint.update(str(ids["user"]["uuid"]) + str(ids["workspace"]["uuid"]) + str(ids["permission"]), "Indexed")
                    #checkpoint.delete(str(ids["user"]["uuid"]) + str(ids["workspace"]["uuid"]) + str(ids["permission"]))
...
if __name__ == '__main__':
    exit_code = BITBUCKET_WORKSPACE_USER_PERMISSIONS().run(sys.argv)
    sys.exit(exit_code)