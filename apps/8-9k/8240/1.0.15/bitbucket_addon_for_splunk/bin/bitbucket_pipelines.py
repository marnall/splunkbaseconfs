...
class BITBUCKET_PIPELINES(smi.Script):
    def __init__(self):
        super(BITBUCKET_PIPELINES, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_pipelines')
        scheme.description = 'Pipelines'
        ...
        for i in slug_list:
            ...
            for j in range(0,val2):
                ...
                for k in data2["values"]:
                    repo = str(k["slug"])
                    url3 = f"https://api.bitbucket.org/2.0/repositories/{i}/{repo}/pipelines?pagelen=50"
                    urls.append(url3)
                    ...
                    while url3:
                        ...
                        for ids in d3["values"]:
                            checkpoint = checkpointer.KVStoreCheckpointer(
                                "bitbucket_pipeline_checkpoint",
                                session_key,
                                "bitbucket_addon_for_splunk"
                            )

                    
                            state = checkpoint.get(str(ids['uuid'].strip('{}'))+str(ids['run_creation_date']))
                            if state is None:
                                event = smi.Event(
                                    data=json.dumps(ids),
                                    index=input_item.get("index"),
                                    sourcetype='bitbucket:pipelines',
                                )
                                ew.write_event(event)
                                count = count + 1
                                checkpoint.update(str(ids['uuid'].strip('{}'))+str(ids['run_creation_date']), "Indexed")
                            #checkpoint.delete(str(ids['uuid'].strip('{}'))+str(ids['run_creation_date']))
...
if __name__ == '__main__':
    exit_code = BITBUCKET_PIPELINES().run(sys.argv)
    sys.exit(exit_code)