...
class BITBUCKET_USER(smi.Script):
    def __init__(self):
        super(BITBUCKET_USER, self).__init__()
    def get_scheme(self):
        
        scheme = smi.Scheme('bitbucket_user')
        logger.debug("Iss",scheme)
        scheme.description = 'Bitbucket User'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        url1 = "https://api.bitbucket.org/2.0/user/"
        ...
        checkpoint = checkpointer.KVStoreCheckpointer(
            "bitbucket_users_checkpoint",
            session_key,
            "bitbucket_addon_for_splunk"
        )
        state = checkpoint.get(str(r1['uuid'].strip('{}')) + str(r1["created_on"]))
        ...
        if state is None:
            event = smi.Event(
                ...
                sourcetype='bitbucket:user',
            )
            ew.write_event(event)
            count = count + 1
            checkpoint.update((str(r1['uuid'].strip('{}')) + str(r1["created_on"])), "Indexed")
        # checkpoint.delete(str(r1['uuid'].strip('{}')) + str(r1["created_on"]))
...
if __name__ == '__main__':
    exit_code = BITBUCKET_USER().run(sys.argv)
    sys.exit(exit_code)
