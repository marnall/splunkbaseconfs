...
class BITBUCKET_REPOSITORY_USER_PERMISSIONS(smi.Script):
    def __init__(self):
        super(BITBUCKET_REPOSITORY_USER_PERMISSIONS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_repository_user_permissions')
        scheme.description = 'Repository User Permissions'
        ...
        for i in slug_list:
            ...
            for j in range(0,val2):
                ...
                for k in data2["values"]:
                    repo = str(k["slug"])
                    url3 = f"https://api.bitbucket.org/2.0/repositories/{i}/{repo}/permissions-config/users?pagelen=50"
                    urls.append(url3)
                    ...
                    while url3:
                        ...
                        for ids in data3["values"]:
                            checkpoint = checkpointer.KVStoreCheckpointer(
                                "bitbucket_repository_users_permission_checkpoint",
                                session_key,
                            ...
                            if state is None:
                                event = smi.Event(
                                    ...
                                    sourcetype='bitbucket:repository user permissions',
                                )
...
if __name__ == '__main__':
    exit_code = BITBUCKET_REPOSITORY_USER_PERMISSIONS().run(sys.argv)
    sys.exit(exit_code)