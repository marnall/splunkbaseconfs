...
class BITBUCKET_PROJECTS(smi.Script):
    def __init__(self):
        super(BITBUCKET_PROJECTS, self).__init__()
    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_projects')
        scheme.description = 'Bitbucket Projects'
            ...
            for j in range(0,val2):
                ...
                for ids in data2["values"]:
                    checkpoint = checkpointer.KVStoreCheckpointer(
                        "bitbucket_project_checkpoints",
                        session_key,
                        "bitbucket_addon_for_splunk"
                    )
            
                    state = checkpoint.get(str(ids["name"]) + str(ids["updated_on"]))

                    if state is None:
                        event = smi.Event(
                            data=json.dumps(ids),
                            index=input_item.get("index"),
                            sourcetype='bitbucket:projects',
                        )
                        ew.write_event(event)
                        count = count + 1
                        checkpoint.update(str(ids["name"]) + str(ids["updated_on"]), "Indexed")
...
if __name__ == '__main__':
    exit_code = BITBUCKET_PROJECTS().run(sys.argv)
    sys.exit(exit_code)