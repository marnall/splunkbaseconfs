...
class BITBUCKET_PULLREQUESTS(smi.Script):
    def __init__(self):
        super(BITBUCKET_PULLREQUESTS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_pullrequests')
        scheme.description = 'Pull Requests'
        ...
        for i in slug_list:
            ...
            for j in range(0,val2):
                ...
                for k in data2["values"]:
                    repo = str(k["slug"])
                    url3 = f"https://api.bitbucket.org/2.0/repositories/{i}/{repo}/pullrequests?state=OPEN&state=MERGED&state=DECLINED&state=SUPERSEDED"
                    urls.append(url3)
                    ...
                    while url3:
                        ...
                        for ids in data3["values"]:
                            checkpoint = checkpointer.KVStoreCheckpointer(
                                "bitbucket_pull_requests_checkpoint",
                                session_key,
                            ...
                            state = checkpoint.get(str(ids["updated_on"]) + str(ids["id"]))
                            ...
                            if state is None:
                                event = smi.Event(
                                    ...
                                    sourcetype='bitbucket:pullrequests',
                                )
                                ew.write_event(event)
                                count = count + 1
                                checkpoint.update((str(ids["updated_on"]) + str(ids["id"])), "Indexed")
                            # checkpoint.delete(str(ids["updated_on"]) + str(ids["id"]))
...
if __name__ == '__main__':
    exit_code = BITBUCKET_PULLREQUESTS().run(sys.argv)
    sys.exit(exit_code)