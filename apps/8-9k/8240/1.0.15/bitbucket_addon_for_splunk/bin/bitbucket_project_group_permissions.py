...
class BITBUCKET_PROJECT_GROUP_PERMISSIONS(smi.Script):
    def __init__(self):
        super(BITBUCKET_PROJECT_GROUP_PERMISSIONS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bitbucket_project_group_permissions')
        scheme.description = 'Project Group Permissions'
        ...
        for i in slug_list:
            ...
            while url2:
                ...
                for j in data2["values"]:
                    key = j['key']
                    url3 = f"https://api.bitbucket.org/2.0/workspaces/{i}/projects/{key}/permissions-config/groups?pagelen=50"
                    urls.append(url3)
                    ...
                    while url3:
                        ...
                        for ids in data3["values"]:
                            checkpoint = checkpointer.KVStoreCheckpointer(
                                "bitbucket_projects_groups_permissions_checkpoint",
                                session_key,
                            ...
                            if state is None:
                                event = smi.Event(
                                    ...
                                    sourcetype='bitbucket:projects group permissions',
                                )
...
if __name__ == '__main__':
    exit_code = BITBUCKET_PROJECT_GROUP_PERMISSIONS().run(sys.argv)
    sys.exit(exit_code)