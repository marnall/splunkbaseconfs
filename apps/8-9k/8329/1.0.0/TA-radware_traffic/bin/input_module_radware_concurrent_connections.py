...
def collect_events(helper, ew):
    ...
    if secondary_vrm_host:
        helper.log_info(f"[VRM-CONN] Starting with primary: {primary_vrm_host}, secondary: {secondary_vrm_host}")
    else:
        helper.log_info(f"[VRM-CONN] Starting with primary: {primary_vrm_host} (no secondary configured)")

    # ========= 2. Define VRMClient class (Conn-per-DP logic) =========
    class VRMClient:
        ...
        def switch_to_secondary(self):
            ...
            if self.secondary_host and not self.primary_failed:
                helper.log_warning(f"[VRM-CONN] Primary VRM ({self.primary_host}) failed, switching to secondary ({self.secondary_host})")
        ...
        def login(self):
            ...
            try:
                ...
                helper.log_info(f"[VRM-CONN] Login OK to {self.active_host}")
            except Exception as e:
                helper.log_error(f"[VRM-CONN] Login failed to {self.active_host}: {e}")
                ...
                if self.switch_to_secondary():
                    ...
                    self.logged_in = True
                    self.last_login_time = _time.time()
                    helper.log_info(f"[VRM-CONN] Login OK to {self.active_host} (secondary)")
                else:
                    raise
        ...
        def login_if_needed(self, force=False):
            ...
            if force or not self.logged_in or time_since_login > 600:
                helper.log_info(f"[VRM-CONN] Re-authenticating to {self.active_host} (time since last login: {time_since_login:.0f}s)")
                try:
                    self.login()
                except Exception as e:
                    helper.log_error(f"[VRM-CONN] Re-authentication failed: {e}")
                    raise
        ...
        def logout(self):
            ...
            try:
                ...
                r.raise_for_status()
                helper.log_info(f"[VRM-CONN] Logout OK from {self.active_host}")
            except Exception as e:
                helper.log_warning(f"[VRM-CONN] Logout failed from {self.active_host}: {e}")
        ...
        def get_mitigation_groups(self):
            ...
            try:
                ...
                if r.status_code == 401 or r.status_code == 403:
                    helper.log_warning("[VRM-CONN] Authentication failed, attempting re-login")
                ...
                if r.status_code == 200:
                    ...
                    helper.log_error(
                        f"[VRM-CONN] get_mitigation_groups: status={r.status_code}"
                    )
                    return []
            except Exception as e:
                helper.log_error(f"[VRM-CONN] Exception in get_mitigation_groups: {e}")
        ...
        def get_mg_devices(self):
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    helper.log_error(
                        f"[VRM-CONN] get_mg_devices: status={r.status_code}"
                    )
                    return []
            except Exception as e:
                helper.log_error(f"[VRM-CONN] Exception in get_mg_devices: {e}")
                return []

        def _build_conn_payload(self, lookback_minutes, selected_devices):
            """
            Build concurrent-connections payload per your example, per DP:
                {
                  "selectedDevices": [{deviceId: "<ip>", networkPolicies: [], ports: []}],
                  "filterString": "",
                  "originatedByDF": false,
                  "protocols": ["UDP","TCP"],
                  "timeInterval": { "from": <ms>, "to": null }
                }
            """
        ...
        def get_dp_concurrent_connections(self, dp_ip, lookback_minutes=5):
            """
            Get concurrent connections stats for a single DefensePro (by IP).
            Uses the API:
               /cyber-controller/api/vrm/v1/traffic/defensepro/concurrent-connections
            with per-DP selectedDevices payload.
            """
            self.login_if_needed()
            url = f"{self.base_url}/cyber-controller/api/vrm/v1/traffic/defensepro/concurrent-connections"
            selected = [{"deviceId": dp_ip, "networkPolicies": [], "ports": []}]
            payload = self._build_conn_payload(lookback_minutes, selected)
            ...
            try:
                ...
                if r.status_code == 401 or r.status_code == 403:
                    helper.log_warning("[VRM-CONN] Authentication failed on concurrent-conn call, attempting re-login")
                    self.login_if_needed(force=True)
                    r = self.session.post(
                        url, json=payload, verify=False, timeout=20
                    )
                ...
                helper.log_error(f"[VRM-CONN] Failed to get concurrent connections from {self.active_host}: {e}")
                # Try secondary on failure
                if not self.primary_failed and self.switch_to_secondary():
                    self.login_if_needed(force=True)
                    url = f"{self.base_url}/cyber-controller/api/vrm/v1/traffic/defensepro/concurrent-connections"
        ...
        def get_all_dp_conn_from_mg(self, lookback_minutes=5):
            """
            For every DefensePro returned by get_mitigation_groups(),
            call concurrent-connections API and return a list of:
                {
                  "dp_name": ...,
                  "ip_address": ...,
                  "mg_groups": [...],
                  "stats": <raw json from concurrent-connections API>
                }
            """
            ...
            for dev in devices:
                ...
                try:
                    stats = self.get_dp_concurrent_connections(ip, lookback_minutes)
                    results.append(
                        {
                    ...
                    helper.log_info(
                        f"[VRM-CONN] Collected concurrent connections for {ip} ({dev.get('dp_name')})"
                    )
                except Exception as e:
                    helper.log_error(f"[VRM-CONN] Failed concurrent-conn query for {ip}: {e}")
    ...
    try:
        ...
        helper.log_error(f"[VRM-CONN] Login failed: {e}")
        return

    all_stats = client.get_all_dp_conn_from_mg(lookback_minutes)
    ...
    for item in all_stats:
        ...
        # { "timeStamp": "1764060945000", "TCP": "991.0", "UDP": "31788.0" }
        for entry in stats.get("data", []):
            ...
            # Add connection values per protocol: TCP, UDP, etc.
            for key, val in row.items():
                ...
                try:
                    ...
                    helper.log_debug(f"[VRM-CONN] Non-numeric conn value for {key}: {val}")
                    event_body[f"{key}_CONN"] = val
                    continue

                event_body[f"{key}_CONN"] = v
                total_conn += v

            # store total concurrent connections across all protocols
            event_body["Total_CONN"] = total_conn
            ...
            event_obj = helper.new_event(
                ...
                sourcetype="radware:conc:conn",
                source=f"radware_vrm://{ip}/concurrent-connections",
                time=event_time,
            )
            ew.write_event(event_obj)
            event_count += 1

    helper.log_info(f"[VRM-CONN] Finished collection. Wrote {event_count} events.")

    # ========= 4. Logout =========
    try:
        client.logout()
    except Exception as e:
        helper.log_error(f"[VRM-CONN] Logout failed: {e}")
    finally:
