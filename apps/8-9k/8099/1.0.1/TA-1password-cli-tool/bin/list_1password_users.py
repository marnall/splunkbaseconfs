...
import input_module_list_1password_users as input_module

bin_dir = os.path.basename(__file__)

'''
'''
class ModInputlist_1password_users(base_mi.BaseModInput):

    def __init__(self):
        use_single_instance = False
        super(ModInputlist_1password_users, self).__init__("ta_1password_cli_tool", "list_1password_users", use_single_instance)
        self.global_checkbox_fields = None

    def get_scheme(self):
        """overloaded splunklib modularinput method"""
        scheme = super(ModInputlist_1password_users, self).get_scheme()
        scheme.title = ("List 1Password Users")
...
if __name__ == "__main__":
    exitcode = ModInputlist_1password_users().run(sys.argv)
    sys.exit(exitcode)
