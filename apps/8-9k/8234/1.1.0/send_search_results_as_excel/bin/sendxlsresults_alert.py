...
def sendemail(recipient, sender, subject, body, argvals, attachment):
    ...
    # robust boolean parsing (works for 0/1/true/false/strings)
    try:
        use_ssl = normalizeBoolean(argvals.get('use_ssl', False))
    except Exception:
        use_ssl = toBool(str(argvals.get('use_ssl', 'false')))
    try:
        use_tls = normalizeBoolean(argvals.get('use_tls', False))
    except Exception:
        use_tls = toBool(str(argvals.get('use_tls', 'false')))
    ...
    msg['From'] = sender
    msg['To'] = recipient
    # use provided body (not bodyText)
    ...
    try:
        # modes:
        #  - use_ssl=True        -> SMTPS (SSL immediately), default to 465 if port left at 25
        #  - use_ssl=False,use_tls=True  -> SMTP then STARTTLS
        #  - use_ssl=False,use_tls=False -> Plain SMTP (what you asked for)
        if use_ssl:
            if port == 25:
                port = 465
            print(f"use SSL (SMTPS) host={host} port={port}", file=sys.stderr)
            smtp = smtplib.SMTP_SSL(host, port)
        else:
            print(f"plain SMTP host={host} port={port}", file=sys.stderr)
            smtp = smtplib.SMTP(host, port)
            if use_tls:
                print("use STARTTLS", file=sys.stderr)
                smtp.ehlo()
                smtp.starttls()
                smtp.ehlo()

        # If you later enable AUTH on your SMTP, uncomment:
        # if username and password:
        #     smtp.login(username, password)

        print("go go gadget - send email!", file=sys.stderr)
        smtp.sendmail(sender, recipient.split(","), msg.as_string())
        logger.info('invocation_id=%s invocation_type="%s" msg="Email sent successfully" rcpt="%s" attachment="%s"' % (INVOCATION_ID, INVOCATION_TYPE, recipient, attachment))
        smtp.quit()
        return
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--execute" :
        ...
        logger.basicConfig(format='%(asctime)s %(levelname)s %(message)s',
                           filename=os.path.join(os.environ['SPLUNK_HOME'],'var','log','splunk','sendxlsresults.log'),
                           filemode='a+', level=logger.INFO)
            ...
            for row in csvreader:
                ...
                for item in row:
                    if not item.startswith('__'):
                        ...
                        sheet.write(row_num, column_num, cellvalue, style)
                        column_num += 1
                row_num += 1
