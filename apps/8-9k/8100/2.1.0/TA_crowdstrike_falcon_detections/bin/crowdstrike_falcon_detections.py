...
import alerts_combined as alerts
from falconpy import APIHarnessV2, _VERSION as falconpy_version

class ModInputcrowdstrike_falcon_detections(base_mi.BaseModInput):

    def __init__(self):
        use_single_instance = False
        super(ModInputcrowdstrike_falcon_detections, self).__init__("ta_crowdstrike_falcon_detections_technical_add_on", "crowdstrike_falcon_detections", use_single_instance)
        self.global_checkbox_fields = None

    def get_scheme(self):
        """overloaded splunklib modularinput method"""
        scheme = super(ModInputcrowdstrike_falcon_detections, self).get_scheme()
        scheme.title = ("CrowdStrike Falcon Detections")
        ...
        scheme.add_argument(smi.Argument("cloud", title="Select Cloud Environment",
                                         ...
                                         required_on_create=True,
                                         required_on_edit=False))
        scheme.add_argument(smi.Argument("product", title="Detection Sources",
                                         description="",
                                         required_on_create=True,
                                         required_on_edit=False))
    ...
    def get_app_name(self):
        return "TA_crowdstrike_falcon_detections"
    ...
    def validate_input(helper, definition):
        ...
        if num_products > 1:
            if 'all' in products:
                raise ValueError(f"Cannot select specific Alert Sources if All has been selected")
    ...
    def collect_events(helper, ew):
        ...
        log_label = f"CrowdStrike Falcon Detections TA {version} {stanza_name} :"
        ...
        helper.log_info(f"{log_label} Configuration - FalconPy SDK version: {falconpy_version}")

        #configure useragent value for API calls
        user_agent = f"Splunk_TA_Falcon_Detects_v{version}"
        ...
        try:
            ...
            helper.log_info(f"{log_label} Resuming from checkpoint: {checkpoint}")

        except Exception as e:
            helper.log_warning(f"{log_label} Unable to retrieve checkpoint: {type(e).__name__}: {e}. Starting from configured start_date or default.")
            checkpoint = ''
        ...
        if checkpoint != '':
            ...
            current_time = datetime.datetime.now(datetime.timezone.utc) + timedelta(minutes=-1)
            start_date = current_time.replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=None).strftime('%Y-%m-%dT%H:%M:%SZ')
        ...
        falcon = APIHarnessV2(client_id=clientid, client_secret=secret, base_url=cs_base_url, proxy=proxy_settings, timeout=const.timeout, user_agent=user_agent)

        #authenticate and validate credentials before collection
        try:
            falcon.authenticate()
        except Exception as auth_err:
            helper.log_error(f"{log_label} Authentication exception: {auth_err}")
            raise RuntimeError(f"Failed to authenticate with CrowdStrike API: {auth_err}")

        if not falcon.authenticated():
            token_status = falcon.token_status
            if token_status is None:
                helper.log_error(f"{log_label} Authentication failed - possible network failure or OAuth2 service unavailable")
            else:
                helper.log_error(f"{log_label} Authentication failed - token_status={token_status}")
            raise RuntimeError("Failed to authenticate with CrowdStrike API")

        helper.log_info(f"{log_label} Successfully authenticated with CrowdStrike API")

        #get product(s)
        products = helper.get_arg('product')
        helper.log_info(f'{log_label} Configuration - Selected products: {products}')

        ta_data = {"Cloud_environment":api_endpoint, "Input":stanza_name, "TA_version":version, "Products":str(products), "Start_date":checkpoint}

        alerts.get_alerts(checkpoint, stanza_checkpoint, chkpt_value, log_label, falcon, ta_data, helper, ew)
    ...
    def get_global_checkbox_fields(self):
        if self.global_checkbox_fields is None:
            ...
            try:
                ...
                self.log_error(f'Get exception when loading global checkbox parameter names. {type(e).__name__}: {e}')
                self.global_checkbox_fields = []
        return self.global_checkbox_fields

if __name__ == "__main__":
    exitcode = ModInputcrowdstrike_falcon_detections().run(sys.argv)
    sys.exit(exitcode)
