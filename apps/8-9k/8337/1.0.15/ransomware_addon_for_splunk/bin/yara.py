...
from yara_helper import validate_input

ADDON_NAME = "ransomware_addon_for_splunk"


class YARA(smi.Script):
    def __init__(self):
        super(YARA, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('yara')
        scheme.description = 'Yara'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        count = 0
        urls = []
        url = "https://api-pro.ransomware.live/yara"
        headers = {
        ...
        checkpoint = checkpointer.KVStoreCheckpointer(
            "ransomware_addon_yara_checkpoints",
            session_key,
        ...
        for entry in data.get("groups", []):
            ...
            if not group_name:
                continue
            detail_url = f"https://api-pro.ransomware.live/yara/{group_name}"
            ...
            event_data = detail_data

            unique_key = hashlib.md5(json.dumps(event_data, sort_keys=True).encode()).hexdigest()
            state = checkpoint.get(unique_key)
            if state is None:
                event = smi.Event(
                    data=json.dumps(event_data),
                    index=input_item.get("index"),
                    sourcetype="ransomware:yara",
                    source="group_yara://yara_group"
                )
