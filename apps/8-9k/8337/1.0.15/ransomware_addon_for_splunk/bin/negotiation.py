...
from negotiation_helper import validate_input

ADDON_NAME = "ransomware_addon_for_splunk"


class NEGOTIATION(smi.Script):
    def __init__(self):
        super(NEGOTIATION, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('negotiation')
        scheme.description = 'Negotiation'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        # with open("/opt/splunk/etc/apps/ransomware.txt", "w") as f:
        #     f.write(f"ACT_KEY - {api_key}")
        ...
        url = "https://api-pro.ransomware.live/negotiations"
        try:
            if proxy_enabled == "1":
        ...
        checkpoint = checkpointer.KVStoreCheckpointer(
            "ransomware_addon_negotiations_checkpoints",
            session_key,
        ...
        for group_entry in group_list:
            ...
            group_url = f"https://api-pro.ransomware.live/negotiations/{group}"
            try:
                if proxy_enabled == "1":
            ...
            for chat in chats:
                ...
                detail_url = f"https://api-pro.ransomware.live/negotiations/{group}/{chat_id}"
                urls.append(detail_url)
                try:
                    if proxy_enabled == "1":
                        detail_resp = requests.get(detail_url, headers=headers, proxies=proxy_dict)
                    else:
                ...
                if state is None:
                    event = smi.Event(
                        ...
                        sourcetype="ransomware:negotiation",
                        source="group_negotiations://chat_detail"
                    )
...
if __name__ == '__main__':
    exit_code = NEGOTIATION().run(sys.argv)
    sys.exit(exit_code)
