...
from csirt_helper import validate_input

ADDON_NAME = "ransomware_addon_for_splunk"


class CSIRT(smi.Script):
    def __init__(self):
        super(CSIRT, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('csirt')
        scheme.description = 'csirt'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        countries = [
            ...
            "TCD", "CHL", "CHN", "COL", "COM", "COG", "COD", "COK", "CRI", "CIV", "HRV",
            "CUB", "CYP", "CZE", "DNK", "DJI", "DMA", "DOM", "ECU", "EGY", "SLV", "GNQ",
            ...
            "SYC", "SLE", "SGP", "SVK", "SVN", "SLB", "SOM", "ZAF", "SSD", "ESP", "LKA",
        ...
        for i in countries:
            url = f"https://api-pro.ransomware.live/csirt/{i}"
            urls.append(url)
            headers = {
            ...
            for j in data["results"]:
                checkpoint = checkpointer.KVStoreCheckpointer(
                    "ransomware_addon_csirt_splunk_checkpoints",
                    session_key,
                    "ransomware_addon_for_splunk"
                )
                unique_key = hashlib.md5(json.dumps(j, sort_keys=True).encode()).hexdigest()
