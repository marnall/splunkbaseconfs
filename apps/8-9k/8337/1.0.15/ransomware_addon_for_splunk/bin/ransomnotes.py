...
from ransomnotes_helper import validate_input

ADDON_NAME = "ransomware_addon_for_splunk"


class RANSOMNOTES(smi.Script):
    def __init__(self):
        super(RANSOMNOTES, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('ransomnotes')
        scheme.description = 'Ransomnotes'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        url = "https://api-pro.ransomware.live/ransomnotes"
        try:
            if proxy_enabled == "1":
                response = requests.get(url, headers=headers, proxies=proxy_dict)
            else:
                response = requests.get(url, headers=headers)
            group_list = response.json().get("groups", [])
        except Exception:
            group_list = []

        # Initialize checkpoint
        checkpoint = checkpointer.KVStoreCheckpointer(
            "ransomware_addon_ransomnotes_checkpoints",
            session_key,
        ...
        for group_entry in group_list:
            ...
            if not group:
                continue
            group_url = f"https://api-pro.ransomware.live/ransomnotes/{group}"
            try:
                if proxy_enabled == "1":
                    group_resp = requests.get(group_url, headers=headers, proxies=proxy_dict)
                else:
            ...
            ransomnotes = group_data.get("ransomnotes", [])

            for note_name in ransomnotes:
                detail_url = f"https://api-pro.ransomware.live/ransomnotes/{group}/{note_name}"
                urls.append(detail_url)
                try:
                    if proxy_enabled == "1":
                        note_resp = requests.get(detail_url, headers=headers, proxies=proxy_dict)
                    else:
                        note_resp = requests.get(detail_url, headers=headers)
                    note_data = note_resp.json()
                except Exception:
                    continue

                formatted_event = note_data
                ...
                if state is None:
                    event = smi.Event(
                        ...
                        sourcetype="ransomware:ransomnote",
                        source="group_ransomnotes://ransomnote_detail"
                    )
...
if __name__ == '__main__':
    exit_code = RANSOMNOTES().run(sys.argv)
    sys.exit(exit_code)
