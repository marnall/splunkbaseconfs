...
from group_detail_helper import validate_input

ADDON_NAME = "ransomware_addon_for_splunk"


class GROUP_DETAILS(smi.Script):
    def __init__(self):
        super(GROUP_DETAILS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('group_details')
        scheme.description = 'Group Details'
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        url = "https://api-pro.ransomware.live/groups"
        if proxy_enabled == "1":
        ...
        checkpoint = checkpointer.KVStoreCheckpointer(
            "ransomware_addon_groupsdetails_checkpoints",
            session_key,
        ...
        for group in group_list.get("groups", []):
            ...
            detail_url = f"https://api-pro.ransomware.live/groups/{group_name}"
            ...
            unique_key = hashlib.md5(json.dumps(group_data, sort_keys=True).encode()).hexdigest()
            ...
            if state is None:
                event = smi.Event(
                    ...
                    sourcetype="ransomware:group_detail",
                    source="group_detail_api://all"
                )
...
if __name__ == '__main__':
    exit_code = GROUP_DETAILS().run(sys.argv)
    sys.exit(exit_code)
