...
active_user_raw_output_file="$SPLUNK_HOME/etc/apps/Splunk_TA_ExposureAnalytics_mac/tmp/user_mac_osx_output.log"
...
do
    if [ -n $aura_user_id[$i] ]; then
        output="$date_time nt_host=$aura_nt_host user_id=${aura_user_id[$i]}"
        [ -n "$user_info[$i]" ] && output+=" user_info=${user_info[$i]}"
        [ -n "$session[$i]" ] && output+=" session=${session[$i]}"
        [ -n "$aura_lastdetect[$i]" ] && output+=" lastdetect=${aura_lastdetect[$i]}"
