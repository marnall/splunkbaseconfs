...
system_software_info_raw_output_file_path="$SPLUNK_HOME/etc/apps/Splunk_TA_ExposureAnalytics_mac/tmp/Software.log"
system_Hardware_info_raw_output_file_path="$SPLUNK_HOME/etc/apps/Splunk_TA_ExposureAnalytics_mac/tmp/Hardware.log"
...
output="$date_time vendor=\"Apple\" asset_type=\"Workstation\""
[ -n $"$Model_Name" ] && output+=" product=$Model_Name"
...
[ -n $"$aura_nt_host" ] && output+=" nt_host=$aura_nt_host"
