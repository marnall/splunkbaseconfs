#!/usr/bin/env python3
"""
DarkStrata Close Alert - Adaptive Response Action

Closes a credential exposure alert in DarkStrata, marking the investigation
as complete.
...
class CloseAlertAction(DarkStrataActionBase):
    """Close a DarkStrata alert."""

    ACTION_NAME = "darkstrata_close_alert"

    def execute(self, params: dict[str, Any]) -> dict[str, Any]:
        """Execute the close alert action."""
        ...
        self.logger.info("Closing alert %s using account %s", alert_id, account_name)

        # Get account configuration
        account_config = self.get_account_config(account_name)

        # Make API call to close the alert
        endpoint = f"/alerts/{alert_id}"
        result = self.make_api_request(account_config, "PATCH", endpoint, data={"status": "CLOSED"})

        self.logger.info(
            "Successfully closed alert %s, new status: %s",
            alert_id,
            result.get("status"),
        )
...
def main() -> None:
    ...
    if log is not None:
        ...
        logger = log.Logs().get_logger(CloseAlertAction.ACTION_NAME)
    else:
        import logging

        logging.basicConfig(level=logging.INFO)
        logger = logging.getLogger(CloseAlertAction.ACTION_NAME)
    ...
    action = CloseAlertAction(session_key=session_key, logger=logger)
