...
def parse_link_counts_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(row, dict):
        ...
        # strict=False: result["columns"] may contain more/fewer than 2 entries
        # depending on the query; we only read outbound_count / inbound_count
        # below, so extra pairs drop silently.
        count_data = dict(zip(count_cols, row, strict=False))
    else:
        return {}
