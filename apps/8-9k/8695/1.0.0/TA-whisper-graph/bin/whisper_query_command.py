...
# Strict allowlist regex applied to validated parameter values: lowercase
# alphanumerics, dot, underscore, colon and hyphen. Anything else causes
# the command to short-circuit with zero events. Validation lives in the
# command itself so dashboards can reference the configured indicator
# token directly without relying on chain-search token interpolation.
INDICATOR_ALLOWLIST_RE = re.compile(r"^[A-Za-z0-9._:\-]+$")


def validate_indicator_param(parameters: dict[str, Any], param_name: str) -> dict[str, Any] | None:
    r"""Validate and normalise an indicator-like parameter.

    Applies the strict allowlist regex ``^[A-Za-z0-9._:\-]+$`` and
    lowercases the value. Returns a new parameters dict with the
    normalised value on success, or ``None`` if the value is missing,
    empty or fails validation (caller should short-circuit).

    Args:
        parameters: Existing parameter dict.
        param_name: The key whose value must be validated.

    Returns:
        New parameters dict with the lowercased value, or None on
        validation failure.
    """
    ...
    if not value:
        return None
    if not INDICATOR_ALLOWLIST_RE.fullmatch(value):
        return None
    result = dict(parameters)
    result[param_name] = value.lower()
    return result
...
class WhisperQueryCommand(GeneratingCommand):
    ...
    # When set to a non-empty parameter name (e.g. ``validate_indicator=indicator``)
    # the command applies the strict allowlist regex ``^[A-Za-z0-9._:\-]+$`` to
    # ``parameters[<name>]`` and lowercases the value before executing the
    # query. If the value is missing, empty or fails validation, the command
    # short-circuits and yields zero events. This lets Dashboard Studio
    # panels reference a defined ``$token$`` directly instead of relying on
    # chain-search token interpolation inside ``params="..."``.
    validate_indicator = Option(name="validate_indicator", require=False, default="")
    ...
    def generate(self):
        ...
        # Allowlist validation for indicator-like parameters. When enabled
        # (``validate_indicator=<param_name>``), reject anything outside
        # ``^[A-Za-z0-9._:\-]+$`` by short-circuiting with zero events.
        # This is the canonical defence against Cypher injection.
        if self.validate_indicator:
            validated = validate_indicator_param(parameters, self.validate_indicator)
            if validated is None:
                logger.info(
                    "action=whisperquery status=short_circuit reason=indicator_validation_failed param=%s",
