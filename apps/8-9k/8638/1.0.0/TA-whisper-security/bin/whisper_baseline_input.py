...
DNSSEC_SOURCETYPE = "whisper:dnssec_compliance"
...
def collect_domain_compliance(
    ...
    """Collect SPF and DNSSEC compliance data for a single domain.
    ...
    Returns:
        Tuple of (spf_event, dnssec_event). Either may be None on error.
    """
    from whisper_compliance_queries import (
        build_dnssec_query,
        build_spf_chain_query,
        build_spf_ip_query,
        parse_dnssec_compliance,
    ...
    # DNSSEC compliance
    try:
        dnssec_result = client.query(build_dnssec_query(), {"domain": domain})
        dnssec_data = parse_dnssec_compliance(dnssec_result.get("rows", []))
        dnssec_event = {
            "domain": domain,
            "last_checked": collected_at,
            **dnssec_data,
        ...
        logger.warning("action=collect_dnssec, status=error, domain=%s, error=%s", domain, exc)
    except Exception:
        logger.exception("action=collect_dnssec, status=error, domain=%s", domain)

    return spf_event, dnssec_event
