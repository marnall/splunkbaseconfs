...
from whisper_enrichment_queries import (
    ...
    build_org_pivot_query,
    build_prefix_threat_query,
    build_web_inbound_links_query,
    build_web_link_counts_query,
    build_web_links_query,
    build_whois_prev_registrar_query,
    build_whois_query,
)
...
from whisper_graph_parsers import (
    ...
    parse_link_counts_result,
    parse_org_pivot_result,
    parse_prefix_threat_result,
    parse_prev_registrar_result,
    parse_web_links_result,
    parse_whois_result,
)
...
def enrich_domain(
                ...
                if asn_val:
                    try:
                        ...
                        if info_rows:
                            ...
                            # Extract ASN threat properties (#208)
    ...
    # Co-host count for the primary IP (#428)
    ...
    # Threat intelligence via explain API (skip if inline data already present)
    if include_threat_intel and fields.get("threat_score") is None:
        try:
    ...
    _safe_query(
        client, fields, build_whois_query, {"hostname": hostname}, parse_whois_result, "WHOIS", hostname, "warning"
    )

    # Previous registrar (#361)
    _safe_query(
        client,
        fields,
        build_whois_prev_registrar_query,
    ...
    # Organization pivot (#361)
    _safe_query(
        client, fields, build_org_pivot_query, {"hostname": hostname}, parse_org_pivot_result, "Org pivot", hostname
    )

    # Web link graph (#361)
    try:
        outbound_result = client.query(build_web_links_query(), {"hostname": hostname})
        inbound_result = client.query(build_web_inbound_links_query(), {"hostname": hostname})
        all_link_rows = outbound_result.get("rows", []) + inbound_result.get("rows", [])
        links = parse_web_links_result(all_link_rows)
        fields.update(links)
        # Get accurate counts (not truncated by LIMIT 25)
        count_result = client.query(build_web_link_counts_query(), {"hostname": hostname})
        counts = parse_link_counts_result(count_result)
        if counts:
            fields.update(counts)
    except WhisperAPIRequestError as exc:
        logger.debug("Web link query failed for %s: %s", hostname, exc)
        fields.setdefault("link_count", 0)
        fields.setdefault("outbound_links", 0)
        fields.setdefault("inbound_links", 0)
        fields.setdefault("suspicious_link_count", 0)

    # GeoIP city-level enrichment for resolved IP
    if primary_ip:
        ip_for_geo = primary_ip[0] if isinstance(primary_ip, list) else primary_ip
        _safe_query(
            client,
            fields,
            build_geoip_query,
    ...
    # Calculate inline risk score (#320: first-class enrichment field)
    _inject_risk_score(fields)

    # Filter out disabled categories (#343: inline threat fields from infrastructure query)
