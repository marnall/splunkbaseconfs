...
DNSSEC status, threat feed listings, etc.) to quantified risk points
and normalizes to a 0–100 scale.
"""
...
DEFAULT_RISK_FACTORS: dict[str, dict[str, Any]] = {
    ...
    "no_dnssec": {
        "description": "Domain lacks DNSSEC signing",
        "points": 15,
    },
    "suspicious_spf": {
...
def calculate_risk_score(
    ...
    # Threat category boolean scoring (#207)
    ...
    # DNSSEC check (domain only)
    if enrichment.get("type") == "domain":
        has_dnssec = enrichment.get("dnssec", enrichment.get("signed_with"))
        if not has_dnssec:
            pts = factors.get("no_dnssec", {}).get("points", 15)
            raw_score += pts
            contributing_factors.append({"factor": "no_dnssec", "points": pts, "detail": "No DNSSEC signing"})

    # Threat feed listings
    # Note: threat_score from explain API is an unbounded float (typically 0-100+),
    # not 0-1 as in earlier API versions.
    feed_count = _to_int(enrichment.get("feed_count", 0))
    threat_score = _to_float(enrichment.get("threat_score", 0.0))

    if feed_count >= 4 or threat_score >= 50:
        pts = factors.get("threat_feed_high", {}).get("points", 80)
        raw_score += pts
        contributing_factors.append(
            {"factor": "threat_feed_high", "points": pts, "detail": f"{feed_count} feeds, score={threat_score}"}
        )
    elif feed_count >= 2 or threat_score >= 10:
        pts = factors.get("threat_feed_medium", {}).get("points", 60)
        raw_score += pts
        contributing_factors.append(
            {"factor": "threat_feed_medium", "points": pts, "detail": f"{feed_count} feeds, score={threat_score}"}
        )
    elif feed_count >= 1 or threat_score > 0:
        pts = factors.get("threat_feed_low", {}).get("points", 40)
        raw_score += pts
        contributing_factors.append(
            {"factor": "threat_feed_low", "points": pts, "detail": f"{feed_count} feeds, score={threat_score}"}
        )
    ...
    # Registrar change detection (#361)
...
def _evaluate_threat_categories(
    ...
    # Includes isSpam, isProxy, isVpn (#361)
    category_checks = [
