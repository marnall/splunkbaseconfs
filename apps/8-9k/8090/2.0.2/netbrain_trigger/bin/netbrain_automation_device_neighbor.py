...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        ...
        try:
            ...
            device_name = trigger_params["device"]
            ...
            if "interface" in trigger_params:
                intf_string = trigger_params["interface"]
                if ',' in intf_string:
                    interface_list = intf_string.split(',')
                else:
                    interface_list = [intf_string]
            else:
                interface_list = []
            ...
            response = netbrain.trigger_dev_nbr_map(incident_subject, stub_name, device_name, include_neighbor, neighbor_type, interface_list)
            PL.print_log("Neighbor map response: " + json.dumps(response))
            mapURl = nb_endpoint + '/' + response['mapUrl']
            log = " Search Name ----- " + alert_Name + " ----- Map URL ----- Neighbor ----- " + mapURl
            PL.print_log(log)
