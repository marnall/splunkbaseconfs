...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        ...
        try:
            ...
            response = netbrain.trigger_existing_map(incident_subject, stub_name, mapId, duplicate_map)
            PL.print_log("Existing map response: " + json.dumps(response))
            mapURl = nb_endpoint + '/' + response['mapUrl']
            log = " Search Name ----- " + alert_Name + " ----- Map URL ----- Existing ----- " + mapURl
            PL.print_log(log)
