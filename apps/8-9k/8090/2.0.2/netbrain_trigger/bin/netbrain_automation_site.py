...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        ...
        try:
            ...
            duplicate_map_temp = trigger_params["duplicate_map"]
            ...
            if duplicate_map_temp == "1":
                duplicate_map = True
            else:
                duplicate_map = False
            netbrain = Netbrain(nb_endpoint,username,password,tenant_name,domain_name)
            response = netbrain.trigger_site_map(incident_subject, stub_name, device_name, duplicate_map)
            PL.print_log("Site map response: " + json.dumps(response))
            mapURl = nb_endpoint + '/' + response['mapUrl']
            log = " Search Name ----- " + alert_Name + " ----- Map URL ----- Site ----- " + mapURl
            PL.print_log(log)
            ...
            # logging.info("@@@ From Logging @@@ " + log)      
            # if response.has_key('mapUrl'):
            #     map_url = nb_endpoint + '/' + response['mapUrl']
            #     now = datetime.now()
            #     current_time = now.strftime("%H:%M:%S")
            #     event = Event(
            #         data= map_url,
            #         stanza= "Splunk Event Test",
            #         time= current_time,
            #         host= "localhost",
            #         index= "main",
            #         source= "netbrain_automation",
            #         sourcetype= "misc",
            #         done=True,
            #         unbroken=True
            #     )
