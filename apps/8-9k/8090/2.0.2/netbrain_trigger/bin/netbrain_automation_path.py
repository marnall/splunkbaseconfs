...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        ...
        try:
            ...
            dest = trigger_params["destination"]
            ...
            log = "netbrain_automation_path ----- " + username + " ----- " + incident_subject
            PL = printLog()
            PL.print_log(log)
            netbrain = Netbrain(nb_endpoint,username,password,tenant_name,domain_name)
            response = netbrain.trigger_path_map(incident_subject, stub_name, source, dest)
            PL.print_log("Path map response: " + json.dumps(response))
            mapURl = nb_endpoint + '/' + response['mapUrl']
            log = " Search Name ----- " + alert_Name + " ----- Map URL ----- Path ----- " + mapURl
            PL.print_log(log)
            sys.exit(0)
        except Exception as e:
