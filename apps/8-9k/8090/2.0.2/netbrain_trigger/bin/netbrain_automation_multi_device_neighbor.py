...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        ...
        try:
            ...
            PL.print_log(json.dumps(trigger_params))
            stub_name = trigger_params["stub_name"]
            devices = trigger_params["devices"]
            ...
            auto_link_type = trigger_params["auto_link_type"]
            neighbor_type = trigger_params["neighbor_type"]
            incident_subject = trigger_params["incident_subject"]
        
            auto_link_temp = trigger_params["auto_link"]
            if auto_link_temp == "1":
                auto_link = True
            else:
                auto_link = False

            include_neighbor_temp = trigger_params["include_neighbor"]
            if include_neighbor_temp == "1":
            ...
            response = netbrain.trigger_multi_devices_map(incident_subject, stub_name, device_list, auto_link_type, neighbor_type, auto_link, include_neighbor)
            PL.print_log("Multi_devices map response: " + json.dumps(response))
            mapURl = nb_endpoint + '/' + response['mapUrl']
            log = " Search Name ----- " + alert_Name + " ----- Map URL ----- Multi_devices ----- " + mapURl
            PL.print_log(log)
