...
logger.basicConfig(level=logger.INFO, format='%(asctime)s %(levelname)s  %(message)s',datefmt='%m-%d-%Y %H:%M:%S.000 %z',
     filename=os.path.join(os.environ['SPLUNK_HOME'],'var','log','splunk','recover_alert.log'),
     filemode='a')

splunkHome=os.environ.get('SPLUNK_HOME')
tmpDir=splunkHome + "/etc/apps/mongo_lookup/upload"
...
class Recover(PersistentServerConnectionApplication):
