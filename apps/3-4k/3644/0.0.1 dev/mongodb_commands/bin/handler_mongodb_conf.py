...
class ConfigApp(admin.MConfigHandler):
  ...
  def handleList(self, confInfo):
    confDict = self.readConf("mongodb")
    if None != confDict:
      ...
      confInfo[stanza].append('hosta', 'notset')
  ...
  def handleEdit(self, confInfo):
    ...
    if int(self.callerArgs.data['port'][0]) <= 0:
      self.callerArgs.data['port'][0] = '27017'
...
write them to the [mongodb] stanza
    in mongodb_commands/local/mongodb.conf  
    '''
        
    self.writeConf('mongodb', 'mongodb', self.callerArgs.data)
