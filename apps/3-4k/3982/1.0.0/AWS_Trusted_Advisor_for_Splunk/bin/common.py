...
def get_credentials(session_key):
    ...
    :return: AWS Keys
    """
    myapp = 'AWS_Trusted_Advisor_for_Splunk'
    try:
    ...
    if entities:
        for stanza in entities.values():
            if stanza['eai:acl']['app'] == myapp:
                if stanza['username'] == 'access_key':
                    access_key = stanza['clear_password']
                if stanza['username'] == 'secret_key':
                    secret_key = stanza['clear_password']
