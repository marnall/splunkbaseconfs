...
class HostFieldValidator(StandardFieldValidator):
    ...
    def is_valid_hostname(self, hostname):
        ...
        # TODO need to validate the IPv4 or IPv6  or hostname
        return True # all(allowed.match(x) for x in hostname.split("."))
...
# Setup the handler
logger = setup_logger(logging.DEBUG, "ResolveRestHandler", "resolve_rest_handler.log")

class ResolveRestHandler(admin.MConfigHandler):
    """
    ...
    VALID_PARAMS           = [ PARAM_PROXY_SERVER, PARAM_PROXY_PORT, PARAM_PROXY_TYPE ]
    ...
    def convertParams(cls, name, params, to_string=False):
        ...
        for key, value in params.items():
            ...
            validator = cls.FIELD_VALIDATORS.get( key )
    ...
    def handleEdit(self, confInfo):
        ...
        try:
            ...
            clearable_params = [ self.PARAM_PROXY_SERVER, self.PARAM_PROXY_PORT, self.PARAM_PROXY_TYPE ]
    ...
    def checkConf(cls, settings, stanza=None, confInfo=None, onlyCheckProvidedFields=False, existing_settings=None):
        ...
        for key, val in settings.items():
            ...
            if stanza is not None and confInfo is not None and key in cls.VALID_PARAMS:
                confInfo[stanza].append(key, val)
...
admin.init(ResolveRestHandler, admin.CONTEXT_NONE)
