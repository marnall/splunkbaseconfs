...
@Configuration()
class RCorrplotMixedCommand(StreamingCommand):
    """
    ...
    upper = Option(
        doc='''
        **Syntax:** **upper=***<method>*
        **Description:** Method to use for the visualization of the
    ...
    def stream(self, records):
        ...
        script = script + '''
            c <- cor(dataset);
            corrplot.mixed(c, upper='%s', lower='%s', bg='%s', title='%s', order='%s');
        ''' % (self.upper, self.lower, self.bg, self.title, self.order)
        ...
        if status // 100 != 2:
            ...
            # Use the session to run the corrplot command
...
dispatch(RCorrplotMixedCommand, sys.argv, sys.stdin, sys.stdout, __name__)
