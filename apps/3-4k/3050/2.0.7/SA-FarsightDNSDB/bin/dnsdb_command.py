...
def cache_read(queryType, session_key, rrType, target, time_first_before, time_first_after, time_last_before, time_last_after):
    """
        Reads data from Splunk KV store
    """
    service = connect(token=session_key, owner='nobody', app='SA-FarsightDNSDB')
    collection = service.kvstore["%s_kvstore" % queryType]
    key = target + '-' + str(rrType) + '-' + time_first_before + '-' + time_first_after + '-' + time_last_before + '-' + time_last_after
    try:
...
def cache_insert(queryType, session_key, rrType, target, output, time_first_before, time_first_after, time_last_before, time_last_after):
    """
...
def run_query(dnsdb, target, req_type, rrtype, bailiwick, time_first_before, time_first_after, time_last_before, time_last_after, target_type="auto"):
    ...
    try:
        if target_type == "auto":
            if req_type == "rrset":
                response = list(dnsdb.lookup_rrset(target, rrtype=rrtype, bailiwick=bailiwick,
                                time_first_before=p_time_first_before, time_first_after=p_time_first_after, 
                                time_last_before=p_time_last_before, time_last_after=p_time_last_after, ignore_limited=True))
            elif req_type == "rdata":
                if is_valid_ip(target):
                    response = list(dnsdb.lookup_rdata_ip(target, rrtype=rrtype,
                                    time_first_before=p_time_first_before, time_first_after=p_time_first_after, 
                                    time_last_before=p_time_last_before, time_last_after=p_time_last_after, ignore_limited=True))
                elif is_raw_rdata(target):
                    response = list(dnsdb.lookup_rdata_raw(target, rrtype=rrtype,
                                    time_first_before=p_time_first_before, time_first_after=p_time_first_after, 
                                    time_last_before=p_time_last_before, time_last_after=p_time_last_after, ignore_limited=True))
                else:
                    response = list(dnsdb.lookup_rdata_name(target, rrtype=rrtype,
                                    time_first_before=p_time_first_before, time_first_after=p_time_first_after, 
                                    time_last_before=p_time_last_before, time_last_after=p_time_last_after, ignore_limited=True))
            else:
            ...
            if req_type == "rrset":
                ...
                if target_type == "ip":
                    response = list(dnsdb.lookup_rdata_ip(target, rrtype=rrtype, bailiwick=bailiwick,
                                    time_first_before=p_time_first_before, time_first_after=p_time_first_after, 
                                    time_last_before=p_time_last_before, time_last_after=p_time_last_after, ignore_limited=True))
                    ...
                    response = list(dnsdb.lookup_rdata_name(target, rrtype=rrtype, bailiwick=bailiwick,
                                    time_first_before=p_time_first_before, time_first_after=p_time_first_after, 
                                    time_last_before=p_time_last_before, time_last_after=p_time_last_after, ignore_limited=True))
...
def main():
    try:
        ...
        if not target:
            error = splunk.Intersplunk.generateErrorResults("Usage: | dnsdb target=ip/hostname "
        ...
        # KV Store caching
        try:
            cache_results = cache_read(req_type, sessionKey, rrtype, target, str(time_first_before), str(time_first_after), str(time_last_before), str(time_last_after))
        except KeyError:
            output_events = run_query(dnsdb, target, req_type, rrtype, bailiwick, time_first_before, time_first_after, time_last_before, time_last_after)
            
            cache_insert(req_type, sessionKey, rrtype, target, output_events, str(time_first_before), str(time_first_after), str(time_last_before), str(time_last_after))
        else:
            if calendar.timegm(time.gmtime()) - int(cache_results["cache_time"]) > 86400:
                output_events = run_query(dnsdb, target, req_type, rrtype, bailiwick, time_first_before, time_first_after, time_last_before, time_last_after)
                cache_insert(req_type, sessionKey, rrtype, target, output_events, str(time_first_before), str(time_first_after), str(time_last_before), str(time_last_after))
            else:
