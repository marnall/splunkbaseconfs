...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("sendmodaction action_name=banhash action=starting_main")
    ...
    try:
        ...
        logger.info("sendmodaction action_name=banhash action=adding_info")
        modaction.addinfo()
        ## process results
        if not os.path.exists(modaction.results_file):
            ...
            sys.exit(0)
        logger.info("sendmodaction action_name=banhash results_file={}".format(modaction.results_file))
        with gzip.open(modaction.results_file, 'rt') as fh:
            for num, result in enumerate(csv.DictReader(fh)):
                logger.info("sendmodaction action_name=banhash result_num={}".format(num))
