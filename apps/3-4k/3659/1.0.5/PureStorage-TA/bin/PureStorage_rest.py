    ...
    if os.path.exists(os.path.join(log_state_dir, 'input.json')):
        with open(os.path.join(log_state_dir, 'input.json'), 'r+') as f:
            ...
            if input_name in data:
                ...
                details = [x for x in details if x['opened'] is not None and x['opened'] > date_var]
                if details:
        ...
        with open(os.path.join(log_state_dir, 'input.json'), "w") as f:
            temp = {input_name: {"log_alert": None, "log_login": None, "log_audit": None}}
            temp[input_name][name] = details[-1].get("opened")
...
class PureStorageREST(Script):
    ...
    def ensure_timefield(self, dict, timefield):
        if not timefield:
            # get the current time, format accordingly
            timefield = "time"
            timeval = datetime.utcnow().strftime(TT_FORMAT)
        else:
            timeval = dict.get(timefield)
            del dict[timefield]
       
        # return an OrderedDict here with timefield as the first entry
        return OrderedDict([(timefield, timeval)] + list(dict.items()))

    def write_events(self, ew, array, data, input_name, source, sourcetype, timefield=None):
        """
        ...
        if isinstance(data, dict):
            data = self.ensure_timefield(data, timefield)
            raw_event = Event()
        ...
        for row in data:
            row = self.ensure_timefield(row, timefield)
            raw_event = Event()
            raw_event.stanza = input_name
            raw_event.source = source
            raw_event.host = array
    ...
    def get_array_details(self, ew, connection, array, input_name):
        ...
        self.write_events(ew, array, array_details, input_name, source=Source.source_array,
                          sourcetype=Source.sourcetype, timefield="time")
    ...
    def get_volume_details(self, ew, connection, array, input_name):
        ...
        for index, row in enumerate(volumes_details):
    ...
    def get_volume_snap_details(self, ew, connection, array, input_name):
        ...
        self.write_events(ew, array, pgroup_list, input_name, source=Source.source_pgroup,
                          sourcetype=Source.sourcetype, timefield=None)
        ...
        self.write_events(ew, array, snapshot_details, input_name, source=Source.source_snapshots,
                          sourcetype=Source.sourcetype, timefield="created")
    ...
    def get_host_details(self, ew, connection, array, input_name):
        ...
        self.write_events(ew, array, hosts_details, input_name, source=Source.source_host, sourcetype=Source.sourcetype, timefield=None)
    ...
    def get_message_details(self, ew, connection, array, input_name):
        ...
        self.write_events(ew, array, alert_details, input_name, source=Source.source_alert_logs,
                          sourcetype=Source.sourcetype, timefield="opened")
