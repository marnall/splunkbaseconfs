...
echo "Configuring"
cmd="$configure --output=splunk --logstdout=true --logstderr=false --enabled=$enabled --conditions=splunk --host=$host --port=$po>
echo $cmd
$cmd
cmd_nocerts= "openssl pkcs12 -in \"client.pkcs12\" -nocerts -nodes -out \"$host-$port_pkcs.cert\"
cmd_clcerts= "openssl pkcs12 -in \"cliient.pkcs12\" -clcerts -nokeys -out \"$host-$port_pkcs.key\"
