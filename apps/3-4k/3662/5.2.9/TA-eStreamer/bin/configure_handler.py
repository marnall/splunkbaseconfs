...
class eNcoreConfig( admin.MConfigHandler ):
    ...
    def _configure( self ):
        ...
        _errorIf( r'PKCS12 file \(.*\) does not exist'.encode("utf-8"), output)
        _errorIf( 'Mac verify error: invalid password?'.encode("utf-8"), output, 'Invalid PKCS12 password?'.encode("utf-8") )
