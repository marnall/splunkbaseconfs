...
class ClockEventsResponse(object):
    ...
    def toCsv(self, output):
        header = ['# clock name', 'clock type', 'availability', 'max deviation','sample count']
        for threshold in self.thresholds:
...
class CorvilApiStatsClient(object):
    ...
    """
    def __init__(self, host, port=5101, username='admin', password='', cne=None, useHttps=False, timeout=90):
        self.host = host
        self.port = port
...
#        print "CLIENT URL:%s %s/%s" % (self.url, username, password)
        ...
        self.sudsClient.timeout = timeout
