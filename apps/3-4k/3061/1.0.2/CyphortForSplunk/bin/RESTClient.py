...
class RESTClient:
    ...
    def _setup_logging(self, ll=logger.INFO):
        log_location = os.path.join(self._splunk_home, 'var', 'log', 'splunk',
                                    self._app_name)

        if not os.path.isdir(log_location):
            os.mkdir(log_location)
        output_file_name = os.path.join(log_location, "restclient.log")
        self._log.basicConfig(format='%(asctime)s %(levelname)s %(message)s', filename=output_file_name,
                              filemode='a+', level=ll, datefmt='%Y-%m-%d %H:%M:%S %z')
        self._log.Formatter.converter = time.gmtime
    ...
    def _format_return(self, obj):
        self._log.debug("output_type = %s" % self._output_type)
        if self._output_type is "json":
            return json.loads(obj)
        else:
            return obj

    def _read(self, url, payload=None):
