...
class KennyLoggins:
    ...
    def get_logger(self, app_name=None, file_name="kenny_loggins", log_level=logging.INFO):
        ...
        formatter = logging.Formatter(
            '%(asctime)s log_level=%(levelname)s pid=%(process)d tid=%(threadName)s file="%(filename)s" function="%(funcName)s" line_number="%(lineno)d" py_version="{}" version="{}" %(message)s'.format(
                sys.version, version.__version__))
