...
def collect_events(helper, ew):
    ...
    quickKey_file_name = f"syslog_events_quickkey_{hostname}.txt"
    ...
    #The checkpoint file contains the quickKey of the last event seen in the previous execution of the script.
    checkpoint_file = os.path.join(os.environ['SPLUNK_HOME'], 'etc', 'apps', 'Splunk_TA_sailpoint', 'tmp', quickKey_file_name)
    try:
    ...
    with open(checkpoint_file, 'r') as f:
         quickKey = f.readlines()
         
    yesterday_dt = datetime.datetime.now() - datetime.timedelta(days = 1)
    #calculate the epoch time in milliseconds which is 1 day in past 
    initial_start_time = int((yesterday_dt - datetime.datetime(1970, 1, 1)).total_seconds() *1000)
    ...
    if len(quickKey) == 1:
        quickKey =quickKey[0]
        queryparams['quickKey'] = quickKey
    else:
        queryparams['startTime'] = initial_start_time
        helper.log_info("No quickKey available. Setting start time in the query param.")     

    syslog_events_url = "{}/plugin/rest/SIEMPlugin/syslog-events".format(identityiq_url)
    
    # Check if syslog_events_url is https.
    if not is_https(helper, syslog_events_url):
        return False
    ...
    response = helper.send_http_request(syslog_events_url, "GET", parameters=queryparams, payload=payload, headers=headers, cookies=None, verify=True, cert=None, timeout=valid_timeout, use_proxy=None)
    ...
    #Retrieve the syslog events from the json.
    syslogEvents = get_json_element(results, ["syslogEvents"])

    #Iterate the syslog events array and create Splunk events for each one
    invalid_response = isListEmpty(syslogEvents)
    if not invalid_response:
        for syslogEvent in syslogEvents:
            
            data = json.dumps(syslogEvent)
        ...
        #Get the quickKey of the last syslog event in the run and save it as a checkpoint key in the checkpoint file    
        list_of_quickKeys = get_json_element(results, ["syslogEvents", "quickKey"])

        new_checkpoint_quickKey = list_of_quickKeys[-1]
        helper.log_info("DEBUG New checkpoint quickKey \n{}".format(new_checkpoint_quickKey))
    
        #Write new checkpoint key to the checkpoint file    
        with open(checkpoint_file, 'r+') as f:
            f.seek(0)
            f.write(str(new_checkpoint_quickKey))
            f.truncate()