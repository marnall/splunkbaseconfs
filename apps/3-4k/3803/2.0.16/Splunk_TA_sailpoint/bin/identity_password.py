...
import modalert_identity_password_helper

class AlertActionWorkeridentity_password(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkeridentity_password, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_identity_password_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkeridentity_password("Splunk_TA_sailpoint", "identity_password").run(sys.argv)
    sys.exit(exitcode)
