...
import modalert_application_accounts_helper

class AlertActionWorkerapplication_accounts(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerapplication_accounts, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_application_accounts_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerapplication_accounts("Splunk_TA_sailpoint", "application_accounts").run(sys.argv)
    sys.exit(exitcode)
