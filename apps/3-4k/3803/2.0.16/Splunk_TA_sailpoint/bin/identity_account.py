...
import modalert_identity_account_helper

class AlertActionWorkeridentity_account(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkeridentity_account, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_identity_account_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkeridentity_account("Splunk_TA_sailpoint", "identity_account").run(sys.argv)
    sys.exit(exitcode)
