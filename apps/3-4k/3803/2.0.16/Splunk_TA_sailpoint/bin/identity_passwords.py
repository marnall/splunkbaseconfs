...
import modalert_identity_passwords_helper

class AlertActionWorkeridentity_passwords(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkeridentity_passwords, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_identity_passwords_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkeridentity_passwords("Splunk_TA_sailpoint", "identity_passwords").run(sys.argv)
    sys.exit(exitcode)
