...
if sys.argv[1] == "--execute":
	...
	if str2bool(config.get('overwrite')):
		print >>sys.stderr, 'INFO Overwriting kvstore collection=%s with data=%s' % (collection, json.dumps(json_results))
		# Delete the collection contents
		try:
			response = request('DELETE', delete_url, "", headers)
			print >>sys.stderr, 'DEBUG server response for collection deletion:', json.dumps(response)
		except urllib2.HTTPError, e:
			print >>sys.stderr, 'ERROR Failed to delete collection:', json.dumps(json.loads(e.read()))
			sys.exit(3)
	else:
		print >>sys.stderr, 'INFO Updating kvstore collection=%s with data=%s' % (collection, json.dumps(json_results))
