	...
	def generate(self):
		...
		for name in f:
			...
			try:
				...
				logger.critical('%s Invalid filename: %s\n\t%s', facility, name, e.message)
				logger.debug('%s Invalid filename: %s', facility, name)
				exit(1)
			...
			if list_only:
				...
				try:
					...
					try:
						...
						try:
							...
							logger.error("%s [Strip '[]'] Error reading modified json input.  Aborting.", facility, name)
				...
				if not self.append:
					...
					try:
						...
						logger.info("%s Deleted collection: " + file_app + "\\" + file_collection, facility)
					except urllib2.HTTPError, e:
						logger.error('%s ERROR Failed to delete collection: ' + json.dumps(json.loads(e.read())), facility)
						sys.exit(3)

				i = 0
				batch_number = 1
				limit = int(limits_cfg.get('max_documents_per_batch_save'))
				logger.debug("%s Max documents per batch save = " + str(limit), facility)
				posted = 0
				...
				while i < content_len:
					...
					logger.debug('%s Batch number: ' + str(batch_number) + '(' + str(sys.getsizeof(batch)) + ' bytes / ' + str(len(batch)) + ' records)', facility)
					...
					try:
						...
						logger.error('%s Failed to update records:', facility, json.dumps(json.loads(e.read())))
						yield {'_time': time.time(), 'source': name, 'app': file_app, 'kvstore': file_collection, 'records': "0", 'result': 'error' }
						# Force out of the while loop
						i = content_len

				logger.info("%s Restored " + str(posted) + " records to " + file_app + "\\" + file_collection, facility)
