...
LOG_FILENAME = os.path.join(SPLUNK_HOME,"var","log","splunk","pubnubalert_app_setuphandler.log")

# Set up a specific logger
logger = logging.getLogger('pubnubalert')
...
class ConfigHandler(splunk.admin.MConfigHandler):
        ...
        try:
            ...
            if self.requestedAction == splunk.admin.ACTION_EDIT:
                ...
                for arg in ['param.activationkey' ]:
                    self.supportedArgs.addOptArg(arg)
                for arg in ['param.pubkey' ]:
                    self.supportedArgs.addOptArg(arg)
                for arg in ['param.subkey' ]:
                    self.supportedArgs.addOptArg(arg)
    ...
    def handleList(self, confInfo):
        ...
        try:
            ...
            entities = entity.getEntities(['storage', 'passwords'], namespace="pubnub_alert", owner='nobody', sessionKey=self.getSessionKey())
            credential_list = []
            credential_key_list = []


            for i, c in entities.items():
                if c['eai:acl']['app'] ==  "pubnub_alert":
                    credential_list.append(c['clear_password'])
                    credential_key_list.append(c['username'])
                 
            global_entities = entity.getEntities(['admin', 'alert_actions','pubnub'], namespace="pubnub_alert", owner='nobody', sessionKey=self.getSessionKey())
            ...
            confInfo['pubnubalert'].append('param.pubkey', pubkey) 
            confInfo['pubnubalert'].append('param.subkey', subkey) 
            confInfo['pubnubalert'].append('param.log_level', log_level) 
            confInfo['pubnubalert'].append('param.activationkey', activationkey)
            confInfo['pubnubalert'].append('credential', "::".join(credential_list))
            confInfo['pubnubalert'].append('credential_key', "::".join(credential_key_list))
    ...
    def handleEdit(self, confInfo):
        ...
        try:
            ...
            if self.callerArgs.data['param.subkey'][0] in [None, '']:
                self.callerArgs.data['param.subkey'][0] = ''

            if self.callerArgs.data['param.pubkey'][0] in [None, '']:
                self.callerArgs.data['param.pubkey'][0] = ''
            ...
            subkey = self.callerArgs.data['param.subkey'][0]
            pubkey = self.callerArgs.data['param.pubkey'][0]
            ...
            try:
                ...
                for i, c in entities.items():
                    if c['eai:acl']['app'] ==  "pubnub_alert":
                        entity.deleteEntity(['storage', 'passwords'],":%s:" % c['username'],namespace="pubnub_alert", owner='nobody', sessionKey=self.getSessionKey())           
            except:  
                e = sys.exc_info()[0]  
                logger.error("Error deleting pubnub_alert credential , perhaps this is the first setup run and it did not yet exist (that is ok) : %s" % e) 
            

            for credential_key,credential in zip(credential_key_str.split('::'),credential_str.split('::')):
                try:
                    logger.debug("creating pubnub_alert credential")
                    new_credential = entity.Entity(['storage', 'passwords'], credential_key, contents={'password':credential}, namespace="pubnub_alert",owner='nobody')
                    entity.setEntity(new_credential,sessionKey=self.getSessionKey())
                except:  
                    e = sys.exc_info()[0]  
                    logger.error("Error creating pubnub_alert credential : %s" % e)

            try:
                logger.debug("creating global settings")
                new_global_settings= entity.Entity(['admin', 'alert_actions'], 'pubnub', contents={'param.log_level':log_level,'param.activationkey':activationkey,'param.pubkey':pubkey,'param.subkey':subkey}, namespace="pubnub_alert",owner='nobody')
