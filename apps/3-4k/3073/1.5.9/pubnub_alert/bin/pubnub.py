...
LOG_FILENAME = os.path.join(SPLUNK_HOME,"var","log","splunk","pubnubalert_app_modularalert.log")
...
EGG_DIR = os.path.join(SPLUNK_HOME,"etc","apps","pubnub_alert","bin")
...
def callback(message):
    logger.debug("Callback handler for sent message %s" % message)

def send_message(settings,search_name):
    logger.debug("%s : Sending message to Pubnub with settings %s" % (search_name,settings))
    ...
    logger.info("%s : Sending message to Pubnub channel=%s with message=%s" % (search_name,channel,message))
    ...
    try:  
        ...
        logger.info("%s : Sent message to Pubnub : %s" % (search_name,message))
        return True  
    except Exception as tre:  
        logger.error("%s : Error sending message to Pubnub: %s" % (search_name,tre))  
        return False  
    except:  
        e = sys.exc_info()[0]  
        logger.error("%s : Error sending message to Pubnub: %s" % (search_name,e))   
        return False  
...
if __name__ == "__main__":  
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":  
        ...
        logger.info("%s : Executing Pubnub Alert" % search_name)
        ...
        payload = json.loads(json_config)
        if not send_message(payload.get('configuration'),search_name):
            logger.error("%s : Failed trying to send Message to Pubnub" % search_name)
            sys.exit(2)
        else:
            logger.info("%s : Message successfully sent to Pubnub" % search_name)
    else:
