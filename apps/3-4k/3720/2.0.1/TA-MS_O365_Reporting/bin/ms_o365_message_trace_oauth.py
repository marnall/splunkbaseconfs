...
import o365_util.auth as azauth
import o365_util.utils as o365utils
import re
...
def get_messages(helper, microsoft_trace_url):
    ...
    try:
        access_token = azauth.get_msg_trace_access_token(client_id, client_secret, tenant_id, helper)
        ...
        if(access_token):
            ...
            proxies = o365utils.get_proxy(helper, "requests")
    
            r = requests.get(microsoft_trace_url, headers=headers, proxies=proxies)
            r.raise_for_status()
            response_json = None
...
def get_events_continuous(helper, ew):
    ...
    check_point_key = "%s_obj_checkpoint_oauth" % helper.get_input_stanza_names()
    messages = None
    ...
    if 'value' in message_response:
        messages = message_response['value']
    ...
    while messages:
        ...
        if nextLink is not None:
            ...
            message_response = get_messages(helper, nextLink)
...
def get_events_once(helper, ew):
    ...
    check_point_key = "%s_once_checkpoint_oauth" % helper.get_input_stanza_names()
...
class ModInputms_o365_message_trace_oauth(base_mi.BaseModInput):

    def __init__(self):
        use_single_instance = False
        super(ModInputms_o365_message_trace_oauth, self).__init__("ta_ms_o365_reporting", "ms_o365_message_trace_oauth", use_single_instance)
        self.global_checkbox_fields = None

    def get_scheme(self):
        """overloaded splunklib modularinput method"""
        scheme = super(ModInputms_o365_message_trace_oauth, self).get_scheme()
        scheme.title = ("Office 365 Message Trace (oauth)")
        ...
        scheme.add_argument(smi.Argument("query_window_size", title="Query Window Size (minutes)",
                                         description="Specify how many minute's worth of data to query each interval. See https://splunkbase.splunk.com/app/3720/#/details for more information.",
                                         required_on_create=False,
                                         required_on_edit=False))
        scheme.add_argument(smi.Argument("delay_throttle", title="Delay Throttle (minutes)",
                                         description="Microsoft may delay trace events up to 24 hours. Specify how close to \"now\" a query may run. See https://splunkbase.splunk.com/app/3720/#/details for more information.",
                                         required_on_create=False,
                                         required_on_edit=False))
...
if __name__ == "__main__":
    exitcode = ModInputms_o365_message_trace_oauth().run(sys.argv)
    sys.exit(exitcode)
