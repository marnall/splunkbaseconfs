...
class Utilities:
    ...
    def get_credential(self, realm, cuser):
        ...
        try:
            ...
            if key not in entities:
                ...
                return six.moves.urllib.parse.unquote(entities[key]["clear_password"])
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            jsondump = {"message": str((e)),
    ...
    def set_kvstore_data(self, kvstore, data):
        try:
            uri = self._build_endpoint_uri(["storage", "collections", "data", kvstore])
            self._log.info("setting kvstore data to have fields: {}".format(json.dumps({"keys": list(data.keys())})))
