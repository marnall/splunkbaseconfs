...
class Utilities:
    ...
    def setup_logger(self, level=logging.INFO):
        ...
        """
        log_location = make_splunkhome_path(['var', 'log', 'splunk', self._app_name])
        self._log = logging.getLogger("{0}_restclient".format(self._app_name))
    ...
    def set_credential(self, crealm, cuser, cpass):
        ...
        if creds is not None:
            ...
            args = {"password": cpass}
        self._debug("URI: {0} Args {1}".format(uri, args))
    ...
    def get_credential(self, realm, cuser):
        ...
        try:
            ...
            # self._debug("found entities: %s" % entities)
    ...
    def set_kvstore_data(self, kvstore, data):
        uri = self._build_endpoint_uri(["storage", "collections", "data", kvstore])
        self._debug("setting kvstore data to %s" % data)
