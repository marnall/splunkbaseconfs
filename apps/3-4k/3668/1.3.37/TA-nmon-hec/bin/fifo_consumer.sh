...
if [ -s $nmon_config_rotated ] && [ -s $nmon_header_rotated ] && [ -s $nmon_data_rotated ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ -f $nmon_timestamp_rotated ]; then
            ...
            cat $nmon_config_rotated $nmon_header_files $temp_file $nmon_data_rotated $nmon_external_rotated | $SPLUNK_HOME/bin/splunk cmd $APP/bin/nmon2kv.sh $nmon2csv_options
        fi
    else
        cat $nmon_config_rotated $nmon_header_files $nmon_data_rotated $nmon_external_rotated | $SPLUNK_HOME/bin/splunk cmd $APP/bin/nmon2kv.sh $nmon2csv_options
    fi
...
if [ -s $nmon_config ] && [ -s $nmon_header ] && [ -s $nmon_data ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        cat $nmon_config $nmon_header_files $temp_file $nmon_data_tmp | $SPLUNK_HOME/bin/splunk cmd $APP/bin/nmon2kv.sh $nmon2csv_options
    else
        cat $nmon_config $nmon_header_files $nmon_data_tmp | $SPLUNK_HOME/bin/splunk cmd $APP/bin/nmon2kv.sh $nmon2csv_options
    fi
