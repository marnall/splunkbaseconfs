...
if ( length($APP) == 0 ) {
    ...
    elsif ( -d "$SPLUNK_HOME/etc/apps/TA-nmon-hec" ) {
        $APP = "$SPLUNK_HOME/etc/apps/TA-nmon-hec";
    }
    elsif ( -d "$SPLUNK_HOME/etc/slave-apps/TA-nmon-hec" ) {
...
if ( !-d "$APP" ) {
    print(
"\n$time ERROR: The Application root directory could not be found, is the TA-nmon/TA-nmon-hec installed ?\n"
    );
    die;
}
