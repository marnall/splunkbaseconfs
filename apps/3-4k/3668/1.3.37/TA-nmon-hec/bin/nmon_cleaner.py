...
if not APP:
    ...
    if is_windows:
        ...
        TA_NMON_HEC_APP = SPLUNK_HOME + '\\etc\\apps\\TA-nmon-hec'
    else:
        TA_NMON_APP = SPLUNK_HOME + '/etc/apps/TA-nmon'
        TA_NMON_APP_HEC = SPLUNK_HOME + '/etc/apps/TA-nmon-hec'

    if is_windows:
        TA_NMON_APP_CLUSTERED = SPLUNK_HOME + '\\etc\\slave-apps\\TA-nmon'
        TA_NMON_APP_CLUSTERED_HEC = SPLUNK_HOME + '\\etc\\slave-apps\\TA-nmon-hec'
    else:
        TA_NMON_APP_CLUSTERED = SPLUNK_HOME + '/etc/slave-apps/TA-nmon'
        TA_NMON_APP_CLUSTERED_HEC = SPLUNK_HOME + '/etc/slave-apps/TA-nmon-hec'
    ...
    elif os.path.exists(TA_NMON_APP_HEC):
        APP = TA_NMON_APP_HEC

    elif os.path.exists(TA_NMON_APP_CLUSTERED_HEC):
        APP = TA_NMON_APP_CLUSTERED_HEC

    else:
        msg = 'The Application root directory could not be found, is the TA-nmon/TA-nmon-hec installed ?'
        logging.error(msg)
        sys.exit(1)
