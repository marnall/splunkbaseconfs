'''
    Author: John Southerland josouthe@cisco.com +1.214.734.8099 (AppDynamics Field Architect)
    Jan 30 2025: First version, retrieve Application Snapshots with deeplinks to the snapshot
'''
import concurrent.futures
import json
import os

bin_dir = os.path.dirname(os.path.abspath(__file__))
import sys
import import_declare_test
from splunklib import modularinput as smi
from splunktaucclib.modinput_wrapper import base_modinput as base_mi


class ModInputappdynamics_application_snapshots(base_mi.BaseModInput):

    def __init__(self):
        use_single_instance = False
        super(ModInputappdynamics_application_snapshots, self).__init__("splunk_ta_appdynamics", "appdynamics_application_snapshots",
                                                                 use_single_instance)
        self.global_checkbox_fields = None

    def get_scheme(self):
        scheme = super(ModInputappdynamics_application_snapshots, self).get_scheme()
        scheme.title = ("Application Snapshots")
        scheme.description = ("AppDynamics Application Snapshots Import")
        scheme.use_external_validation = True
        scheme.streaming_mode_xml = True
        scheme.add_argument(smi.Argument("name", title="Name",
                                         description="",
                                         required_on_create=True))
        scheme.add_argument(smi.Argument("global_account", title="Global Account",
                                         description="Select the account to be used to authenticate to AppDynamics.",
                                         required_on_create=True,
                                         required_on_edit=False))
        scheme.add_argument(smi.Argument("duration", title="Time Duration (in minutes)",
                                         description="The time period (in minutes) that you wish to retrieve data for.  e.g.:  5 = retrieve data for the past 5 minutes. [5-60]",
                                         required_on_create=True,
                                         required_on_edit=False))
        scheme.add_argument(smi.Argument("application_list", title="Applications to Collect Metrics for",
                                         description="select the applications",
                                         required_on_create=False,
                                         required_on_edit=False))
        scheme.add_argument(smi.Argument("metrics_to_collect", title="Metrics to Collect",
                                         description="select the metrics you want to ingest",
                                         required_on_create=True,
                                         required_on_edit=False))

        return scheme

    def get_app_name(self):
        return "Splunk_TA_AppDynamics"

    def get_account_fields(self):
        account_fields = []
        return account_fields

    def get_checkbox_fields(self):
        checkbox_fields = [
            'need_props',
            'need_exit_calls',
            'first_in_chain',
            'archived'
        ]
        return checkbox_fields

    def get_global_checkbox_fields(self):
        if self.global_checkbox_fields is None:
            checkbox_name_file = os.path.join(bin_dir, 'global_checkbox_param.json')
            try:
                if os.path.isfile(checkbox_name_file):
                    with open(checkbox_name_file, 'r') as fp:
                        self.global_checkbox_fields = json.load(fp)
                else:
                    self.global_checkbox_fields = []
            except Exception as e:
                self.log_error('Get exception when loading global checkbox parameter names. ' + str(e))
                self.global_checkbox_fields = []
        return self.global_checkbox_fields

    def validate_input(helper, definition):
        pass

    def collect_events(helper, ew):
        from controller_service import ControllerService
        from splunk_service import SplunkService
        from ucc_utils import Util
        from appdynamics_utils import normalize_controller_url
        import json

        helper.log_info("Collecting Snapshot metrics")


        #  Process each account input in inputs.conf separately
        #  Get the properties for each input (stanzas in inputs.conf)
        stanzas = helper.input_stanzas
        for stanza_name in stanzas:
            opt_duration = helper.get_arg('duration')
            if type(opt_duration) == dict:
                opt_duration = opt_duration[stanza_name]
            opt_global_account = helper.get_arg('global_account')
            opt_metrics = helper.get_arg('metrics_to_collect')
            if type(opt_metrics) == dict:
                opt_metrics = opt_metrics[stanza_name]
            opt_app_list = helper.get_arg('application_list')
            if type(opt_app_list) == dict:
                opt_app_list = opt_app_list[stanza_name]
            opt_need_props = helper.get_arg('need_props')
            if type(opt_need_props) == dict:
                opt_need_props = opt_need_props[stanza_name]
            opt_need_props = bool(opt_need_props)
            opt_need_exit_calls = helper.get_arg('need_exit_calls')
            if type(opt_need_exit_calls) == dict:
                opt_need_exit_calls = opt_need_exit_calls[stanza_name]
            opt_need_exit_calls = bool(opt_need_exit_calls)
            opt_first_in_chain = helper.get_arg('first_in_chain')
            if type(opt_first_in_chain) == dict:
                opt_first_in_chain = opt_first_in_chain[stanza_name]
            opt_first_in_chain = bool(opt_first_in_chain)
            opt_archived = helper.get_arg('archived')
            if type(opt_archived) == dict:
                opt_archived = opt_archived[stanza_name]
            opt_archived = bool(opt_archived)
            opt_execution_time_in_milis = helper.get_arg('execution_time_in_milis')
            if type(opt_execution_time_in_milis) == dict:
                opt_execution_time_in_milis = opt_execution_time_in_milis[stanza_name]
            appd_controller_url = normalize_controller_url(opt_global_account['appd_controller_url'])
            idx = Util.get_output_index(helper, stanza_name)
            if type(idx) == dict:
                idx = idx[stanza_name]
            st = helper.get_sourcetype()
            if type(st) == dict:
                st = st[stanza_name]

        helper.log_info(f"Applications: {opt_app_list} Snapshots: {opt_metrics}")
        helper.log_debug(f"controller: {appd_controller_url} account: {opt_global_account}")
        max_workers = Util.get_max_workers(helper.context_meta["session_key"])

        controller = ControllerService(
            helper=helper,
            global_account=opt_global_account,
            duration=opt_duration,
            session_key=helper.context_meta["session_key"],
            source=helper.get_arg('name')
        )

        splunk = SplunkService( helper, idx, st, ew)

        ''' end of declarations, followed by the actual code'''

        def get_section_as_list(data):
            # This, this is why i hate python
            if not isinstance(data, list):
                data = [data]  # Force it into a list if it's a single item
            return data

        def get_app_name(application_name):
            return application_name.split('|', 2)[0]

        def get_app_id(application_name):
            return application_name.split('|', 2)[1]


        '''The logic is all below here....'''
        if not opt_app_list:
            opt_app_list = []
            for app in get_section_as_list(controller.get_all_app_list()["apmApplications"]):
                if app["active"]:
                    opt_app_list.append(f"{app['name']}|{app['id']}")
        opt_app_list = get_section_as_list(opt_app_list)
        opt_metrics = get_section_as_list(opt_metrics)
        options = {}
        user_exp = []
        for opt in opt_metrics:
            user_exp.append(opt.upper())
        options['user_experience'] = user_exp
        options['need_props'] = opt_need_props
        options['need_exit_calls'] = opt_need_exit_calls
        options['first_in_chain'] = opt_first_in_chain
        options['archived'] = opt_archived
        options['execution_time_in_milis'] = opt_execution_time_in_milis
        def process_snapshot_query(app):
            try:
                app_name = get_app_name(app)
                app_id = get_app_id(app)
                for snapshot in controller.get_snapshots_by_app(app_id, options):
                    helper.log_debug(f"Snapshot Data: {json.dumps(snapshot)}")
                    splunk.send_data("appdynamics_snapshots", snapshot)
            except Exception as e:
                splunk.log_exception(e)
                helper.log_warning("Error fetching snapshot data for %s: %s", app_id, e)

        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                results = list(executor.map(process_snapshot_query, opt_app_list))
        except Exception as e:
            splunk.log_exception(e)

        splunk.log_events_ingested()
        helper.log_info("Completed Collecting Application Snapshots")


if __name__ == "__main__":
    exitcode = ModInputappdynamics_application_snapshots().run(sys.argv)
    sys.exit(exitcode)
