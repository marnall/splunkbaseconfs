...
logger = log.Logs().get_logger("appdynamics_list_security_applications")
...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):  # pylint: disable=invalid-name
        ...
        logger.debug("Running helper to get list of security applications")
        ...
        try:
            applications = controller.get_application_security_list()
        except Exception as e:
            confInfo.addErrorMsg(f"Could not get list of security applications, check {controller.get_client_id()} has permissions needed")
            raise Exception(f"Could not get list of security applications, check {controller.get_client_id()} has permissions needed")

        for app in get_section_as_list(applications):
            logger.debug(f"app: {app}")
            confInfo[f"{app['applicationName']}"].append('label', f"{app['applicationName']}")
