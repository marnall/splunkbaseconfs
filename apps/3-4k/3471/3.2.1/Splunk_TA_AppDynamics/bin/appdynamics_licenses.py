'''
    ...
    Feb 14 2025: First version, retrieve license entitlements and usage data
'''
import json
import os
...
class ModInputappdynamics_licenses(base_mi.BaseModInput):

    def __init__(self):
        use_single_instance = False
        super(ModInputappdynamics_licenses, self).__init__("splunk_ta_appdynamics", "appdynamics_licenses",
                                                                        use_single_instance)
        self.global_checkbox_fields = None

    def get_scheme(self):
        scheme = super(ModInputappdynamics_licenses, self).get_scheme()
        scheme.title = ("Controller License Usage")
        scheme.description = ("AppDynamics Controller License Usage")
    ...
    def collect_events(helper, ew):
        ...
        helper.log_info("Collecting Controller License Usage")
        ...
        try:
            for data in controller.get_abl_license_usage():
                helper.log_debug(f"ABL License Usage Data: {json.dumps(data)}")
                splunk.send_data("appdynamics_abl_licenses", data)
            for data in controller.get_ibl_license_usage():
                helper.log_debug(f"IBL License Usage Data: {json.dumps(data)}")
                splunk.send_data("appdynamics_ibl_licenses", data)
        except Exception as e:
            splunk.log_exception(e)

        splunk.log_events_ingested()
        helper.log_info("Completed Collecting Controller License Usage")


if __name__ == "__main__":
    exitcode = ModInputappdynamics_licenses().run(sys.argv)
    sys.exit(exitcode)
