...
from netskope_utils import send_notification, check_input_config, read_conf_file
...
def check_existing_CSV_input(helper):
    ...
    check_input_config(
        ...
        stanza_to_search="netskope_alerts_v2_csv://",
        is_event=False,
        is_csv_input=False
    )
...
def collect_events(helper, ew):
    ...
    try:
        ...
        ec = netskope_iterator_collector.AlertsCollector(helper, ew)
        workers = []

        lock = threading.Lock()
        for alert_type in alert_types:
            netskope_iterator_collector.event_retry_count[alert_type] = 0
            t = threading.Thread(target=netskope_iterator_collector.BaseCollector.thread_runner, name="thread-{}".format(alert_type), args=(alert_type, ec, hist_type_complete, lock, workers))
            t.daemon = True
            t.start()
            workers.append(t)
        ...
        end_datetime = helper.get_arg("end_datetime")
        if end_datetime is not None:
            if hist_type_complete[0] == len(alert_types):
                message = "App: '{}' Input: '{}'. Historical data collection has been completed. Please disable this input.".format(
