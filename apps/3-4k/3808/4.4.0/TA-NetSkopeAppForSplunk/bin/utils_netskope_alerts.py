...
class NetskopeAlertsModel(DataInputModel):
    ...
    def validate(self, name, data, existing=None):
        ...
        if data['alert_type'].strip() == "":
            data['alert_type'] = data['alert_type'].strip()
        ...
        if data.get('collection_type') == 'realtime':
            ...
            if end_datetime < start_datetime:
                raise RestError(
                    ...
                    "End Datetime should be greter than Start Datetime. Please enter valid timerange."
                )
        ...
        super(NetskopeAlertsModel, self).validate(name, data, existing)
