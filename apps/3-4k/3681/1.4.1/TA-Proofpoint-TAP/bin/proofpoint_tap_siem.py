...
_LOGGER = logger_manager.setup_logging("ta_proofpoint_tap")
...
class PfptSiemScript(Script):
    """Main class for Proofpoint TAP SIEM."""
    ...
    INPUT_TYPE = "proofpoint_tap_siem"
    VALIDATE_SSL = True
    ...
    def encrypt_password(self, username, password, input_name):
        ...
        try:
            ...
            _LOGGER.info("{}: Credentials updated successfully.".format(input_name))
    ...
    def mask_password(self, location, input_name):
        ...
        try:
            for input_def in self.admin_client.inputs.list(self.INPUT_TYPE):
                ...
                new_input.refresh()
                _LOGGER.info("{}: inputs.conf updated successfully.".format(input_name))
                break
        except Exception as e:
            raise ValueError('An error occured while updating inputs.conf: {}'.format(str(e)))
    ...
    def get_password(self, username):
        ...
        try:
            ...
            raise ValueError(
                'An error occured while fetching credentials from storage/passwords service: {}'.format(str(e)))
    ...
    def set_authentication_information(self, items):
        ...
        proxy_protocol = items.get('proxy_protocol', None)
    ...
    def validate_input(self, definition):
        ...
        try:
            ...
            proxy_protocol = definition.parameters.get('proxy_protocol', None)
            ...
            if proxy_port is not None and proxy_port.strip() == "":
                proxy_port = None
            ...
            if resp.status_code not in (200, 204):
                raise ValueError(
                    'Could not access TAP SIEM using provided credentials. Status code: {} : {}  '.format(
                        str(resp.status_code), resp.reason))
        except Exception as e:
            _LOGGER.error("{}: {}".format(input_name, str(e)))
            raise ValueError(e)
    ...
    def retrieve_last_poll_time(self):
        ...
        if last_poll_time_dt < self.max_past:
            _LOGGER.warning(
                "{}: Previous poll time {}, is too far in the past. Returning maximum data available.".format(
                    self.input_name, last_poll_time_dt))
            return self.max_past
    ...
    def query_and_save(self, start_time_s, end_time_s):
        """
        Performs a single request to the TAP SIEM API, using any configured proxy.
        ...
        if resp.status_code == 204:
            _LOGGER.info("{}: Empty content returned from {} to {}".format(self.input_name,
                                                                           start_time_s, end_time_s))
        elif resp.status_code != 200:
            raise Exception("Querying from {} to {}, but SIEM server returned status code: {}, messgae: {}"
                            .format(start_time_s, end_time_s, resp.status_code, resp.reason))

        _LOGGER.info("{}: Successful query from {} to {}".format(self.input_name, start_time_s,
                                                                 end_time_s))
    ...
    def save_events(self, data):
        ...
        _LOGGER.info("{}: Collected {} events.".format(self.input_name, event_count))
    ...
    def stream_events(self, inputs, ew):
        ...
        for full_input_name, items in inputs.inputs.items():
            ...
            try:
                ...
                _LOGGER.error("{}: {}".format(self.input_name, e))
                continue
        _LOGGER.info("Collected a total of {} events for {}".format(
            str(self.total_event_count), self.input_name))
        _LOGGER.info("Completed data collection for {} in {} seconds.".format(
            self.input_name, str(time() - start_time)))
    ...
    def download_intervals(self):
        ...
        if final_time_dt <= initial_time_dt:
            raise ValueError(
                'End of interval must be after start of interval. Interval Range : {} to {}'.format(
                    initial_time_dt, final_time_dt))
        if (final_time_dt - initial_time_dt) > timedelta(hours=12):
            raise ValueError('Interval cannot be greater than 12 hours. Interval Range : {} to {}'.format(
                initial_time_dt, final_time_dt))
    ...
    def get_proxies(
        ...
        if proxy_server and proxy_port:
            uri = "{0}:{1}".format(proxy_server, proxy_port)
            if proxy_username and proxy_password:
                ...
                uri = "{0}://{1}".format(proxy_protocol, uri)
        if uri:
