...
class FSSplunkRestApiWrapper(object):
	...
	def updateIndexMacrosConf(self, index):
		self.logger.info('Updating macros.conf in app: [' + fsct_defaults.FS_APP_NAME + '] with index: [' + index + ']')
		macrovalue = 'index="' + index + '"'
		request    = six.moves.urllib.request.Request(self.local_server + '/servicesNS/nobody/' + fsct_defaults.FS_APP_NAME + '/properties/macros/get_index/', data = six.moves.urllib.parse.urlencode({ 'definition' : macrovalue }).encode('utf-8'), headers = { 'Authorization' : ('Splunk %s' % self.session_key) })
