...
class TAVMwareHydraWorker(HydraWorker):
	...
	def loginToTarget(self, target, user, password, realm=None):
		...
		for retry_count in range(4):
			if Connection.update_connection(target, username=user, password=password, wsdl_cache_dir=self.wsdl_cache_dir()):
	...
	def isSessionValid(self, session):
		...
		if Connection.update_connection(target, session_key=session_key, cookie=cookie, wsdl_cache_dir=self.wsdl_cache_dir()):
	...
	def wsdl_cache_dir(self):
		"""
		Returns the directory to cache WSDL files in.
		:return:
		"""
		return make_splunkhome_path(['etc', 'apps', self.app, 'local', 'wsdl_cache'])
