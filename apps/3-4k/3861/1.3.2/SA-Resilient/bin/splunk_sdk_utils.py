...
class SplunkSdkUtils:
    ...
    def map_config_result(self, config, result):
        ...
        #
        tokens = re.findall(r"\$(.+?)\$", mapping_str)
        ...
        try:
            ...
            self.logger.error(u"Unable to convert string {} back to dict in result.".format(mapping_str))
            ret_mapping = mapping
    ...
    def map_modular_action_fields(self, modular_action, config):
        ...
        try:
            ...
            self.logger.error(u"Unable to convert string {} back to dict in modular action.".format(mapping_str))
            ret_mapping = config
    ...
    def map_search_information(self, search_info, config):
        ...
        try:
            ...
            self.logger.error(u"Unable to convert string {} back to dict in search.".format(mapping_str))
    ...
    def map_substitute_dollar(self, config):
        ...
        try:
            ...
            self.logger.error(u"Unable to convert string {} back to dict after '$' substituion.".format(mapping_str))
            # if there is an error, return the original (unmapped) config
            ret_mapping = config
