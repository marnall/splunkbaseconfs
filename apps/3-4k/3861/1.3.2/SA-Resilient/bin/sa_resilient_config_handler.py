...
from app_constants import SA_APP_NAME, SA_ACTION_NAME

logger = ModularAction.setup_logger("resilient_config_handler")
...
class ResilientConfigHandler(PersistentServerConnectionApplication):
    """
    ...
    def handle(self, in_string, **kwargs):
        ...
        setup_data['verify'] = self.stringToBool(setup_data['verify'])   
        ...
        try:
            # test the connection to Resilient
            self.testConnection(setup_data, password)
        except requests.exceptions.SSLError as e:
            # Log the error, post a message to splunk, and report a failure to the UI
            logger.error("Resilient connection failed due to SSL Error.")
            logger.error("Verify from config resulted in verify parameter [%s]",
                         str(setup_data['verify']))
            logger.error(traceback.format_exc())
            msg = {"name":"Resilient configuration not saved due to SSLError when connecting to Resilient host."}
            self.service.post(self.MSG_URL, name=self.APP_NAME, value=msg)

            text = "Resilient connection test failed.\n" + msg["name"] + "\nConsult $SPLUNK_HOME/var/log/splunk/resilient_config_handler.log for details."
            return self.formatResponse(False, text)

        except Exception as e:
            # Log the error, post a message to splunk, and report a failure to the UI
            logger.error("Resilient connection verification failed: " + str(e))
            logger.error(traceback.format_exc())
            msg = {"name": "Verify your credentials are correct."}
            self.service.post(self.MSG_URL, name=self.APP_NAME, value=msg)
            
            text = "Resilient connection test failed due to an exception.\n" + msg["name"] + "\nConsult $SPLUNK_HOME/var/log/splunk/resilient_config_handler.log for details."
            return self.formatResponse(False, text)

        # gather the supported artifact types
        conf_data = self.splunksdk.getConfig(self.service)
        artifact_types = self.parseArtifactTypes(conf_data)
        ...
        if "properties.splunk_notable_event_id" not in defs:
            field_msg = """Warning: splunk_notable_event_id custom field not present in Resilient field definitions!\n
        ...
        html = self.resilient_client.generate_alert_html(self.ACTION_NAME,
							 defs,
                                                         int(setup_data['num_artifacts']),
                                                         artifact_types)
        ...
        text = "Successfully connected to Resilient. Configuration saved."
        if not field_use:
            text += "\n\n" + field_msg
    ...
    def testConnection(self, conf, password):
        ...
        if useAPIKeys:
            # connect to Resilient with API keys
            success = self.resilient_client.connect(conf, conf['verify'], None, password)

        else:
            # connect to Resilient with username and password
            success = self.resilient_client.connect(conf, conf['verify'], password)

        return success

    def getSplunkClient(self, sessionKey):
        #inststantiate a splunk client
        service = client.connect(token=sessionKey, autologin=True, app=self.APP_NAME)
        return service
    
    def getPassword(self, password_id):
        # get the secret stored for sa_resilient admin
    ...
    def parseArtifactTypes(conf_data):
        ...
        """
        artifact_types = []
        for row in csv.reader(conf_data['artifact_types'].splitlines(), skipinitialspace=True):
            artifact_types += row
    ...
    def stringToBool(string):
        """
        covnert true/false, 1/0 string inputs to a boolean variable
        """
    ...
    def formatResponse(success, text, status=200):
        ...
        :param text: string detailing reason for success/failure. Displyed by browser in most cases.
