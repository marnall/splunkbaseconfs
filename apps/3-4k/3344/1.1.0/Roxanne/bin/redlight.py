...
def set_color_red_alert(access_token, lightid):
        req = requests.put('https://api.lifx.com/v1/lights/'+str(lightid)+'/state', data={"power": "on", "color": "red saturation:0.5", "brightness": 0.5}, headers={'Authorization': 'Bearer '+access_token})
        return req
...
class LIFXRedAlertCommand(StreamingCommand):
    """ %(synopsis)
    ...
    def stream(self, events):
       ...
       for event in events:
		    ...
		    #Create a new process for each access_token
                    set_color_red_alert(token,self.lightid)
            yield event

dispatch(LIFXRedAlertCommand, sys.argv, sys.stdin, sys.stdout, __name__)
