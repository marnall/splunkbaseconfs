...
def set_color_green_alert(access_token, lightid):
        req = requests.put('https://api.lifx.com/v1/lights/'+str(lightid)+'/state', data={"power": "on", "color": "green saturation:0.5", "brightness": 0.5}, headers={'Authorization': 'Bearer '+access_token})
        return req
...
class LIFXGreenAlertCommand(StreamingCommand):
    """ %(synopsis)
       ...
       for event in events:
            ...
            for item in settings.iteritems():
                for key in item[1].iteritems():
                    ...
                    set_color_green_alert(token,self.lightid)
            yield event

dispatch(LIFXGreenAlertCommand, sys.argv, sys.stdin, sys.stdout, __name__)
