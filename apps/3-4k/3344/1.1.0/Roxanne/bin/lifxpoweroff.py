...
def power_off_devices(access_token, lightid):
        req = requests.put('https://api.lifx.com/v1/lights/'+str(lightid)+'/state', data={'power' : 'off'}, headers={'Authorization': 'Bearer '+access_token})
        return req
@Configuration(local=True)
class LIFXPowerOffCommand(StreamingCommand):
    """ %(synopsis)
       ...
       for event in events:
            ...
            for item in settings.iteritems():
                for key in item[1].iteritems():
                    ...
                    power_off_devices(token,self.lightid)
            yield event

dispatch(LIFXPowerOffCommand, sys.argv, sys.stdin, sys.stdout, __name__)
