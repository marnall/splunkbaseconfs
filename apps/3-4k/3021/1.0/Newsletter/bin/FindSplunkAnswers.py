...
for url in data:
	#print "(1) I am pulling in URL: " + url
	r = requests.get(url)
	h = HTMLParser.HTMLParser()
...
for url in answersURLs:
	#print "(2) I am pulling in URL: " + url
	r = requests.get(url)
	h = HTMLParser.HTMLParser()
	title = ""
	date = ""
	score = ""
	#print r.content + "\n\n**********************\n"
	match = re.search(r"\<title\>\s+(?P<title>.*?)\W*Question | Splunk Answers", r.content, re.DOTALL) 
	if match:
		#print "Got a match!"
		title = match.group('title')
	#	print "Title: " + title

	match = re.search(r"Asked:.*?(?P<date>\w* \d*(, \d*)? at \d*:\d* \w*)", r.content, re.DOTALL) 
	if match:
		#print "Got a match!"
		date = match.group('date')
	#	print "Date: " + date

	match = re.search(r"class=\"score[^\"]*\">(?P<score>\d*)", r.content, re.DOTALL) 
	if match:
