...
import modalert_awake_get_ip_detail_helper

class AlertActionWorkerawake_get_ip_detail(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerawake_get_ip_detail, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_awake_get_ip_detail_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerawake_get_ip_detail("TA-awake-security-splunk-ar-add-on", "awake_get_ip_detail").run(sys.argv)
    sys.exit(exitcode)
