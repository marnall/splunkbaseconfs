...
class EMSendEmailAlertAction(AbstractCustomAlertAction):
    ...
    def make_email_body(self, result, payload):
        ...
        if managed_by_type == 'entity':
            ...
            group_definition_section = GROUP_DEFINITION_HTML.format(group_filter=group_filter)
    ...
    def _make_alert_url(self, managed_by_type, managed_by_id, alert_name):
        ...
        # hostname in em_send_email takes precedence over email
        em_send_email_stanza = self.get_alert_action_setting('em_send_email')
        custom_hostname = em_send_email_stanza.get('hostname', self.ssContent.get('hostname'))
    ...
    def execute(self, results, payload):
        ...
        self.ssContent = self.ssContent if self.ssContent else self.get_alert_action_setting('email')
        ...
        for row in results:
            ...
            if state_change != 'no' and state_change in email_states \
                    and row.get('current_state') != 'None':
