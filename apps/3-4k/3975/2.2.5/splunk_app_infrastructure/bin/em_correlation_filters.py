...
def deserialize(data, **kwargs):
    ...
    return BooleanFilter(filter_type,
                         [deserialize(f, **kwargs) for f in params.get('filters', [])])
    ...
    if isinstance(cor_filter, BasicFilter):
        ...
        if len(cor_filter.filters) == 1:
            ...
            if cor_filter.type == BooleanFilter.ALLOWED_BOOL_TYPE[0]:
                compressed_filters = [cf for cf in [compress(inside_filter)
                                                    for inside_filter in cor_filter.filters]
                                      if cf is not None]
                field_dict = {}
                for f in compressed_filters:
                    if isinstance(f, BasicFilter):
                        field_dict.setdefault((f.field['name'], f.type), []).extend(f.values)
                    else:
                        new_filters.append(f)
                for field_and_type, values in field_dict.items():
            ...
            # if it's an AND filter -- compress the filters inside and don't do anything
                ...
                new_filters = [cf for cf in [compress(inside_filter)
                                             for inside_filter in cor_filter.filters]
                               if cf is not None]
        ...
        raise ValueError("Unknown filter type: \'%s\'" % type(cor_filter))
    ...
    def __eq__(self, other):
        return isinstance(other, BooleanFilter) and \
            ...
            reduce(lambda a, b: a and b,
                   [self.filters[i] == other.filters[i] for i in range(len(self.filters))])
...
class EntityFilter(object):
    ...
    def __init__(self,
        ...
        :param char_replacement_rules: list - an array of arrays where each sub-array
               contain two elements [<old_string>, <new_string>], used to replace the appearances of <old_string> with
               <new_string> in the dimension value (splitted if `multi_value_delim` is also defined) of an entity
        :param multi_value_delim: string - if not None, meaning the dimension of the entity is supposed to be treated
               as a multi-value field delimited by the input delimiter
        """
        ...
        self.multi_value_delim = multi_value_delim
    ...
    def convert_basic(self, entity):
        ...
        values = em_common.always_list(dimensions.get(self.dimension_name))

        if self.multi_value_delim is not None:
            multi_vals_list = [v.split(self.multi_value_delim) for v in values]
            values = []
            for multi_vals in multi_vals_list:
        ...
        if self.char_replacement_rules_dict:
            for old, new in self.char_replacement_rules_dict.items():
...
def create_entity_log_filter(entity, related_collectors):
    ...
    for c in related_collectors:
        event_correlators = c.correlation_rules
        or_correlator = BooleanFilter('or')
        for correlator_name, correlator_data in event_correlators.items():
    ...
    for c in related_collector:
        ...
        for correlator_name, correlator_data in event_correlators.items():
            if correlator_data:
                ...
                for entity in collector_entities_match.get(collector_name, []):
                    # set up entity filter
                    entity_filters_raw = correlator_data['entity_filters']
                    entity_filter = deserialize(entity_filters_raw, entity=entity)
