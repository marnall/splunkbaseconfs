...
import modalert_zenith_unquarantine_helper

class AlertActionWorkerzenith_unquarantine(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerzenith_unquarantine, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            ...
            if not self.validate_params():
                return 3 
            status = modalert_zenith_unquarantine_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerzenith_unquarantine("TA-ziften", "zenith_unquarantine").run(sys.argv)
    sys.exit(exitcode)
