...
import modalert_zenith_kill_process_by_pid_helper

class AlertActionWorkerzenith_kill_process_by_pid(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerzenith_kill_process_by_pid, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            ...
            if not self.validate_params():
                return 3 
            status = modalert_zenith_kill_process_by_pid_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerzenith_kill_process_by_pid("TA-ziften", "zenith_kill_process_by_pid").run(sys.argv)
    sys.exit(exitcode)
