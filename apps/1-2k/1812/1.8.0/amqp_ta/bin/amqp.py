...
def update_xml_input(xml_str):
    ...
    for i, c in credentials_list:
        if c['eai:acl']['app'] ==  "amqp_ta":
          amqp_username = c['username']
          if not amqp_username == "activation_key":
              amqp_password = c['clear_password']
              replace_key='<param name="username">%s</param>' % amqp_username
              replace_key_with_elements = '%s<param name="password">%s</param>' % (replace_key,amqp_password)
