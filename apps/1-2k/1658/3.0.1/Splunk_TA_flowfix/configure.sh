...
BINARY_DIR="$TA_PATH"/nfdump-binary
ASCII_DIR="$TA_PATH"/nfdump-ascii
...
rm -rf "$TA_PATH"/bin/*.pid

# how long to keep the ascii flows
...
do
   printf "\nListener #"$n"\n"
   printf "\nNetflow [v5], [v7], [v9] or [IPFIX] [ Default: v5 ]: "
   read flowType
   if [[ -z "$flowType" ]]; then
        flowType="v5"
   fi
   ...
   printf ""$TA_PATH"/bin/"$OS"_"$CPU_TYPE"_"$CPU_ARCH"/nfcapd -p "$portNum" "$bindIP" -T all -t "$rollSeconds" -l "$TA_PATH"/nfdump-binary/"$portNum" -P "$TA_PATH"/bin/nfcapd-"$n"-"$flowType"-"$bindIP"-"$portNum".pid -D\n" >> "$TA_PATH"/bin/flowfix.sh
   printf "fi\n" >> "$TA_PATH"/bin/flowfix.sh

   mkdir "$BINARY_DIR"/"$portNum"
done
...
printf "\nfind "$TA_PATH"/nfdump-ascii -type f -mtime +"$keepDays" -exec rm -f {} \\;" >> "$TA_PATH"/bin/flowfix.sh 

# write out TA-flowfix/local/inputs.conf
printf "[monitor://"$TA_PATH"/nfdump-ascii]\n" > "$TA_PATH"/local/inputs.conf
...
printf "interval = 60\n"  >> "$TA_PATH"/local/inputs.conf
printf "sourcetype = netflow\n"  >> "$TA_PATH"/local/inputs.conf
printf "disabled = false"  >> "$TA_PATH"/local/inputs.conf
