...
function crude_urlencode()
# encode reservers/unsafe url/bash characters
{
    echo "$1" | sed -e '{
s/\//%252F/g
s/\*/%2A/g
...
# build a command name suitable for use in a REST target
{
    ...
    echo ${1/$(dirname $cmd)\//.%252Fbin%252F}
}
...
{
    url_name=$(crude_urlencode "$1")
    if [ $remote_server_uri != "false" ]; then
        echo `$SPLUNK_HOME/bin/splunk _internal call /servicesNS/nobody/$remote_server_app_name/data/inputs/script/$url_name -uri $remote_server_uri | grep "name=\"$2" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//' -e "s/<s:key name=\"$2\">//" -e 's/<\/s:key>//g'`
    else
        echo `$SPLUNK_HOME/bin/splunk _internal call /servicesNS/nobody/$server_app_name/data/inputs/script/$url_name | grep "name=\"$2" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//' -e "s/<s:key name=\"$2\">//" -e 's/<\/s:key>//g'`
    fi
}
...
# given an endpoint and a post string, set the value
{
    ...
    url_name=$(crude_urlencode "$1")
    if [ $remote_server_uri != "false" ]; then
        setter_response=`$SPLUNK_HOME/bin/splunk _internal call /servicesNS/nobody/$remote_server_app_name/data/inputs/script/$url_name -uri $remote_server_uri -post:$2 $3`
    else
        setter_response=`$SPLUNK_HOME/bin/splunk _internal call /servicesNS/nobody/$server_app_name/data/inputs/script/$url_name -post:$2 $3`
    fi
...
# given a script name, enable it
{
    ...
    url_name=$(crude_urlencode "$1")
    if [ $remote_server_uri != "false" ]; then
        enable_response=`$SPLUNK_HOME/bin/splunk _internal call /servicesNS/nobody/$remote_server_app_name/data/inputs/script/$url_name/enable -uri $remote_server_uri -method POST`
    else
        enable_response=`$SPLUNK_HOME/bin/splunk _internal call /servicesNS/nobody/$server_app_name/data/inputs/script/$url_name/enable -method POST`
    fi
    handle_rest_response "$enable_response" "enable"
}
...
# given a script name, disable it
{
    ...
    url_name=$(crude_urlencode "$1")
    if [ $remote_server_uri != "false" ]; then
        disable_response=`$SPLUNK_HOME/bin/splunk _internal call /servicesNS/nobody/$remote_server_app_name/data/inputs/script/$url_name/disable -uri $remote_server_uri -method POST`
    else
        disable_response=`$SPLUNK_HOME/bin/splunk _internal call /servicesNS/nobody/$server_app_name/data/inputs/script/$url_name/disable -method POST`
    fi
    handle_rest_response "$disable_response" "disable"
}
...
{
    if [ $remote_server_uri != "false" ]; then
       echo "$($SPLUNK_HOME/bin/splunk list exec -uri "$remote_server_uri" | sed -ne 's/^Command:\s*\(.*\)\s*$/\1/p')"
    else
       echo "$($SPLUNK_HOME/bin/splunk list exec | sed -ne 's/^Command:\s*\(.*\)\s*$/\1/p')"
    fi
}
...
# if enabled show the interval and last run time 
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > SHOW INPUT STATUS ***"
    echo ""
    input_counter=0
    ...
    for line in $script_list; do
        case "$line" in 
           *Splunk_TA_hadoopops* ) get_scripted_input_status "$line"; input_counter=`expr $input_counter + 1`;
        esac
    done
    IFS=$oldIFS
    echo ""
...
#enables all endpoints
{
    ...
    for line in $script_list; do
        case "$line" in 
           *Splunk_TA_hadoopops* ) echo "enabling $line"; input_endpoint=$(build_scripted_input_endpoint "$line"); enable_scripted_input "$input_endpoint";;
        esac
    done
    IFS=$oldIFS
    set -f
...
# disables all inputs
{
    ...
    for line in $script_list; do
        case "$line" in 
            *Splunk_TA_hadoopops* ) echo "disabling $line"; input_endpoint=$(build_scripted_input_endpoint "$line"); disable_scripted_input $input_endpoint;;
        esac
    done
    IFS=$oldIFS
    set -f
...
# clone all inputs from local to remote_server_uri
{
    if [ $_remote_server_uri == "false" ]; then
        ...
        for line in $script_list; do
            case "$line" in
                *Splunk_TA_hadoopops* ) echo ""; echo "    cloning $line to $server_name"; echo ""; scripted_clone "$line" 
            esac
        done
        set -f
        for line in $MONITOR_INPUTS; do
...
# batch enable all inputs
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > ENABLE ALL INPUTS ***"
    echo ""
...
# batch disable all inputs
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > DISABLE ALL INPUTS ***"
    echo ""
    echo "You are currently managing Splunk server '$server_name'"
    echo ""
...
# confirm local to remote config copy
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > COPY LOCAL CONFIG TO REMOTE ***"
    echo ""
    echo "You are currently managing Splunk server '$server_name'"
    echo ""
...
# manage one input
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > CHOOSE INPUT TO MANAGE ***"
    echo ""
    echo "You are currently managing Splunk server '$server_name'"
    echo ""
    echo "--> Manage Input '$1'"
...
# choose one input, then enable/disable/change interval
{
    ...
    for line in $script_list; do
        case "$line" in 
           *Splunk_TA_hadoopops* ) echo " $input_counter - $line"; selection_list[$input_counter]=$line; input_counter=`expr $input_counter + 1`;
        esac
    done
    IFS=$oldIFS
    set -f
...
# the aptly named 'manage inputs' menu
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > MANAGE INPUTS ***"
    echo ""
    echo "You are currently managing Splunk server '$server_name'"
    echo ""
    echo "    Please choose from one of the following options:"
    ...
    if [ "$remote_server_uri" != "false" ] && [ "$server_hadoopopsfwd_app_installed" = "true" ]; then
...
# the aptly named install menu
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > INSTALL/UPGRADE MENU***"
    echo ""
    echo "You are currently managing Splunk server '$server_name'"
    echo ""
    echo " Please enter the full URI string indicating where the app resides"
    echo ""
    echo "   -> for example, 'https://localhost/apps/hadoopopsfwd_app_new.tgz'"
    echo ""
    echo -n "Enter URI: "
    read install_uri 
...
# the aptly named main menu
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > MAIN MENU ***"
    echo ""
    echo "You are currently managing Splunk server '$server_name'"
    echo ""
    echo "    Please choose from one of the following options:"
    echo ""
    echo "1 - show Splunk_TA_hadoopops input status"
    echo "2 - manage Splunk_TA_hadoopops inputs"
...
{
    if [ $remote_server_uri != "false" ]; then
        remote_server_hadoopopsfwd_app_installed="true"
        remote_server_app_name="$1"
    else
        server_hadoopopsfwd_app_installed="true"
        server_app_name="$1"
    fi
}
...
{
    if [ $remote_server_uri != "false" ]; then 
        if [ $remote_server_hadoopopsfwd_app_installed != "false" ]; then
            set_server_has_app_enabled
        else
            unset_server_has_app_enabled
        fi 
    else
        if [ $server_hadoopopsfwd_app_installed != "false" ]; then
            set_server_has_app_enabled
        else
            unset_server_has_app_enabled
        fi
...
{
    if [ $remote_server_uri != "false" ]; then
        remote_server_has_hadoopopsfwd_app_enabled="true"
    else
        server_has_hadoopopsfwd_app_enabled="true"
    fi
}
...
{
    if [ $remote_server_uri != "false" ]; then
        remote_server_has_hadoopopsfwd_app_enabled="false"
    else
        server_has_hadoopopsfwd_app_enabled="false"
    fi
}
...
function check_for_hadoopopsfwd_app
# can't manage the unix app if there is nothing to manage
{
    ...
    for line in $app_output; do
        case "$line" in  
           *Splunk_TA_hadoopops* ) set_app_installed "Splunk_TA_hadoopops";;
    ...
    if [ $remote_server_uri = "true" ]; then
        if [ $remote_server_has_hadoopopsfwd_app_enabled = "true" ]; then
            main_menu
        else
            ...
            case $want_install_app in
                y ) install_menu; check_for_hadoopopsfwd_app;; 
                * ) splunk_remote_logout; prerequisites;;
            esac
        fi
    else
        if [ $server_has_hadoopopsfwd_app_enabled = "true" ]; then
            main_menu
        else
...
# then route to main_menu
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > LOCAL LOGIN ***"
    echo ""
...
# then route to main_menu
{
    clear
    echo ""
    echo "*** Splunk> Splunk_TA_hadoopops command-line setup > REMOTE LOGIN ***"
    echo ""
...
# changing credentials on the same remote instance
{
    ...
    if [ "$?" = "0" ]; then
        ...
        remote_server_uri="false"
        remote_server_hadoopopsfwd_app_installed="false"
        remote_server_has_hadoopopsfwd_app_enabled="false"
        echo ""
...
# often followed by call to splunk_login
{
    ...
    remote_server_uri="false"
    server_name="false"
    server_hadoopopsfwd_app_installed="false"
    server_has_hadoopopsfwd_app_enabled="false"
    remote_server_hadoopopsfwd_app_installed="false"
    remote_server_has_hadoopopsfwd_app_enabled="false"
    clear
...
# executes a stored queue of command line options and arguments
{
    if [ ! -n "$__QUEUE" ]; then
        ...
        server_name=$(get_server_name)
        check_for_hadoopopsfwd_app   cli
        echo ""
        echo " authenticated to $server_name"
        echo "" 
        for key in ${!__QUEUE[@]}; do
            execute_command "${__QUEUE[$key]}"
        done
    fi
}
...
{
    __QUEUE+=("$ACTION $ACTION_TARGET")
}

### MAIN ###

. `dirname $0`/common.sh
. `dirname $0`/.introspect
