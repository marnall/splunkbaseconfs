...
LOG_FILENAME = os.path.join(SPLUNK_HOME,"var","log","splunk","mqttmodinput_app_setuphandler.log")

# Set up a specific logger
logger = logging.getLogger('mqttmodinput')
...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        try:
            ...
            entities = entity.getEntities(['storage', 'passwords'], namespace="mqtt_ta", owner='nobody', sessionKey=self.getSessionKey())
            credential_list = []
            credential_key_list = []


            for i, c in entities.items():
                if c['eai:acl']['app'] ==  "mqtt_ta":
                    username = c['username']
                    if not username == "activation_key":
                        credential_list.append(c['clear_password'])
                        credential_key_list.append(username)
                 
            confInfo['mqttmodinput'].append('credential', "::".join(credential_list))
            confInfo['mqttmodinput'].append('credential_key', "::".join(credential_key_list))
    ...
    def handleEdit(self, confInfo):
        ...
        try:
            ...
            try:
                ...
                for i, c in entities.items():
                    if c['eai:acl']['app'] ==  "mqtt_ta":
                        username = c['username']
                        if not username == "activation_key":
                            entity.deleteEntity(['storage', 'passwords'],":%s:" % c['username'],namespace="mqtt_ta", owner='nobody', sessionKey=self.getSessionKey())           
            except:  
                e = sys.exc_info()[0]  
            ...
            for credential_key,credential in zip(credential_key_str.split('::'),credential_str.split('::')):
                try:
                    logger.debug("creating mqtt_ta credential")
                    new_credential = entity.Entity(['storage', 'passwords'], credential_key, contents={'password':credential}, namespace="mqtt_ta",owner='nobody')
                    entity.setEntity(new_credential,sessionKey=self.getSessionKey())
                except:  
                    e = sys.exc_info()[0]  
                    logger.error("Error creating mqtt_ta credential : %s" % e)
