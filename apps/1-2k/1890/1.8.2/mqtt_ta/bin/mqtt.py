...
def update_xml_input(xml_str):
    ...
    for i, c in credentials_list:
        if c['eai:acl']['app'] ==  "mqtt_ta":
          mqtt_username = c['username']
          if not mqtt_username == "activation_key":
              mqtt_password = c['clear_password']
              replace_key='<param name="username">%s</param>' % mqtt_username
              replace_key_with_elements = '%s<param name="password">%s</param>' % (replace_key,mqtt_password)
