...
logging = logger_manager.get_logger("apic_session")
logging_response = logger_manager.get_logger("session_response")
        ...
        while not self._exit:
            ...
            try:
                ...
                if resp.ok:
                    ...
                    logging.error("Could not re-login to APIC.")
                    self._login_timeout = 30
            except ConnectionError:
...
class Subscriber(threading.Thread):
    ...
    def _send_subscription(self, url, only_new=False):
        ...
        if "subscriptionId" not in resp_data:
            logging.error(
                "Did not receive proper subscription response from APIC for url %s response: %s", url, resp_data
            )
            resp = requests.Response()
            resp.status_code = 404
            resp._content = '{"error": "Could not send subscription to APIC"}'
    ...
    def _open_web_socket(self, use_secure=True):
        ...
        if use_secure:
            ...
            self._ws_url = "wss://%s/socket%s" % (self._apic.ipaddr, self._apic.token)
        else:
            self._ws_url = "ws://%s/socket%s" % (self._apic.ipaddr, self._apic.token)
...
class Session(object):
    ...
    def __init__(
        ...
        if key and cert_name:
            ...
            try:
                ...
                logging.error("Are you sure you provided the private key? (Not the certificate)")
                logging.error(
                    "Make sure that you had converted your key to RSA private key. if not, follow the steps given in setup guide for the same."
                )
                raise TypeError("Could not load private key file %s" % (self.key))
        else:
    ...
    def push_to_apic(self, url, data, timeout=None):
        ...
        logging.info("Posting url: %s", post_url)
        ...
        if self.cert_auth and not (self.appcenter_user and self._subscription_enabled and self._logged_in):
            ...
            resp = self.session.post(
                post_url, data=data, verify=self.verify_ssl, timeout=timeout, proxies=self._proxies, cookies=cookies
            )
        ...
        logging_response.debug('Response for the url: %s is %s.', post_url, resp.text)
        return resp
    ...
    def get(self, url, timeout=None):
        ...
        if resp.status_code == 403:
            ...
            resp = self.session.get(
                get_url + "&page=%s&page-size=10000" % page_number,
                timeout=timeout,
                verify=self.verify_ssl,
                proxies=self._proxies,
                cookies=cookies,
            ...
            logging.error("Received error: %s %s", str(resp.status_code), resp.text)
            retries = 3
            while retries > 0:
        ...
        logging.info(resp)
        logging_response.debug("API Response for the url %s is %s.", get_url, resp.text)
        return resp
