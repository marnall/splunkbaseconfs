...
LOG_FILENAME = os.path.join(SPLUNK_HOME,"var","log","splunk","jmsmodinput_app_setuphandler.log")

# Set up a specific logger
logger = logging.getLogger('jmsmodinput')
...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        try:
            ...
            entities = entity.getEntities(['storage', 'passwords'], namespace="jms_ta", owner='nobody', sessionKey=self.getSessionKey())
            credential_list = []
            credential_key_list = []
            ...
            confInfo['jmsmodinput'].append('credential', "::".join(credential_list))
            confInfo['jmsmodinput'].append('credential_key', "::".join(credential_key_list))
            ...
            try:
                ...
                for i, c in entities.items():
                    if c['eai:acl']['app'] ==  "jms_ta":
                        username = c['username']
                        if not username == "activation_key":
                            entity.deleteEntity(['storage', 'passwords'],":%s:" % c['username'],namespace="jms_ta", owner='nobody', sessionKey=self.getSessionKey())           
            except:  
                e = sys.exc_info()[0]  
                logger.error("Error deleting jms_ta credential , perhaps this is the first setup run and it did not yet exist (that is ok) : %s" % e) 
            

            for credential_key,credential in zip(credential_key_str.split('::'),credential_str.split('::')):
                try:
                    logger.debug("creating jms_ta credential")
                    new_credential = entity.Entity(['storage', 'passwords'], credential_key, contents={'password':credential}, namespace="jms_ta",owner='nobody')
                    entity.setEntity(new_credential,sessionKey=self.getSessionKey())
                except:  
                    e = sys.exc_info()[0]  
                    logger.error("Error creating jms_ta credential : %s" % e)
