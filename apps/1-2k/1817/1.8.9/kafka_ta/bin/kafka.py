...
def update_xml_input(xml_str):
    ...
    for i, c in credentials_list:
        if c['eai:acl']['app'] ==  "kafka_ta":
          kafka_username = c['username']
          if not kafka_username == "activation_key":
              kafka_password = c['clear_password']
              replace_key='<param name="kafka_username">%s</param>' % kafka_username
              replace_key_with_elements = '%s<param name="kafka_password">%s</param>' % (replace_key,kafka_password)
