...
LOG_FILENAME = os.path.join(SPLUNK_HOME,"var","log","splunk","kafkamodinput_app_setuphandler.log")

# Set up a specific logger
logger = logging.getLogger('kafkamodinput')
...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        try:
            ...
            entities = entity.getEntities(['storage', 'passwords'], namespace="kafka_ta", owner='nobody', sessionKey=self.getSessionKey())
            credential_list = []
            credential_key_list = []


            for i, c in entities.items():
                if c['eai:acl']['app'] ==  "kafka_ta":
                    username = c['username']
                    if not username == "activation_key":
                        credential_list.append(c['clear_password'])
                        credential_key_list.append(username)
                 
            confInfo['kafkamodinput'].append('credential', "::".join(credential_list))
            confInfo['kafkamodinput'].append('credential_key', "::".join(credential_key_list))
            ...
            try:
                ...
                for i, c in entities.items():
                    if c['eai:acl']['app'] ==  "kafka_ta":
                        username = c['username']
                        if not username == "activation_key":
                            entity.deleteEntity(['storage', 'passwords'],":%s:" % c['username'],namespace="kafka_ta", owner='nobody', sessionKey=self.getSessionKey())           
            except:  
                e = sys.exc_info()[0]  
                logger.error("Error deleting kafka_ta credential , perhaps this is the first setup run and it did not yet exist (that is ok) : %s" % e) 
            

            for credential_key,credential in zip(credential_key_str.split('::'),credential_str.split('::')):
                try:
                    logger.debug("creating kafka_ta credential")
                    new_credential = entity.Entity(['storage', 'passwords'], credential_key, contents={'password':credential}, namespace="kafka_ta",owner='nobody')
                    entity.setEntity(new_credential,sessionKey=self.getSessionKey())
                except:  
                    e = sys.exc_info()[0]  
                    logger.error("Error creating kafka_ta credential : %s" % e)
