...
Splunk.Module.SimpleResultsTableExtended = $.klass(Splunk.Module.AbstractPagedModule, {
    ...
    onLoadStatusChange: function($super, statusInt) {
        ...
        if (statusInt == Splunk.util.moduleLoadStates.HAS_CONTEXT) {
            ...
            if (context === null) {
                this.logger.warn('getContext could not retrieve a context object when called from SimpleResultsTableExtended\'s onLoadStatusModule.');
                return;
            }
	...
	params["defaultColor"] = this.getParam('defaultColor');
	params["fieldToColor"] = this.getParam('fieldToColor');
	params["colorCode"] = this.getParam('colorCode');
    ...
    /**
     * SimpleResultsTableExtended can have child modules.
    ...
    onResultsRendered: function() {
        //this.logger.debug('SimpleResultsTableExtended.onResultsRendered - START');
        var context = this.getContext();
