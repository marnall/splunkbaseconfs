...
# logging setup
import logging
logger = logging.getLogger('splunk.appserver.controllers.module.SimpleResultsTableExtended')
...
class SimpleResultsTableExtended(module.ModuleHandler):
    '''
    Provides module content for the SimpleResultsTableExtended.  The arguments supported 
    ...
    def generateResults(self, host_app, client_app, sid, defaultColor=None, fieldToColor=None, colorCode=None, count=1000, 
        ...
        if not sid:
            raise Exception('SimpleResultsTableExtended.generateResults - sid not passed!')
        ...
        try:
            ...
            logger.error('SimpleResultsTableExtended could not find the job %s. Exception: %s' % (sid, e))
        ...
        if len(ordered):
            ...
            logger.debug('SimpleResultsTableExtended - localMin=%s localMax=%s adjustedMin=%s adjustedMax=%s' % (localMin, localMax, adjustedMin, adjustedMax))
	    ...
	    if colorCode != None:
	    	colorCodeComparisons = colorCode.split(':')
		...
		if colorCode != None:
			if fieldToColor == None:
            			raise Exception('fieldToColor not set as a parameter')
			try:
				if field == fieldToColor:
					colorSet = False
					intValue = float(fieldValues[0].value)
					for comparison in colorCodeComparisons:
						compareFields = comparison.split(',')
						if len(compareFields) != 3:
							raise Exception('Invalid number of arguments in colorCode')
						if compareFields[0] != '<' and compareFields[0] != '>' and compareFields[0] != '=':
							raise Exception('Invalid Entry for comparison Type.  Expected <,> or =')

						if compareFields[0] == '<':
							if intValue<float(compareFields[1]):
								output.append(' style="background-color:%s" ' % compareFields[2])	
								colorSet = True
								break
						if compareFields[0] == '>':
							if intValue>float(compareFields[1]):
					...
					if colorSet == False and defaultColor != None:
						output.append(' style="background-color:%s" ' % defaultColor)	
			except ValueError:
				raise Exception('Not a valid comparison field')
			except:
				raise Exception('Error when trying to apply Color Code to field')
