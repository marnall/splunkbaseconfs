...
UNIT_NAMES = { 's':['second', 'seconds'], 'h':['hour', 'hours'], 'd':['day','days'], 'w':['week', 'weeks'], 'm':['month','months'], 'q':['quarter','quarters'] }
def unitName(unit):
    return UNIT_NAMES.get(unit, unit)

UNIT_STRFTIME = { 'h': ['%I%p','%s'], 'd':['%b%d','%s'], 'w':['%b%d', 'week_of_%s'], 'm':['%b','%s'], 'q':['%b','quarter_of_%s'] }
...
def usage():
    si.generateErrorResults(" 'timeunit' argument required, such as s (seconds), h (hours), d (days), w (weeks), y (years). Optionally prefix with a number: 600s (10 minutes), 2w (2 weeks). Optionally add another argument to specify the time-range label: series=[short,exact,relative]")
    exit(-1)
