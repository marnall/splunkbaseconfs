...
_APP_HOME = _SPLUNK_HOME + "/etc/apps/Roost/"
...
SCHEME = """<scheme>
    <title>Roost</title>
    <description>Get data from Your Nest Thermostat.</description>
    <use_external_validation>true</use_external_validation>
    <streaming_mode>xml</streaming_mode>
    <endpoint>
        <args>
            <arg name="username">
                <title>Nest Account Username</title>
                <description>Your Nest Account Username</description>
            </arg>
            <arg name="password">
                <title>Password</title>
                <description>Your Nest Account password</description>
            </arg>
        </args>
    </endpoint>
</scheme>
...
def run():
    ...
    config = get_config()  
    NestDevice = Nest(config['username'], config['password'])
    NestDevice.login()
    stanza = config["name"]
    ...
    logging.info("source=%s sourcetype=nest stanza=%s operation=running_api"%(source,stanza))
    NestDevices = NestDevice.get_devices()
    for device in NestDevices:
         do_event("%s"%(json.dumps(NestDevices[device])),"nest",source)
    logging.info("source=%s sourcetype=nest stanza=%s operation=running_structure"%(source,stanza))
    NestStructures = NestDevice.get_structures()
    for struct in NestStructures:  
          do_event("%s"%(json.dumps(NestStructures[struct])),"nest",source)
    if (datetime.now().minute > 5 ):
        NestSchedules = NestDevice.get_schedules()
        for sched in NestSchedules:
            do_event("%s"%(json.dumps(NestSchedules[sched])),"nest",source)
        
    do_done_event("nest",source)
    end_stream()
