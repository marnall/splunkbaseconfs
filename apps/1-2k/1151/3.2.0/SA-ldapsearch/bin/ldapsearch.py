...
class LdapSearchCommand(GeneratingCommand):
    ...
    def generate(self):
        ...
        try:
            #check if NTLM is checked
            if configuration.ntlmv2:
            ...
            with ldap3.Connection(
                    ...
                    authentication=authentication) as connection:
            ...
            if isinstance(value, bytes):
                ...
                for i in range(len(value)):
                    if isinstance(value[i], bytes):
                        ...
                        value[i] = str(value[i])
                    elif isinstance(value[i], datetime.time):
                        value[i] = str(value[i])
                    elif isinstance(value[i], datetime.datetime):
                        value[i] = str(value[i])
                    elif isinstance(value[i], datetime.timedelta):
                        value[i] = str(value[i])
                    elif isinstance(value[i], datetime.timezone):
                        value[i] = str(value[i])
                    elif isinstance(value[i], datetime.tzinfo):
                        value[i] = str(value[i])
