...
class BaseInputRestHandler(admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        for name, props in inputs_dict.items():
            item = confInfo[name]
    ...
    def handleEdit(
        ...
        common_new_input = next(iter(new_inputs_dict.values()))
        for key, value in common_new_input.items():
            # Skip group fields
            if key in self.group_fields:
                continue

            for item in old_inputs_dict.values():
        ...
        if has_group_fields:
            ...
            for name, value in new_inputs_dict.items():
    ...
    def _create_inputs(self, inputs_dict, disable_action=None):
        ...
        for index, (name, value) in enumerate(inputs_dict.items(), 1):
            try:
                logger.info(
    ...
    def _find_input_by_value(self, value, inputs_dict):
        ...
        for name, input_value in inputs_dict.items():
            matched = True
            for key in self.group_fields:
