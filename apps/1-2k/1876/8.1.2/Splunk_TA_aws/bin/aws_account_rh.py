...
class AccountRestHandler(admin.MConfigHandler):
    ...
    def handleRemove(
        ...
        # If account is auto-dicovered then abort the deletetion operation
        account = self.get(self.callerArgs.id)
        if account.get("iam"):
            raise RestError(400, "Auto Discovered AWS Account can not be deleted.")
