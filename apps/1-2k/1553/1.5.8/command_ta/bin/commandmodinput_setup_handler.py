...
LOG_FILENAME = os.path.join(SPLUNK_HOME,"var","log","splunk","commandmodinput_app_setuphandler.log")
...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        try:
            ...
            entities = entity.getEntities(['storage', 'passwords'], namespace="command_ta", owner='nobody', sessionKey=self.getSessionKey())
            credential_list = []
            credential_key_list = []


            for i, c in entities.items():
                if c['eai:acl']['app'] ==  "command_ta":
                    credential_list.append(c['clear_password'])
                    credential_key_list.append(c['username'])
                 
            confInfo['commandmodinput'].append('credential', "::".join(credential_list))
            confInfo['commandmodinput'].append('credential_key', "::".join(credential_key_list))
    ...
    def handleEdit(self, confInfo):
        ...
        try:
            ...
            try:
                ...
                for i, c in entities.items():
                    if c['eai:acl']['app'] ==  "command_ta":
                        entity.deleteEntity(['storage', 'passwords'],":%s:" % c['username'],namespace="command_ta", owner='nobody', sessionKey=self.getSessionKey())           
            except:  
                e = sys.exc_info()[0]  
                logger.error("Error deleting command_ta credential , perhaps this is the first setup run and it did not yet exist (that is ok) : %s" % e) 
            

            for credential_key,credential in zip(credential_key_str.split('::'),credential_str.split('::')):
                try:
                    logger.debug("creating command_ta credential")
                    new_credential = entity.Entity(['storage', 'passwords'], credential_key, contents={'password':credential}, namespace="command_ta",owner='nobody')
                    entity.setEntity(new_credential,sessionKey=self.getSessionKey())
                except:  
                    e = sys.exc_info()[0]  
                    logger.error("Error creating command_ta credential : %s" % e)
