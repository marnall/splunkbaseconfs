...
#--------------------------------------------------
#Examples of many other handlers for you to follow
#--------------------------------------------------
...
class TeamViewerEventLogHandler:

    def __init__(self,**args):
        pass

    # send the JSON Audit Events to Splunk for indexing
    def processResponse(self,raw_response_output):

        endpoint_response = json.loads(raw_response_output)

        for event in endpoint_response["AuditEvents"]:
          print_xml_stream(json.dumps(event))


        return endpoint_response["ContinuationToken"]
    ...
    def __call__(self, response_object,raw_response_output,response_type,req_args,endpoint,oauth2=None):       
        ...
        if response_type == "json":
            ...
            #the post request json payload
            post_payload = json.loads(req_args["data"])
            
            #process the initial response results
            continuation_token = self.processResponse(raw_response_output)
                 
            #page over any remaining results using the continuation token if it exists
            while continuation_token:

                #update post payload with next continuation token
                post_payload["ContinuationToken"] = continuation_token
                req_args["data"] = json.dumps(post_payload)
            ...
            #clear out last continuation token
            post_payload["ContinuationToken"] = None
            
            #update post payload with new date range
            post_payload["startDate"] = post_payload["endDate"]
            next_end_date = datetime.now() 
            post_payload["endDate"] = next_end_date.strftime("%Y-%m-%dT%H:%M:%SZ")

            #persist the post payload for the next request
            req_args["data"] = json.dumps(post_payload)
...
class AtlassianAuditLogsHandler:
    ...
    def processResponse(self,raw_response_output):
        ...
        #split out the data array into individual events for indexing
        for event in endpoint_response["data"]:
            print_xml_stream(json.dumps(event))

         
        #follow any pagination links in the response    
        next_link = endpoint_response["links"]["next"]
        next_cursor = endpoint_response["meta"]["next"]

        return next_link,next_cursor
    ...
    def __call__(self, response_object,raw_response_output,response_type,req_args,endpoint,oauth2=None):       
        ...
        if response_type == "json":
            ...
            next_link,next_cursor = self.processResponse(raw_response_output)

            if not "params" in req_args:
                req_args["params"] = {}
            
            #perform pagination
            while next_link and next_cursor:

                #set cursor for next request
                req_args["params"]["cursor"] = next_cursor

                #perform next request of the pagination
                next_response = requests.get(next_link,**req_args)  

                raw_response_output = next_response.text

                #parse subsequent JSON response from pagination
                next_link,next_cursor = self.processResponse(raw_response_output)


            #clean up from pagination
            req_args["params"]["cursor"] = None


            #update the "from" and "to" request parameters for the next primary request that fires off
            #times are in unix epoch milliseconds according to the docs

            #set the next "from" time to the latest "to" time
            latest_time = req_args["params"]["to"]

            if latest_time:

                #set the next "from" time to the latest "to" time
                req_args["params"]["from"] = latest_time

                #increment the next "to" time by 1 day
                #obvisouly 1 day is purely an example , you should adust this as necessary to whatever your polling frequency is
                req_args["params"]["to"] = int(latest_time) + (86400 * 1000)
