...
def sendEmail(results, settings, keywords, argvals):
    ...
    if argvals.get('attachment_name'):
        ssContent['action.email.attachment_name'] = argvals.get('attachment_name')
    if argvals.get('sendxlsx'):
        ssContent['action.email.sendxlsx'] = normalizeBoolean(argvals.get('sendxlsx'))
...
def buildAttachments(settings, ssContent, results, email, jobCount):
    ...
    sendxlsx    = normalizeBoolean(ssContent.get('action.email.sendxlsx', False))
    ...
    type        = ssContent['type']
    attachment_name = ssContent['action.email.attachment_name']
    logger.info("entering build attachments...")
    ...
    if sendxlsx:
        time.sleep(10)
        xlsxAttachment = MIMEBase("application","octet-stream")
        filename="/opt/splunk/var/run/splunk/" + attachment_name
        xlsxAttachment.set_payload(open(filename, "rb").read())
        encoders.encode_base64(xlsxAttachment)
        #xlsxAttachment.add_header('Content-Disposition', 'attachment; filename="/opt/splunk/var/run/splunk/excelexport_20200131_V2.xlsx"')
        filename=attachment_name
        xlsxAttachment.add_header('Content-Disposition', 'attachment; filename="'+filename+'"')
        toAttach.append(xlsxAttachment)
