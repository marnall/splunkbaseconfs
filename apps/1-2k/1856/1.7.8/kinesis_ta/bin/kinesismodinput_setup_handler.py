...
LOG_FILENAME = os.path.join(SPLUNK_HOME,"var","log","splunk","kinesismodinput_app_setuphandler.log")

# Set up a specific logger
logger = logging.getLogger('kinesismodinput')
...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        try:
            ...
            entities = entity.getEntities(['storage', 'passwords'], namespace="kinesis_ta", owner='nobody', sessionKey=self.getSessionKey())
            credential_list = []
            credential_key_list = []


            for i, c in entities.items():
                if c['eai:acl']['app'] ==  "kinesis_ta":
                    username = c['username']
                    if not username == "activation_key":
                        credential_list.append(c['clear_password'])
                        credential_key_list.append(username)
                 
            confInfo['kinesismodinput'].append('credential', "::".join(credential_list))
            confInfo['kinesismodinput'].append('credential_key', "::".join(credential_key_list))
            ...
            try:
                ...
                for i, c in entities.items():
                    if c['eai:acl']['app'] ==  "kinesis_ta":
                        username = c['username']
                        if not username == "activation_key":
                            entity.deleteEntity(['storage', 'passwords'],":%s:" % c['username'],namespace="kinesis_ta", owner='nobody', sessionKey=self.getSessionKey())           
            except:  
                e = sys.exc_info()[0]  
                logger.error("Error deleting kinesis_ta credential , perhaps this is the first setup run and it did not yet exist (that is ok) : %s" % e) 
            

            for credential_key,credential in zip(credential_key_str.split('::'),credential_str.split('::')):
                try:
                    logger.debug("creating kinesis_ta credential")
                    new_credential = entity.Entity(['storage', 'passwords'], credential_key, contents={'password':credential}, namespace="kinesis_ta",owner='nobody')
                    entity.setEntity(new_credential,sessionKey=self.getSessionKey())
                except:  
                    e = sys.exc_info()[0]  
                    logger.error("Error creating kinesis_ta credential : %s" % e)
