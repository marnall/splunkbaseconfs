...
def update_xml_input(xml_str):
    ...
    for i, c in credentials_list:
        if c['eai:acl']['app'] ==  "kinesis_ta":
          ...
          if not aws_access_key_id == "activation_key":
              aws_secret_access_key = c['clear_password']
              replace_key='<param name="aws_access_key_id">%s</param>' % aws_access_key_id
              replace_key_with_elements = '%s<param name="aws_secret_access_key">%s</param>' % (replace_key,aws_secret_access_key)
