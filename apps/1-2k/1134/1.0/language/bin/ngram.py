...
# Copyright (c) 2012 Petri Kallberg
# - modified original script to work with Splunk
...
if __name__ == '__main__':
	...
	# By default analyze _raw (usually not a good idea)
	# and assign results into lang -field of event.
	keywords, options = splunk.Intersplunk.getKeywordsAndOptions()
	field_input = options.get('text', '_raw')
	field_output = options.get('lang', 'lang')
	
	# Assume LM -directory is in the same directory as this script
	n = _NGram()
	l = NGram(os.path.join(sys.path[0],'LM'))
	
	results = splunk.Intersplunk.readResults(None, None, False)
	
	for event in results:
		if field_input not in event:
			continue
		
		try:
			event[field_output] = l.classify(event[field_input])
		except Exception:
			continue
