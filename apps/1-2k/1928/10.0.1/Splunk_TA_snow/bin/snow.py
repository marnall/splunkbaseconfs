...
def _check_delay(delay, input_name, logger=COMMON_LOGGER):
    if not delay:
        logger.error("Field 'delay' is required for input '{}'".format(input_name))
        return False

    try:
        if 0 <= int(delay) < snow_consts.DELAY_LIMIT:
            return True
        else:
            return False
    except ValueError as e:
        msg = (
            "Got unexpected value {} of 'delay' field for input '{}'. "
            "Delay should be in between 0 and {} seconds. "
            "You can either change it in inputs.conf file or edit "
            "'Delay' on Inputs page.".format(delay, input_name, snow_consts.DELAY_LIMIT)
        )
        snow_utility.add_ucc_error_logger(
            logger=logger,
            logger_type=snow_consts.GENERAL_EXCEPTION,
...
class SNOW(smi.Script):
    ...
    def validate_input(self, definition, logger=COMMON_LOGGER):
        ...
        if not _check_delay(delay, config_metadata["name"], logger=logger):
            raise Exception(
                "Delay should be an integer for input '{}', And its value should be between 0 and {}.".format(
                    config_metadata["name"], snow_consts.DELAY_LIMIT
                )
            )
    ...
    def stream_events(self, inputs, ew):
        ...
        try:
            ...
            try:
                ...
                if not input_item.get("delay"):
                    input_item["delay"] = 0
                    _LOGGER.warning(
                        "No delay specified for input '{}'."
                        "Using default delay=0 for now.".format(input_item["name"])
                    )
                elif _check_delay(
                    input_item["delay"],
                    input_item["name"],
                    ...
                    input_item["delay"] = int(input_item["delay"])
                else:
                    return
