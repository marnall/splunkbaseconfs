...
class ModularAlertBase(ModularAction):
    def __init__(self, ta_name, alert_name):
        ...
        self.log_filename = f"{snow_consts.LOGFILE_PREFIX}_{alert_name}"
        self._logger = create_log_object(self.log_filename)
        ...
        try:
            with gzip.open(self.results_file, "rt") as rf:
                for num, result in enumerate(csv.DictReader(rf)):
                    result.setdefault("rid", str(num))
                    if result["rid"] == "":
                        self.log_info(f"Setting default value of rid={str(num)}")
                        result["rid"] = str(num)
                    self.update(result)
                    self.invoke()
                    break
