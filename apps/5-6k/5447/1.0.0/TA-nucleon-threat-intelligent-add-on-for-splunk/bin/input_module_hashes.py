...
# create post request from nucleon hashes api
def request(global_username,global_password,global_usrn,global_client_id,reset,limit,helper):
    ...
    req = Request("POST", 'https://api.nucleoncyber.com/feed/hash', auth=(global_username,global_password), data=body)
...
def modify_array(active_threat_array,active_threat_summary,timeStamp,ew,helper):
    ...
    for event in active_threat_array:
        ...
        # set date by general GMT time  - need to be change if you need the date by your time zone
        # event["date_timeStamp_main_GMT"] =str(datetime.fromtimestamp(timeStamp))
...
def collect_events(helper, ew):
    ...
    in this case foler name is :
    ta_nucleon_add_on_for_threat_intelligent_hashes.log
    """
    ...
    helper.log_debug("Started fetching assets information")
    ...
    if(json_payload.get('status')==1 and json_payload.get('summary').get("total")==0):
        ...
        exit()
    helper.log_info(" count of fetched data from hashes api: "+str(len(json_payload.get('data'))))
