...
class TWILIO_MONITOREVENTS(smi.Script):

    def __init__(self):
        super(TWILIO_MONITOREVENTS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('twilio_monitorevents')
        scheme.description = 'twilio:monitorevents'
    ...
    def stream_events(self, inputs, ew):
        ...
        for ids in r["events"]:
            ...
            state = checkpoint.get(str(ids["sid"]) + str(ids["event_date"]))
            if state is None:

                event = smi.Event(
                    data=json.dumps(ids, indent=2),
                    index=input_item.get("index"),
                    sourcetype='twilio:monitorevents',
                )
                ew.write_event(event)
                count = count + 1
                checkpoint.update(str(ids["sid"]) + str(ids["event_date"]), "Indexed")
            # checkpoint.delete(str(ids["sid"]) + str(ids["event_date"]))

        while r["meta"]["next_page_url"] is not None:
            nexturl = r["meta"]["next_page_url"]
            url_without_https = nexturl.replace("https", "")
            url = "https" + url_without_https
            if proxy_enabled == "1":
                response = requests.get(url, auth=HTTPBasicAuth(account_sid, token), proxies=proxy_dict)
            else:
                response = requests.get(url, auth=HTTPBasicAuth(account_sid, token))
            ...
            for ids in r["events"]:
                ...
                if r["meta"]["next_page_url"] is None:
                    break
...
if __name__ == '__main__':
    exit_code = TWILIO_MONITOREVENTS().run(sys.argv)
    sys.exit(exit_code)
