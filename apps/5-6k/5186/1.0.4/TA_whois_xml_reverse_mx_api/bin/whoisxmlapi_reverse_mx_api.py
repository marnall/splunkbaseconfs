...
class WXAReverseMxCommand(GeneratingCommand):
    mail_server = Option(require=True)
    ...
    def generate(self):
        api_config = self._get_application_config("whoisxmlapi_reverse_mx.conf", "whoisxmlapi_reverse_mx")

        session_key = self.metadata.searchinfo.session_key

        try:
            service = client.connect(token=session_key, app="TA_whois_xml_reverse_mx_api")
        ...
        if not WXAReverseMxCommand._validate_search_term(self.mail_server):
            self.logger.error('Given search term is invalid')
            exit(1)

        latest_name = ''

        for i in range(WXAReverseMxCommand.__MAX_ITERATIONS):
            api_response = ''
            try:
                api_response = self._send_api_request(self.mail_server, latest_name, api_config)
            ...
            for row in domains_list:
                yield {'domain_name': row['name'],
                       'first_seen': datetime.utcfromtimestamp(int(row['first_seen'])).isoformat(' '),
                       'last_visit': datetime.utcfromtimestamp(int(row['last_visit'])).isoformat(' ')}
    ...
    @staticmethod
    def _send_api_request(_mail_server, _latest_name, _api_config):
        payload = {
    ...
    def _validate_search_term(term):
        match_ip = WXAReverseMxCommand.__IP_REGEX.search(term)
        match_domain = WXAReverseMxCommand.__DOMAIN_NAME_REGEX.search(term)
        if match_ip:
            try:
...
dispatch(WXAReverseMxCommand, sys.argv, sys.stdin, sys.stdout, None)
