...
from endpoints.detection_endpoint import endpoint
from endpoints.oat_endpoint import endpoint as oat_endpoint
...
class XDRConfigMigrationHandler(ConfigMigrationHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.oat_endpoint = oat_endpoint
        super(XDRConfigMigrationHandler, self).__init__(*args, **kwargs)
        self.oat_handler = RestHandler(
    ...
    def handleEdit(self, confInfo):
        ...
        if disabled is None:
            detection_global_account = self.payload.get("global_account")
            disabled = self.get_input_payload("disabled")
            if not is_true(disabled):
                self.is_able_to_enable(detection_global_account=detection_global_account)
            self.edit_hook(
                session_key=self.getSessionKey(),
                config_name=self._get_name(),
                stanza_id=self.callerArgs.id,
            ...
            detection_global_account = self.get_input_payload("global_account")
            self.is_able_to_enable(detection_global_account=detection_global_account)
            return self.handler.enable(self.callerArgs.id)
        
    def is_able_to_enable(self, detection_global_account):
        decrypt = self.callerArgs.data.get(
            self.ACTION_CRED,
            [False],
        )
        decrypt = is_true(decrypt[0])
        result = self.oat_handler.all(
                decrypt=decrypt,
                count=0,
        )
        for entity in result:
            ...
            if detection_global_account==global_account and not disabled :
                raise ValueError("Unable to enable XDR_Detection. XDR_OAT and XDR_Detection cannot be enabled for one account at the same time. Disable XDR_OAT in order to enable XDR_Detection")
        return result
