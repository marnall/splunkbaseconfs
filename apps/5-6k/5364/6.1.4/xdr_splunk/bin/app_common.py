...
def get_version():
    VERSION_DIR = os.path.join(
        os.environ["SPLUNK_HOME"], 'etc', 'apps', 'xdr_splunk', 'default', 'app.conf')
...
def set_input_setting(section, key, value):
    local_path = os.path.join(
        os.environ["SPLUNK_HOME"], 'etc', 'apps', 'xdr_splunk', 'local')
...
def format_iso_time(ft_rule='%Y-%m-%dT%H:%M:%S.000Z', delta_sec=0):
...
def gen_context_path(input_name, customer_id):
    encode_str = base64.b64encode(bytes(input_name, 'utf-8'))
    suffix = hashlib.sha1(encode_str).hexdigest()
    if customer_id is None:
        return (os.path.join(os.environ["SPLUNK_HOME"], 'etc', 'apps', 'xdr_splunk', 'data'), 'status-{}.json'.format(suffix))
    return (os.path.join(os.environ["SPLUNK_HOME"], 'etc', 'apps', 'xdr_splunk', 'data'), 'status-{}-{}.json'.format(suffix, customer_id))


def fetch_context(input_name, customer_id,  init_val={}):
    ck_path, file_name = gen_context_path(input_name, customer_id)
...
def update_context(input_name, customer_id, key, value):
    ck_path, file_name = gen_context_path(input_name, customer_id)
    status_file = os.path.join(ck_path, file_name)
    if not os.path.exists(ck_path):
        os.makedirs(ck_path, 0o755)
    if not os.path.exists(status_file):
        ...
        try:
            ...
            helper.log_error(
                f"JSONDecodeError@{status_file} when update_context, INPUT:{input_name}, custom_id:{customer_id}")
            status = {}
        status[key] = value
        with open(status_file, 'w') as json_file:
            json.dump(status, json_file)
...
def del_context(input_name, customer_id, key):
    ck_path, file_name = gen_context_path(input_name, customer_id)
    status_file = os.path.join(ck_path, file_name)
    if not os.path.exists(ck_path):
        os.makedirs(ck_path, 0o755)
        ...
        while attempt_times >= 0:
            try:
                response = requests.request(
                    method, url, params=parameters, headers=headers, json=payload, timeout=timeout, proxies=proxies)
...
def update_tpc_metrics(endpoint, headers, proxies):
    try:
        ...
        resp_text = res.text if res else ""
        helper.log_error(
            f"fail to update metrics to tpc with response: {resp_text}")

def query_params_parse(url):
    if url == None:
        return None
