...
RESULT=$(curl -s -k https://$API_HOST/ocs/v1.php/apps/metrics/api/v1/metrics\?users\=true\&files\=false\&shares\=false\&quota\=false\&userData\=false\&format\=json -H "OC-MetricsApiKey: $METRICSAPIKEY")
if [ ! -z "$RESULT" ]; then
	echo $RESULT
else
