...
class ModInputcrowdstrike_device_flight_control_json(base_mi.BaseModInput):

    def __init__(self):
        use_single_instance = False
        super(ModInputcrowdstrike_device_flight_control_json, self).__init__("ta_crowdstrike_falcon_devices", "crowdstrike_device_flight_control_json", use_single_instance)
        self.global_checkbox_fields = None

    def get_scheme(self):
        """overloaded splunklib modularinput method"""
        scheme = super(ModInputcrowdstrike_device_flight_control_json, self).get_scheme()
        scheme.title = ("CrowdStrike Device Flight Control JSON")
        ...
        """
        scheme.add_argument(smi.Argument("member_cid", title="Flight Control Member CID",
                                         description="Enter the CrowdStrike Customer ID for the child instance the data will be collected from",
                                         required_on_create=True,
        ...
        scheme.add_argument(smi.Argument("cloud", title="Select Cloud Environment",
                                         description="Select the appropriate cloud environment for the Flight Control Child CID",
                                         required_on_create=True,
                                         required_on_edit=False))
    ...
    def collect_events(helper, ew):
        ...
        log_label = f'CrowdStrike Device TA FFC {version} {stanza_name} :'
        ...
        #get member CID
        member_cid = helper.get_arg('member_cid')
        helper.log_info(f'{log_label} Configuration: member CID - {member_cid}')
        ...
        try:
            falcon = APIHarnessV2(client_id=clientid, client_secret=secret,
                                  ...
                                  user_agent=user_agent, member_cid=member_cid,
                                  timeout=const.timeout, ssl_verify=True)
        ...
        if not falcon.authenticated():
            ...
            if status is None:
                ...
                helper.log_error(
                    ...
                    f'Verify client_id, client_secret, API scopes (hosts:read), '
                    f'cloud environment ({base_url}), and member_cid ({member_cid})'
                )
                raise RuntimeError(f'Authentication failed — HTTP {status}: {reason}')

        helper.log_info(f'{log_label} Authentication successful (Flight Control, member_cid={member_cid})')

        #collect list of device IDs
...
if __name__ == "__main__":
    exitcode = ModInputcrowdstrike_device_flight_control_json().run(sys.argv)
    sys.exit(exitcode)