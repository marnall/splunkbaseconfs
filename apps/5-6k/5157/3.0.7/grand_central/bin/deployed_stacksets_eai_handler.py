...
class DeployedStacksetsEAIHandler(base_eai_handler.BaseEAIHandler):
    ...
    def handleCreate(self, confInfo):
        ...
        confInfo -- The object containing the information about what is being requested.
        """
        logger.info('Stackset template deployment requested -- here')
        ...
        logger.info('HERE: {}'.format(str(aws_regions)))

        passwords_conf_payload = self.simple_request_eai(aws_secret_key_link_alternate, 'list', 'GET')
        SECRET_KEY = passwords_conf_payload['entry'][0]['content']['clear_password']

        try:
            logger.info('aws_regions[0]: {}'.format(str(aws_regions[0])))
            client = boto3.client(
                'cloudformation', 
            ...
            logger.error(e)
            raise admin.InternalException('Error creating boto3 client %s' % e)
        ...
        try:
            ...
            logger.error(e)
            raise admin.InternalException('Error deploying Stackset %s' % e)
        ...
        deployed_stacksets_rest_path = '/servicesNS/%s/%s/deployed_stacksets/%s' % (
            'nobody', self.appName, quote_plus(params['name']))
    ...
    def handleRemove(self, confInfo):
        ...
        deployed_stacksets_rest_path = '/servicesNS/%s/%s/deployed_stacksets/%s' % (
            'nobody', self.appName, quote_plus(name))
        deployed_stacksets_eai_response_payload = self.simple_request_eai(deployed_stacksets_rest_path,
                                                                                'read', 'GET')
        ...
        aws_regions = list([str(x) for x in aws_regions])
