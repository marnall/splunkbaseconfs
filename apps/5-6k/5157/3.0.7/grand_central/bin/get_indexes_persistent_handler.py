...
logfile = os.sep.join([os.environ['SPLUNK_HOME'], 'var',
                       'log', 'splunk', 'persistent_handler.log'])
logging.basicConfig(filename=logfile, level=logging.DEBUG)
debug_logger = log_helper.setup(
    logging.INFO, 'GetIndexesPersistentHandler', 'get_indexes_persistent_handler.log')
...
class GetIndexesPersistentHandler(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        try:
            ...
            indexes = [x.name for x in s.indexes]
            return json.dumps({
                'payload': indexes,
                'status': 200
            })
        except Exception as e:
            debug_logger.error('Error getting indexes: {}'.format(e))
            return json.dumps({
                'status': 500,
                'payload': str(e)
            })
...
def simple_request_eai(url, action, method, session_key, params=None):
    ...
    try:
        ...
        debug_logger.error(
            'action=http_internal_request state=error error="%s" pid=%s guid=%s' % (e, pid, guid))
        raise Exception('Unable to %s %s entry. %s' % (action, url, e))
    debug_logger.info(
        'action=http_internal_request state=end pid=%s guid=%s' % (pid, guid))

    try:
        payload = json.loads(content)
    except Exception as e:
        debug_logger.error(
            'action=http_internal_request state=error error="%s"' % e)
        raise Exception('Unable to parse %s response payload.' % url)

    if response.status not in [200, 201]:
        message = simple_request_messages_to_str(response.messages)
        debug_logger.error(
            'action=http_internal_request state=error error="%s"' % message)
        raise Exception(
