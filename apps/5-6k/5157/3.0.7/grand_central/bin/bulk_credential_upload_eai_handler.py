...
class BulkCredentialUploadEAIHandler(base_eai_handler.BaseEAIHandler):
    ...
    def handleCreate(self, confInfo):
        ...
        if 'credential_file' in params and params['credential_file'] != '0' and params['credential_file'] != '':
            credentials = params['credential_file'].split('\n')
            header = credentials.pop(0).split(',')
            index_access_key_id = header.index("Access key ID")
            index_secret_access_key = header.index("Secret access key")
        else:
            credentials = []
        

        for credential in credentials:
            row = credential.split(',')
            aws_access_key = row[index_access_key_id].strip()
            SECRET_KEY = row[index_secret_access_key].strip()
            try:
        ...
        logger.info("Bulk credential upload request DONE")
