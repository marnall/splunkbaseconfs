...
if sys.version_info >= (3, 0):
    ...
    sys.path.insert(
        0, os.path.sep.join([bl, "bin", "lib", "python3.7", "site-packages"])
    )
...
class KennyLoggins:
    ...
    def get_logger(
        self, app_name=None, file_name="kenny_loggins", log_level=logging.INFO
    ):
        log_location = make_splunkhome_path(["var", "log", "splunk", app_name])
        # logging.setLoggerClass(SplunkLogger)
        useLogCFG = True
        LOGGING_DEFAULT_CONFIG_FILE = make_splunkhome_path(
            ["etc", "apps", app_name, "default", "log.cfg"]
        )
        LOGGING_LOCAL_CONFIG_FILE = make_splunkhome_path(
            ["etc", "apps", app_name, "local", "log.cfg"]
        )
        LOGGING_DEFAULT_CONFIG_FILE_CONF = make_splunkhome_path(
            ["etc", "apps", app_name, "default", "apl_logging.conf"]
        )
        LOGGING_LOCAL_CONFIG_FILE_CONF = make_splunkhome_path(
            ["etc", "apps", app_name, "local", "apl_logging.conf"]
        )
        LOGGING_STANZA_NAME = app_name
        _log = logging.getLogger("{}".format(file_name))
        if not os.path.isdir(log_location):
            os.mkdir(log_location)
        if os.path.isfile(LOGGING_DEFAULT_CONFIG_FILE_CONF) or os.path.isfile(
            LOGGING_LOCAL_CONFIG_FILE_CONF
        ):
            useLogCFG = False
        output_file_name = os.path.join(log_location, "{}.log".format(file_name))
        _log.propogate = False
        _log.setLevel(log_level)
        f_handle = handlers.RotatingFileHandler(
            output_file_name, maxBytes=25000000, backupCount=5
        )
        ...
        try:
            if not useLogCFG:
                _log.debug(
                    "action=setting_levels source=apl_logging.conf local='{}' default='{}'".format(
                        LOGGING_LOCAL_CONFIG_FILE_CONF, LOGGING_DEFAULT_CONFIG_FILE_CONF
                    )
                )
                splunk.setupSplunkLogger(
                    _log,
                    LOGGING_DEFAULT_CONFIG_FILE_CONF,
                    LOGGING_LOCAL_CONFIG_FILE_CONF,
                    LOGGING_STANZA_NAME,
                )
            else:
                _log.debug(
                    "action=setting_levels source=log.cfg local='{}' default='{}'".format(
            ...
            _log.error(
                "Failed to setup Logger {3}:{4}: {1}:{0}, setting log_level to {2}".format(
                    e, type(e), log_level, app_name, file_name
                )
            )
            _log.setLevel(log_level)
        return _log
...
class Utilities:
    ...
    def _make_get_request(self, uri, args=None):
        return rest.simpleRequest(
            uri, getargs=args, sessionKey=self._session_key, raiseAllErrors=True
        )

    def _make_post_request(self, uri, args=None, usejson=False):
        if not usejson:
            return rest.simpleRequest(
                uri, postargs=args, sessionKey=self._session_key, raiseAllErrors=True
            )
        else:
            return rest.simpleRequest(
                uri, jsonargs=json.dumps(args), sessionKey=self._session_key
            )
    ...
    def check_collection_exists(self, collection_name, do_create=False, **kwargs):
        ...
        try:
            ...
            self._log.info(
                "action=check_collection_exists do_create={} sc={}".format(
                    do_create, type(entry)
                )
            )
            self.check_transform_exists(
                t_name, do_create=do_create, collection=c_name, external_type="kvstore"
            )
            ...
            self._log.warning(
                "action=check_collection_exists name={} do_create={} {}".format(
                    c_name, do_create, e
                )
            )
            ret_obj = {}
            if do_create:
                ...
                sr, sc = self._make_post_request(
                    self._build_endpoint_uri(["storage", "collections", "config"]),
                    args=args,
                )
                entry = json.loads(sc.decode("utf-8"))
                self._log.info(
                    "action=check_collection_exists do_create={} response={}".format(
                        do_create, json.dumps(entry)
                    )
                )
                self.check_transform_exists(
                    t_name,
    ...
    def check_transform_exists(self, transform_name, do_create=False, **kwargs):
        ...
        try:
            ...
            entry = json.loads(sc.decode("utf-8"))
            self._log.info(
                "action=check_transform_exists do_create={} sc={}".format(
                    do_create, json.dumps(entry)
                )
            )
            return entry
            ...
            self._log.warning(
                "action=check_transform_exists name={} do_create={} {}".format(
                    t_name, do_create, e
                )
            )
            if do_create:
                ...
                sr, sc = self._make_post_request(
                    self._build_endpoint_uri(["configs", "conf-transforms"]),
                    args=kwargs,
                )
                self._log.info(
                    "action=check_transform_exists do_create={} ".format(do_create)
                )
    ...
    def get_search_results_by_sid(self, sid, **kwargs):
        try:
            ...
            server_response, server_content = self._make_get_request(
                uri, args={"count": kwargs.get("count", 10000), "output_mode": "json"}
            )
    ...
    def send_single_event(self, event={}, **kwargs):
        try:
            ...
            server_response, server_content = self._make_post_request(
                "{}?index={}&sourcetype={}&source={}".format(
                    uri,
                    kwargs.get("index", "main"),
                    kwargs.get("sourcetype", "utilities_call"),
                    kwargs.get("source", "utilities_call"),
                ),
    ...
    def set_credential(self, crealm, cuser, cpass):
        ...
        if creds is not None:
            ...
            uri = self._build_endpoint_uri(
                ["storage", "passwords", "{0}:{1}".format(crealm, cuser)]
            )
            args = {"password": cpass}
    ...
    def get_configuration(self, filename, stanza):
        try:
            rc, o = self._make_get_request(
                self._build_endpoint_uri(
                    ["configs", "conf-{}".format(filename), stanza]
                ),
                args={"output_mode": "json"},
            )
            self._log.debug(
                "action=get_configuration typeobj={} rc={} typerc={} compare={}".format(
                    type(o), rc.status, type(rc.status), (rc.status == 200)
                )
            )
            if rc.status == 200:
            ...
            self._log.error(
                ...
                'exception_arguments="{}" filename="{}" exception_line="{}" '.format(
                    str(e), type(e), e, fname, exc_tb.tb_lineno
                )
            )
            return {}
    ...
    def get_credential(self, realm, cuser):
        ...
        try:
            self._log.info(
                "realm={} cuser={} app={}".format(realm, cuser, self._app_name)
            )
            ...
            if key not in entities:
                ...
                try:
                    ...
                    if clear_pass is None:
                        ...
                        return None
                    self._log.debug(
                        "action=get_credential msg=found_clear_password realm={} cuser={} ".format(
                            realm, cuser
                        )
                    )
                    return clear_pass
            ...
            self._log.error(
                'message="{}" exception_type="{}" '
                'exception_arguments="{}" filename="{}" line="{}" '.format(
                    str(e), type(e), e, fname, exc_tb.tb_lineno
                )
            )
    ...
    def get_kvstore_status(self):
        ...
        headers, data = self._make_get_request(
            self._build_endpoint_uri(["kvstore", "status"]),
            args={"output_mode": "json"},
        )
    ...
    def kvstore_batch_save(self, kvstore, data):
        try:
            uri = self._build_endpoint_uri(
                ["storage", "collections", "data", kvstore, "batch_save"]
            )
            ...
            for chunk in chunks:
                ...
                self._debug(
                    "ksvtore={} action=batch_save chunk_length={}".format(
                        kvstore, len(chunk)
                    )
                )
    ...
    def set_kvstore_data(self, kvstore, data):
        try:
            uri = self._build_endpoint_uri(["storage", "collections", "data", kvstore])
            self._log.info(
                "setting kvstore data to have fields: {}".format(
                    json.dumps({"keys": data.keys()})
                )
            )
    ...
    def delete_kvstore_item(self, kvstore, key):
        try:
            uri = self._build_endpoint_uri(
                ["storage", "collections", "data", kvstore, key]
            )
            self._log.info(
                "action=deleting_kvstore_item kvstore={} key={}".format(kvstore, key)
            )
            return self._make_delete_request(uri, usejson=True)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
    ...
    def build_proxy_string(proxy):
        ...
        if proxy["proxy_url"]:
            ...
            proto = "http"
            if (
                proxy.get("use_ssl") is "true"
                or "{}".format(proxy.get("use_ssl")) is "1"
            ):
                proto = "https"
            proxy_string = "{}://{}".format(proto, proxy_url)
            if proxy.get("proxy_user"):
                # quote() the credentials to account for special characters.
                # See https://bugs.python.org/issue33342
                credentials_string = (
                    f"{quote(proxy['proxy_user'])}:{quote(proxy['proxy_pass'])}"
                )
                proxy_string = "{}://{}@{}".format(proto, credentials_string, proxy_url)
    ...
    def verify_https(url):
        if not url.startswith("https"):
            raise Exception(f"URL {url} must be HTTPS")
