...
class CreateAlertModularAction(ModularAction):
    def __init__(
        ...
        self._verify_splunk = verify_splunk
        try:
            kl = KennyLoggins()
            ...
            self._log.debug(
                "action=instantiate_utilities session_key_length={}".format(
                    len(self.sessionKey)
                )
            )
            self.utils = Utilities(app_name=app_name, session_key=self.sessionKey)
            self._log.debug(
                "action=get_configuration global_configuration={}".format(
                    dumps(global_configuration)
                )
            )
            self.configuration = self.utils.get_configuration(
                global_configuration.get("filename", ""),
                global_configuration.get("stanza", ""),
            )
            self._log.debug(
                "action=get_configuration config={}".format(self.configuration)
            )
            for k, v in self.configuration.items():
                self._log.debug("action=setting_attr {}={}".format(k, v))
                setattr(self, k, v)
            self._log.debug(
                "action=get_settings settings={}".format(self.configuration)
            )
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            self._log.error(
                "action=fatal_error exception_line={} file={}  message={}".format(
                    exc_tb.tb_lineno, fname, e
                )
            )
    ...
    def _load_results(self, form="rb"):
    ...
    def _load_kvstore(self, kvstore_name):
        try:
            ...
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            self._log.error(
                "function=_load_kvstore exception_line={} file={}  message={}".format(
                    exc_tb.tb_lineno, fname, e
                )
            )
    ...
    def _delete_kvstore_items(self, kvstore_name, data):
        try:
            # delete_kvstore_item
            res = [
                self.utils.delete_kvstore_item(kvstore_name, x.get("_key"))
                for x in data
                if x.get("_key") is not None
            ]
    ...
    def _save_kvstore(self, kvstore_name, field_name, data):
        try:
            res = self.utils.kvstore_batch_save(
                kvstore_name,
                [
                    {
                        "_key": zz.get("_key", "{}".format(uuid.uuid4())),
                        field_name: zz.get("value", ""),
                    }
                    for zz in data
                ],
            )
        ...
        try:
            ...
            with open(path) as fh:
                ...
                for line in lookup:
                    keys.append(line)
            self._log.debug(
                "action=load_csv sc={} url={} path={}".format(sc, url, path)
            )
            return keys
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            self._log.error(
                "function=_load_lookup exception_line={} file={}  message={}".format(
                    exc_tb.tb_lineno, fname, e
                )
            )
    ...
    def _save_lookup(self, lookup_name, data):
        url = self._build_url(["data", "lookup-table-files", lookup_name])
        tmpfile = make_splunkhome_path(
            ["etc", "apps", self.app_name, "lookups", "tmp_{}".format(lookup_name)]
        )
        ...
        self._log.info(
            "action=save_lookup name={} status_code={}".format(
                lookup_name, r.status_code
            )
        )
        try:
            os.remove(tmpfile)
    ...
    def _create_lookup(self, lookup_name, data):
        ...
        self._log.info(
            "action=create_lookup name={} status_code={} ret={} url={}".format(
                lookup_name, r.status_code, dumps(r.json()), url
            )
        )
        os.remove(tmpfile)
        return data
    ...
    def _post(self, url, data):
        try:
            ...
            with self.with_suppress_insecure_request_warnings():
                return requests.post(
                    url=url,
                    data=data,
                    headers={"Authorization": "Splunk " + self.getSessionKey()},
                    verify=self._verify_splunk,
                )
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            self._log.error(
                "function=_post exception_line={} file={}  message={}".format(
                    exc_tb.tb_lineno, fname, e
                )
            )
    ...
    def _delete(self, url):
        try:
            ...
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            self._log.error(
                "function=_delete line={} file={}  message={}".format(
                    exc_tb.tb_lineno, fname, e
                )
            )
    ...
    def _get(self, url):
        try:
            ...
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            self._log.error(
                "function=_get exception_line={} file={}  message={}".format(
                    exc_tb.tb_lineno, fname, e
                )
            )
    ...
    def get_evtidx(self, evt):
        try:
            ...
            evt = re.compile(r"index\s*=\s*([\w\d_-]+)")
            ...
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            self._log.error(
                "function=get_evtidx exception_line={} file={}  message={}".format(
                    exc_tb.tb_lineno, fname, e
                )
            )
    ...
    def _get_spl(self, sid):
        ...
        if sc == 200:
            self._log.debug(
                "response {}".format(dumps(r.json()["entry"][0]["content"]["request"]))
            )
    ...
    @contextmanager
    def with_suppress_insecure_request_warnings(self):
        """
        Context manager to suppress InsecureRequestWarning from requests when verify_splunk is False.
        This is useful when making requests to Splunk instances with self-signed certificates or without SSL verification.
        :return: None
        """
        if self._verify_splunk is False:
