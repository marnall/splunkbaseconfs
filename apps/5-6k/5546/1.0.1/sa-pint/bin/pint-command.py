...
#handler
@Configuration()
class pintCommand(StreamingCommand):
  #examples
  #url        = Option(require=True)
  #paramMap   = Option(require=False)
  inputField  = Option(require=True)
  fromUnit    = Option(require=True)
  toUnit      = Option(require=True)
    ...
    for record in records:
      if record[inputField]:
        try:
            ...
            q = ureg.Quantity
            q = q(float(record[inputField]),fromUnit)
            nq = q.to(toUnit)
...
dispatch(pintCommand, sys.argv, sys.stdin, sys.stdout, __name__)