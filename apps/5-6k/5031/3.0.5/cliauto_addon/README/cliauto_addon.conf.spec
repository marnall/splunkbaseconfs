# This file describes the cliauto_addon.conf file that is included
# with the CLI Auto Addon for Splunk app.
# 
# cliauto_addon.conf is meant to correspond with cliauto_cmds.conf
#
...
[main]
enable_event_backup = <no | yes> 
* If enable_event_backup = yes, the events of last job are backed up to a log file $SPLUNK_HOME/cliauto_addon/cliauto_addon.log
...
* The app_pid_collection KVStore stores the job/pid history.
...
* Default max length of data input strings of Custom REST API. Min = 1, Max = 500000.

absolute_input_data_length_max = <number of characters>
* Absolute max length of data input strings of Custom REST API. Min = 1, Max = 1000000.
