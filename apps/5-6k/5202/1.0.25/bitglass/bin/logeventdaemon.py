...
class LastLog:
    ...
    def update(self, dtime, ll, logtype=None, resp=None):
        if logtype is None:
            logtype = self.logtype
        elif logtype in self.subLogs:
            return self.subLogs[logtype].update(dtime, ll, resp=resp)
        ...
        if ll:
            ...
            if resp:
                # add ratelimit to log directly
                headers = resp.headers
                # app.logger.debug("  updateRateLimit %s; %s" % (logtype, repr(headers)))
                if 'x-ratelimit-remaining' in headers:
                    ll['_x_ratelimit_remaining'] = headers.get('x-ratelimit-remaining')
                    ll['_x_ratelimit_reset'] = headers.get('x-ratelimit-reset', '').replace(' ', 'T').replace('+00:00', 'Z')
            ...
            if self.get(logtype=logtype):
                ...
                if self.get('_x_ratelimit_remaining', logtype):
                    del self.log[logtype]['_x_ratelimit_remaining']
                if self.get('_x_ratelimit_reset', logtype):
                    del self.log[logtype]['_x_ratelimit_reset']
    ...
    def updateError(self, conf, errormsg, resettime, logtype=None):
        ...
        if type(errormsg) is HTTPError:
            # update error with the rate limit
            headers = errormsg.response.headers
            app.logger.debug("updateError %s; %s; %s; %s" % (str(errormsg), resettime, logtype, repr(headers)))
            self.log[logtype]['_x_ratelimit_remaining'] = headers.get('x-ratelimit-remaining')
            self.log[logtype]['_x_ratelimit_reset'] = headers.get('x-ratelimit-reset', '').replace(' ', 'T').replace('+00:00', 'Z')
        self.dump()
...
def getAPIToken(logData, conf, logType):
    ...
    try:
        ...
        app.logger.info("getAPIToken %s; %s" % (logType, d))
    except Exception as ex:
...
def restCall(_,
    ...
    if auth_token is None or auth_token == '':  # nosec
        ...
        if any(a() for a in [lambda: username is None or username == '',
            ...
            app.logger.error(">restCall 401 %s%s with no username/pwd/auth" % (url, endpoint))
    ...
    with requests.Session() as s:
        ...
        if isinstance(dataParams, dict):
            # Assume json
            app.logger.info("   >>>POST: json: %s" % (repr(dataParams)))
            ...
            app.logger.info("   >>>GET: %s" % (repr(dataParams)))
...
def drainLogEvents(ctx, conf, logType, logData=None, dtime=None, nextPageToken=None):
    app.logger.info(">drainLogEvents: logType:%s; logData:%s; dtime:%s; nextPageToken:%s" % (logType, logData, dtime, nextPageToken))
    ...
    try:
        ...
        drained = False
        while not drained:
            # handle 429 which sets dtime > now scenario
            if logTime > datetime.utcnow():
                app.logger.info("  DrainLogEvents lt:%s; logTime %s is future time, skip" % (logType, logTime))
                drained = True
                break
            ...
            if len(data) == 0:
                app.logger.debug("  drainLogEvents: %s empty data %s" % (logType, logData))
                drained = True
            else:
                logTime, lastLog = processLogEvents(ctx, conf, logType, data, nextPageToken, logTime, isSyslog)
                app.logger.debug("  drainLogEvents after process: logType: %s; logTime: %s; lastLog: %s" % (logType, logTime, lastLog))
            ...
            if isSyslog:
                ...
                status.lastLog = lastLogFile.update(dtime, lastLog, logType, resp=r)    # type: ignore[union-attr]
        ...
        if SKIPPED_REQUEST_ERROR not in msg:
            app.logger.error(msg)
            lastLogFile.updateError(conf, ex, None, logType)     # type: ignore[union-attr]
    ...
    for lt in log_types:
        ...
        try:
            if lastLogFile.get(logtype=lt):     # type: ignore[union-attr]
                ...
                overRateLimit = lastLogFile.get('_x_ratelimit_remaining', lt) == "0"
                if overRateLimit:
                    rateLimitReset = lastLogFile.get('_x_ratelimit_reset', lt)
                    app.logger.info("lt %s is over RateLimit and will reset at %s" % (lt, rateLimitReset))
                    dtime[lt] = datetime.strptime(rateLimitReset, TIME_FORMAT_URL)
