...
class DragosManageIOCInactiveListCommand(GeneratingCommand):
    ...
    def generate(self):
        ...
        try:
            ...
            self._verify_local_search_head(service_with_app)
    ...
    def _verify_local_search_head(self, service):
        # If you are running the input on a forwarder then you must run this command
        # directly on the search head
        
        if 'dragos_iocs' in service.inputs:
            ioc_input = service.inputs['dragos_iocs']
            if 'search_head_configuration' in ioc_input:
                if ioc_input['search_head_configuration'] == 'remote':
                    raise Exception('Please run this command on the search head, not the forwarder.')
