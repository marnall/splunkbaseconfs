...
#         - Device ID Field: the field name that contains the device id to quarantine.
import sys
import os
import json
import logging
...
class VmwareQuarantineDevice(VmwareCBCAlertAction):
                ...
                for num, result in enumerate(csv.DictReader(fh)):
                    try:
                        ...
                        if device_id is None:
                            ...
                            # hostname = "name:{}".format(device_id)
                            # hostname = "hostNameExact:{}".format(device_id)
                            result["device_id"] = device_id
                            try:
                                self._log.info("action=quarantine_try filter={}, cb_type={}".format(device_id, type(self.cb)))
                                device = None
                                if result.get("org_key", None) is not None and self._use_multi_tenant:
                                    org_key = result.get("org_key")
                                    self._log.debug("action=multi_tenant_api_usage org_key={}".format(org_key))
                                ...
                                self._log.info("device={}, type_of_device={}".format(device, type(device)))
                                self._log.debug("action=device_found device={} next=quarantine".format(device_id))
                                r = device.quarantine(True)
                                self._log.info("action=finished_quarantine device={} r={}".format(device, r))
                                result["alert_action_quarantine"] = True
                                self.addevent(json.dumps(dict(result)),
                                              sourcetype="vmware:alert_action:{}".format(_alert_name))
...
if __name__ == '__main__':
    ...
    modaction = None
    try:
        logger.info("action=instantiating alert={}".format(_alert_name))
        modaction = VmwareQuarantineDevice(sys.stdin.read())
        modaction.main()
