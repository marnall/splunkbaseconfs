...
- A new `censyssearch` streaming command to enrich search results by IP for banners, device description, web page titles, open ports, and TLS names
...
The app supports Censys' search product. 

**Search users** configure your API ID and secret under the "manage apps" menu, and for the Censys app click "set up". Find your API credentials and copy them here. This will unlock the "censyssearch" streaming command. 

## Usage

The "censyssearch" streaming command lets you enrich results by IP address. Syntax: `... | censys [ip address field] {title, protocols, banners, description, tls_names}`


### In-line search

The app introduces a new streaming command, `censyssearch`, which allows you to perform lookups for each record based on the IP address. The command takes two arguments, in order:
...
In searches containing IP addresses, this `censyssearch` command can be used to enrich events. For example:
