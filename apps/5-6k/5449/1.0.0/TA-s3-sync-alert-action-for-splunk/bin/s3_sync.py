...
import ta_s3_sync_alert_action_for_splunk_declare

import os
import sys

from alert_actions_base import ModularAlertBase
import modalert_s3_sync_helper

class AlertActionWorkers3_sync(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkers3_sync, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_s3_sync_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkers3_sync("TA-s3-sync-alert-action-for-splunk", "s3_sync").run(sys.argv)
    sys.exit(exitcode)
