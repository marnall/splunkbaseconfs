...
class WXAReverseNsCommand(GeneratingCommand):
    name_server = Option(require=True)
    ...
    def generate(self):
        api_config = self._get_application_config("whoisxmlapi_reverse_ns.conf", "whoisxmlapi_reverse_ns")

        session_key = self.metadata.searchinfo.session_key

        try:
            service = client.connect(token=session_key, app="TA_whois_xml_reverse_ns_api")
        ...
        if not WXAReverseNsCommand._validate_search_term(self.name_server):
            self.logger.error('Given search term is invalid')
            exit(1)

        latest_name = ''

        for i in range(WXAReverseNsCommand.__MAX_ITERATIONS):
            api_response = ''
            try:
                api_response = self._send_api_request(self.name_server, latest_name, api_config)
    ...
    def _validate_search_term(term):
        match_ip = WXAReverseNsCommand.__IP_REGEX.search(term)
        match_domain = WXAReverseNsCommand.__DOMAIN_NAME_REGEX.search(term)
        if match_ip:
            try:
...
dispatch(WXAReverseNsCommand, sys.argv, sys.stdin, sys.stdout, None)
