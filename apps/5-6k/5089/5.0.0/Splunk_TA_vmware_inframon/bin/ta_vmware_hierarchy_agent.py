...
from hydra_inframon.hydra_common import HydraRuntimeAdapter
...
#Setup Logger
_hierarchy_log_level_name = get_hydra_log_level("hierarchy_agent", default="INFO")
if _hierarchy_log_level_name == "WARN":
	_hierarchy_log_level_name = "WARNING"
_HIERARCHY_LOG_LEVEL = getattr(logging, _hierarchy_log_level_name, logging.INFO)
logger = setupLogger(log_format='%(asctime)s %(levelname)s [TAVMwareHierarchyAgent] %(message)s', level=_HIERARCHY_LOG_LEVEL, log_name="ta_vmware_inframon_hierarchy_agent.log", logger_name="ta_vmware_inframon_hierarchy_agent")
...
def _validate_runtime_adapter(adapter):
	"""
	Exercise the remote hydra runtime health endpoint with the adapter's current
	credential and return the resulting HTTP status.
	"""
	rsp, payload = adapter._runtime_request("health", method="GET", timeout=30)
	return int(payload.get("status", rsp.status))

def get_node_adapters(local_host_path, local_session_key, pool_name):
	"""
	Given the local session key return an iterable of HydraRuntimeAdapters to
	...
	@rtype: list
	@return: iterable of authenticated HydraRuntimeAdapters to all DCN's
	"""
	#Establish node list
	...
	for node_stanza in node_stanzas:
		if node_stanza.pool_name == pool_name:
			auth_token = SplunkStoredCredential.get_password(node_stanza.name, node_stanza.user, "Splunk_TA_vmware_inframon", session_key=local_session_key, host_path=local_host_path)
			if not auth_token:
				logger.warning("[get_node_adapters] missing stored credential for node=%s of pool=%s; skipping cache distribution for this node", node_stanza.name, pool_name)
				continue

			try:
				direct_adapter = HydraRuntimeAdapter(node_stanza.name, auth_token)
				direct_status = _validate_runtime_adapter(direct_adapter)
				if direct_status == 200:
					adapter_list.append(direct_adapter)
					continue
				if direct_status not in [401, 403]:
					logger.warning("[get_node_adapters] runtime health check failed for node=%s of pool=%s with status=%s; skipping cache distribution for this node", node_stanza.name, pool_name, direct_status)
					continue

				legacy_session_key = getSessionKey(node_stanza.user, auth_token, hostPath=node_stanza.name)
				if not legacy_session_key:
					logger.warning("[get_node_adapters] failed to get legacy session key for node=%s of pool=%s; skipping cache distribution for this node", node_stanza.name, pool_name)
					continue
				legacy_adapter = HydraRuntimeAdapter(node_stanza.name, legacy_session_key)
				legacy_status = _validate_runtime_adapter(legacy_adapter)
				if legacy_status == 200:
					adapter_list.append(legacy_adapter)
					logger.warning("[get_node_adapters] node=%s of pool=%s is using legacy username/password authentication path", node_stanza.name, pool_name)
				else:
					logger.warning("[get_node_adapters] legacy runtime health check failed for node=%s of pool=%s with status=%s; skipping cache distribution for this node", node_stanza.name, pool_name, legacy_status)
			except Exception as e:
				logger.exception("[get_node_adapters] failed to establish runtime adapter for node=%s of pool=%s due to error=%s", node_stanza.name, pool_name, str(e))
...
def get_datastore_info(target, user, password):
	...
	try:
		...
		for returnval in resp_xml.getElementsByTagName("returnval"):
			...
			match = re.match(r'ds:\/\/\/vmfs\/volumes\/(.*?)\/[\"\n\s\t\b]?.*$', ds_url)
			if match:
...
def distribute_hierarchy_cache(nodes, target, vms_by_host, ds_info_by_uuid):
	...
	hydra runtime adapters

	@type nodes: list of HydraRuntimeAdapter
	@param nodes: list of authenticated runtime adapters to DCN's
	@type target: str
	...
	for hga in nodes:
		try:
			logger.info("[distribute_hierarchy_cache] setting cache batch for target=%s node=%s cache_items=%s", target, hga.splunkd_uri, len(cache_items))
