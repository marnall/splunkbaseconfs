...
logger = setup_logging('ta_bitsight_bitsight')


class BITSIGHT(smi.Script):
    """Class for modular input."""

    def __init__(self):
        """Initialize the BITSIGHT Class."""
        self.global_checkbox_fields = None
        super(BITSIGHT, self).__init__()

    def get_scheme(self):
        """Overloaded splunklib modularinput method."""
        scheme = smi.Scheme('bitsight')
        scheme.description = (
    ...
    def engine(self, input_name, input_item, meta_configs, ew):
        ...
        # Explicit function call for alerts data
        params['endpoint_name'] = 'alerts'
        params['endpoint_url'] = ENDPOINT_DISPATHCER['alerts']['url']
        params['selected_companies_guid_list'] = [x['company_guid'] for x in self.selected_companies_map]
        params['selected_companies_map'] = self.selected_companies_map
        ENDPOINT_DISPATHCER['alerts']['function'](params)
    ...
    def _validate_interval(self, input_item):
        ...
        try:
            ...
            if interval < 300 or interval > 14515200:
                ...
                return False
            return True
        except ValueError:  # SyntaxError will be raised in case of empty value
            logger.error('Invalid value of parameter "Interval".')
        except (SyntaxError, TypeError):  # TypeError will be raised if key will not found in the conf
            logger.error('Interval field is not provided or invalid.')
        return False
    ...
    def _validate_start_date(self, input_item):
        ...
        try:
            formatted_date = datetime.datetime.strptime(provided_date, "%Y-%m-%d")
    ...
    def _setup_company_mapping(self, input_item):
        """Set up company mapping and configuration."""
        meta_configs = self._input_definition.metadata
        self.session_key = meta_configs['session_key']
        self.server_uri = meta_configs['server_uri']
        self.bsobject = BitsightCompanyGuidMapper(self.session_key, "spm")
    ...
    def _setup_api_configuration(self, meta_configs):
        """Set up API configuration and validate URL."""
        ...
        if url:
            ...
            elif url_prefix != "https":
                logger.info("Invalid url. Using the default url for further processing.")
        ...
        try:
            ...
            logger.error(f"{self.server_uri} is not in http(s)://hostname:port format: {str(e)}")
            return False
        ...
        try:
            ...
            for name, input_item in inputs.inputs.items():
                ...
                if not all([
                    ...
                    input_item.get("company_tree_multiselect")
                ]):
                    return False

                # Setup company mapping and configuration
                meta_configs = self._setup_company_mapping(input_item)

                # Setup and validate API configuration
                if not self._setup_api_configuration(meta_configs):
                    return False
                ...
                logger.info(
                    f"Data Indexed Successfully for all endpoints. "
...
if __name__ == '__main__':
    exit_code = BITSIGHT().run(sys.argv)
    sys.exit(exit_code)
