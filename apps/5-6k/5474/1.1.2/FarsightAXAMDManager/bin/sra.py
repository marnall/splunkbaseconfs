...
class SRAInput(Script):
    """ SRA modular input"""

    def get_scheme(self):
        scheme = Scheme("Farsight SRA")
        scheme.description = "Pulls data from SRA channel"
        ...
        channels_arg = Argument(
            name="channels",
            title="Channels(comma delimited)",
            description="https://www.farsightsecurity.com/Technical/fsi-sie-channel-guide.pdf",
            data_type=Argument.data_type_string,
            required_on_create=True,
            required_on_edit=True,
        )
    ...
    def stream_events(self, inputs, ew):
        for input_name, input_item in inputs.inputs.items():
            ...
            channels = [int(x.strip()) for x in input_item["channels"].split(',')]
            ...
            try:
                ...
                make_error_message(
                    "Error in SRA input %s: %s - Logid: %s" % (input_name, detail, e['logid']),
                    sessionKey)


if __name__ == "__main__":
    try:
        SRAInput().run(sys.argv)
    except Exception as e:  # pylint disable=broad-except
        logger.error("SRA error: " + str(e))
