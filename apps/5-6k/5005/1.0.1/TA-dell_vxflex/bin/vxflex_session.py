...
class VxFlexRequestError(Exception):
    def __init__(self, msg, status_code):
        super(VxFlexRequestError, self).__init__(msg)
        self.status_code = status_code

class VxFlexSession(object):
    """
    ...
    def __init__(self, account, session_key, modinput_helper=None, logger=None):
        """
        VxFlexSession class to connect to VxFlex REST API and request to them.
        ...
        self.state = VxFlexSession.AVAILABLE
        self.modinput_helper = modinput_helper
        self.credential_manager = vxflex_utilities.CredentialManager(session_key)

        # Logger
        if not logger:
            self.logger = vxflex_utilities.get_logger(session_key, 'vxflex_session', account.name)
        
        # Proxy 
        self.proxy_uri = vxflex_utilities.get_proxy_uri(session_key, logger)
        self.ssl_verification, self.http_scheme = vxflex_utilities.get_additional_parameters(session_key)
    ...
    def _handle_status_code(self, status_code):
        ...
        if 200 <= status_code < 300:
            ...
            msg = "VxFlex OS Gateway is disabled. Contact your VxFlex OS administrator to enable it."
            ...
            msg = "Internal server error is received. Please check in the VxFlex system and try again."
        else:
            msg = "Invalid status code"
        raise VxFlexRequestError("Status code:{} - {}".format(status_code, msg), status_code=status_code)
 
    def _request(self, url, method="GET", params=None, data=None, headers={}, auth=None):
        """
        Request to the VxFlex endpoints
        """
    ...
    def request(self, url, method="GET", params=None, data=None):
        ...
        Try 3 times with VxFlex session token of 1. token_cache 2. ckpt_file 3. authenticate
    ...
    def authenticate(self):
        ...
        try:
            ...
            res = self._request(vxflex_utilities.AUTH_URL, method="GET", auth=(self.account.username, self.account.password))
            token = str(res).strip('"')
