...
from vxflex_account import VxFlexAccount


def timer(func):
    """
    Timer keeps track of execution time for any function
    It is hardly related to VxFlexCommonDataCollector as it is using some of class variables
    """
...
class VxFlexCommonDataCollector(object):
    """
    ...
    def __init__(self, helper, event_writer, logger_name):
        ...
        self.logger = vxflex_utilities.get_logger(self.session_key, logger_name, system_name, input_name=self.input_name)
        
        # initialize session obj for all the requests
        self.vxflex_account_obj = VxFlexAccount(system_name, endpoint, username, password)
        self.session_obj = self.vxflex_account_obj.get_request_session(self.session_key, self.helper, self.logger)
    ...
    def get_system_id(self):
        ...
        """
        checkpoint_name = "{}_system".format(self.vxflex_account_obj.name)
        ...
        if not self.system_id:
            ...
            self.logger.debug("Getting system_id using system_endpoint={}.".format(vxflex_utilities.SYSTEM_ENDPOINT))
            sys_response = self.session_obj.request(url=vxflex_utilities.SYSTEM_ENDPOINT)
