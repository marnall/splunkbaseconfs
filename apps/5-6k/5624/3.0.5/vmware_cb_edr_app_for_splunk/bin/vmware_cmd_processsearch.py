...
class SplunkProcess(Process):

    def __init__(self, *args, **kwargs):
        super(SplunkProcess, self).__init__(*args, **kwargs)
...
class VMwareEDRProcessSearchCommand(GeneratingCommand):
    """ %(synopsis)
    ##Syntax
    %(syntax)
    ##Description
    ...
    def squash_error_data(self, ret_dict, cb_url="Unknown"):
        try:
            ...
            return {'sourcetype': 'vmware:cb:edr:cmd:processsearch:error', '_time': time.time(),
    ...
    def squash_data(self, data_dict, cb_url="Unknown"):
        try:
            ...
            if data_dict is None:
                ...
                return {'sourcetype': 'vmware:cb:edr:cmd:processsearch', '_time': time.time(),
    ...
    def generate(self):
        ...
        try:
            ...
            self._log.info("clients {}".format(any([x is None for x in self._clients])))
            if any([x is None for x in self._clients]):
                yield {'sourcetype': 'vmware:cb:edr:cmd:processsearch:error', '_time': time.time(),
                    'source': 'custom_command', '_raw': {"error": "Authentication hosts not assigned. Please configure the search command for authentication."}}
            else:
                p = mp.Pool(2)
...
dispatch(VMwareEDRProcessSearchCommand, sys.argv, sys.stdin, sys.stdout, __name__)
