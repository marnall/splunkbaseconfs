...
class VMwareEDRSensorSearchCommand(GeneratingCommand):
    """ %(synopsis)
    ##Syntax
    %(syntax)
    ##Description
    ...
    def threaded_action(self, org_name, cb_url, c):
        try:
            ...
            self._log.error("ThreadedAction {}".format(self.squash_error_data(errobj, cb_url)))
    ...
    def squash_error_data(self, ret_dict, cb_url="Unknown"):
        try:
            ...
            return {'sourcetype': 'vmware:cb:edr:cmd:sensorsearch:error', '_time': time.time(),
    ...
    def squash_data(self, data_dict, cb_url="Unknown"):
        try:
            ...
            ret_dict = {}
            for x in data_dict.get_info():
                self._log.info("x={}".format(x))
                v = data_dict.get(x)
                ret_dict[x] = str(v)
            return {'sourcetype': 'vmware:cb:edr:cmd:sensorsearch', '_time': time.time(),
                    'source': cb_url, '_raw': ret_dict}
        except Exception as e:
            self._catch_error(e)
    ...
    def generate(self):
        ...
        try:
            ...
            if any([x is None for x in self._clients]):
                yield {'sourcetype': 'vmware:cb:edr:cmd:sensorsearch:error', '_time': time.time(),
...
dispatch(VMwareEDRSensorSearchCommand, sys.argv, sys.stdin, sys.stdout, __name__)
