...
@Configuration()
class Zen2HanCommand(StreamingCommand):
    ...
    def stream(self, records):
        ...
        for record in records:
            if self.exclude != None:
                if self.exclude in options:
                    options[self.exclude] = False
                try:
                    record[self.outfield]= mojimoji.zen_to_han(record[target_field], kana=options['kana'], digit=options['digit'], ascii=options['ascii'])
                except KeyError:
                    continue
            else:
                try:
                    record[self.outfield]= mojimoji.zen_to_han(record[target_field]) 
                except KeyError:
                    continue

            yield record

dispatch(Zen2HanCommand, sys.argv, sys.stdin, sys.stdout, __name__)
