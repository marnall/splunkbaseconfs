...
AGARI_API_VERSION = "v1"
OAUTH_ENDPOINT = "oauth/token"
CP_ENDPOINT = "cp"
REQUESTS_TIMEOUT = 180
VERIFY_SSL = True
APP_NAME = os.path.abspath(__file__).split(os.sep)[-3]
AGARI_SETTINGS_CONF = "ta_agari_settings"
AGARI_ACCOUNT_CONF = "ta_agari_account"
AGARI_HOSTNAME = "api.agari.com"
...
def get_proxy_config():
    ...
    proxy_configuration = read_conf_file(
        GetSessionKey().session_key, AGARI_SETTINGS_CONF, stanza="proxy"
    )
...
def make_agari_url(platform_url, endpoint, api_version=AGARI_API_VERSION):
    """Create Agari URL."""
    if not (
