...
def get_audit_logs( helper, ew, base_url, access_token, page_num, start_time_obj, audit_logs_list ):
    ...
    url_to_query = base_url + '/audit?page=' + str(page_num)
    ...
    if r.status_code != 200:    # requests.codes.ok
        error_msg = 'SANDFLY_ERROR(get_audit_logs): {} {}'
        helper.log_error( error_msg.format( r.status_code, r.reason ) )
        return False, None, audit_logs_list
    ...
    helper.log_info( 'Audit Page Number : [{}]'.format( page_num ) )
    helper.log_info( 'More Results Flag : [{}]'.format( more_results ) )
    helper.log_info( 'Scroll ID         : [{}]'.format( scroll_id ) )
    helper.log_info( 'Audit Logs Total  : [{}]'.format( total_count ) )
    ...
    for d in hits_list:
        t_time_obj = datetime.strptime(d['timestamp'], "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)
        if t_time_obj > start_time_obj:
            if last_timestamp_obj is None:
                last_timestamp_obj = t_time_obj
                helper.log_debug('get_audit_logs: the first last timestamp [{}]'.format(last_timestamp_obj))
            elif t_time_obj > last_timestamp_obj:
                last_timestamp_obj = t_time_obj
                helper.log_debug('get_audit_logs: update the last timestamp [{}]'.format(last_timestamp_obj))
            audit_logs_list.append( d )
            count += 1

    helper.log_debug('get_audit_logs: count [{}]'.format(count))
    helper.log_debug('get_audit_logs: last timestamp [{}]'.format(last_timestamp_obj))

    if count < (total_count - 1):
        helper.log_debug('get_audit_logs: count [{}] < total_count [{}] set more_results to False'.format(count, total_count-1))
        more_results = False

    return more_results, last_timestamp_obj, audit_logs_list


def do_process_audit_logs( helper, ew, audit_logs_list ):
    count = 0

    if len(audit_logs_list) > 0:
        for d in audit_logs_list:
            t = {}
            t['timestamp'] = d['timestamp']
            del d['timestamp']
            t['event_type'] = 'sandfly_audit_log'
    ...
    helper.log_debug('do_process_audit_logs: count [{}]'.format(count))
...
#        b_test_mode = False
    ...
    stanza_checkpoint_timestamp = 'sandfly_audit_logs_{}_timestamp'.format(my_stanza_name)
...
#        helper.log_debug('test mode enabled - delete checkpoint key: [{}]'.format(stanza_checkpoint_timestamp))
    ...
    if checkpoint_ts is None:
        ...
        start_time_obj = datetime.strptime(checkpoint_ts, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)
    ...
    more_results, last_timestamp_obj, audit_logs_list = get_audit_logs( helper, ew, base_url, access_token, page_num, start_time_obj, audit_logs_list )
    helper.log_debug('page [{}] last timestamp [{}]'.format(page_num, last_timestamp_obj))
    if audit_logs_list is not None:
        total_count += do_process_audit_logs( helper, ew, audit_logs_list )
        helper.log_debug('page [{}] total count [{}]'.format(page_num, total_count))
        audit_logs_list.clear()

    while more_results is True:
        page_num += 1
        more_results, t_last_timestamp_obj, audit_logs_list = get_audit_logs( helper, ew, base_url, access_token, page_num, start_time_obj, audit_logs_list )
        if last_timestamp_obj is None:
            last_timestamp_obj = t_last_timestamp_obj
            helper.log_debug('last timestamp was None use page [{}] last timestamp [{}]'.format(page_num, last_timestamp_obj))
        elif t_last_timestamp_obj > last_timestamp_obj:
            last_timestamp_obj = t_last_timestamp_obj
            helper.log_debug('update page [{}] last timestamp [{}]'.format(page_num, last_timestamp_obj))
        else:
            helper.log_debug('old last timestamp [{}]'.format(last_timestamp_obj))
        if audit_logs_list is not None:
            total_count += do_process_audit_logs( helper, ew, audit_logs_list )
            helper.log_debug('page [{}] total count [{}]'.format(page_num, total_count))
            audit_logs_list.clear()
    ...
    helper.log_debug('total count    [{}]'.format(total_count))
    helper.log_debug('get_audit_logs success')

    if last_timestamp_obj is not None:
        last_timestamp_str = last_timestamp_obj.strftime( "%Y-%m-%dT%H:%M:%S.%fZ" )
