...
class ModularInput:
    ...
    # Begin Methods
    def __init__(self, app_name=None, scheme=None, cim_fields=None, **kwargs):
        try:
            if scheme is None:
                scheme = {}
            ...
            self.splunk_index("main")
            self.tracking_uuid = (
                kwargs["tracking_uuid"]
                if "tracking_uuid" in kwargs
                else str(uuid.uuid4())
            )
            ...
            self._app_home = os.path.join(
                self._splunk_home, "etc", "apps", self._app_name
            )
            ...
            raise e
        self.log.debug(
            "Starting MI __init__ : app_name={} scheme={} ".format(app_name, scheme)
        )
    ...
    def _build_scheme(self, scheme={}):
        ...
        """
        tmp = "{}{}{}{}".format(
            "<scheme><title>{}</title>".format(scheme["title"]),
    ...
    def _base_encode(self, num, alphabet=BASE62):
        ...
        """
        self.log.info(
            "action=_base_encode num={} alphabet={} type={}".format(
                num, alphabet, type(num)
            )
        )
    ...
    def _base_decode(self, sstring, alphabet=BASE62):
        ...
        """
        self.log.info(
            "action=_base_decode num={} alphabet={}".format(sstring, alphabet)
        )
    ...
    def _require_configuration(self, config, key):
        if key not in config:
            raise Exception(
                "Invalid configuration received from Splunk: key '{}' is missing.".format(
                    key
                )
            )
    ...
    def _checkpoint(self, key, value=False, checkpoint_time=None, is_object=False):
        chkpointfile = os.path.join(
            self._config["checkpoint_dir"], "{}_{}".format(self.host(), key)
        )
        if not value:
            chk_time = 0
            self._loaded_checkpoints[key] = (
            ...
            self.dbg(
                ...
                load_time=self._loaded_checkpoints[key],
                is_object=is_object,
                value=value,
                checkpoint_name=key,
                file=chkpointfile,
            ...
            try:
                if os.path.isfile(chkpointfile):
                    ...
                    self.debug(
                        ...
                        "found a value in the file for {} : {}".format(key, chk_time)
                    )
                    if not is_object:
                    ...
                    chk_time = (
                        wibbly_wobbly_timey_wimey - datetime.utcfromtimestamp(0)
                    ).total_seconds()
                    chk_time = float(chk_time)
                    if is_object:
    ...
    def _get_config(self):
        ...
        try:
                    ...
                    if stanza_name:
                        ...
                        for param in params:
                            ...
                            if (
                                ...
                                and param.firstChild.nodeType
                                == param.firstChild.TEXT_NODE
                            ):
                                data = param.firstChild.data
                                config[param_name] = data
                                self.log.debug(
                                    "XML: '{}' -> '{}'".format(param_name, data)
                                )
            ...
            raise Exception(
                "Error getting Splunk configuration via STDIN: {}".format(str(e))
            )
        return config
    ...
    def _multiple_events(self, events, time_field="timestamp", **kwargs):
        ...
        for evt in events:
            self.print_event(json.dumps(evt), time_field=time_field, **kwargs)
    ...
    def sourcetype(self, sourcetype=None):
        if sourcetype:
            self._SOURCETYPE = sourcetype
        return self._SOURCETYPE

    def splunk_index(self, splunk_index=None):
        if splunk_index:
    ...
    def _get_checkpointfile(self, key):
        return os.path.join(
            self._config["checkpoint_dir"], "{0}_{1}".format(self.host(), key)
        )
    ...
    def _get_checkpoint(self, key):
        ...
        self.debug(
            "set checkpoint load time to: {}".format(self._loaded_checkpoints[key])
        )
        try:
            if os.path.isfile(chkpointfile):
                self.debug("File Exists: {}".format(chkpointfile))
    ...
    def set_checkpoint(self, key, checkpoint_time=None, isObject=False):
        ...
        return self._checkpoint(
            key, value=True, checkpoint_time=checkpoint_time, is_object=isObject
        )
    ...
    def _set_lock(self, run_id=""):
        ...
        time_to_force_unlock = 86400
        if os.path.exists(lockfile):
            now = time.time()
            with open(lockfile, "r") as lf:
                cnts = lf.read().split(";;")
                self.fatal(
                    action="setting_lockfile",
                    msg="lockfile exists",
                    lockfile=lockfile,
                    contents=";;".join(cnts),
                )
            ...
            if time_since_lock > time_to_force_unlock:
                ...
                if self._remove_lock():
                    self._set_lock(run_id)
                return True
            else:
                self.warn(
                    action="waiting_to_remove_lock",
                    lockfile=lockfile,
                    time_since_lock=time_since_lock,
                    time_to_force_unlock=time_to_force_unlock,
                    time_until_unlock=(time_to_force_unlock - time_since_lock),
    ...
    def _remove_lock(self):
        ...
        if os.path.exists(lockfile):
            try:
                ...
                self.warn(
                    action="failed_to_remove_lockfile", lockfile=lockfile, exception=e
                )
    ...
    def print_event(
        ...
        **kwargs,
        ...
        try:
            ...
            self.log.debug(
                "successful parse of JSON data: Is Dict: {}".format(
        ...
        if isinstance(event_data, dict) or _isJson:
            if time_field not in event_data and "timestamp" not in event_data:
                ...
                self.log.debug(
                    "setting timestamp to generated time: {}".format(
                ...
                self.log.debug(
                    "setting timestamp to time_field {} time: {}".format(
                        event_data[time_field], event_data["timestamp"]
                    )
                )
            else:
                self.log.debug(
                    "unknown condition: time_field {} ".format(event_data[time_field])
                )
        ...
        explicit_time_tag = ""
        if my_time is not None and (
            isinstance(my_time, float) or isinstance(my_time, int)
        ):
        ...
        eventxml = "".join(
            [
                f"<event>{explicit_time_tag}",
                f"<data><![CDATA[{self._escape(event_data)}]]></data>",
                f"<sourcetype>{self._escape(st)}</sourcetype>",
                f"<source>{self._escape(s)}</source>",
                f"<host>{self._escape(self.host())}</host>",
                "<done /></event>\n",
            ]
        )
        # f"<index>{self._escape(self.splunk_index())}</index>",
        # self.log.debug(f"action=print_event {eventxml}")
    ...
    def print_done_event(self):
        eventxml = "<event><data></data><sourcetype>{}</sourcetype><source>{}</source><host>{}</host><done/></event>\n".format(
            self._escape(self.sourcetype()),
            self._escape(self.source()),
    ...
    def print_error(self, s):
        ...
        self.sourcetype("{}:error".format(self._app_name))
        self.log.error(
            "host={} sourcetype={} source={} {}".format(
                self.host(), self.sourcetype(), self.source(), s
            )
        )
        self._print(
            "<event><data>{}</data><index>_internal</index></event>".format(
                self._escape(s)
            )
        )
