...
# encoding = utf-8
# 
# Proofpoint Web Isolation Handler
#
...
class ModInputproofpoint_web_isolation(base_mi.BaseModInput):

    def __init__(self):
        use_single_instance = False
        super(ModInputproofpoint_web_isolation, self).__init__("ta_proofpoint_isolation", "proofpoint_web_isolation",
                                                               use_single_instance)
        self.global_checkbox_fields = None

    def get_scheme(self):
        """overloaded splunklib modularinput method"""
        scheme = super(ModInputproofpoint_web_isolation, self).get_scheme()
        scheme.title = ("Proofpoint Web Isolation")
        scheme.description = (
    ...
    def collect_events(helper, ew):
        ...
        helper.log_debug("Base URL: {0}".format(config.PFPT_WEB_ISO_ENDPOINT))
        helper.log_debug("HTTP Method: {0}".format(config.PFPT_WEB_ISO_METHOD))
        ...
        while True:
            response = None
            try:
                response = helper.send_http_request(config.PFPT_WEB_ISO_ENDPOINT, config.PFPT_WEB_ISO_METHOD,
                                                    parameters,
...
if __name__ == "__main__":
    exitcode = ModInputproofpoint_web_isolation().run(sys.argv)
    sys.exit(exitcode)
