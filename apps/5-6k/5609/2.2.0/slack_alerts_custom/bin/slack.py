...
## J081 - Editing Here
    err = "PRINT 01 " + str(repr(payload.get("result").get("index")))
    log(err)
    err2= "PRINT 02 " + str(repr(payload.get("result")))
    log(err2)


## JO81 - Return false for alerts in ONB index
    index=payload.get("result").get("index")
    if index :
        index=index.split("-")[0]
        if index=="onai" or index=="onai2" or index=="onaic" :
            return True
        else:
            log("WARN Droping slack alert requests for non onai index -" + index )
            return False
    else:
        log("WARN No index entry found default action sending slack alert")
        return True
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        #with open("/opt/splunk/var/log/splunk/slackcustom.log","w") as f:
