...
def collect_events(helper, ew):
    ...
    helper.log_info("Collecting data for Cisco Advanced Phishing Protection. Start Time: {} End Time: {}".format(start_date, end_date))
    ...
    if api_data:
       indexing_done = splunk_data_ingestion(helper, ew, api_data, index, source, "cisco:app:status")
       api_data = {}
    ...
    if len(policy_actions) == 6:
        ...
        for policy in policy_events(helper, access_token, api_host, start_date, end_date, None, policy_details=True, state=state, config_start_date=config_start_date, stanza_name=stanza_name):
            ...
            if api_data:
                indexing_done = splunk_data_ingestion(helper, ew, api_data, index, source, "cisco:app:policy_events")
            if messages_flag:
                for policy in api_data.get("policy_details"):
                    ...
                    if message:
                        ...
                        message_data_ingestion(helper, ew, encapsulate_event("messages", message, ts_field="date", options={"timestamp_format":"%Y-%m-%dT%H:%M:%S%z"}), index, source, "cisco:app:messages")
                    else:
