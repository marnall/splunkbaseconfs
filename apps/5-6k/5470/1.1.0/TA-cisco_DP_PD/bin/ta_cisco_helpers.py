...
def get(helper, access_token, url, key, params={}):
    ...
    while True:
        try:
            ...
            helper.log_debug("Response got for from Cisco server for url: {}. Status Code: {}".format(url, rslt.status_code))
            if rslt.status_code == 200:
                data = rslt.json()
                helper.log_debug("Response 200 got from Cisco server for url: {}".format(url))
                return data[key]
            elif rslt.status_code == 429:
                # rate limit response - back-off and try again
                helper.log_debug("Response 429 got from Cisco server for url: {}. Sleeping for sometime.".format(url))
                time.sleep(0.1)
                continue
            else:
                try:
...
def encapsulate_event(event_type, event_data, ts_field=None, alert_id=None, options=None):
    options = options or {}
