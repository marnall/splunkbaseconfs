...
def collect_events(helper, ew):
    ...
    api_host = cisco_utils.CISCO_HOSTNAME
    ...
    if api_data:
       indexing_done = splunk_data_ingestion(helper, ew, api_data, index, source, "cisco:dp:status")
       api_data = {}
    ...
    if api_data["alerts"]:
        indexing_done = splunk_data_ingestion(helper, ew, api_data, index, source, "cisco:dp:alerts")
    ...
    if threat_feeds_mode == "on":
        ...
        if api_threat_data:
            indexing_done = splunk_data_ingestion(helper, ew, api_threat_data, index, source, "cisco:dp:threatfeeds")
    ...
    if failure_data_mode == "all":
        ...
        if api_failure_all:
            indexing_done = splunk_data_ingestion(helper, ew, api_failure_all, index, source, "cisco:dp:failures")
        ...
        if api_failure_data:
            indexing_done = splunk_data_ingestion(helper, ew, api_failure_data, index, source, "cisco:dp:failures")
    api_data = {}
