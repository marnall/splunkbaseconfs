...
USER_AGENT = "CiscoSplunk DP_APP_Integration/v1.1.0"
...
class AccountValidator(Validator):
    """To Validate Token of Cisco Account."""

    def validate(self, value, data):
        """Validate Cisco token given by user."""
        client_id = data.get("client_id")
        client_secret = data.get("client_secret")
        hostname = cisco_utils.CISCO_HOSTNAME
        try:
            ...
            if (not client_secret) or (isinstance(client_secret, six.string_types) and len(client_secret.strip()) <= 0):
                ...
                return False
            proxies = cisco_utils.create_requests_proxy_dict()
            session = cisco_utils.requests_retry_session()
            headers = {
                "content-type": "application/json",
                "content-type": "application/x-www-form-urlencoded",
                "User-Agent": str(USER_AGENT)
            }
            payload = "client_id={}&client_secret={}".format(
                client_id, client_secret)
            url = cisco_utils.make_cisco_url(
                platform_url=hostname, endpoint=cisco_utils.OAUTH_ENDPOINT)
            response = session.post(
                url,
                headers=headers,
                data=payload,
                verify=cisco_utils.get_verify_flag(),
                proxies=proxies,
                timeout=cisco_utils.REQUESTS_TIMEOUT,
            )
            ...
            try:
                ...
                self.put_msg('{} {}. Cause -> '.format(
                    'Could not connect to provided Cisco Account.',
                    'Please recheck Cisco credentials or Proxy settings.',
                    str(e)))
                return False
            ...
            if response.get("error", "").strip() == "invalid_client":
                self.put_msg(
                    "Invalid Client ID or Client Secret. Please enter valid Cisco Client ID and Client Secret."
                )
                return False
