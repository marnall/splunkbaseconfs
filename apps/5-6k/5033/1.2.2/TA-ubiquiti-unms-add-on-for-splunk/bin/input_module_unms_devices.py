...
def validate_input(helper, definition):
    unms_devices = definition.parameters.get('unms_devices', None)
pass

def collect_events(helper, ew):
    token = helper.get_global_setting("api_token")
    unms_ip = helper.get_global_setting("unms_ip")
    unms_st = helper.get_arg("unms_devices")
    ...
    for device in devices_json:
            url_2 = 'http://' + unms_ip + '/nms/api/v2.1/devices/' + device['identification']['id']
