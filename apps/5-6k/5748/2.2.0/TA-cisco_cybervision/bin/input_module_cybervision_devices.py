...
def collect_events(helper, ew):
    ...
    try:
        ...
        while True:
            ...
            data = utils.request_get(helper, "devices", config_details, params)
            for dictionary in data:
                device_id = dictionary["id"]
                dictionary["vulnerabilities"] = utils.request_get(
                    helper,
                    "devices/{}/vulnerabilities".format(device_id),
                    config_details,
                )
            if data and (page == 1):
            ...
            utils.ingest_in_splunk(
                helper, ew, data, sourcetype, additional_fields, source="Devices"
            )
        ...
        helper.log_error(utils.DATA_COLLECTION_ERROR.format("Devices", e))
