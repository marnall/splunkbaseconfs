...
__author__ = 'ksmith'
_MI_APP_NAME = 'GoogleWorkspaceForSplunk Classroom Modular Input'
_SPLUNK_HOME = make_splunkhome_path([""])
sys.path.insert(0, make_splunkhome_path(["etc", "apps", _APP_NAME, "lib"]))
kl = KennyLoggins()
log = kl.get_logger(_APP_NAME, "classroom-modularinput", logger.INFO)

modular_input = GSuiteModularInput(app_name=_APP_NAME, scheme={
    "title": "Google Workspaces Classroom Data",
    "description": "Consumes Classroom Data from GWorkspaces",
    "args": [
        {"name": "guid",
        ...
        {"name": "servicename",
         "description": "Google Workspaces Classroom Service Name to consume",
         "title": "Service Name",
         "required": True
         },
        {"name": "write_courses",
         "description": "Google Workspaces Classroom course data to consume",
         "title": "Write Courses"
         },
        {"name": "course_ids",
...
def run():
    log.info("action=start_modular_input name=ga_classroom")
    ...
    try:
        ...
        modular_input.setup_gw("classroom")
        modular_input.get_classroom()
