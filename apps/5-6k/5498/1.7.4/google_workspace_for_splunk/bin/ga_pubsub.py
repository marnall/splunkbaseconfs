...
__author__ = 'ksmith'
_MI_APP_NAME = 'GoogleWorkspaceForSplunk PubSub Modular Input'
_SPLUNK_HOME = make_splunkhome_path([""])
sys.path.insert(0, make_splunkhome_path(["etc", "apps", _APP_NAME, "lib"]))
kl = KennyLoggins()
log = kl.get_logger(_APP_NAME, "pubsub-modularinput", logger.INFO)

modular_input = GSuiteModularInput(app_name=_APP_NAME, scheme={
    "title": "Google Workspaces PubSub Data",
    "description": "Consumes PubSub Data from GWorkspaces",
    "args": [
        {"name": "guid",
        ...
        {"name": "max_messages", "description": "Max Messages to pull", "title": "Max Messages"},
        {"name": "project", "description": "The PubSub Project", "title": "Project", "required": True},
        {"name": "subscription", "description": "The PubSub subscription", "title": "Subscription ID", "required": True},
        {"name": "credential",
...
def run():
    log.info("action=start_modular_input name=ga_pubsub")
    ...
    try:
        ...
        modular_input.setup_gw("pubsub")
        modular_input.get_pubsub()
