...
__author__ = 'ksmith'
_MI_APP_NAME = 'GoogleWorkspaceForSplunk Alert Center Modular Input'
_SPLUNK_HOME = make_splunkhome_path([""])
sys.path.insert(0, make_splunkhome_path(["etc", "apps", _APP_NAME, "lib"]))
kl = KennyLoggins()
log = kl.get_logger(_APP_NAME, "alerts-modularinput", logger.INFO)

modular_input = GSuiteModularInput(app_name=_APP_NAME, scheme={
    "title": "Google Workspaces Alert Center",
    "description": "Consumes Alert Center from GWorkspaces",
    "args": [
        {"name": "guid",
...
def run():
    ...
    try:
        ...
        modular_input.setup_gw("alerts")
        modular_input.get_alerts_report()
