...
class ModularInput:
    ...
    def _print_debug(self, s):
        myStr = "app={} evt_source={} evt_sourcetype={} evt_host={} {}".format(
    ...
    def _checkpoint(self, key, value=False, checkpoint_time=None, is_object=False):
        ...
        if not value:
            ...
            self.debug("action=read process=checkpoint load_time={}".format(self._loaded_checkpoints[key]))
            try:
                if os.path.isfile(chkpointfile):
                    self.debug("action=read process=checkpoint file={}".format(chkpointfile))
                    chk_time = self._read_file(chkpointfile)
                    self.debug("action=read process=checkpoint key={} contents={}".format(key, chk_time))
                    if not is_object:
                    ...
                    self.debug("action=read process=checkpoint sub_action=set_default_time key={}".format(key))
                    ...
                    self.debug("action=read process=checkpoint object={}".format(chk_time))
            except Exception as e:
                self._catch_error(e)
            self.debug("action=read process=checkpoint returning_object={}".format(chk_time))
            return chk_time
        else:
            try:
    ...
    def print_event(self, event_data, time_field="timestamp", explicit_time=None, sourcetype=None, source=None, **kwargs):
        st = self.sourcetype()
        s = self.source()
        if sourcetype:
            st = sourcetype
        ...
        if isinstance(event_data, dict) or _isJson:
            if time_field not in event_data and "timestamp" not in event_data:
                ...
                self.log.debug("NOT (time_field not in event_data and \"timestamp\" not in event_data)  NOR (time_field in event_data and \"timestamp\" not in event_data) : time_field_value=\"{}\" time_field={} ".format(event_data[time_field], time_field))
        ...
        eventxml = "<event>{}<data><![CDATA[{}]]></data><sourcetype>{}</sourcetype><source>{}</source><host>{}</host><done /></event>\n".format(
            explicit_time_tag, self._escape(event_data), self._escape(st), self._escape(s),
    ...
    def print_error(self, s):
        tmp = self.sourcetype()
        self.sourcetype("{}:error".format(self._app_name))
        self.log.error("evt_host={} evt_sourcetype={} evt_source={} {}".format(self.host(), self.sourcetype(), self.source(), s))
