...
class ModInputDocuSign_Accounts(Script):

	# Define some global variables
	INPUT_TYPE = "docusign_accounts"
	MASK = "*********"

	def get_scheme(self):
		""" Called by Splunk to determine the configuration inputs for this module. """

		scheme = Scheme("DocuSign Accounts")
		scheme.description = ("Ingests DocuSign Account data")
	...
	def get_access_token(self, authServer, userId, clientId, rsaKeyClear):
		...
		later = now + 3600
		claim = {"iss": clientId, "aud": parsed_authServer.netloc, "iat": now, "exp": later, "scope": "impersonation organization_read signature", "sub": userId}
...
if __name__ == "__main__":
	...
	logger = setup_logger(ModInputDocuSign_Accounts.INPUT_TYPE)

	## Run the Modular Input
	exitcode = ModInputDocuSign_Accounts().run(sys.argv)
	sys.exit(exitcode)
