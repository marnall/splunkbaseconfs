    ...
    def get_and_write_events(self, ew, lensUrl, access_token, fetchLimit, cursor):
        ...
        try:
            ...
            if len(lens_data['data']) == 0:
                logger.info(f'events_received=0 from cursor={cursor}, end_cursor={lens_data["endCursor"]}')
                return lens_data["endCursor"]
            
            ## Write out all the events to the event writer, override the timestamp with the event's timestamp
            monitor_events_count, monitor_alerts_count = (0, 0)
            for ev in lens_data["data"]:
                if ev["object"] == 'Alert':
                    monitor_alerts_count += 1
                else:
            ...
            logger.info("events_received=%s from cursor=%s, end_cursor=%s, monitor_events_count=%s, monitor_alerts_count=%s" % (len(lens_data["data"]),cursor,lens_data["endCursor"],monitor_events_count,monitor_alerts_count))
