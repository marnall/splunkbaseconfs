...
LOG_FILENAME = os.path.join(SPLUNK_HOME,"var","log","splunk","criblalert_app_setuphandler.log")
...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        try:
            ...
            entities = entity.getEntities(['storage', 'passwords'], namespace="cribl_alert", owner='nobody', sessionKey=self.getSessionKey())
            ...
            for i, c in entities.items():
                if c['eai:acl']['app'] ==  "cribl_alert":
                    credential_list.append(c['clear_password'])
                    credential_key_list.append(c['username'])


            global_entities = entity.getEntities(['admin', 'alert_actions','cribl'], namespace="cribl_alert", owner='nobody', sessionKey=self.getSessionKey())
            ...
            confInfo['criblalert'].append('param.log_level', log_level) 
            confInfo['criblalert'].append('param.activationkey', activationkey) 
            confInfo['criblalert'].append('credential', "::".join(credential_list))
            confInfo['criblalert'].append('credential_key', "::".join(credential_key_list))
    ...
    def handleEdit(self, confInfo):
        ...
        try:
            ...
            try:
                ...
                for i, c in entities.items():
                    if c['eai:acl']['app'] ==  "cribl_alert":
                        entity.deleteEntity(['storage', 'passwords'],":%s:" % c['username'],namespace="cribl_alert", owner='nobody', sessionKey=self.getSessionKey())           
            except:  
                e = sys.exc_info()[0]  
                logger.error("Error deleting cribl_alert credential , perhaps this is the first setup run and it did not yet exist (that is ok) : %s" % e) 
            

            for credential_key,credential in zip(credential_key_str.split('::'),credential_str.split('::')):
                try:
                    logger.debug("creating cribl_alert credential")
                    new_credential = entity.Entity(['storage', 'passwords'], credential_key, contents={'password':credential}, namespace="cribl_alert",owner='nobody')
                    entity.setEntity(new_credential,sessionKey=self.getSessionKey())
                except:  
                    e = sys.exc_info()[0]  
                    logger.error("Error creating cribl_alert credential : %s" % e)
            
            try:
                logger.debug("creating global settings")
                new_global_settings= entity.Entity(['admin', 'alert_actions'], 'cribl', contents={'param.log_level':log_level,'param.activationkey':activationkey}, namespace="cribl_alert",owner='nobody')
