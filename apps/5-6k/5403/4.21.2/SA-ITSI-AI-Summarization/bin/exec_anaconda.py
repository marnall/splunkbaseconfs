...
from util.base_util import get_apps_path, get_app_version
...
def setup_psc_python_path():
    """Sets up the PSC Python path based on the platform and architecture."""
    ...
    if not os.path.isdir(sa_path):
        raise RuntimeError(f"Failed to find Python for Scientific Computing Add-on ({sa_scipy})")

    system_path = os.path.join(sa_path, "bin", SUPPORTED_SYSTEMS[system])
...
def exec_anaconda():
    ...
    if not os.access(python_path, os.X_OK):
        try:
            if system[0] != 'Windows':
                ...
                logger.debug(f"Set executable permissions on {python_path}")
        except OSError as e:
            if e.errno == 30:
                logger.warning(
                    f"Failed to set executable permissions on {python_path}: "
                    "This is expected if the file system is read-only."
                )
            else:
                logger.warning(
                    f"Failed to set executable permissions on {python_path}: {e}"
                )
        except Exception as e:
            logger.error(
                f"Unexpected error while setting executable permissions on {python_path}: {e}"
            )
    ...
    try:
        ...
        if e.errno == errno.EACCES:
            # Fallback: run the wrapper via bash
            os.execl("/bin/bash", "bash", python_path, *sys.argv)
    except Exception:
