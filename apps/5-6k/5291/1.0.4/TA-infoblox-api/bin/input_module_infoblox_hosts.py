...
def collect_events(helper, ew):
    ...
    global_next_page = 0
    global_resource = helper.get_global_setting("rest_api_home")+"/record:host?_paging=1&_return_as_object=1&_max_results="+helper.get_arg('item_limit')+"&_return_fields="+helper.get_arg('requested_fields')
    ...
    try:
        hosts_response = helper.send_http_request(global_resource, "GET", parameters=None, payload=None, headers=dict_headers, cookies=None, verify=False, cert=None, timeout=opt_global_timeout, use_proxy=False)
        hosts_json = hosts_response.json()
        
    except Exception as e:
        err = "Error during Infoblox get /record:host method. Response cannot be converted to json. Check credentials or URL.\nException: "+str(e)
        helper.log_error(err)
    ...
    try:
        length = len(hosts_json['result'])
        for i in range(length):
            ew.write_event(helper.new_event(source=global_source, index=global_index, sourcetype=global_source_type, data=json.dumps(hosts_json['result'][i])))
    ...
    try:
        global_next_page = hosts_json['next_page_id']
    ...
    while global_next_page != 0:
        #prepare request
        try:
            hosts_response = helper.send_http_request(global_paged_resource+global_next_page, "GET", parameters=None, payload=None, headers=dict_headers, cookies=None, verify=False, cert=None, timeout=opt_global_timeout, use_proxy=False)
            hosts_json = hosts_response.json()
        
        except Exception as e:
            err = "Error during Infoblox get next page of /record:host method. Response cannot be converted to json. Check credentials or URL.\nException: "+str(e)
            helper.log_error(err)
            #ew.write_event(helper.new_event(source=global_source, index=global_index, sourcetype=global_source_type, data=err))
