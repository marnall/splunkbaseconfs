...
@Configuration()
class FeedDomainRiskCommand(EventingCommand):
    """This custom search command makes a request to the domainrisk-feed-(api) API endpoint and
    retrieves realtime domain risk information for apex-level domains, regardless of observed traffic

    Inherits from the EventingCommand custom search type. Override the `transform` method as the entrypoint to this script

    Example:
        | dtfeeddomainrisk after=-60
    """
    ...
    def transform(self, records):
        ...
        self.dt_log = DTLogger(
            "feed_domainrisk", os.path.basename(__file__), self.get_user(), self.feature
        )
        self.dt_log.info("starting feed_domainrisk.py")
        ...
        try:
            ...
            results = api.realtime_domain_risk(
                after=self.after,
...
dispatch(FeedDomainRiskCommand, sys.argv, sys.stdin, sys.stdout, __name__)
