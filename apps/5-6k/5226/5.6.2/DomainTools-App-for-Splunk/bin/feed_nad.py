...
@Configuration()
class FeedNadCommand(EventingCommand):
    """This custom search command makes a request to the newly-active-domains-feed-(api) API endpoint and
    retrieves apex-level domains (e.g. example.com but not www.example.com) DomainTools has newly observed in our DNS sensor network
    ...
    def transform(self, records):
        ...
        self.dt_log = DTLogger(
            "feed_nad", os.path.basename(__file__), self.get_user(), self.feature
        )
        self.dt_log.info("starting feed_nad.py")
...
dispatch(FeedNadCommand, sys.argv, sys.stdin, sys.stdout, __name__)
