...
@Configuration()
class FeedDomainHotlistCommand(EventingCommand):
    """This custom search command makes a request to the domainhotlist-feed-(api) API endpoint and
    retrieves high-risk, apex-level domains that are observed by DomainTools' global sensor network to be active within 24 hours

    Inherits from the EventingCommand custom search type. Override the `transform` method as the entrypoint to this script

    Example:
        | dtfeeddomainhotlist after=-60
    """
    ...
    def transform(self, records):
        ...
        self.dt_log = DTLogger(
            "feed_domain_hotlist", os.path.basename(__file__), self.get_user(), self.feature
        )
        self.dt_log.info("starting feed_domain_hotlist.py")
        ...
        try:
            ...
            for response in results.response():
                ...
                yield {
                    ...
                    "phishing_risk": feed_result.get("phishing_risk"),
                    "malware_risk": feed_result.get("malware_risk"),
                    "spam_risk": feed_result.get("spam_risk"),
                    "proximity_risk": feed_result.get("proximity_risk"),
                    "overall_risk": feed_result.get("overall_risk"),
                    "expires": feed_result.get("expires"),
                }
...
dispatch(FeedDomainHotlistCommand, sys.argv, sys.stdin, sys.stdout, __name__)
