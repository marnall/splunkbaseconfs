...
try:
    ...
    if params:
        decoded = json.loads(unquote(params))
        api_key = decoded.get("farsight_key")
    else:
        api_key = get_credentials(settings["sessionKey"])

    proxy = get_proxy(settings["sessionKey"])
    ...
    if proxy != "":
        ...
        dnsdb = dnsdb2.Client(api_key, swclient=swclient, version=version)
    ...
    try:
        ...
        raise Exception("Authorization failed. Check Farsight API key")
