...
def get_flex_function(client, query_type, match_type):
    ...
    function_map = {
        "rdata": {"regex": client.flex_rdata_regex, "glob": client.flex_rdata_glob},
        "rrnames": {
...
try:
    dt_log = DTLogger("dnsdb", os.path.basename(__file__), "unknown", "flexsearch")
    ...
    if not query:
        error = splunk.Intersplunk.generateErrorResults(
            "Usage: | dtdnsdbflex query_type=<rdata|rrnames> match_type=<glob|regex> query=<glob or regex pattern> "
