...
@Configuration()
class EscalateIrisDetectCommand(EventingCommand):
    """This custom search command makes a request to the Iris Detect Escalation API endpoint and outputs the results.

        Inherits from the EventingCommand custom search type. Override the `transform` method as the entrypoint to this script

        Example:
            | dtirisdetectescalate escalation_type="blocked" domain_id="<domain_id>"
    """
    ...
    escalation_type = Option(
        doc="""
                **Syntax:** **escalation_type=***<params>*
                **Description:** Add domains to blocklist or escalate to Google Safe Browse. """,
        default="blocked",
        require=True,
    )
    ...
    def transform(self, records):
        ...
        self.dt_log.info("EscalateIrisDetectCommand: starting escalate_iris_detect.py")

        self.dt_log.info("EscalateIrisDetectCommand: Escalating domain in Iris Detect")
        try:
            api_wrapper = DtApiWrapper(self.service, self.dt_log)
            api = api_wrapper.create_dt_api()

            response = api.iris_detect_escalate_domains([self.domain_id], self.escalation_type)
            self.dt_log.info(f"Detectdashboard-action: {self.feature}")
            if self.feature.lower() != "detectdashboard-action":
                ...
                for result_data in iris_detect_results_kvstore.data.query(query=json.dumps({"dt_domain_id": self.domain_id})):
                    if result_data.get("dt_escalations"):
                        result_data["dt_escalations"].append(self.escalation_type)
                    else:
                        result_data["dt_escalations"] = [self.escalation_type]
                    result_data["dt_state"] = "watched"
            ...
            self.dt_log.error(e)
            raise Exception(dt_exception_messages.escalated) from e

        self.dt_log.info("EscalateIrisDetectCommand: completed escalate_iris_detect.py")


dispatch(EscalateIrisDetectCommand, sys.argv, sys.stdin, sys.stdout, __name__)
