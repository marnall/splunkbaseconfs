...
def setup_logger(level):
    ...
    log_file = os.path.join(
        sys.path[0], "..", "..", "..", "..", "var", "log", "splunk", "honeydb.log"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger = setup_logger(logging.ERROR)
        logger.error(
            "Sensor Data Error: File %s data read error %s ", jsonfile, jsonerror
        )
        sys.exit()
    ...
    if ("X-HoneyDb-ApiId" in args) and ("X-HoneyDb-ApiKey" in args):
        ...
        logger = setup_logger(logging.ERROR)
        logger.error(
            "Sensor Data Error: HoneyDB args X-HoneyDb-ApiId OR/AND X-HoneyDb-ApiKey missing in file : ./%s ",
            jsonfile,
        )
        sys.exit()
    ...
    if apiId and apiKey:
        ...
        try:
            ...
            logger = setup_logger(logging.ERROR)
            logger.error(
                "Sensor Data Error: problem initializing from_id file : ./%s",
                from_id_file,
            )
        ...
        # call api
        try:
            url = "https://honeydb.io/api/sensor-data{}?sensor-data-date={}&from-id={}".format(
                mydata, sensor_data_date, from_id
            )
            ...
            if response.status_code != 200:
                logger = setup_logger(logging.ERROR)
                logger.error(
                    "Sensor Data Error: API error with status code: %s ",
        ...
        try:
            ...
            if eventjson:
                ...
                try:
                    ...
                    logger = setup_logger(logging.ERROR)
                    logger.error(
                        "Sensor Data Error: problem writing from_id file : .%s",
                        from_id_file,
                    )
            ...
            logger = setup_logger(logging.ERROR)
            logger.error(
                "Events API call failed . Please check your authentication key or check with HoneyDB support team. API response code : %s",
                response.status_code,
            )
            sys.exit()
    else:
        logger = setup_logger(logging.ERROR)
        logger.error(
            "HoneyDB API ID and API Key can not be blank. Please add your API ID and Key to the honeydb.json file."
        )
        sys.exit()
