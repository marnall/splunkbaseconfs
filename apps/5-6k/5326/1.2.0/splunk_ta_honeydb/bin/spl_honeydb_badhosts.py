...
if __name__ == "__main__":
    ...
    try:
        ...
        logger = setup_logger(logging.ERROR)
        logger.error(
            "Bad Hosts Error: File %s data read error %s ", jsonfile, jsonerror
        )
        sys.exit()
    ...
    if ("X-HoneyDb-ApiId" in args) and ("X-HoneyDb-ApiKey" in args):
        ...
        logger = setup_logger(logging.ERROR)
        logger.error(
            "Bad Hosts Error: HoneyDB args X-HoneyDb-ApiId OR/AND X-HoneyDb-ApiKey missing in file : ./%s ",
            jsonfile,
        )
        sys.exit()
    ...
    if apiId and apiKey:
        ...
        if response.status_code != 200:
            logger = setup_logger(logging.ERROR)
            logger.error(
                "Bad Hosts Error: API error with status code: %s ", response.status_code
            )
        ...
        try:
            ...
            logger = setup_logger(logging.ERROR)
            logger.error(
                "Bad Hosts API call failed . Please check your authentication key or check with HoneyDB Support team. API response code: %s",
        ...
        logger = setup_logger(logging.ERROR)
        logger.error(
            "HoneyDB API key ID and Secret Key can not be blank. Please Enter the right keys"
        )
        sys.exit()
