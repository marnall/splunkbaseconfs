...
class Devices(base_script.BaseScript):
    """Class for collecting Devices data from Cisco Meraki."""

    def __init__(self):
        """Initialize the Devices class."""
        super(Devices, self).__init__()
        self.logfile_prefix = (
            "splunk_ta_cisco_meraki_devices"
        )
        self.sourcetype = utils.DEVICES_SOURCETYPE

    def get_scheme(self):
        """Overloaded splunklib modularinput method to get scheme for the modular input."""
        scheme = smi.Scheme("devices")
        scheme.description = "Devices"
