...
def get_clients_from_file():
    fp = open('../local/all_unmonitored_clients.conf','r')
    contents = fp.read()
    content_list = contents.split("\n")
    if(content_list[1] == "None"):
        return []
    client_list = []
    for i in range(1,len(content_list)-1):
        client_list.append(content_list[i])
    return client_list
...
def get_commserver():
    ...
    contents = fp.read()
    content_list = contents.split("\n")
    webserver = content_list[0].strip("]")
    webserver = webserver.split("[")[1]
    return webserver

try:
    SL.make_entry("monitor_all_clients", "**********Started monitioring operation in monitor_all_clients script**************")
    ...
    if(not check_index_conf()):
        ...
        index_flag = True
    client_list_old = get_clients_from_file()
    commserve = get_commserver()
    SL.make_entry("monitor_all_clients","Getting commcell credentials")
    username,password,url = helper.get_credentials(commserve, session_key)
    SL.make_entry("monitor_all_clients","Getting authcode associated with commcell")
    auth_code = helper.get_authcode(username, password, url)

    client_conf_status = {}
    SL.make_entry("monitor_all_clients","started to write to client.conf and new_client.conf")
    failed_clients = []
    deconfig_clients = []
    client_list = []
    ...
    for i in client_list_old:
        if(i not in buffered_clients):
            fp_buffer.write(i + "\n")
    fp_buffer.close()

    for i in client_list_old:
        if(i in buffered_clients):
            SL.make_entry("monitor_all_clients", "Client " + i + " already in processing")
            continue

        client_list.append(i)
        SL.make_entry("monitor_all_clients","checking Splunk software status for client " + i)
        try:
            client_prop = helper.client_prop_byname(auth_code, i, url)
            is_deleted_client = client_prop["clientProperties"][0]["clientProps"]["IsDeletedClient"]
            if(is_deleted_client):
                SL.make_entry("start_monitoring", i + " is a de-configured client, skipping it for monitoring")
                deconfig_clients.append(i)
                clean_operation.clean(i, commserve)
                continue
            client_id = str(client_prop["clientProperties"][0]["client"]["clientEntity"]["clientId"])
            info_list = installation_helper.check_software_status(i, commserve, client_id, auth_code, url)
        except Exception as excp:
            SL.make_entry("monitor_all_clients","ERROR Failed to check software status for client " + i + " Check client connectivity")
            failed_clients.append(i)
            clean_operation.clean(i, commserve)
            helper.commit_failed_clients(i, commserve, 'Check software status')
            continue
        ...
        if not software_status:
            SL.make_entry("monitor_all_clients","Splunk Software Not initialized on the client " + i)
            SL.make_entry("monitor_all_clients","Checking for Software on the local machine")
            status, path = helper.check_software_available(os_type)
            indexer_ip_port = get_indexer_info()
            if(status and indexer_ip_port != ""):
                SL.make_entry("monitor_all_clients","Software found and started remote installation on client machine")
                try:
                    installation_helper.install_software(os_type, client_id, i, path, indexer_ip_port, url, auth_code)
                except Exception as excp:
                    SL.make_entry("monitor_all_clients","ERROR Software installation failed for client " + i + " " + str(excp) + " Try to manually install the forwarder and then select the client for monitoring")
                    failed_clients.append(i)
                    clean_operation.clean(i, commserve)
                    helper.commit_failed_clients(i, commserve, "Install software")
                    continue

                SL.make_entry("monitor_all_clients","Software installation completed successfully")
                client_conf_status[i] = True
                software_pre_installed = False
            else:
                SL.make_entry("monitor_all_clients","Software or indexer info not found, staging client for further processing")
                fp_software = open("../local/software_status.conf","a")
                fp_software.write("[" + commserve  + "]\n")
                fp_software.write("clientname:" + i + '\n')
                fp_software.write("ostype:" + os_type + '\n')
                fp_software.close()
                software_pre_installed = True
                client_conf_status[i] = False
        else:
            SL.make_entry("monitor_all_clients","Splunk Software found on client machine, staging client for further processing")
            client_conf_status[i] = False
            software_pre_installed = True
    ...
    SL.make_entry("monitor_all_clients","Started collecting addition information about clients")
    for client_name in client_list:
        try:
            if client_name not in failed_clients and client_name not in deconfig_clients:
                ...
                is_deleted_client = client_prop["clientProperties"][0]["clientProps"]["IsDeletedClient"]
                if(is_deleted_client):
                    SL.make_entry("monitor_all_clients", client_name + " is a de-configured client, skipping it for monitoring")
                ...
                SL.make_entry("monitor_all_clients","Getting log directory of the client " + client_name)
                try:
                    ...
                    SL.make_entry("monitor_all_clients","Failed to get log directory of client " + str(excp))
                    clean_operation.clean(client_name, commserve)
                    helper.commit_failed_clients(client_name, commserve, 'Get log dir')
                    continue
                SL.make_entry("monitor_all_clients","Log Directory path detected " + log_dir)
                ...
                if index_flag and client_conf_status[client_name]:
                    ...
                    SL.make_entry("monitor_all_clients","Passing monitoring command to client with index " + index_name)
                    try:
                        installation_helper.monitor_file(client_ip, index_name, log_dir)
                    except Exception as excp:
                        SL.make_entry("monitor_all_clients","ERROR Failed to monitor client " + client_name)
                        clean_operation.clean(client_name, commserve)
                        helper.commit_failed_clients(client_name, commserve, 'Monitor log files')
                        continue
                    installation_helper.restart_splunk(client_ip)
                    SL.make_entry("monitor_all_clients","Monitoring Log Files Successfully for client " + client_name)
            ...
            SL.make_entry("monitor_all_clients","ERROR Failed to get additional information about the client " + client_name)
            clean_operation.clean(client_name, commserve)
    ...
    SL.make_entry("monitor_all_clients","monitor_all_clients finished execution")

except Exception as excp:
    exc_type, exc_obj, exc_tb = sys.exc_info()
    entry_content = "ERROR " + str(exc_tb.tb_lineno) + ' ' + str(excp)
    SL.make_entry("monitor_all_clients", entry_content)
