"""
KV Store Backup search command
...
class KVStoreBackupCommand(GeneratingCommand):
	...
	def generate(self):
		...
		app_list = kv.get_server_apps(splunkd_uri, auth_header, self.app)
		logger.debug("Apps list: %s" % str(app_list))
		collection_list = kv.get_app_collections(splunkd_uri, auth_header, self.collection, self.app, app_list, self.global_scope)
		...
		for collection in collection_list:
			...
			result, message, total_record_count = kv.download_collection(logger, splunkd_uri, auth_header, entry_app, collection_name, output_file, self.compression)
