...
def get_server_apps(uri, auth_header, app = None):
	apps = []
	if app is not None:
		apps.append(app)
	else:
		...
		[response, response_code] = request('GET', apps_uri, "", headers)
...
def get_app_collections(uri, auth_header, selected_collection, selected_app, app_list, global_scope):
	...
	for app in app_list:
		...
		try:
			...
			if response_code == 200:
				...
				eprint(f"Response code: {response_code}, Response: {response}")
...
def copy_collection(logger, source_auth_header, source_uri, target_auth_header, target_uri, app, destapp, collection, create, append):
	# Enumerate all of the collections in the app (if an app is selected)
	...
	try:
		...
		result, message, record_count = download_collection(logger, source_uri, source_auth_header, app, collection, output_file, True)
		...
		if (result == "success") and not append:
			# Delete the target collection prior to uploading
			delete_start_time = time.time()
			response_code = delete_collection(logger, target_uri, target_auth_header, destapp, collection)
			delete_end_time = time.time()
			logger.debug("Response code for pre-upload deletion of collection %s/%s: %d", app, collection, response_code)

		if result == "success":
			if create:
				# See if the source collection exists on the destination
				collection_check = get_app_collections(target_uri, target_auth_header, collection, destapp, [destapp], False)
				logger.debug("Response for pre-creation check of collection %s/%s: %d", destapp, collection, collection_check)
				if len(collection_check) == 0: # Collection does not exist
					# Get the schema of the source collection
					[schema, response_code] = get_collection_schema(logger, source_uri, source_auth_header, app, collection)
					logger.debug('Schema response for collection %s/%s: %s', destapp, collection, json.dumps(schema))
					# Write the collection and schema to the remote destination/app
					[result, response_code] = create_collection(logger, target_uri, target_auth_header, destapp, collection, schema)
			upload_start_time = time.time()
			result, message, posted = upload_collection(logger, target_uri, target_auth_header, destapp, collection, output_file)
		...
		retval = { "app": app, "collection": collection, "result": result, 
		...
		if destapp is not None:
			retval['destapp'] = destapp
		return retval

	except BaseException as e:
		raise Exception("Error copying collection %s/%s from %s to %s: %s" % (app, collection, source_host, target_host, repr(e)))

def delete_collection(logger, remote_uri, target_auth_header, app, collection):
	...
	# Set request headers
	headers = {
		'Authorization': target_auth_header,
		'Content-Type': 'application/json'}
	...
	try:
		...
		logger.debug('Server response for deletion of collection %s/%s: (%d) %s', app, collection, response_code, response)
		if response_code == 200:
...
def download_collection(logger, remote_uri, auth_header, app, collection, output_file, compress=False):
	# Set request headers
	headers = {
		'Authorization': auth_header,
		'Content-Type': 'application/json'
	...
	try:
		...
		while (loop_record_count is None or loop_record_count == batch_size):
			...
			[response, response_code] = request('GET', remote_data_url, '', headers)
			response = response.decode('utf-8')
			logger.debug('Download response %d for collection %s/%s: %s', response_code, app, collection, response[:100] if len(response) > 100 else response)
...
def upload_collection(logger, remote_uri, remote_auth_header, app, collection, file_path):
	# Set request headers
	headers = {
	...
	if not append:
		# Delete the collection contents
		try:
			delete_collection(logger, splunkd_uri, auth_header, file_app, file_collection)
...
def get_collection_schema(logger, uri, auth_header, app, collection):
	# Set request headers
	headers = {
		'Authorization': auth_header,
		'Content-Type': 'application/json'
	...
	# Config options
	url_tmpl_collection_config = '%(server_uri)s/servicesNS/%(owner)s/%(app)s/storage/collections/config/%(collection)s?output_mode=json'

	try:

		# Build the URL
		remote_data_url = url_tmpl_collection_config % dict(
			server_uri = uri,
			owner = 'nobody',
			app = app,
			collection = collection)
		...
		[response, response_code] = request('GET', remote_data_url, '', headers)
		response = response.decode('utf-8')
		#logger.debug('get_collection_config response %s', response)
		#logger.debug('get_collection_config response_code %s', response_code)

		if response_code == 200:
			logger.info('Retrieved KV store collection config successfully: %s/%s' % (app, collection))
			schema = json.loads(response)['entry'][0]['content']
			# Parse the schema
			for setting in list(schema.keys()):
				for startFilter in ['replication_', 'eai:', 'profiling', 'disabled']:
					if setting.startswith(startFilter):
						if setting in list(schema.keys()):
							del schema[setting]
							continue
				if not setting in list(schema.keys()):
					continue
				if schema[setting] == 'undefined':
					del schema[setting]
					continue
			...
			schema = {}
			logger.error(f'Error retrieving config for collection={collection}: \n\t{response}')
			result = "error"
			message = response

	except BaseException as e:
		logger.error(f'Failed to retrieve config for collection={collection}: {repr(e)}', exc_info=True)
		result = "error"
		message = repr(e)

	return schema, response_code

def create_collection(logger, uri, auth_header, app, collection, schema):
	# Set request headers
	headers = {
		'Authorization': auth_header,
		'Content-Type': 'application/json'
	...
	# Config options
	url_tmpl_collection_create = '%(server_uri)s/servicesNS/%(owner)s/%(app)s/storage/collections/config?output_mode=json'
	url_tmpl_collection_schema = '%(server_uri)s/servicesNS/%(owner)s/%(app)s/storage/collections/config/%(collection)s?output_mode=json'

	try:
		# Build the create URL
		create_url = url_tmpl_collection_create % dict(
			server_uri = uri,
			owner = 'nobody',
			app = app)
		
		# Build the schema URL
		schema_url = url_tmpl_collection_schema % dict(
			server_uri = uri,
			owner = 'nobody',
			app = app,
			collection = collection)
		...
		# Download the data from the collection
		[response, response_code] = request('POST', create_url, f'name={collection}', headers)
		response = response.decode('utf-8')
		logger.debug('create response %s %s', response_code, response)
		#logger.debug('get_collection_config response_code %s', response_code)

		if response_code == 201:
			logger.info('Created KV store collection successfully: %s/%s' % (app, collection))
			result = "success"
			message = "Created collection"
			if schema is not None:
				try:
					# Write the schema to the collection
					[response, response_code] = request('POST', schema_url, schema, headers)
					response = response.decode('utf-8')
					logger.debug('schema response %s %s', response_code, response)

					result = "success"
					message = "Created collection with schema"
				except Exception as e:
					logger.error(f'Error adding schema to collection={collection}: \n\t{response}')
					result = "error"
					message = response
		else:
			schema = {}
			logger.error(f'Error creating collection={collection}: \n\t{response}')
			result = "error"
			message = response

	except BaseException as e:
		logger.error(f'Error creating collection={collection}: {repr(e)}', exc_info=True)
		result = "error"
		message = repr(e)
