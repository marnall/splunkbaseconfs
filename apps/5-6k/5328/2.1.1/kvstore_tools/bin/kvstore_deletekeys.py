"""
KV Store Collection Bulk Record Deleter search command
...
class KVStoreDeleteKeysCommand(StreamingCommand):
	...
	def stream(self, events):
		...
		self.auth_header = 'Splunk ' + self.session_key
		...
		app_list = kv.get_server_apps(self.splunkd_uri, self.auth_header, self.app)
		collection_list = kv.get_app_collections(self.splunkd_uri, self.auth_header, self.collection, self.app, app_list, True)
