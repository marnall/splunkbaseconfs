...
class KVStorePushCommand(GeneratingCommand):
	...
	def generate(self):
		...
		if 'run_kvstore_push' in current_user_capabilities or 'run_kvst_all' in current_user_capabilities or current_user in['splunk-system-user', 'admin', 'sc_admin']:
		...
		for host in target_list:
			...
			local_app_list = kv.get_server_apps(splunkd_uri, local_auth_header, self.app)
			local_collection_list = kv.get_app_collections(splunkd_uri, local_auth_header, self.collection, self.app, local_app_list, self.global_scope)
			...
			for local_collection in local_collection_list:
				...
				try:
					yield(kv.copy_collection(logger, local_auth_header, splunkd_uri, remote_auth_header, remote_uri, collection_app, self.destapp, collection_name, self.create, self.append))
				except BaseException as e:
