...
class KVStoreRestoreCommand(GeneratingCommand):
	...
	def generate(self):
		...
		auth_header = 'Splunk ' + session_key
		...
		for name in backup_file_list:
			...
			if (name.endswith('.json') or name.endswith('.json.gz')) and len(name_split)==3:
				...
				file_app = self.destapp if self.destapp is not None else name_split[0]
				...
				if list_only:
					...
					if data_bytes > 0:
						if not self.append:
							...
							try:
								...
								if not collection_id in deleted_collections:
									kv.delete_collection(logger, splunkd_uri, auth_header, file_app, file_collection)
						...
						try:
							result, message, record_count = kv.upload_collection(logger, splunkd_uri, auth_header, file_app, file_collection, name)
