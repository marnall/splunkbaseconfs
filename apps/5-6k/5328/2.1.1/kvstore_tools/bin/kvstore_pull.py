...
class KVStorePullCommand(GeneratingCommand):
	...
	destapp = Option(
		doc='''
			Syntax: destapp=<appname>
			Description: The app to upload collections to''',
			require=False)
	
	create = Option(
		doc='''
			Syntax: create=[true|false]
			Description: Create the collection if it doesn't exist''',
			require=False)
	...
	def generate(self):
		...
		local_auth_header = 'Splunk ' + local_session_key
		...
		if 'run_kvstore_pull' in current_user_capabilities or 'run_kvst_all' in current_user_capabilities or current_user in['splunk-system-user', 'admin', 'sc_admin']:
		...
		# Support a destination app
		if self.destapp:
			if self.app:
				logger.debug('Destination App: %s' % self.destapp)
			else:
				ui.exit_error("Must provide 'app' setting when specifying 'destapp'")
		else:
			if self.app:
				self.destapp = self.app
			else:
				self.destapp = None

		if self.create:
			logger.debug('Create: %s' % self.create)
		else:
			self.create = False
		...
		try:
			...
			logger.debug('remote_user %s', remote_user)
		except BaseException as e:
		...
		try:
			...
			if remote_user != '':
				...
				remote_auth_header = remote_service.token
			else:
				# Use the password as an API token of no username is supplied
				remote_auth_header = f'Bearer {remote_password}'
		...
		try:
			remote_app_list = kv.get_server_apps(remote_uri, remote_auth_header, self.app)
		except (urllib.error.HTTPError, BaseException) as e:
			ui.exit_error('Failed to get apps on remote Splunk instance: %s' % repr(e))
		
		try:
			remote_collection_list = kv.get_app_collections(remote_uri, remote_auth_header, self.collection, self.app, remote_app_list, self.global_scope)
		except (urllib.error.HTTPError, BaseException) as e:
			ui.exit_error('Failed to get collections list on remote Splunk instance: %s' % repr(e))
		...
		for remote_collection in remote_collection_list:
			...
			try:
				yield(kv.copy_collection(logger, remote_auth_header, remote_uri, local_auth_header, splunkd_uri, collection_app, self.destapp, collection_name, self.create, self.append))
