...
class IPQS(GeneratingCommand):
    ...
    value_type = Option(require=True, default=None)
    ...
    def generate(self):
        try:
            ...
            if not self.value_type:
                yield self.gen_record(
                    _time=time.time(),
                    error="Please provide a value type. Supported types are username|password|email",
                )
                return
            ...
            if usercreds is not None:
                ...
                return
            if "email" in self.value_type.lower():
                validated_ioc_name = Validation.validating_ioc("email", self.value)
