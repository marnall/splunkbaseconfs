...
class RebootDeviceAlertAction(object):
    ...
    def implement_alert_action(self, logger, **kwargs):
        ...
        logger.info('Rebooting device {device_id}'.format(
            device_id=device_id))
        ...
        if device_id == '':
            ...
            with redirect_stdout(f):
                ...
                api_call.reboot_device(device_id)
...
if __name__ == '__main__':
    ...
    logger = logger_for_input('reboot_device_alert_action')

    logger.info("Alert Action Reboot Device triggered")

    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        payload = json.loads(sys.stdin.read())
        try:
            modular_alert = RebootDeviceAlertAction(**payload)
