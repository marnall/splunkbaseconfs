...
class GetExtensionAlertAction(object):
    ...
    def implement_alert_action(self, logger, **kwargs):
        ...
        def log_summary(call_output, response):
            ...
            :return: None
            '''
            response_text_temp = response.text
        ...
        logger.info('Getting extension details {extension_name}'.format(
            extension_name=extension_name
        ))

        event_data = self.params['configuration']
        ...
        with redirect_stdout(f):
            ...
            # Note: pass version when extension install events are updated to include it
            api_call.get_extension_permission_action(extension_id, '', event_data)

        call_output = f.getvalue()

        response_text_temp = remove_response_text_on_success(api_call.response)
        response_text_temp = api_call.response
...
if __name__ == '__main__':
    ...
    logger = logger_for_input('get_extension_alert_action')

    logger.info("Get extension alert action triggered")

    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        payload = json.loads(sys.stdin.read())
        try:
            modular_alert = GetExtensionAlertAction(**payload)
