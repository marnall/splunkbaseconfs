...
def ComputeExtensionsList(extensions_list, device_list, data):
  ...
  Args:
    ...
    device_list: [splunk added] - list of device_profile_extension data
    ...
    for browser in device['browsers']:
      ...
      for profile in browser['profiles']:
        ...
        for extension in profile['extensions']:
          ...
          device_profile_extension = {
              "machineName": device['machineName'],
              "orgUnitPath": device['orgUnitPath'],
              "deviceId": device['deviceId'],
              "profileName": profile['name'],
              "chromeSignedInUserEmail": profile.get('chromeSignedInUserEmail', ''),
              "extensionId": extension['extensionId'],
              "name": extension.get('name', ''),
              "version": extension.get('version', ''),
              "disabled": extension.get('disabled', 'False'),
              "installType": extension.get('installType', '')
          }

          if 'version' in extension:
            key = key + ' @ ' + extension['version']
            device_profile_extension['version'] = extension['version']
          ...
          device_list.append(device_profile_extension)
...
def main(args):
  ...
  # [SPLUNK] modified for getting private key from variable
  ...
  try:
    ...
    base_request_url = 'https://admin.googleapis.com/admin/directory/v1.1beta1/customer/{}/devices/chromebrowsers'\
        .format(args.customerID)
    request_parameters = ''
    ...
    while True:
      ...
      ComputeExtensionsList(extensions_list, device_list_splunk, data)
      ...
      # [SPLUNK] disabled max_browsers limit
    ...
    write_lookup(device_list_splunk, args.device_profile_extension_list_csv)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '-d',
      '--device_profile_extension_list_csv',
      metavar='FILENAME',
      default='./device_profile_extension_list_csv',
      help='Generate an device, profile and extension list to the specified CSV '
      'file')
  parser.add_argument(
      '-m',
  ...
  if args.max_browsers_to_process is not None and args.max_browsers_to_process <= 0:
