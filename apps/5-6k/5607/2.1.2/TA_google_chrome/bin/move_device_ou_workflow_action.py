...
@Configuration()
class movedeviceou(StreamingCommand):
    sa_name = Option(
        doc='''
    ...
    def stream(self, records):
        ...
        try:
            ...
            dir_device_id = str(self.directory_device_id)
            ...
            user_info = f'username: {username}, triggered Move Device OU workflow action'
            logger.info(user_info)
            move_log = f'Moving device {dir_device_id} to OU: {ou_name}'
            logger.info(move_log)
            api_call = GoogleWorkflowCalls(sa_key=sa_key, customerID=customerID, admin_email=admin_email)
            f = io.StringIO()
            with redirect_stdout(f):
                api_call.move_os_device_to_OU(ou_name, dir_device_id)
...
dispatch(movedeviceou, sys.argv, sys.stdin, sys.stdout, __name__)
