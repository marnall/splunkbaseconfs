...
class DisableDeviceAlertAction(object):
    ...
    def implement_alert_action(self, logger, **kwargs):
        ...
        device_id = str(self.params['configuration']['device_id'])
        api_call = GoogleWorkflowCalls(sa_key=sa_key,
                                       customerID=customerID,
                                       admin_email=admin_email
                                       )
        f = io.StringIO()
        logger.info('Disabling device {device_id}'.format(
            device_id=device_id))

        if device_id == '':
            logger.info('The device_id field is empty. Skipping...')
        else:

            with redirect_stdout(f):
                # captures stdout prints into a variable

                api_call.disable_device(device_id)
...
if __name__ == '__main__':
    ...
    logger = logger_for_input('disable_device_alert_action')

    logger.info("Alert Action Disable Device triggered")

    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        payload = json.loads(sys.stdin.read())
        try:
            modular_alert = DisableDeviceAlertAction(**payload)
