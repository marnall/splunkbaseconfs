...
@Configuration()
class wipedevice(StreamingCommand):
    sa_name = Option(
        doc='''
    ...
    def stream(self, records):
        ...
        try:
            ...
            user_info = f'username: {username}, triggered Wipe Device workflow action'
            logger.info(user_info)
            move_log = f'Wipe Device: {directory_device_id}'
            logger.info(move_log)
            api_call = GoogleWorkflowCalls(sa_key=sa_key, customerID=customerID, admin_email=admin_email)
            f = io.StringIO()
            with redirect_stdout(f):
                api_call.wipe_device(directory_device_id)
...
dispatch(wipedevice, sys.argv, sys.stdin, sys.stdout, __name__)
