...
@Configuration()
class disabledevice(StreamingCommand):
    sa_name = Option(
        doc='''
    ...
    def stream(self, records):
        ...
        try:
            ...
            user_info = f'username: {username}, triggered Disable Device workflow action'
            logger.info(user_info)
            move_log = f'Disabling device {directory_device_id}'
            logger.info(move_log)
            api_call = GoogleWorkflowCalls(sa_key=sa_key, customerID=customerID, admin_email=admin_email)
            f = io.StringIO()
            with redirect_stdout(f):
                api_call.disable_device(directory_device_id)
...
dispatch(disabledevice, sys.argv, sys.stdin, sys.stdout, __name__)
