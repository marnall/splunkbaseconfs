...
class MoveUserOuAlertAction(object):
    ...
    def implement_alert_action(self, logger, **kwargs):
        ...
        device_user = str(self.params['configuration']['device_user'])
        ...
        logger.info('Moving profile {device_user} to OU: {OU}'.format(
            device_user=device_user,
            OU=ou_path))
        if device_user == '':
            logger.info('The device_user field is empty. Skipping...')
        else:
            with redirect_stdout(f):
                # captures stdout prints into a variable

                api_call.move_user_to_OU(ou_path, device_user)
...
if __name__ == '__main__':
    ...
    logger = logger_for_input('move_user_ou_alert_action')

    logger.info("Alert Action Move User to OU triggered")

    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        payload = json.loads(sys.stdin.read())
        try:
            modular_alert = MoveUserOuAlertAction(**payload)
