...
class WipeUserAlertAction(object):
    ...
    def implement_alert_action(self, logger, **kwargs):
        ...
        f = io.StringIO()
        logger.info('Wipe users from Device: {device_id}'.format(
            device_id=device_id))
...
if __name__ == '__main__':
    ...
    logger = logger_for_input('wipe_user_alert_action')

    logger.info("Alert Action Wipe All Users From Device triggered")

    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        payload = json.loads(sys.stdin.read())
        try:
            modular_alert = WipeUserAlertAction(**payload)
