...
@Configuration()
class wipeuser(StreamingCommand):
    sa_name = Option(
        doc='''
    ...
    def stream(self, records):
        ...
        try:
            ...
            directory_device_id = str(self.directory_device_id)
            ...
            user_info = f'username: {username}, triggered Wipe All Users from Device workflow action'
            logger.info(user_info)
            move_log = f'Wipe users from Device: {directory_device_id}'
            logger.info(move_log)

            api_call = GoogleWorkflowCalls(sa_key=sa_key, customerID=customerID, admin_email=admin_email)
            f = io.StringIO()
            with redirect_stdout(f):
                api_call.wipe_user(directory_device_id)
...
dispatch(wipeuser, sys.argv, sys.stdin, sys.stdout, __name__)
