...
class WipeDeviceAlertAction(object):
    ...
    def implement_alert_action(self, logger, **kwargs):
        ...
        f = io.StringIO()
        logger.info('Wipe Device: {device_id}'.format(
            device_id=device_id))
        ...
        if device_id == '':
            ...
            with redirect_stdout(f):
                ...
                api_call.wipe_device(device_id)
...
if __name__ == '__main__':
    ...
    logger = logger_for_input('wipe_device_alert_action')

    logger.info("Alert Action Wipe Device triggered")

    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        payload = json.loads(sys.stdin.read())
        try:
            modular_alert = WipeDeviceAlertAction(**payload)
