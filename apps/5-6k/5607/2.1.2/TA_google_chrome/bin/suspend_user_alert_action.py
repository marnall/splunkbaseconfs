...
class SuspendUserAlertAction(object):
    ...
    def implement_alert_action(self, logger, **kwargs):
        ...
        f = io.StringIO()
        logger.info('Suspending profile {device_user}'.format(
        ...
        if device_user == '':
            ...
            with redirect_stdout(f):
                ...
                api_call.suspend_user(device_user)
...
if __name__ == '__main__':
    ...
    logger = logger_for_input('suspend_user_alert_action')

    logger.info("Alert Action Suspend User triggered")

    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        payload = json.loads(sys.stdin.read())
        try:
            modular_alert = SuspendUserAlertAction(**payload)
