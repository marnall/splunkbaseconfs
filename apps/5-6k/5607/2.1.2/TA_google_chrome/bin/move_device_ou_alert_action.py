...
class MoveDeviceOuAlertAction(object):
    ...
    def implement_alert_action(self, logger, **kwargs):
        ...
        deviceID = str(self.params['configuration']['device_id'])
        ...
        if deviceID == '':
            ...
            with redirect_stdout(f):
                ...
                api_call.move_os_device_to_OU(ou_path, deviceID)
...
if __name__ == '__main__':
    ...
    logger = logger_for_input('move_device_ou_alert_action')

    logger.info("Alert Action Move Device to OU triggered")

    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        payload = json.loads(sys.stdin.read())
        try:
            modular_alert = MoveDeviceOuAlertAction(**payload)
