...
### Update to point directly to the app's bin folder to grab correct version.
# Precedence applies, and will pull the wrong version from other apps.
...
class KennyLoggins:
    ...
    def get_logger(self, app_name=None, file_name="kenny_loggins", log_level=logging.INFO, format_string=None):
        if format_string is None:
            format_string = '%(asctime)s log_level=%(levelname)s pid=%(process)d tid=%(threadName)s file="%(filename)s" function="%(funcName)s" line_number="%(lineno)d" version="{}" %(message)s'.format(
                version.__version__)
...
class Utilities:
    ...
    def check_collection_exists(self, collection_name, do_create=False, **kwargs):
        ...
        try:
            ...
            self._log.warning("action=check_collection_exists name={} do_create={} {}".format(c_name, do_create, e))
            ret_obj = {}
            if do_create:
                args = {"name": c_name, "output_mode": "json"}
                self._log.info("action=create_collection args={}".format(args))
                sr, sc = self._make_post_request(self._build_endpoint_uri(["storage", "collections", "config"]),
                                                 args=args)
                entry = json.loads(sc.decode("utf-8"))
                self._log.info(
    ...
    def check_transform_exists(self, transform_name, do_create=False, **kwargs):
        ...
        try:
            ...
            self._log.warning("action=check_transform_exists name={} do_create={} {}".format(t_name, do_create, e))
            if do_create:
                kwargs["name"] = t_name
    ...
    def get_configuration(self, filename, stanza):
        try:
            ...
            self._log.debug("action=get_configuration typeobj={} rc={} typerc={} compare={}".format(type(o), rc.status,
                                                                                                    type(rc.status),
                                                                                                    (rc.status == 200)))
            if rc.status == 200:
            ...
            self._log.error('message="{}" exception_type="{}" '
                            'exception_arguments="{}" filename="{}" exception_line="{}" '.format(str(e), type(e), e,
                                                                                                 fname,
                                                                                                 exc_tb.tb_lineno))
            return {}
    ...
    def get_credential(self, realm, cuser):
        ...
        try:
            ...
            self._log.error('message="{}" exception_type="{}" '
                            'exception_arguments="{}" filename="{}" line="{}" '.format(str(e), type(e), e, fname,
                                                                                       exc_tb.tb_lineno))
            raise Exception(json.dumps(jsondump))
