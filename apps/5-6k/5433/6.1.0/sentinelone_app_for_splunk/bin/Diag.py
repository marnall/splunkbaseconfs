...
def collect_diag_info(diag, options=None, global_options=None, app_dir=None, **kwargs):
    ...
    try:
        ...
        diag.add_rest_endpoint("/services/configs/conf-inputs", os.path.join("02_rest", "conf_inputs.xml"))
        diag.add_rest_endpoint("/services/configs/conf-sentinelone_settings", os.path.join("02_rest", "sentinelone_settings.xml"))
    except:
        pass
    ...
    if not os.path.exists(system_logs):
        ...
        for x in ["splunkd.log", "python.log", "relaymodaction.log", "python_modular_input.log", "splunkd_stderr.log"]:
            for y in glob.glob(os.path.join(system_logs, f"{x}*")):
                if os.path.exists(y):
                    diag.add_file(y, os.path.join("03_system_logs", x))
