...
def setup_logging():
    ...
    logging_file_name = "Moog_Integration.log"
...
def send_req(url, headers, req_json, proxies=None, verify=False):
    ...
    logger.info("Successfully sent the event data to Moogsoft")
    logger.info("Response received from Moogsoft: {}".format(str(res)))
    return res
...
def decrypt_secrets(settings):
    ...
    passwords = service.storage_passwords.list(owner="nobody", app="Moogsoft", sharing="app")
...
def execute(rest_url, req_data, moog_cert=None):
    """
    Execute the POST request on the Moogsoft
    :param rest_url: Moogsoft Integration URL
    ...
    try:
        ...
        raise Exception(
            "An error has occurred while executing POST request with Moogsoft Integration. Error message:"
...
def keep_required_config(config):
    """
    Keep only selected alert attributes from the setup page in config which will be push to the Moogsoft.
...
def main():
    """
    This function initialises Moogsoft App
    """
    setup_logging()
    ...
    try:
        ...
        if is_https_url(rest_url) and 'moogcertificate' in config.keys():
            try:
                ...
                logger.warning("Trying to connect to Moogsoft Integration without client certificate from App: {}"
                               .format(APP_NAME))
        ...
        logger.error("Moogsoft alert script execution failed: {}".format(str(exception)))
