    ...
    try:
        for sc in range(length): 
            ...
            while new_results:
                ...
                event = helper.new_event(source=inputsource, index=helper.get_output_index(), sourcetype=helper.get_sourcetype(),data=str(data2['total']))
                ew.write_event(event)
            
                event = helper.new_event(source=inputsource, index=helper.get_output_index(), sourcetype=helper.get_sourcetype(),data=str(data2['start']))
                ew.write_event(event)
