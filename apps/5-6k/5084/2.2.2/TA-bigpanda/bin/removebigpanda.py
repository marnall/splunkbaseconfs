...
if __name__ == "__main__":
    ...
    try:
        ...
        # Remove the bigpanda alert action from each result
        for result in results:
            actions = ""
            if result.get('actions'):
                actions = result.get('actions').split(',')
            # Get the saved search URL
            search_url = result.get('id')
            # Modify Actions to remove the BigPanda action
            actions = ",".join([action for action in actions if action != bp_action]).strip()
            result['actions'] = actions
            # Remove the action from the result
            result[f"action.{bp_action}"] = 0
            # Normalize the url to save to localhost
            url = normalize_url(search_url, actions)
            if not url:
                logger.error(f"Unable to remove {bp_action} from saved search: {search_url}")
                continue
            # Disable proxy usage
            if not use_proxy:
                disable_proxy()
            ...
            try:
                logger.info(f"Removing {bp_action} from saved search: {url}")
    ...
    logger.info(f"Removed action.{bp_action} from {len(updated_results)} saved searches.")
