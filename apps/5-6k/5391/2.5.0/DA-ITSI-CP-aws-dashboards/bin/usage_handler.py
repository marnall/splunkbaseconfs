...
class UsageHandler(admin.MConfigHandler):
    ...
    def _get_index_volume_by_sourcetype(cls, splunk_service):
        index_volume_by_sourcetype = 'search `cp-aws-dashboards-summary-index` earliest=-7d@d latest=@d report=aws_indexed_data_volume | timechart span=1d useother=f max(sum_mb) as volume by series | fillnull value=0 | eval time = strftime(_time, "%F") | fields - _time, _span, _spandays'
