...
class BillingReportType(admin.MConfigHandler):
    ...
    def handleCreate(self, confInfo):
        ...
        nav_path_default = make_splunkhome_path(['etc', 'apps', 'DA-ITSI-CP-aws-dashboards', 'default', 'data', 'ui', 'nav'])
        nav_path_local = make_splunkhome_path(['etc', 'apps', 'DA-ITSI-CP-aws-dashboards', 'local', 'data', 'ui', 'nav'])
        ...
        response = Rest.simpleRequest("/servicesNS/nobody/DA-ITSI-CP-aws-dashboards/data/ui/nav/default", sessionKey= self.getSessionKey(), method='POST', postargs={'eai:data': XMLString})

        # Refresh the nav endpoint so that our changes take effect
        nav_entity = en.getEntities('data/ui/nav/_reload', sessionKey = self.getSessionKey(), namespace='DA-ITSI-CP-aws-dashboards', owner='nobody')
