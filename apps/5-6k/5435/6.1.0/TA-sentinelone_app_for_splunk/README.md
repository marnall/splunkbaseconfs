...
| TA-sentinelone_app_for_splunk:error:event               | Internal Error Logging                            | Errors that occur during threat event processing.                                                    |
| TA-sentinelone_app_for_splunk:error                     | Internal Error Logging                            | Errors that occur during processing.                                                                 |
...
- sentineloneapi
  ...
  - Sample Usage
    - `| rest "/servicesNS/-/TA-sentinelone_app_for_splunk/configs/conf-authhosts" splunk_server=local | fields + url | rename url as management | sentineloneapi activity_types`
...
- Single Instance 
  ...
  - Only the SentinelOne App ([TA-sentinelone_app_for_splunk](https://splunkbase.splunk.com/app/5433/>))
...
- Single Instance + Heavy Forwarder 
  ...
  - Single Instance:
    ...
    - SentinelOne App ([TA-sentinelone_app_for_splunk](https://splunkbase.splunk.com/app/5433/>))
...
If a support representative asks for it, a support diagnostic file can be generated. Use the following command to generate the file. Send the resulting file to support.
    ...
    $SPLUNK_HOME/bin/splunk diag --collect=app:TA-sentinelone_app_for_splunk
...
- Configurations Dashboard
  ...
  - There is a dashboard that is not within the navigation. It can be loaded using the following url: `https://<yourSplunk>/<language>/TA-sentinelone_app_for_splunk/sentinelone_current_configurations/`. This dashboard displays configured information for each input, and how they interact with each other configuration.
