            ...
            if type(events) == list and len(events) > 0:
                ...
                if conf.has_section(STANZA):
                    with open(fname, 'w+') as configfile:
                        conf.set(STANZA, 'timestamp', str(datetime.datetime.strptime(
                            timestamp.split('.')[0], '%Y-%m-%d %H:%M:%S') + datetime.timedelta(seconds=1)))
...
def print_xml_stream(s):
    print("<stream><event unbroken=\"1\"><data>{}</data><done/></event></stream>".format(encodeXMLText(s)))
