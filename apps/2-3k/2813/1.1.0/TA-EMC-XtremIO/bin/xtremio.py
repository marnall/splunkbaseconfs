...
def getCredentials(sessionKey, host):
    ...
    try:
        ...
        logging.error("Could not get {} credentials from splunk. Error: {}".format(myapp, str(e)))
...
def do_run():
    ...
    try:
        ...
        while True:
            ...
            for endpoint in endpoints:
                ...
                try:
                    ...
                    logging.error("HTTP Request error: {} for endpoint {}".format(str(e), str(endpoint)))
                    time.sleep(float(backoff_time))
                except Exception as e:
        ...
        logging.error("Looks like an error: {}".format(str(e)))
        sys.exit(2)


def replaceTokens(raw_string, req_args):
    try:
        substitution_tokens = re.findall(r"\$(?:\w+)\$", raw_string)
        for token in substitution_tokens:
            endpoints = getattr(tokens, token[1:-1])(raw_string, req_args)
        return endpoints
    except Exception:
        e = sys.exc_info()[1]
        logging.error("Looks like an error substituting tokens: {}".format(str(e)))
...
def handle_output(response, output, type, req_args, endpoint, STANZA):
    try:
        ...
        logging.error("Looks like an error handle the response output: {}".format(str(e)))


# prints validation error data to be consumed by Splunk
def print_validation_error(s):
    print("<error><message>{}</message></error>".format(encodeXMLText(s)))


# prints XML stream
def print_xml_single_instance_mode(s):
    print("<stream><event><data>{}</data></event></stream>".format(encodeXMLText(s)))
...
def get_validation_config():
    ...
    if item_node:
        ...
        for param in params_node:
            ...
            logging.debug("Found param {}".format(name))
