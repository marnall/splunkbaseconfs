        ...
        if len(sys.argv) >1:
            for arg in sys.argv[1:]:
                if arg.lower().startswith("appendclass="):
                    ...
                    errString = "xsCreateCDContext-F-003: Invalid argument: " + arg
                    raise Exception(errString) 
        else:
            raise Exception("xsCreateCDContext-F-001: Usage: xsCreateCDContext name=<string> terms=<conceptlist-option> (type=<contexttype-option>)? (<fuzzyvalues-option>)*")
