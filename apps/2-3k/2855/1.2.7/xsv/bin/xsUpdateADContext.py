...
if __name__ == '__main__':
    ...
    search = ''
    shape = 'strapezoid'
    term_list = 'outlier,typical,outlier'
    uom = ''
    writeRole = '*'
        ...
        if len(sys.argv) >1:
            for arg in sys.argv[1:]:
                if arg.lower().startswith("appendclass="):
                    ...
                    errString = "xsUpdateADContext-F-003: Invalid argument: " + arg
                    raise Exception(errString) 
        else:
            raise Exception("xsUpdateADContext-F-001: Usage: xsUpdateADContext name=<string> terms=<conceptlist-option> (type=<contexttype-option>)? (<fuzzyvalues-option>)*")
