...
def send_octoblu_request(url, body, user_agent=None):
    if url is None:
    ...
    try:
        ...
        if 200 <= res.code < 300:
            print >> sys.stderr, "INFO Octoblu receiver responded with HTTP status=%d" % res.code
            return True
        else:
            print >> sys.stderr, "ERROR Octoblu receiver responded with HTTP status=%d" % res.code
            return False
    except urllib2.HTTPError, e:
        print >> sys.stderr, "ERROR Error sending Octoblu request: %s" % e
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not send_octoblu_request(url, json.dumps(body), user_agent=user_agent):
            sys.exit(2)
    except Exception, e:
