...
def collect_diag_info(diag, options=None, global_options=None, app_dir=None, **args):
    logging.info(
        "feature=diagnostic action=collect message=collecting checkpoint files"
    )

    checkpoint_src_dir = os.path.join(
        os.environ["SPLUNK_HOME"], "var", "lib", "splunk", "modinputs", "server", "splunk_app_db_connect"
    )
...
def simple_collect(diag, checkpoint_src_dir, checkpoint_dest_dir):
    logging.info(
        "feature=diagnostic action=collect message=collecting checkpoint files in mode simple (hot checkpoint files)"
    )
...
def all_collect(diag, checkpoint_src_dir, checkpoint_dest_dir):
    logging.info(
        "feature=diagnostic action=collect message=collecting checkpoint files in mode all (hot and cold checkpoint files)"
    )
    files = os.listdir(checkpoint_src_dir)
    for f in files:
