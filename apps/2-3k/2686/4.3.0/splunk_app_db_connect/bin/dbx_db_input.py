...
class DbxDbInputRunner:
    ...
    def __init__(self):
        ...
        self.server_db_input_url = "http://localhost:" + str(task_server_port) + "/api/inputs/{}/run"
        self.modinput_name_regex = re.compile(r"dbx_db_input://([\w.-]+)$")
    ...
    def stream_events(self):
        ...
        logger.info(f"feature=input action=execute python_version={sys.version}")

        try:
            modinput_name = next(iter(input_definition.inputs.keys()))
            db_input_name = self.modinput_name_regex.search(modinput_name).group(1)
            logger.debug(f"feature=input action=execute name={db_input_name}")
            ...
            verify_ssl = self._verify_ssl(self.server_db_input_url)
            response = s.post(url=self.server_db_input_url.format(db_input_name), headers=headers, verify=verify_ssl)

            if response.status_code != 200:
                if response.status_code == 303:
                    logger.info(
                        f"feature=input action=execute name={db_input_name} status=skipped message=input was executed on other node, "
                        + f"status_code={response.status_code}, content={response.content}"
                    )
                else:
                    logger.warning(
                        f"feature=input action=execute name={db_input_name} status=failed "
                        + f"status_code={response.status_code}, content={response.content}"
                    )
            else:
                logger.info(
                    f"feature=input action=execute name={db_input_name} status=success content={response.content}"
                )
        except Exception as e:
            logger.error(f"feature=input action=execute name={db_input_name} status=failed cause={e}")
        finally:
            logger.info(
                f"feature=input action=execute name={db_input_name} status=success execution_time={time.time() - start_time}"
            )
