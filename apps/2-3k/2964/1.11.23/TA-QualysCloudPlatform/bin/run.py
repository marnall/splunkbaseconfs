...
parser.add_option("--csam", "--log-csam-assets",
					action="store_true", dest="log_csam_api", default=False,
					help="Log CSAM Asset Info")

parser.add_option("--cert", "--log-certview-certificates",
					action="store_true", dest="log_cert_api", default=False,
					help="Log Certificates Info")
...
if options.log_csam_api:
	log_csam_api = True
	data_input_running = "cyber_security_asset_management"

if options.log_cert_api:	
	log_cert_api = True
	data_input_running = "certview_certificates"
...
is_behind_gateway = log_fim_events_api or log_fim_ignored_events_api or log_fim_incidents_api or log_edr_api or log_csi_api or log_csc_api or log_sem_api or log_pcrs_api or log_csam_api or log_cert_api
...
try:
	...
	if log_csam_api:
		cp = './cyber_security_asset_management'
		qlogger.info("Running CSAM input")
		csamPopulator = qualysModule.qualys_log_populator.QualysCSAMPopulator(settings=appConfig, checkpoint=cp, host=h, index=i, start_date=start_date,event_writer = ew)
		csamPopulator.run()

	if log_cert_api:
		cp = './certview_certificates'
		qlogger.info("Running CertView input")
		certviewPopulator = qualysModule.qualys_log_populator.QualysCertViewPopulator(settings=appConfig, checkpoint=cp, host=h, index=i, start_date=start_date,event_writer = ew)
