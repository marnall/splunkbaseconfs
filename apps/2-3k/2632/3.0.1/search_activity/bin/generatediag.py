...
#TSIDX Event Count
SearchTSIDXCount = '| tstats local=t count as SHCount from `SA_SearchHistory` by _time span=1d | append [| tstats local=t count as ECount from `SA_Events` by _time span=1d] | stats sum(*) as * by _time | fillnull value=0'
...
#User Activity
SearchSAUserActivity = '| tstats count from `SA_Events` where myapp="search_activity" by myapp myview user _time span=1s  | sort user | streamstats count as UserUnique by user | eval UserUnique=if(UserUnique>1,null,1) | streamstats sum(eval(if(UserUnique>0,UserUnique,null)))  as UserUnique | fields - user'

#Anonymous User Activity
SearchAnonyActivity = '|stats count as bogus    | append [| tstats local=t  count sum(total_run_time) as total_run_time sum(NumExports) as NumExports from `SA_SearchHistory`  where * (SearchHead=*) * * by _time user searchtype span=1d | stats sum(count) as NumSearches sum(eval(if(searchtype="adhoc",count,0))) as NumAdHocSearches sparkline(sum(count)) as UsageTrend sum(total_run_time) as HoursOfSearchTime sum(eval(if(searchtype="adhoc",total_run_time,0))) as HoursOfAdHocSearchTime sum(NumExports) as NumberOfExports by user| where user!="splunk-system-user" | eval HoursOfSearchTime = round(HoursOfSearchTime/3600,2) | eval HoursOfAdHocSearchTime = round(HoursOfAdHocSearchTime/3600,2) | sort - count ]        | append [| tstats prestats=t local=t count from `SA_SearchHistory`  where * (SearchHead=*) * * searchtype=adhoc by searchcommands , user| stats count by searchcommands, user| eval maturity=`eval_high_complexity_search_commands` | eval maturity=`eval_med_complexity_search_commands` | eval maturity=`eval_low_complexity_search_commands`  | stats sum(eval(if(searchcommands="search",count,null))) as search_num sum(maturity) as maturity by user | eval "search_complexity_score"=if(maturity/search_num>`max_search_complexity_rating`,`max_search_complexity_rating`,1+round(maturity/search_num,0)) | fields user search_complexity_score | sort - search_complexity_score | streamstats count as complexity_order | eventstats count as NumComplexityUsers]        | append  [| tstats local=t avg(accuracy) as Accuracy from `SA_SearchHistory` where * (SearchHead=*) * *  accuracy>0 total_run_time>0 result_count>0 searchtype=adhoc by user| sort - Accuracy | streamstats count as accuracy_order | eventstats count as NumAccuracyUsers]       | append  [ | tstats  local=t avg(total_run_time) as runtime from `SA_SearchHistory` where * (SearchHead=*) * *   searchtype=adhoc by user | eval runtime=round(runtime,0) | stats avg(runtime) as runtime by user | fields user runtime | sort - runtime | streamstats count as runtime_order | eventstats count as NumRuntimeUsers]     | append [| tstats  local=t count sum(ShouldInvestigate) as ShouldInvestigate from `SA_SearchHistory` where * (SearchHead=*) * *  by user | sort - ShouldInvestigate | streamstats count as investigate_order | eventstats count as NumInvestigateUsers]    | append [| tstats local=t count from `SA_Events` where * (SearchHead=*) * *  type=scheduler by user status  	| append [| tstats local=t count  from `SA_SearchHistory` where * (SearchHead=*) * * by user search_status ] | stats sum(eval(if(status="skipped",count,0))) as SkippedScheduled sum(eval(if(status="success",count,0))) as SuccessfulScheduled sum(eval(if(search_status="failed",count,0))) as FailedAdHoc sum(eval(if(search_status="success",count,0))) as SuccessfulAdHoc by  user | eval PercentFailed=round(100*(SkippedScheduled+FailedAdHoc)/(SuccessfulScheduled+SuccessfulAdHoc)) | sort - PercentFailed | streamstats count as FailureOrder | eventstats max(FailureOrder) as NumUsersFailure ]    | eval "Accuracy Order" = accuracy_order . " out of " . NumAccuracyUsers     | eval "Search Complexity Order" = complexity_order . " out of " . NumComplexityUsers     | eval "Slow Search Runtime Order" = runtime_order . " out of " . NumRuntimeUsers      | eval "Search Failure Order" = FailureOrder . " out of " . NumUsersFailure  | eval "Questionable Search Order" = investigate_order . " out of " . NumInvestigateUsers   | eventstats values(*Order*) as *Order* by user  | fields user NumSearches NumAdHocSearches UsageTrend HoursOfSearchTime HoursOfAdHocSearchTime NumberOfExports "Accuracy Order" "Search Complexity Order" "Search Failure Order" "Questionable Search Order" "Slow Search Runtime Order" mail | sort - NumSearches | where NumSearches>0 | streamstats count as user'

#Volume Indexed
SearchVolumeIndexed = 'search earliest=-24h@h latest=@h index=_internal source=*license_usage.log "Type=RolloverSummary" | stats sum(b) as sumb | eval sumGB = round(sumb/1024/1024/1024,4)'

#Num Servers
SearchNumServers = '| metasearch index=* earliest=-15min | dedup splunk_server  | stats dc(splunk_server) as NumServers'

#Lifetime of Search
SearchLifetime = '| tstats local=t count dc(user) as user avg(result_count) as avg_result_count avg(scan_count) as avg_scan_count median(total_run_time) as median_total_run_time sum(total_run_time) as total_trt from `SA_SearchHistory` where (searchtype=adhoc* OR searchtype=scheduled* OR searchtype=summarization* OR searchtype=dashboard*) by time_bucket | eval "total_trt avg hrs per day"=round(total_trt/3600/30,1) | streamstats sum(total_trt) as Aggregated_Total_Run_Time | eventstats max(total_trt) as max_trt max(Aggregated_Total_Run_Time) as Max_Ag  | eval PercentageOfTotalRuntime = round(total_trt/max_trt * 100,2) | eval AggregatePercentageOfTotalRuntime = round(Aggregated_Total_Run_Time/Max_Ag * 100,2) | foreach PercentageOfTotalRuntime AggregatePercentageOfTotalRuntime [fieldformat <<FIELD>> = <<FIELD>> . "%" ]| fields - Max_Ag max_trt | fillnull avg_result_count avg_scan_count median_total_run_time total_trt PercentageOfTotalRuntime "total_trt avg hrs per day" value=0'
