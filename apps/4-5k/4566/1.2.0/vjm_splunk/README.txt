Value Journey Manager for Splunk = 1.0.0
Description = The Value Journey Manager for Splunk application is used for managing, analyzing usage, and indexing the sample data used by the Value Journey for Splunk applications Reports, Dashboards, and Walkthroughs.

Required Secondary Application:
Value Journey for Splunk - Version 1.0.0
https://splunkbase.splunk.com/app/4565/

For documentation, either use the documentation dashboard within the Application UI. 
	(Value Journey Manager for Splunk: Configuration --> VJM - Overview)

Value Journey Manager for Splunk - Installation Location:
	- Single Splunk System Environment: 
		- Splunk Search Head/Indexer
	- Distributed: 
		- Required: Splunk Search Head:
			- If you want to have the example data collection on a seperate Splunk System,
			then just install the Value Journey Manager for Splunk application on that System
			and then disable the Scripted Inputs on the Splunk Search head.  
				- Note: Currently the Data Collection only works on UX/Mac OSX systems.
		- More Information is provided in the Value Journey Manager for Splunk - VJM - Overview dashboard.

Value Journey Manager for Splunk - Requireed Application:
	- The Value Journey Manager for Splunk application requires the Value Journey for Splunk application.
		- The Value Journey for Splunk application contains all the Example Dashboards, Reports,
		and Walkthroughs that the "User" will be journeying through.   They leverage the 
		Example data that is indexed using the Scripted Inputs of the Value Journey for Splunk
...
2. Create the Value Journey for Splunk "Owner" and "User" Accounts.
	- Create the "Owner" account, who will be the individual(s) that will be monitoring the 
progress of the "Users" and providing assistance if needed.
	- Create the "User" account, who will be the individual(s) that will be walking through
...
3. Create the required indexes (vj_ex, vj_walk, vj_sgs)  
4. Load the Current Day's Example Data (Using either the "VJM - Getting Started or VJM - Config - Load Example Data" Dashboards)
...
5. Use the applications email, or manually email the Users their login credentials.
	- Once the User has gone through the initial, quick, Configuration Wizard, they will
