...
FILEDIR           = '/pub/CDC/observations_germany/climate/daily/kl/recent/'
...
def ReadStations(fp, dLastEvents) :
    ...
    for strLine in fp:
        ...
        data=l[:6]
        land=l[-1]
        stationName=u' '.join(l[6:-1])
        dStations[stationName]= dict(zip(lKeys, l))
        # Insert Country field
        dStations[stationName]['Country']=u'DE'
        try :
...
def WriteStations(d, fp) :
    ...
    for k,v in d.iteritems() :
        strFormat = u','.join(u'{}={}'.format(x,y) for (x,y) in v.items())+u'\n'
...
def WriteLastEvents(strFile, dStations):
    try:
        ...
        for k,v in dStations.iteritems() :
            if type(k) == type(2) :
...
def WriteToSplunk(qOut, lEvents, dStation):
    ...
    for x in lEvents:
        ...
        if theDate > lastDate:
            ...
            strFormat += u','.join(u'{}={}'.format(k.lstrip(),v.lstrip()) for (k,v) in x.items())
...
def OutputThread(fp, qOut, dStations) :
    ...
    while True:
        ...
        if s=='STOP' :
            return
        try:
            fp.write(s.encode(encoding='iso-8859-1'))
        except Exception as e:
            sys.stderr.write(str(e)+'\n')
...
def ReadZipArchive(fp) :
    ...
    for strName in zFp.namelist():
        ...
        if match:
            ...
            fpProduct=StringIO.StringIO(zFp.read(strName))
            dReader=csv.DictReader(fpProduct, delimiter=';')
            l=[]
            for x in dReader:
                # We have to create unicode strings......just to make sure we do this for keys and values
                d={}
                for k,v in x.iteritems() :
                    if v.lstrip()!='-999':
                        d[k.decode(encoding='iso-8859-1')]=v.decode(encoding='iso-8859-1')
                l.append(d)
...
def main() :
    ...
    p.add_option("-s", "--stations", dest="stations", action='store_true',\
