...
class NutanixALERTS(Script):
    ...
    def stream_events(self,inputs,ew):
        ...
        try:
            ...
            ew.log('INFO', 'Accessing Nutanix Rest API for alert data using Username:%s,Password:%s,IP:%s,Ports:%s' %(Username, Password, IP, Port))
            ...
            dataset = req.get(endpoint+'alerts/',params=param,verify=False)
            if dataset.status_code == 200:
                for item in dataset.json()['entities']:
                    event = Event()
                    event.stanza = input_name
                    event.source = 'nutanix:alerts'
                    event.host = endpoint+'alerts/'
                ...
                ew.log('ERROR','Splunk was unable to retrieve data from the Nutanix Prism API. The following error was encountered when accessing the Nutanix API Endpoint to obtain data about the alerts in a cluster : %s') %e
