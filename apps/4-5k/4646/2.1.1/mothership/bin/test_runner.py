...
def cli_arguments():
    ...
    parser.add_argument('-p', '--password', default='password', help='Splunk password.')
...
class TestRESTHandler(unittest.TestCase):
    def create_helper(self, endpoint, postargs, user='nobody'):
        return rest.simpleRequest('/servicesNS/%s/mothership/%s' % (user, endpoint), getargs=dict(output_mode='json'), postargs=postargs, method='POST', sessionKey=self.session_key)

    def delete_helper(self, endpoint, name, user='nobody'):
        return rest.simpleRequest(
            '/servicesNS/%s/mothership/%s/%s' % (user, endpoint, six.moves.urllib.parse.quote_plus(name)),
    ...
    def list_helper(self, endpoint, get_args=None, user='nobody'):
        ...
        if get_args is not None:
            rest_get_args.update(get_args, user=user)
        return rest.simpleRequest('/servicesNS/%s/mothership/%s' % (user, endpoint),
            getargs=dict(output_mode='json'), method='GET', sessionKey=self.session_key)

    def read_helper(self, endpoint, name, user='nobody'):
        return rest.simpleRequest(
            '/servicesNS/%s/mothership/%s/%s' % (user, endpoint, six.moves.urllib.parse.quote_plus(name)),
            getargs=dict(output_mode='json'), method='GET',
            sessionKey=self.session_key)
    ...
    def simple_request_delete(self, path):
        return rest.simpleRequest(
            path,
            getargs=dict(output_mode='json'), method='DELETE',
            sessionKey=self.session_key)
