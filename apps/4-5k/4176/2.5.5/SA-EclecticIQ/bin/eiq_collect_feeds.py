...
def export_csv_to_kv(feed_id, text, count=0, diff_flag=False):
    global app_name, sessionKey, splunk_info
    ...
    if 'diff' not in csvreader.fieldnames:
        ...
        for row in csvreader:
            ...
            if version_family == 'new' or EIQ_VERSION == "FC-Essentials":
                format_to_kv(data_to_add, "source.name", row["source.names"])
            else:
                format_to_kv(data_to_add, "source.name", row["source.name"])
        ...
        for row in csvreader:
            ...
            if row['diff'] == 'add':
                ...
                if version_family == 'new' or EIQ_VERSION == "FC-Essentials":
                    ...
                    data_to_add["source"] = {"name_iq": row["source.name"]}
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Get the config from the sa-eclecticiq.conf file and
        ...
        if EIQ_VERSION == "FC-Essentials":
            for item in feeds_info:
                item["name"] = "FC feed ID=" + str(item["id"])
        ...
        for item in feeds_info:
            ...
            if blocks:
                ...
                for block in blocks:
                    ...
                    export_csv_to_kv(item['id'], data_from_block, counter, flag)
                    counter += 1
                    flag = False
