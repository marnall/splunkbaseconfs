...
import modalert_mbcr_alert_sa_helper

class AlertActionWorkermbcr_alert_sa(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkermbcr_alert_sa, self).__init__(ta_name, alert_name)
    ...
    def validate_params(self):
        ...
        if not self.get_param("detection_id"):
            self.log_error('detection_id is a mandatory parameter, but its value is None.')
            return False
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_mbcr_alert_sa_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkermbcr_alert_sa("mbcr", "mbcr_alert_sa").run(sys.argv)
    sys.exit(exitcode)
