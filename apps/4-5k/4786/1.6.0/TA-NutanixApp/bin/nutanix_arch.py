    ...
    def stream_events(self,inputs,ew):
	    ...
	    if Password != self.MASK:
	      ...
	      self.mask_password(service, Username, Module_Name, ew)
            ...
            ew.log('INFO', 'Accessing Nutanix Rest API for host data using Username:%s,IP:%s,Ports:%s' %(Username, IP, Port))
            ...
            ew.log('ERROR',' Splunk was unable to retrieve data from the Nutanix Prism API. The following error was encountered when accessing the Nutanix API Endpoint to obtain architecture data: [[%s]]' %err_msg)
