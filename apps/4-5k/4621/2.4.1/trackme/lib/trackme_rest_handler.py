...
try:
    from trackme_libs_get_data import AuthenticationExpiredError
except ImportError:
    # Defensive: define a placeholder so the except clause never matches if import fails
    class AuthenticationExpiredError(Exception):
        pass

try:
    from trackme_libs_logging import set_request_logger, reset_request_logger
except ImportError:
    # Defensive: if the logging helpers are unavailable, fall back to no-ops so
    # shared-lib traces simply route to the root logger as before.
    def set_request_logger(logger):
        return None

    def reset_request_logger(token):
        return None
...
class RESTHandler(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        try:
            ...
            if function_to_call is not None:
                ...
                # Bind this handler's named logger as the effective logger for
                # the duration of the request so shared-lib helpers
                # (trackme_libs.*) log into THIS handler's per-handler file
                # (with their tenant_id context), instead of the unconfigured
                # root logger where they would silently vanish. Reset in
                # finally so the context never leaks to the next request on a
                # reused thread. No process-global handler mutation occurs
                # (the leak-prone pattern removed in PR #1712).
                _log_token = (
                    set_request_logger(self.logger)
                    if self.logger is not None
                    else None
                )
                try:
                    ...
                    if _log_token is not None:
                        reset_request_logger(_log_token)
            else:
                if self.logger is not None:
                    self.logger.warn(
        ...
        except AuthenticationExpiredError as auth_exception:
            if self.logger is not None:
                self.logger.warning(
                    "Request failed due to expired or invalid Splunk session, "
                    "this is typically transient and resolves on next user action, "
                    "exception=%s",
                    str(auth_exception),
                )
            return {
                "payload": json.dumps({
                    "action": "failure",
                    "response": "Splunk session has expired, please refresh the page to re-authenticate.",
                }),
                "status": 401,
            }
    ...
    def convert_to_dict(self, query):
        ...
        for key, val in query:
            ...
            # existing entry a list and add this one
