...
filehandler = RotatingFileHandler(
    "%s/var/log/splunk/trackme_splk_fqm_advisor_helper.log" % splunkhome,
    mode="a",
    maxBytes=10000000,
    backupCount=1,
)
...
class SplkFqmAdvisorExecutor(GeneratingCommand):
    tenant_id = Option(
        doc="""
        **Syntax:** **tenant_id=****
    ...
    component = Option(
        doc="""
        **Syntax:** **component=****
        **Description:** The component category: must be fqm.""",
        require=True,
        default=None,
        validate=validators.Match("component", r"^fqm$"),
    )

    def generate(self, **kwargs):
        # Python version check — FQM Advisor requires Python 3.13+
                ...
                "_raw": json.dumps(
                    {
                        "status": "error",
                        "message": (
                            f"AI FQM Advisor requires Python 3.13+. "
        ...
        from trackme_libs_ai_fqm_advisor import (
            start_fqm_advisor_from_search_context,
        ...
        # (unified flag — replaces the old per-advisor ai_fqmadvisor_enabled).
        #
        ...
        #
        # Pre-flight check 3: FQM component enabled for the tenant
        #

        component_info_key = "component_splk_fqm"
        try:
        ...
        if schema_version_upgrade_in_progress:
            yield_json = {
                ...
                "component": self.component,
                "response": (
                    f'tenant_id="{self.tenant_id}", FQM Advisor: schema upgrade is currently in progress, '
        ...
        # Aliases for backward-compatible variable naming downstream.
        ai_fqmadvisor_mode = ai_components_advisor_mode
        ...
        #
        # FQM specific: exclude @global aggregated entities — per the FQM Advisor
        # system prompt, @global entities cannot be fixed individually (they
        # aggregate all fields of a sourcetype) and the Advisor should only
        # operate on per-field entities (fqm_type="field").
        #
        ...
        base_query = f"""\
                ...
                | where fqm_type="field"{priority_clause}
        ...
        if ai_components_advisor_min_days_between_reviews > 0:
            review_filter = f"""
                | join type=left object [
                    search index={tenant_summary_idx} sourcetype="trackme:ai_agent:fqm_advisor:*"
        ...
        # Sort: RED entities first, then by name
        ...
        try:
                ...
                if isinstance(item, dict):
                    ...
                    if not eligible:
                        ...
                        logging.debug(
                            ...
                            f'skipped from FQM Advisor by AI automated filter, reason="{reason}"'
                        )
                        continue
            ...
            if (
                ...
                logging.info(
                    f'tenant_id="{self.tenant_id}", component="{self.component}", '
                    f'AI automated-action filter skipped entities from FQM Advisor: '
        ...
        if len(entities_toprocess) > 0:
            for entity_record in entities_toprocess:
                ...
                logging.info(
                    f'tenant_id="{self.tenant_id}", component="{self.component}", '
                    f'starting FQM Advisor for object="{object_value}", mode="{ai_fqmadvisor_mode}"'
                )

                status = "unknown"
                job_id = None
                try:
                    agent_result = start_fqm_advisor_from_search_context(
                    ...
                    msg = (
                        f'tenant_id="{self.tenant_id}", component="{self.component}", '
                        f'object="{object_value}", FQM Advisor failed with exception="{str(e)}"'
                    )
                    logging.error(msg)
            ...
            results_dict = {
                ...
                "component": self.component,
                "action": "success",
                "results": "AI FQM Advisor job successfully executed",
                ...
                handler_events_records.append(
                    {
                        ...
                        "object_category": f"splk-{self.component}",
                        "handler": f"trackme_fqm_advisor_tracker_tenant_{self.tenant_id}",
                        "handler_message": "Entity was reviewed by AI FQM Advisor.",
                        "handler_troubleshoot_search": (
                            f'index=_internal '
                            f'(sourcetype=trackme:custom_commands:trackmesplkfqmadvisorhelper) '
            ...
            results_dict = {
                ...
                "component": self.component,
                "action": "success",
                "results": "AI FQM Advisor job executed but there were no entities to review at this time",
                "run_time": round((time.time() - start), 3),
                "mode": ai_fqmadvisor_mode,
                "entities_count": 0,
                "upstream_search_query": search_query,
        ...
        report_name = f"trackme_fqm_advisor_tracker_tenant_{self.tenant_id}"
...
dispatch(SplkFqmAdvisorExecutor, sys.argv, sys.stdin, sys.stdout, __name__)
