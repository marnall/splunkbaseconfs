...
filehandler = RotatingFileHandler(
    "%s/var/log/splunk/trackme_splk_flx_threshold_advisor_helper.log" % splunkhome,
    mode="a",
    maxBytes=10000000,
    backupCount=1,
)
...
class SplkFlxThresholdAdvisorExecutor(GeneratingCommand):
    tenant_id = Option(
        doc="""
        **Syntax:** **tenant_id=****
    ...
    component = Option(
        doc="""
        **Syntax:** **component=****
        **Description:** The component category: must be flx.""",
        require=True,
        default=None,
        validate=validators.Match("component", r"^flx$"),
    )

    def generate(self, **kwargs):
        # Python version check — FLX Threshold Advisor requires Python 3.13+
                ...
                "_raw": json.dumps(
                    {
                        "status": "error",
                        "message": (
                            f"AI FLX Threshold Advisor requires Python 3.13+. "
        ...
        from trackme_libs_ai_flx_threshold import (
            start_flx_threshold_advisor_from_search_context,
        )
        ...
        # (unified flag — replaces the old per-advisor ai_flxthreshold_enabled).
        #
        ...
        #
        # Pre-flight check 3: FLX component enabled for the tenant
        #

        component_info_key = "component_splk_flx"
        try:
        ...
        ai_flxthreshold_mode = ai_components_advisor_mode
        ...
        # filter expression may reference (see Feed Lifecycle helper for
        # rationale; the filter engine treats absent fields as empty).
        base_query = f"""\
        ...
        if ai_components_advisor_min_days_between_reviews > 0:
            review_filter = f"""
                | join type=left object [
                    search index={tenant_summary_idx} sourcetype="trackme:ai_agent:flx_threshold_advisor:*"
        ...
        try:
                ...
                if isinstance(item, dict):
                    ...
                    if not eligible:
                        ...
                        logging.debug(
                            ...
                            f'skipped from FLX Threshold Advisor by AI automated filter, reason="{reason}"'
                        )
                        continue
            ...
            if (
                ...
                logging.info(
                    f'tenant_id="{self.tenant_id}", component="{self.component}", '
                    f'AI automated-action filter skipped entities from FLX Threshold Advisor: '
        ...
        if len(entities_toprocess) > 0:
            for entity_record in entities_toprocess:
                ...
                logging.info(
                    f'tenant_id="{self.tenant_id}", component="{self.component}", '
                    f'starting FLX Threshold Advisor for object="{object_value}", mode="{ai_flxthreshold_mode}"'
                )

                status = "unknown"
                job_id = None
                try:
                    agent_result = start_flx_threshold_advisor_from_search_context(
                    ...
                    msg = (
                        f'tenant_id="{self.tenant_id}", component="{self.component}", '
                        f'object="{object_value}", FLX Threshold Advisor failed with exception="{str(e)}"'
                    )
                    logging.error(msg)
            ...
            results_dict = {
                ...
                "component": self.component,
                "action": "success",
                "results": "AI FLX Threshold Advisor job successfully executed",
                ...
                handler_events_records.append(
                    {
                        ...
                        "object_category": f"splk-{self.component}",
                        "handler": f"trackme_flx_threshold_advisor_tracker_tenant_{self.tenant_id}",
                        "handler_message": "Entity was reviewed by AI FLX Threshold Advisor.",
                        "handler_troubleshoot_search": (
                            f'index=_internal '
                            f'(sourcetype=trackme:custom_commands:trackmesplkflxthresholdadvisorhelper) '
            ...
            results_dict = {
                ...
                "component": self.component,
                "action": "success",
                "results": "AI FLX Threshold Advisor job executed but there were no entities to review at this time",
                "run_time": round((time.time() - start), 3),
                "mode": ai_flxthreshold_mode,
                "entities_count": 0,
                "upstream_search_query": search_query,
        ...
        report_name = f"trackme_flx_threshold_advisor_tracker_tenant_{self.tenant_id}"
...
dispatch(SplkFlxThresholdAdvisorExecutor, sys.argv, sys.stdin, sys.stdout, __name__)
