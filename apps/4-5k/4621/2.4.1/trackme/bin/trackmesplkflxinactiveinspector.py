...
filehandler = RotatingFileHandler(
    f"{splunkhome}/var/log/splunk/trackme_splkflx_inactive_inspector.log",
    mode="a",
    maxBytes=10000000,
    backupCount=1,
)
...
class SplkFlxInactiveEntitiesInspector(GeneratingCommand):
    ...
    def generate(self, **kwargs):
        ...
        # end of main
        logging.info(
            f'tenant_id="{self.tenant_id}", instance_id={instance_id}, trackmesplkflxinactiveinspector is starting'
        )
        ...
        # Data collection
        data_collection_name = f"kv_trackme_flx_tenant_{self.tenant_id}"
        data_collection = self.service.kvstore[data_collection_name]

        # Hybrid trackers registry — used to resolve per-tracker options such as
        # enable_zero_kpis_when_inactive. One lookup per unique tracker_name per cycle
        # (cached by normalized tracker_name).
        hybrid_trackers_collection_name = (
            f"kv_trackme_flx_hybrid_trackers_tenant_{self.tenant_id}"
        )
        try:
            hybrid_trackers_collection = self.service.kvstore[
                hybrid_trackers_collection_name
            ]
        except Exception:
            hybrid_trackers_collection = None
        tracker_zero_kpi_cache = {}

        def parse_tracker_names(raw):
            """Normalize the entity's tracker_name field into a list of names.

            Entities created under concurrent-tracker support store ``tracker_name`` as a
            JSON-encoded array of normalized names (see ``trackmesplkflxparse``). Legacy
            entities may store a plain string. Returns ``[]`` when nothing is parseable.
            """
            if not raw:
                return []
        ...
        def lookup_zero_kpis_flag(tracker_name):
            """Return the registry flag for ONE tracker (True/False), cached."""
            if not tracker_name:
                return True
            if tracker_name in tracker_zero_kpi_cache:
                return tracker_zero_kpi_cache[tracker_name]
            enabled = True
            if hybrid_trackers_collection is not None:
                try:
                    # registry _key is sha256 of the same (normalized) tracker name
                    # the entity stores in its tracker_name JSON array, see
                    # post_flx_tracker_create / post_flx_converging_tracker_create.
                    tracker_key = hashlib.sha256(
                        tracker_name.encode("utf-8")
                    ).hexdigest()
                    registry_record = hybrid_trackers_collection.data.query_by_id(
                        tracker_key
                    )
                    flag_value = registry_record.get("enable_zero_kpis_when_inactive")
                    if flag_value is not None:
                        enabled = bool(int(flag_value))
                except Exception:
                    # absent registry record or transient KV error: keep default-enabled
                    pass
            tracker_zero_kpi_cache[tracker_name] = enabled
            return enabled

        def is_zero_kpis_when_inactive_enabled(raw_tracker_name):
            """Resolve the enable_zero_kpis_when_inactive flag for an entity.

            Default-enabled when the registry record is missing or lacks the field —
            existing trackers (registry records pre-dating the field) inherit the new
            behaviour without modification. For an entity managed by multiple
            concurrent trackers, the flag is the AND of every tracker's flag — i.e. if
            ANY tracker has explicitly opted out, no zero-fill happens for that entity.
            """
            tracker_names = parse_tracker_names(raw_tracker_name)
            if not tracker_names:
                return True
            for name in tracker_names:
                if not lookup_zero_kpis_flag(name):
                    return False
            return True

        def collect_numeric_kpis(value, sink):
            """Recursively collect numeric KPI keys into ``sink`` (dict), skipping
            booleans and the ``status`` key.

            Entity ``metrics`` may be either the legacy flat shape
            ``{"events_count": 42, ...}`` or the tracker-keyed shape introduced for
            concurrent trackers ``{"<tracker_name>": {"events_count": 42, ...}, ...}``.
            We flatten any nested dicts so that every KPI ends up at the top level of
            the emitted payload (matching the metrics-index field-expansion contract).
            """
        ...
        def build_inactive_metrics_payload(record):
            """Build the metrics payload emitted by the inactive tracker for one entity.

            Always carries ``status=2``. When the entity's tracker(s) have
            ``enable_zero_kpis_when_inactive`` enabled (default), every numeric KPI key
            in the entity's last-seen ``metrics`` blob is added with value ``0``.
            Handles both the legacy flat metrics shape and the tracker-keyed shape used
            by concurrent trackers. When the last-seen metrics blob is missing or
            unparseable, the payload falls back to the historical ``{"status": 2}``-only
            emission.
            """
            payload = {}
            if is_zero_kpis_when_inactive_enabled(record.get("tracker_name")):
                ...
                if raw_metrics:
                    ...
                    if isinstance(last_metrics, dict):
                        collect_numeric_kpis(last_metrics, payload)
            payload["status"] = 2
            return payload
        ...
        (
            ...
            collection_records_keys,
            collection_records_dict,
        ) = self.get_collection_records("flx")

        logging.info(
            f'tenant_id="{self.tenant_id}", instance_id={instance_id}, component="flx", successfully retrieved {len(collection_records_keys)} records in the KVstore collection={data_collection_name}, run_time={round(time.time()-collection_records_get_start, 3)}'
        )
        ...
        for record in collection_records:
            ...
            # get status (handle tracker-keyed JSON format from concurrent trackers)
            status_value = record.get("status")
            try:
                if isinstance(status_value, str):
                    try:
                        status_by_tracker = json.loads(status_value)
                        if isinstance(status_by_tracker, dict):
                            # Tracker-keyed format: apply worst-status logic (2 > 3 > 1)
                            status_values = list(status_by_tracker.values())
                            if status_values:
                                if 2 in status_values:
                                    record_status = 2  # Red
                                elif 3 in status_values:
                                    record_status = 3  # Orange
                                else:
                                    record_status = 1  # Green
                            else:
                                record_status = 1  # Default to green if empty
                        else:
                            # Not a dict, treat as old format integer string
                            record_status = int(status_value)
                    except (json.JSONDecodeError, TypeError):
                        # If parsing fails, treat as old format integer string
                        record_status = int(status_value)
                elif isinstance(status_value, dict):
                    # Already a dict (tracker-keyed format)
                    status_values = list(status_value.values())
                    if status_values:
                        if 2 in status_values:
                            record_status = 2  # Red
                        elif 3 in status_values:
                ...
                logging.error(
                    f'instance_id="{instance_id}", failed to parse status for object="{object_name}", '
                    f'value="{status_value}", exception="{str(e)}"'
                )
                record_status = 1  # Default to green on error
            ...
            if not max_sec_inactive:
                ...
                try:
                    if isinstance(max_sec_inactive, str):
                        try:
                            parsed = json.loads(max_sec_inactive)
                            if isinstance(parsed, dict):
                                # Tracker-keyed format: extract minimum value (excluding 0)
                                non_zero_values = [float(v) for v in parsed.values() if float(v) > 0]
                                if non_zero_values:
                                    max_sec_inactive = min(non_zero_values)
                                else:
                            ...
                            max_sec_inactive = float(max_sec_inactive)
                    elif isinstance(max_sec_inactive, dict):
                        # Already a dict (tracker-keyed format)
                        non_zero_values = [float(v) for v in max_sec_inactive.values() if float(v) > 0]
                        if non_zero_values:
                            max_sec_inactive = min(non_zero_values)
                        else:
                            max_sec_inactive = 0
                    ...
                    logging.error(
                        f'instance_id="{instance_id}", failed to parse max_sec_inactive for object="{object_name}", '
                        f'value="{record.get("max_sec_inactive")}", exception="{str(e)}"'
                    )
                    max_sec_inactive = 0
            ...
            if entity_is_inactive:
                ...
                if (
                    ...
                    try:
                        trackme_flx_gen_metrics(
                            ...
                            object_id,
                            metric_index,
                            json.dumps(build_inactive_metrics_payload(record)),
                        )
                    except Exception as e:
                        error_msg = f'Failed to call trackme_flx_gen_metrics with exception="{str(e)}"'
                        logging.error(error_msg)
                    ...
                    # fix current_state consistency if needed
                    if record_current_state != "red":
                        try:
                            ...
                            logging.info(
                                f'successfully updated record current_state to red, record="{json.dumps(record, indent=2)}"'
                            )
                    ...
                    # summary state event - build a concise event matching active tracker format
                    summary_event = {
                        "object_state": record.get("object_state", "red"),
                        "status_message": record.get("status_message", "entity is red due to inactivity"),
                        "anomaly_reason": record.get("anomaly_reason", "inactive"),
                        "object": object_name,
                    ...
                    # include current timestamp to ensure a unique event_id across runs
                    summary_event["event_id"] = hashlib.sha256(
                        (json.dumps(summary_event) + str(time.time())).encode()
                    ).hexdigest()
                    try:
                        trackme_gen_state(
                            index=trackme_summary_idx,
                            sourcetype="trackme:state",
                            source=f"current_state_tracking:splk-flx:{self.tenant_id}",
                            event=summary_event,
                        )
                        logging.debug(
                            f'TrackMe summary event created successfully, record="{json.dumps(summary_event, indent=1)}"'
                        )
                    except Exception as e:
                        logging.error(
                            f'TrackMe summary event creation failure, record="{json.dumps(summary_event, indent=1)}", exception="{str(e)}"'
                        )
                ...
                # if the entity is inactive but does not yet have a status red
                    ...
                    try:
                        ...
                        # Generate the summary event - concise format matching active tracker output
                        summary_event = {
                            "object_state": record.get("object_state", "red"),
                            "status_message": record.get("status_message", "entity is red due to inactivity"),
                            "anomaly_reason": record.get("anomaly_reason", "inactive"),
        ...
        if report_objects_list:
            ...
            try:
                ...
                logging.error(
                    f'tenant_id="{self.tenant_id}", component="flx", could not send notification event, exception="{e}"'
                )
        ...
        if errors_count > 0:
            if self.register_component and self.tenant_id and self.report:
                try:
                    ...
                    logging.error(
                        f'tenant_id="{self.tenant_id}", instance_id={instance_id}, component="flx", Failed to call trackme_register_tenant_object_summary with exception="{str(e)}"'
                    )
        else:
            if self.register_component and self.tenant_id and self.report:
                try:
        ...
        # end of main
        logging.info(
            f'tenant_id="{self.tenant_id}", instance_id={instance_id}, trackmesplkflxinactiveinspector has terminated, run_time="{round(time.time() - start, 3)}"'
        )
