...
filehandler = RotatingFileHandler(
    "%s/var/log/splunk/trackme_splk_component_health_advisor_helper.log" % splunkhome,
    mode="a",
    maxBytes=10000000,
    backupCount=1,
)
...
class SplkComponentHealthAdvisorExecutor(GeneratingCommand):
    tenant_id = Option(
        doc="""
        **Syntax:** **tenant_id=****
    ...
    component = Option(
        doc="""
        **Syntax:** **component=****
        **Description:** The component category: wlk or mhm.""",
        require=True,
        default=None,
        validate=validators.Match("component", r"^(?:wlk|mhm)$"),
    )

    def generate(self, **kwargs):
        # Python version check — Component Health Advisor requires Python 3.13+
                ...
                "_raw": json.dumps(
                    {
                        "status": "error",
                        "message": (
                            f"AI Component Health Advisor requires Python 3.13+. "
        ...
        from trackme_libs_ai_component_health import (
            start_component_health_advisor_from_search_context,
        )
        ...
        # (unified flag — replaces the old per-advisor
        # ai_wlkadvisor_enabled / ai_mhmadvisor_enabled).
        #
        ...
        if component.lower() not in components_list:
            yield {
                "_time": time.time(),
                "_raw": json.dumps(
                    {
                        ...
                        "status": "info",
                        "message": (
                            f"Component '{component}' is not in ai_components_advisor_list "
        ...
        component_info_key = f"component_splk_{component}"
        try:
                ...
                "_raw": json.dumps(
                    {
                        ...
                        "status": "info",
                        "message": (
                            f"Component {component} is disabled for this tenant "
                            f"({component_info_key}=0), no action taken."
                        ),
                    }
                ),
            }
        ...
        ai_advisor_mode = str(vtenant_account.get("ai_components_advisor_mode", "inspect"))
        ai_advisor_provider_name = str(vtenant_account.get("ai_components_advisor_provider_name", ""))
        provider_name = ai_advisor_provider_name if ai_advisor_provider_name else None

        try:
            max_runtime = int(vtenant_account.get("ai_components_advisor_max_runtime_sec", 14400))
        except (ValueError, TypeError):
            max_runtime = 14400

        try:
            min_days = int(vtenant_account.get("ai_components_advisor_min_days_between_reviews", 30))
        except (ValueError, TypeError):
            min_days = 30

        # Shared AI Agent automated-action filter — see ML Advisor helper
        # for the full rationale. CSV is consumed both as the cheap SPL
        # pre-filter and as the Python post-filter; both must agree.
        ...
        logging.info(
            f'tenant_id="{self.tenant_id}", component="{component}", '
            f'ai_components_advisor_mode="{ai_advisor_mode}", '
            f'ai_components_advisor_provider_name="{ai_advisor_provider_name}", '
            f'ai_components_advisor_max_runtime_sec="{max_runtime}", '
            f'ai_components_advisor_min_days_between_reviews="{min_days}", '
        ...
        # Priority filter — skip entities whose `priority` is not in the
        # shared `ai_automated_priority_filter` list. Empty list = no SPL
        # pre-filter; the Python post-filter still applies the match-all
        # path via ``is_ai_automated_eligible_for_entity``.
        ...
        # filter expression may reference (see Feed Lifecycle helper).
        base_query = f"""\
                | trackmegetcoll tenant_id="{self.tenant_id}" component="{component}"
                | where monitored_state="enabled"{priority_clause}
                | table keyid, object, object_state, priority, tags, labels, data_index, data_sourcetype"""

        if min_days > 0:
            review_filter = f"""
                | join type=left object [
                    search index={tenant_summary_idx} sourcetype="trackme:ai_agent:component_health_advisor:{component}:*"
                        earliest=-{min_days}d latest=now
                        tenant_id="{self.tenant_id}" component="{component}" automated="true"
        ...
        # Sort: RED entities first, then ORANGE, then the rest — alphabetically within tier
        ...
        try:
                ...
                if isinstance(item, dict):
                    ...
                    if not eligible:
                        ...
                        logging.debug(
                            ...
                            f'skipped from Component Health Advisor by AI automated filter, reason="{reason}"'
                        )
                        continue
            ...
            if (
                ...
                logging.info(
                    f'tenant_id="{self.tenant_id}", component="{component}", '
                    f'AI automated-action filter skipped entities from Component Health Advisor: '
        ...
        if len(entities_toprocess) > 0:
            for entity_record in entities_toprocess:
                ...
                logging.info(
                    f'tenant_id="{self.tenant_id}", component="{component}", '
                    f'starting Component Health Advisor for object="{object_value}", mode="{ai_advisor_mode}"'
                )

                status = "unknown"
                job_id = None
                try:
                    agent_result = start_component_health_advisor_from_search_context(
                    ...
                    msg = (
                        f'tenant_id="{self.tenant_id}", component="{component}", '
                        f'object="{object_value}", Component Health Advisor failed with exception="{str(e)}"'
                    )
                    logging.error(msg)
            ...
            results_dict = {
                ...
                "component": component,
                "action": "success",
                "results": "AI Component Health Advisor job successfully executed",
                ...
                handler_events_records.append(
                    {
                        ...
                        "object_category": f"splk-{component}",
                        "handler": f"trackme_{component}_component_health_advisor_tracker_tenant_{self.tenant_id}",
                        "handler_message": "Entity was reviewed by AI Component Health Advisor.",
                        "handler_troubleshoot_search": (
                            f'index=_internal '
                            f'(sourcetype=trackme:custom_commands:trackmesplkcomponenthealthadvisorhelper) '
            ...
            results_dict = {
                ...
                "component": component,
                "action": "success",
                "results": "AI Component Health Advisor job executed but there were no entities to review at this time",
                "run_time": round((time.time() - start), 3),
                "mode": ai_advisor_mode,
                "entities_count": 0,
                "upstream_search_query": search_query,
        ...
        report_name = f"trackme_{component}_component_health_advisor_tracker_tenant_{self.tenant_id}"
...
dispatch(SplkComponentHealthAdvisorExecutor, sys.argv, sys.stdin, sys.stdout, __name__)
