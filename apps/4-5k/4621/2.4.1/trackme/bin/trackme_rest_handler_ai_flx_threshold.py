...
"""
TrackMe REST Handler — AI FLX Threshold Advisor

Endpoints:
    POST   /trackme/v2/ai_flx_threshold/threshold_advisor         — Start agent analysis
    GET    /trackme/v2/ai_flx_threshold/threshold_advisor_status  — Poll agent job status
    DELETE /trackme/v2/ai_flx_threshold/threshold_advisor_cancel  — Cancel running job
"""

__name__ = "trackme_rest_handler_ai_flx_threshold.py"
...
logger = setup_logger("trackme.rest.ai.flx_threshold", "trackme_rest_api_ai_advisor_flx_threshold.log")
...
from trackme_libs_ai_flx_threshold import (
    start_flx_threshold_advisor_async,
)


class TrackMeHandlerAiFlxThreshold_v2(trackme_rest_handler.RESTHandler):
    def __init__(self, command_line, command_arg):
        super(TrackMeHandlerAiFlxThreshold_v2, self).__init__(
            command_line, command_arg, logger
        )

    # Resource group description
    def get_resource_group_desc_ai_flx_threshold(self, request_info, **kwargs):
        response = {
            "resource_group_name": "ai_flx_threshold",
            "resource_group_desc": (
                "AI FLX Threshold Advisor agent endpoints. These endpoints enable AI-powered "
                "analysis and calibration of Flex Object entity threshold configurations, "
                "including metric coverage assessment, threshold value calibration, variable "
                "threshold slot management, and decommission recommendations."
            ),
        }
        return {"payload": response, "status": 200}

    # -----------------------------------------------------------------
    # POST /trackme/v2/ai_flx_threshold/threshold_advisor — Start analysis
    # -----------------------------------------------------------------
    def post_threshold_advisor(self, request_info, **kwargs):
        """
        Start a FLX Threshold Advisor agent analysis for a Flex Object entity.

        POST body (JSON):
            tenant_id (required): Tenant identifier
            component (required): Component type — flx
        ...
        if describe:
            return {
                "payload": {
                    "describe": (
                        "Start a FLX Threshold Advisor agent analysis for "
                        "a Flex Object (FLX) entity. The advisor inspects "
                        "the entity's metric distributions and current "
                        "thresholds, then proposes calibrated threshold "
                        "values. ``inspect`` mode is read-only with "
                        "calibration recommendations; ``act`` mode applies "
                        "threshold changes automatically. Component: "
                        "``flx`` only."
                    ),
                    "resource_desc": (
                        "Start an asynchronous FLX Threshold Advisor "
                        "agent run for a Flex Object entity"
                    ),
                    "resource_spl_example": (
                        '| trackme url="/services/trackme/v2/ai_flx_threshold/threshold_advisor" '
                        'mode="post" body="{\'tenant_id\': \'mytenant\', '
                        '\'component\': \'flx\', \'object_id\': \'<entity_key>\', '
                        '\'mode\': \'inspect\'}"'
                    ),
                    "options": [
                        {
                            ...
                            "component": (
                                "Required. Must be ``flx`` (FLX Threshold "
                                "Advisor only operates on Flex Object "
                                "entities)."
                            ),
                            "object": (
                            ...
                            "mode": (
                                ...
                                "read-only) or ``act`` (apply threshold "
        ...
        valid_components = {"flx"}
        if component not in valid_components:
            return {
                "payload": {
                    "error": f"Invalid component '{component}'. FLX Threshold Advisor supports: {sorted(valid_components)}"
                },
                "status": 400,
            }
        ...
        # Start the agent
        try:
            result = start_flx_threshold_advisor_async(
            ...
            logger.info(
                f'function=post_threshold_advisor, tenant_id="{tenant_id}", '
            ...
            logger.error(f"FLX Threshold Advisor start failed: {e}", exc_info=True)
            return {"payload": {"error": f"Failed to start FLX Threshold Advisor: {e}"}, "status": 500}

    # -----------------------------------------------------------------
    # GET /trackme/v2/ai_flx_threshold/threshold_advisor_status
    # -----------------------------------------------------------------
    def get_threshold_advisor_status(self, request_info, **kwargs):
        """
        Get the status of a FLX Threshold Advisor agent job.

        Query parameters:
            job_id (required): The job identifier returned by POST /threshold_advisor

        Returns:
            {
                "status": "running" | "complete" | "error" | "cancelled",
                "result": { FlxThresholdAdvisorResult } | null,
                "error": "..." | ""
            }
        """
        describe = trackme_parse_describe_flag(request_info)
        if describe:
            return {
                "payload": {
                    "describe": (
                        "Poll the status of a FLX Threshold Advisor agent "
                        "job. Returns ``running`` / ``complete`` / "
                        "``error`` / ``cancelled``, the live progress "
                        "feed, and (on completion) the structured "
                        "``FlxThresholdAdvisorResult`` payload."
                    ),
                    "resource_desc": (
                        "Poll a FLX Threshold Advisor agent job's "
                        "status, progress, and result"
                    ),
                    "resource_spl_example": (
                        '| trackme url="/services/trackme/v2/ai_flx_threshold/threshold_advisor_status" '
    ...
    # -----------------------------------------------------------------
    # DELETE /trackme/v2/ai_flx_threshold/threshold_advisor_cancel
    # -----------------------------------------------------------------
    def delete_threshold_advisor_cancel(self, request_info, **kwargs):
        """
        Cancel a running FLX Threshold Advisor agent job.
        ...
        if describe:
            return {
                "payload": {
                    "describe": (
                        "Cancel a running FLX Threshold Advisor agent "
                    ...
                    "resource_spl_example": (
                        '| trackme url="/services/trackme/v2/ai_flx_threshold/threshold_advisor_cancel" '
        ...
        logger.info(f'function=delete_threshold_advisor_cancel, job_id="{job_id}"')
