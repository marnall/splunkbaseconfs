...
class TrackMeHandlerSplkTagPoliciesRead_v2(trackme_rest_handler.RESTHandler):
    def __init__(self, command_line, command_arg):
        super(TrackMeHandlerSplkTagPoliciesRead_v2, self).__init__(
    ...
    def post_tag_policies_list_transforms(self, request_info, **kwargs):
        ...
        if describe:
            response = {
                "describe": "This endpoint lists Splunk lookup transforms available to the calling user (or to a configured remote Splunk deployment account), used by the tag policy configuration UI to populate the lookup-name picker.",
                "resource_desc": "List available Splunk lookup transforms (local or remote) for tag policy configuration",
                "resource_spl_example": '| trackme mode=post url="/services/trackme/v2/splk_tag_policies/tag_policies_list_transforms" body="{\'filter\': \'tag\', \'account\': \'local\'}"',
                "options": [
                    {
    ...
    def post_tag_policies_get_lookup_fields(self, request_info, **kwargs):
        ...
        if describe:
            response = {
                ...
                "resource_desc": "Get fields from a lookup transform for tags policy field mapping",
                "resource_spl_example": '| trackme mode=post url="/services/trackme/v2/splk_tag_policies/tag_policies_get_lookup_fields" body="{\'lookup_name\': \'my_asset_lookup\'}"',
                "options": [
                    {
    ...
    def post_tag_policies_get_entity_fields(self, request_info, **kwargs):
        ...
        if describe:
            response = {
                "describe": "This endpoint returns the available entity fields for mapping in lookup-based tags policies, it requires a POST call with the following information:",
                "resource_desc": "Get available entity fields per component for tags policy lookup field mapping",
                "resource_spl_example": '| trackme mode=post url="/services/trackme/v2/splk_tag_policies/tag_policies_get_entity_fields" body="{\'component\': \'dsm\'}"',
                "options": [
                    {
    ...
    # Execute a Splunk search and return fields for search-based tag policies
    def post_tag_policies_execute_search(self, request_info, **kwargs):
        ...
        if describe:
            response = {
                "describe": "This endpoint executes a Splunk SPL search and returns fields and sample rows for search-based tags policy configuration, it requires a POST call with the following information:",
                "resource_desc": "Execute a Splunk SPL search and return fields and sample rows for search-based tags policy configuration",
                "resource_spl_example": '| trackme mode=post url="/services/trackme/v2/splk_tag_policies/tag_policies_execute_search" body="{\'tenant_id\': \'mytenant\', \'component\': \'dsm\', \'search_query\': \'| inputlookup my_asset_lookup\', \'earliest\': \'-5m\', \'latest\': \'now\'}"',
                "options": [
                    {
                        "tenant_id": "REQUIRED. The tenant identifier",
