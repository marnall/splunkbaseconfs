...
class AlertActionWorkertrackme_free_style_rest_call(ModularAlertBase):
    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkertrackme_free_style_rest_call, self).__init__(
            ta_name, alert_name
        )
    ...
    def validate_params(self):
        if not self.get_param("endpoint_url"):
            self.log_error(
                "endpoint_url is a mandatory parameter, but its value is None."
            )
            return False
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_trackme_free_style_rest_call_helper.process_event(
                self, *args, **kwargs
            )
...
if __name__ == "__main__":
    exitcode = AlertActionWorkertrackme_free_style_rest_call(
        "trackme", "trackme_free_style_rest_call"
    ).run(sys.argv)
    sys.exit(exitcode)
