...
filehandler = RotatingFileHandler(
    os.path.join(splunkhome, "var", "log", "splunk", "trackme_trackmesplkslaclass.log"),
    mode="a",
    maxBytes=10_000_000,
    backupCount=1,
)
...
class TrackMeHandlePriority(GeneratingCommand):
    ...
    def generate(self, **kwargs):
        ...
        # target
        target_url = f"{self._metadata.searchinfo.splunkd_uri}/services/trackme/v2/splk_sla_policies/write/sla_policies_apply"
        ...
        sla_policies_collection_name = (
            f"kv_trackme_{self.component}_sla_policies_tenant_{self.tenant_id}"
        )
        ...
        provenance = f"trackmesplkslaclass:{self.component}:tenant_{self.tenant_id}"
        (
            ...
            self.service, sla_policies_collection_name, page=1, page_count=0, kvcollection_mode=kvcollection_mode, provenance=provenance, logger=logging
        )
        ...
        if not proceed or schema_version_upgrade_in_progress:
            ...
            if not schema_version_upgrade_in_progress:
                ...
                trackme_register_tenant_object_summary(
                    ...
                    f"trackme_{self.component}_sla_tracker_tenant_{self.tenant_id}",
                    "success",
                    time.time(),
                    str(time.time() - start),
