...
class Kill_Checks(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        task_id = "N/A"
        if "task_id" in args['form_parameters'] :
            task_id = args['form_parameters']['task_id']

        # Windows OS (not supporting ps -ef )
        if "pid" in args['form_parameters'] :
            pid = args['form_parameters']['pid']
            os.kill(int(pid),signal.SIGTERM)
            logger.info('task_id="%s" task_status="CANCELLED" check="N/A" status="cancelled" category="N/A" total="N/A" job_status="cancelled" search_order="N/A" pid="%s"' % (task_id, pid))
            return {'payload': 'killed', 'status': 200}

        # Linux/Mac/Unix (ps -ef)
        output = subprocess.check_output("ps aux | grep scma", shell=True)
        lines = output.decode().split("\n")
        for cmd in lines :
            if "/scma/bin/scma.py" in cmd :
                arguments = cmd.split(" ")
                if sys.version_info >= (3, 0):
                    arguments = list(filter(len, arguments))
                else :
                    arguments = filter(len, arguments)

                pid = arguments[1];

                if "type" in args['form_parameters'] :
                    if "ko" in args['form_parameters']['type'] :
                        if ",100," in cmd :
                            os.kill(int(pid),signal.SIGTERM)
                    else :
                        if 'order=2-' in cmd :
