...
class LoadResults(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        results = args['form_parameters']["results"]
        
        tar = tarfile.open(results)
        f = tar.extractfile("scma_aaa_results.csv")
        lines = f.readlines()
        f.close()
        ...
        with tarfile.open(results) as tar:
            subdir_and_files = [
                ...
                if not tarinfo.name.startswith("scma_aaa_results.csv")
            ]
            tar.extractall(members=subdir_and_files,path=reports_folder)  # nosemgrep
