...
class SplunkBaseAuth(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        try:
            ...
            r = requests.post('https://splunkbase.splunk.com/api/account:login', data=payload)
            
        except requests.exceptions.SSLError as e:
            return {'payload': json.dumps({'msg': str(e)}), 'status': r.status_code}

        if r.status_code == 200:
            return {'payload': r.text.split("<id>")[1].split("</id>")[0], 'status': r.status_code}
