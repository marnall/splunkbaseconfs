...
class SplunkAuth(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        username = args['form_parameters']["username"]
        password = args['form_parameters']["password"]

        response = ""

        try:
            r = requests.get('https://api.splunk.com/2.0/rest/login/splunk', verify=False, auth=(username, password))  # nosemgrep
        except requests.exceptions.SSLError as e:
            return {'payload': json.dumps({'msg': str(e)}), 'status': r.status_code}

        if r.status_code == 200:
            return {'payload': r.json()["data"]["token"], 'status': r.status_code}
        if r.status_code == 401:
            response = json.dumps(r.json())
        else:
            response = json.dumps({'msg': 'Error receiving data from API. Please try again later.','status_code': r.status_code})
