...
def run_btool(*args):
    ...
    #appFilter = ["system/default", "leaflet_maps_app", "location_tracker_app", "sankey_diagram_app", "simple_xml_examples", "splunk_monitoring_console", "timeline_app", "parallel_coordinates"]
    appFilter = ["/local/"]


    for line in re.split('\n?(?=\/opt)',stdout.decode('utf-8')):
        line = line.strip()
        should_skip = False
        for x in appFilter:
            if x not in line:
...
def get_btool_internal_host():
    ...
    for (app, stanza, lines) in run_btool("server", "list-debug"):
        if re.match(r"general", stanza):
            for line in lines:
                hostMatch = re.search(r"serverName\s*=\s*([^\s\v]+)$", line)
