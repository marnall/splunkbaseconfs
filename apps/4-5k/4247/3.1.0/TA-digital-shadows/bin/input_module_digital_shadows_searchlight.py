...
from digital_shadows_utility import set_input, get_request_parameters, scroll_incidents, retrieve_verbose_incidents, \
...
def collect_events(helper, ew):
    ...
    try:
        ...
        retrieve_private_incidents(helper, authorization, proxy, ew, input_stanza, input_name)
...
def retrieve_private_incidents(helper, authorization, proxy, ew, input_stanza, input_name):
    ...
    try:
        ...
        private_incident = checkpoint.get('private_incident') or since
        private_incident_alerted = checkpoint.get('private_incident_alerted') or private_incident

        #Fetch alerted:false
        info_alerted_false = get_request_parameters('incidents', authorization, helper, final_time, private_incident=private_incident)

        # This method will retrieve private incidents in batch and writes them to splunk
        scroll_incidents(helper, proxy, ew, info_alerted_false, 'incidents')
        checkpoint["private_incident"] = final_time
        helper.save_check_point(input_name, checkpoint)

        #Fetch alerted:true
        info_alerted_true = get_request_parameters('incidents', authorization, helper, final_time,"true", private_incident=private_incident, private_incident_alerted=private_incident_alerted)

        # This method will retrieve private incidents in batch and writes them to splunk
        scroll_incidents(helper, proxy, ew, info_alerted_true, 'incidents')
        checkpoint = helper.get_check_point(input_name) or dict()
        checkpoint["private_incident_alerted"] = final_time
        helper.save_check_point(input_name, checkpoint)
