...
ioc_id_list = []
MAPPING_SOURCETYPE = {
    'incidents':'ds:searchlight:alerts',
    'intel_incidents': 'ds:searchlight:intel:incidents',
    'credential:compromise': 'ds:searchlight:credentials',
    'risk_pipeline': 'ds:searchlight:riskpipeline',
    'intel_ioc_incidents': 'ds:searchlight:intel:iocs',
    'verbose': 'verbose'
}
...
def write_to_splunk(helper, r_json, ew, incident_private_intel, alerted=None):
    ...
    try:
        ...
        if input_stanza[input_name]['verbose'] == '1' and incident_private_intel == 'incidents':
            for content in r_json:
                ...
                event = helper.new_event(data=json.dumps(content), time=None,
                                     host=None, index=input_stanza[input_name]['index'], source='digital_shadows',
        ...
        if incident_private_intel == 'incidents':
            if alerted == "false":
                ...
                checkpoint["private_incident_alerted"] = checkpoint_event["modified"]
            ...
            if response.status_code == 200:
                ...
                if total == 0:
                    ...
                    if incident_private_intel == 'verbose':
                        ...
                        write_to_splunk(helper, r_json['content'], ew, incident_private_intel, alerted=info['payload']['filter']['alerted'])
...
def get_incidents_header(types, daterange, offset, alerted=None):
    ...
    return {
        "filter": {
            ...
            "withFeedback": 'true',
            "withoutFeedback": 'true',
            "alerted":'false' if alerted is None else 'true',
            "withTakedown": 'true',
...
def get_request_parameters(incident_private_intel, authorization, helper, finaltime, alerted=None, private_incident=None, private_incident_alerted=None):
    ...
    try:
        ...
        if incident_private_intel == 'intel_incidents':
            ...
            # This condition checks for non alerted incidents checkpoint 
            if checkpoint and checkpoint.get('private_incident') and alerted is None:
                helper.log_debug('Checkpoint is present:' + str(checkpoint))
                date = private_incident + '/' + finaltime
            # This condition checks for alerted incidents checkpoint
            elif checkpoint and checkpoint.get('private_incident_alerted') and alerted is "true":
                helper.log_debug('Checkpoint is present:' + str(checkpoint))
                date = private_incident_alerted + '/' + finaltime
            # This condition used to get non alerted incidents checkpoint in case of upgrade and new input scenario for alerted incidents
            elif checkpoint and not checkpoint.get('private_incident_alerted') and alerted is "true":
                helper.log_debug('Checkpoint is not present for alerted incidents. Taking start time of non alerted incidents   :' + str(checkpoint.get('private_incident')))
                date = private_incident_alerted + '/' + finaltime
            # This condition used to get start datetime from input configuration in case of new input scenario for non alerted incidents
            else:
                helper.log_debug('Checkpoint not present, Starting Time:' + str(since) + private_incident)
                date = private_incident + 'Z' + '/' + finaltime
            info['payload'] = get_incidents_header(get_types(helper), date, '0', alerted)
...
def getProxySettings(my_app, entities):
    ...
    proxy_settings_conf = os.path.join(
        make_splunkhome_path(["etc", "apps", my_app, "local", "ta_digital_shadows_settings.conf"]))
    proxies = {}
