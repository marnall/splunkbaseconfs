...
"""Graph Query template to pull the list of all audit logs"""
AUDITLOGS_GRAPHQL_QUERY_TEMPLATE = """
query GetAuditLogs {{
  users(count: 1000, page: 1) {{
    edges {{
      node {{
        ... on FullUserProfile {{
          id
...
def collect_events(helper, ew):
    ...
    while not all_params['page_load_success'] and \
        ...
        debug(helper, all_params, 
             "Try: {try}. Requesting audit logs from AssetNote...")
        graphql_query = AUDITLOGS_GRAPHQL_QUERY_TEMPLATE.format(**all_params)
        ...
        try:
            ...
            raw_err_msg = str(e)
            err_msg  = "Error in send_http_request for requesting audit logs in try: {try}. "
        ...
        if status_code == 200:
            ...
            all_params['page_load_success'] = False
            debug(helper, all_params, 
                "Sleeping {backoff_time}s before re-requesting audit logs...")
            time.sleep(all_params['backoff_time'])


    # ------------------------------------------------------------------------
    # Parse the JSON response with the audit logs from Assetnote
    # ------------------------------------------------------------------------
    
    if status_code == 200:

        debug(helper, all_params,
             "Parsing response to get the list of all audit logs JSON...")
        resp_json = resp.json()
        
        debug(helper, all_params, 
            "Getting the audit logs...")
        if 'data' in resp_json:
            if 'users' in resp_json['data']:
                audit_logs_json = resp_json['data']['users']['edges']
                all_params['audit_logs_count'] = len(audit_logs_json)

        debug(helper, all_params, 
            "Adding audit logs to Splunk...")
        
        new_event = helper.new_event(json.dumps(audit_logs_json, indent=4),
        ...
        info(helper, all_params, 
            "Writing an event to Splunk index: {assetnote_index}, sourcetype: {assetnote_sourcetype}, source: {assetnote_source}, count: {audit_logs_count}...")
        
        ew.write_event(new_event)
    else:
        
        info(helper, all_params, "Error encountered when retrieving the audit logs...")
