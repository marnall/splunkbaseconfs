  ...
  assetGroups(f: {{field: "id", op: EQ, value: "{ag_id}"}}) {{
    edges {{
      node {{
        id
        name
        ipRanges(count: {page_count}, page: {page_num}, f:[{pull_time_after}]) {{
          pageInfo {{
            hasNextPage
          }}
          edges {{
  ...
  assetGroups(f: {{field: "id", op: EQ, value: "{ag_id}"}}) {{
    edges {{
      node {{
        id
        name
        domains(count: {page_count}, page: {page_num}, f:[{pull_time_after}]) {{
          pageInfo {{
            hasNextPage
          }}
          edges {{
...
def collect_events(helper, ew):
    ...
    last_pull_time = checkpoint_helper.get_check_point('ag_last_pull_time')

    if not last_pull_time:
        info(helper, {}, 
            "ASSETGROUPS INFO: Last pull time checkpoint not set, pulling all data for asset groups from Assetnote.")
        ...
        info(helper, {"last_pull_time": last_pull_time}, 
            "ASSETGROUPS INFO: Last pull time checkpoint IS set. Pulling all data from {last_pull_time}.")
                    ...
                    for edge in resp_json_ip_ranges_edges:
                        if 'node' in edge:
                            if 'cidr' in edge['node']:
                                ...
                                whoisContact = edge['node']['whoisContact']
                                all_params['ip_range'] = ip_range
                                debug(helper, all_params,
                                      "Adding cidr: {ip_range} to ip_ranges")
                                ipObj = {'cidr':ip_range, 'whoisContact':whoisContact}
                    ...
                    if 'pageInfo' in resp_json_ip_ranges:
                        ...
                        if 'hasNextPage' in resp_json_page_info:
                            ...
                            if get_next_page == False or get_next_page == 'false' or get_next_page == 'False':
                                ...
                                checkpoint_helper.save_check_point('ag_last_pull_time', current_time)
    ...
    for asset_group in asset_groups:
        ...
        while get_next_page:
            ...
            asset_group_domains_per_page['ag_id'] = all_params['ag_id']
            asset_group_domains_per_page['ag_name'] = all_params['ag_name']
                    ...
                    for edge in resp_json_domains_edges:
                        if 'node' in edge:
                            if 'name' in edge['node']:
                                ...
                                domain_obj = {'name': domain, 'whois': whoisContact}
                                asset_group_domains_per_page['domains'].append(domain_obj)
