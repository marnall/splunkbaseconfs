...
def download_zip(zip_dir, file_name):
    r = requests.get(GITHUB_URL)
    with open(os.path.join(zip_dir, file_name),'wb') as f:
        f.write(r.content)
...
def get_new_files_from_github(zip_dir):
    ...
    download_zip(zip_dir,"master.zip")
    os.chdir(zip_dir)
...
def _get_csv(url, file_name):
    ...
    with open(os.path.join(LOOKUPS_DIRECTORY,file_name),'wb') as f:
        f.write(r.content)
