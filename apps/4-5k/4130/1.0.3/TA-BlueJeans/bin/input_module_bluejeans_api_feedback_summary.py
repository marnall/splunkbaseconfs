...
def collect_events(helper, ew):
    ...
    try:
        ...
        # Ex: If current day when script runs is: 2018-06-09, then starttime = 2018-06-07T23:59.59, endtime = 2018-06-08T23:59.59
        ...
        url = base_api_indigo_url + "/v2/enterprise/" + str(enterprise_id) + "/indigo/feedback/count"
        ...
        helper.log_debug("input_type={0:s} input={1:s} message='Requesting commit data from Feedback Summary API.' url='{2:s}' parameters='{3:s}'".format(inputtype, inputname, url, json.dumps(parameter)))

        # Return API response code
        r_status = response.status_code
        helper.log_debug("input_type={0:s} input={1:s} message='Feedback Summary API response code' status_code={2:d}".format(inputtype,inputname, r_status))

        # Return API request status_code
        if r_status is not 200:
            helper.log_error("input_type={0:s} input={1:s} message='Feedback Summary API request unsuccessful.' status_code={2:d}".format(inputtype,inputname,r_status))
        ...
        if checkpoint_value:
            helper.log_debug("input_type={0:s} input={1:s} message='Event already pushed for Feedback Summary data at timestamp key {2:s}. Skipping this..'".format(inputtype, inputname, eventTimestamp))
        else:
            event['timestamp'] = eventTimestamp
            helper.log_debug('\n\n eventTimestamp: ' + str(eventTimestamp))
            helper.log_debug("input_type={0:s} input={1:s} message='Pushing Feedback Summary data Event to Splunk with timestamp {2:s}'".format(inputtype, inputname, eventTimestamp))
