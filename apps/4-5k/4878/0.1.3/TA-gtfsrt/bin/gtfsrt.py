...
SCHEME = """<scheme>
    ...
    <endpoint>
        <args>
            ...
            <arg name="icon">
                <title>Realtime Location Track Icon</title>
                <description>Enter ambulance, bicycle, bus, car, motorcycle, plane, ship, space-shuttle, subway, taxi, train, or truck.</description>
                <data_type>string</data_type>
                <validation>
                validate(icon in ("","ambulance", "bicycle", "bus", "car", "motorcycle", "plane", "ship", "space-shuttle", "subway", "taxi", "train", "truck"), "Invalid Icon")
                </validation>
                <required_on_create>false</required_on_create>
            </arg>
        </args>
    ...
    if(response.status_code == 200):
        ...
        for entity in feed.entity:
            if entity.HasField('vehicle') and entity.vehicle.timestamp > lasttime:
                if "icon" in config:
                    print("<event><time>{}</time><source>{}</source><sourcetype>gtfsrt:vehicle</sourcetype><data>id=\"{}\" lat={} lng={} status=\"{}\" route=\"{}\" trip=\"{}\" icon={}</data></event>".format(entity.vehicle.timestamp,config["feed"],entity.vehicle.vehicle.id, entity.vehicle.position.latitude,entity.vehicle.position.longitude, entity.vehicle.current_status, entity.vehicle.trip.route_id, entity.vehicle.trip.trip_id,config["icon"]))
                else:
            ...
            if entity.HasField('alert') and (len(entity.alert.active_period) == 0 or any((x.start < feed.header.timestamp and x.end > feed.header.timestamp) for x in entity.alert.active_period)):
                print("<event><time>{}</time><source>{}</source><sourcetype>gtfsrt:alert</sourcetype><data>header=\"{}\" description=\"{}\" url=\"{}\"</data></event>".format(feed.header.timestamp,config["feed"],entity.alert.header_text.translation[0].text,entity.alert.description_text.translation[0].text,entity.alert.url.translation[0].text))
        print("</stream>")
        open(checkpointfile, "w").write(str(feed.header.timestamp))
    else:
        logging.error("{} returned status {}".format(config["feed"],response.status_code))
