...
def setup_logger(level, name, file_name, use_rotating_handler=True):
    ...
    formatter = logging.Formatter('%(asctime)s  %(filename)s@%(lineno)5d : %(levelname)8s - %(message)s')
...
class RestHandler(admin.MConfigHandler):
    ...
    def setup(self):
        ...
        self.logger.debug("setup info : self.requestedAction %s " % self.requestedAction)
	...
	if 1:
	    ...
	    for debug_args in self.supportedArgs.args:
		self.logger.debug("setup info : self.supportedArgs %s " % debug_args.argName)
    ...
    def handleEdit(self, confInfo):
        ...
        self.logger.debug("POST EDIT ACTION  	stanza :  self.callerArgs.id = %s " % self.callerArgs.id )
        self.logger.debug("POST EDIT ACTION parameters :  self.callerArgs = %s " % self.callerArgs )
        ...
        try:
            ...
            if name is not None:
                for stanza, settings in confDict.items():
                    if stanza == name:
                        is_found = True
        		self.logger.debug("POST EDIT ACTION :  find stanza %s " % self.callerArgs.id )
            ...
            for p in clearable_params:
       	    ...
       	    self.logger.debug("POST EDIT ACTION :  before write ")
            # Write out the updated conf
       	    self.logger.debug("POST EDIT ACTION :  before write  file %s, name %s, validated_params %s" % (self.conf_file, name, validated_params))
	    
	    #del confDict[name]
	    #confDict[name] = validated_params

	    if self.handleEditCreate_prefix_callback is not None:
		self.handleEditCreate_prefix_callback()

            self.writeConf(self.conf_file, name, validated_params)
       	    self.logger.debug("POST EDIT ACTION :  after write ")

	    if self.handleEditCreate_suffix_callback is not None:
		self.handleEditCreate_suffix_callback()
    ...
    def handleCreate(self, confInfo):
        """
        Handles Creates to the configuration options

        Arguments
        confInfo -- The object containing the information about what is being requested.
        """

        self.logger.debug("POST CREATE ACTION  	  stanza :  self.callerArgs.id = %s " % self.callerArgs.id )
        self.logger.debug("POST CREATE ACTION parameters :  self.callerArgs = %s " % self.callerArgs )
        ...
        try:
            ...
            if name is not None:
                for stanza, settings in confDict.items():
                    if stanza == name:
                        is_found = True
        		self.logger.debug("POST CREATE ACTION :  find stanza %s " % self.callerArgs.id )
            ...
            # Stop if we could find the name, suppose to be the first time to add the config record 
            if is_found:
                raise admin.NotFoundException("Already have a stanza for the given name '%s'" % (name))
            ...
            # Check the configuration settings
            cleaned_params = self.checkConf(new_settings, name, confInfo, existing_settings=None)
            ...
            for p in clearable_params:
       	    ...
       	    self.logger.debug("POST CREATE ACTION :  before write ")
            # Write out the updated conf
       	    self.logger.debug("POST CREATE ACTION :  before write  file %s, name %s, validated_params %s" % (self.conf_file, name, validated_params))
	    ...
	    '''	
            ...
            self.writeConf(self.conf_file, name, validated_params)
       	    self.logger.debug("POST CREATE ACTION :  after write ")
		...
		self.handleEditCreate_suffix_callback()
            ...
            self.logger.exception("Exception generated while performing create")
    ...
    def handleRemove(self, confInfo):
        """
        Handles removes to the configuration options

        Arguments
        confInfo -- The object containing the information about what is being requested.
        """

        self.logger.debug("POST REMOVE ACTION  	  stanza :  self.callerArgs.id = %s " % self.callerArgs.id)
        self.logger.debug("POST REMOVE ACTION parameters :  self.callerArgs = %s " % self.callerArgs )
        ...
        try:
            ...
            if name is not None:
                for stanza, settings in confDict.items():
                    if stanza == name:
                        is_found = True
        		self.logger.debug("POST REMOVE ACTION :  find stanza %s " % self.callerArgs.id )
            ...
            # Get the settings that are being set
            new_settings = copy.copy(existing_settings)

	    for key in new_settings:
		new_settings[key] = "INVALID"
            ...
            for p in clearable_params:
       	    ...
       	    self.logger.debug("POST EDIT REMOVE ACTION :  before write ")
            # Write out the updated conf
       	    self.logger.debug("POST EDIT REMOVE ACTION :  before write  file %s, name %s, validated_params %s" % (self.conf_file, name, validated_params))
	    ...
	    self.writeConf(self.conf_file, name, new_settings)
	
	    '''
	    for stanza, settings in confDict.items(): 
		self.writeConf(self.conf_file, stanza, settings)
       	    	self.logger.debug("POST EDIT REMOVE ACTION :  write stanza %s " % stanza)
	    '''

       	    self.logger.debug("POST REMOVE ACTION :  after write ")
