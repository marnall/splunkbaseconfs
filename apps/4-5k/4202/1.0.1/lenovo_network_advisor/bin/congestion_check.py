...
if __name__ == '__main__':
    ...
    if config.get(section_name, 'Congestion') != 'Enable':
	sys.exit(0)
    ...
    logfile = defconfig.get('switch_properties', 'Congestion_Log_File')
    interval = defconfig.get('switch_properties', 'Congestion_Interval')

    logger =  FieldLogHandler(logfile, 4, 5, INFO, 'TRAFFIC_DROPS')
    conn = Connection(params)
    if (set_bst_feature(conn) != 1):
	sys.exit(0)
	
    rpt = get_bst_congestion_queues_intf_up_instance(conn)
    if (rpt):
        splunk_congestion_log(conn.ip, rpt, logger) 
