...
from utils import string, number
...
class SonarSplunkServiceConfiguration:
    ...
    def request_body(self):
        body = {
            LICENSE: self.get_secret_field(LICENSE_FIELD),
            INDEX: self.index,
        ...
        limit = self.limit or self.app_configuration[LIMIT_FIELD]
    ...
    def validate_required_fields(self, cfg):
        ...
        for field in secret_fields:
            if SonarSplunkServiceConfiguration.is_blank_string(self.get_secret_field(field[KEY])):
                missing_fields.append(field)
