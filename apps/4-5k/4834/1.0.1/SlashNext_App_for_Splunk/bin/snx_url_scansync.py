...
@Configuration()
class SnxUrlScanSyncCommand(GeneratingCommand):
    # Class variables
    ...
    def generate(self):
        ...
        try:
            ...
            self.snx_logger.info('Running "snxurlscansync" Command')
            ...
            if self.url is not None:
                self.snx_logger.info('Checking URL Scan Sync for URL: {0}'.format(self.url))
                # We perform a URL Scan Sync command
                ...
                if response.ok:
                    ...
                    if data.get('errorNo') == 0:
                        ...
                        msg = 'Your URL Scan request is submitted to the cloud and may take up-to 60 ' \
                              ...
                              'action with scan_id = {0} or running the same "snxurlscansync" action one more time'\
...
dispatch(SnxUrlScanSyncCommand, sys.argv, sys.stdin, sys.stdout, __name__)
