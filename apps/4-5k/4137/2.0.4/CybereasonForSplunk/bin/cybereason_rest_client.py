...
class CybereasonClient:
    ...
    def __init__(self, base_url=None, username=None, password=None, proxy=None, modular_input=None, auth_type=None, utils=None, cred_realm=None, **kwargs):
        try:
            ...
            self.headers = {"Content-Type": "application/json", "User-Agent": "CybereasonSplunkIntegration/2.0.4 (target="+self._get_server_address()+")"}
        ...
        try:
            if data is None:
                if self._useproxy:
                    ...
                    ret = self.session.get(url=url, headers=self.headers, verify=True, proxies=self.proxies)
                else:
                    log.debug("action=call_get use_proxy=false")
                    ret = self.session.get(url=url, headers=self.headers, verify=True)
                return ret
            else:
                if self._useproxy:
                    log.debug("action=call_post use_proxy=true")
                    ret = self.session.post(url=url, headers=self.headers, verify=True, data=data, proxies=self.proxies)
                else:
                    log.debug("action=call_post use_proxy=false url={} data={} headers={}".format(url, data, self.headers))
                    try:
                        ret = self.session.post(url=url, headers=self.headers, verify=True, data=data)
                    except Exception as e:
