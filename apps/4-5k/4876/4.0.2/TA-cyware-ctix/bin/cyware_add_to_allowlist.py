...
# encoding = utf-8
"""Alert action worker for cyware_add_to_allowlist."""
...
from ta_cyware_ctix import modalert_cyware_add_to_allowlist_helper


class AlertActionWorkercyware_add_to_allowlist(ModularAlertBase):
    """Alert action worker for adding indicators to allowlist."""

    def __init__(self, ta_name, alert_name):
        """Initialize the alert action worker."""
        super(AlertActionWorkercyware_add_to_allowlist, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_cyware_add_to_allowlist_helper.process_event(
                self, *args, **kwargs
            )
...
if __name__ == "__main__":
    exitcode = AlertActionWorkercyware_add_to_allowlist("TA-cyware-ctix", "cyware_add_to_allowlist").run(sys.argv)
    sys.exit(exitcode)
