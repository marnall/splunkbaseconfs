...
def setup_logger():
    access_settings = cli_lib.getMergedConf("sn_sec_instance_es")
    level = access_settings['splunk_log']['logLevel']
    log_file_name = "TA_ServiceNow_ES_addon"
...
logger = setup_logger()
logger.info("checking log hit in event ingest file")

DEFAULT_ENDPOINT = "/api/sn_sec_splunkes/notable_event_ingestion"
...
def get_event(session_key, sid, offset, event_id):
    try:
...
def post_to_servicenow_using_token(access_settings, data_values, session_key, event_incident_endpoint, realm):
    ...
    if expiry_time < current_time:
        ...
        if response["status_code"] == 200:
            ...
            handle_response(session_key, response.status_code, response.url, "security incident")
    else:
...
def post_to_servicenow(session_key, **kwargs):
    logger.info("Invoking post_to_servicenow")
    try:
        realm = kwargs["realm"]
        instance_conf = cli_lib.getMergedConf("sn_sec_instance_es")
        ...
        if auth_type == "OAuth":
            ...
            logger.info("Invoking els epart")
...
def main():
    try:
        ...
        event_id = get_event_id(sys.argv)
        realm = sn_util.getRealm(get_instance(sys.argv))

        event = get_event(session_key, sid, offset, event_id)
        if event:
