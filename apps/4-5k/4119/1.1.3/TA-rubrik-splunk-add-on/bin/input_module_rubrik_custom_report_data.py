...
def collect_events(helper, ew):
    ...
    global_report_id = helper.get_arg("report_id")
    verify_ssl = helper.get_arg("verify_ssl_certificates")
    ...
    # get report data
    url = ('https://'+global_rubrik_node+'/api/internal/report/'+str(global_report_id)+'/table')
    has_more = True
    cursor = None
    payload = {}
    while has_more:
        ...
        data = helper.send_http_request(url, method='POST', parameters=parameters, headers=headers, verify=verify_ssl, timeout=timeout, use_proxy=False,payload=payload)
        if data.status_code != 200:
            failed_http_call(helper, data)
        data = data.json()
        has_more = data['hasMore']
        cursor = data['cursor']
        for entry in data['dataGrid']:
            this_record = {}
            for i in range(len(data['columns'])):
                this_record[data['columns'][i]] = entry[i]
            this_record['clusterName'] = cluster_name
            this_record['reportId'] = global_report_id
