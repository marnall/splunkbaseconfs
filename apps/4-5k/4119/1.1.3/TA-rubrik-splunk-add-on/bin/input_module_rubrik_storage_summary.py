...
def collect_events(helper, ew):
    ...
    #  get storage summary
    url = ('https://'+global_rubrik_node+'/api/internal/stats/system_storage')
    parameters = {}
    response = helper.send_http_request(url, method='GET', parameters=parameters, headers=headers, verify=verify_ssl, timeout=timeout, use_proxy=False)
    if response.status_code != 200:
        failed_http_call(helper, response)
    ...
    output['used'] = response['used']
    output['available'] = response['available']
    output['snapshot'] = response['snapshot']
    output['liveMount'] = response['liveMount']
    output['miscellaneous'] = response['miscellaneous']
    # return output
