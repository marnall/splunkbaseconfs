...
def collect_events(helper, ew):
    ...
    helper.log_debug('Cluster name for '+global_rubrik_node+' identified as '+cluster_name)
    # get archive location details
    ...
    for location in response['data']:
        for bw_type in ['Incoming','Outgoing']:
            parameters = {}
            parameters['data_location_id'] = location['id']
            parameters['range'] = '-15min'
            parameters['bandwidth_type'] = bw_type
            url = ('https://'+global_rubrik_node+'/api/internal/stats/archival/bandwidth/time_series')
            response = helper.send_http_request(url, method='GET', parameters=parameters, headers=headers, verify=verify_ssl, timeout=timeout, use_proxy=False)
            if response.status_code != 200:
                failed_http_call(helper, response)
            response = response.json()
            for entry in response:
                # format output
                this_record = {}
                this_record['_time'] = entry['time']
                this_record['locationId'] = location['id']
                this_record['locationName'] = location['name']
                this_record['clusterName'] = cluster_name
                this_record['type'] = bw_type
                this_record['value'] = entry['stat']
                # return output
