...
def collect_events(helper, ew):
    ...
    url = ('https://'+global_rubrik_node+'/api/internal/managed_volume?is_relic=false&primary_cluster_id=local')
    # get managed vol details
    response = helper.send_http_request(url, method='GET', parameters=parameters, headers=headers, verify=verify_ssl, timeout=timeout, use_proxy=False)
    if response.status_code != 200:
        failed_http_call(helper, response)
    response = response.json()
    ...
    exported_count = 0 # count of exported MVs
    writable_count = 0 # count of writable MVs
    nfs_count = 0 # count of NFS type MVs
    smb_count = 0 # count of SMB type MVs
    for mv in response['data']:
        ...
        if mv['shareType'] == 'NFS':
            ...
            smb_count += 1
    output['exported'] = exported_count
    output['writable'] = writable_count
    output['nfs'] = nfs_count
    output['smb'] = smb_count
    # return output
