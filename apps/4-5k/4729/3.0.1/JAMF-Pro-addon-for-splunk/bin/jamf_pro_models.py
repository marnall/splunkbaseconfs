...
_logger = logging.getLogger("jamf_pro_models")
...
class jamf_pro_computer:
    ...
    def build_from_string(self, computer, source_type):
        ...
        try:
            ...
            # Malformed XML would otherwise crash the input loop. Leave the
            # class-level empty default in place and log so the operator can
            # correlate the missing event with a parse error.
            preview = (computer[:200] if isinstance(computer, str) else str(computer)[:200])
            _logger.error(
                "jamf_pro_computer.build_from_string failed to parse XML (source_type=%s): %s | preview=%r",
                source_type, e, preview,
            )
    ...
    def paginate(self, *args, **kwargs):
        ...
        fin_xml = list()
        # Element.getchildren() was removed in Python 3.9; iterate directly.
...
class jamf_pro_mobile_device:
    ...
    def build_from_string(self, mobile_device, source_type):
        ...
        if source_type == "mobile_device":
            try:
                ...
                # input_module_jamf.py:134 calls this (not build_from_xml), so
                # a malformed mobile-device XML from Jamf would otherwise crash
                # the input. Leave the class-level empty default in place and
                # log so the operator can correlate empty events with the parse error.
                preview = (mobile_device[:200] if isinstance(mobile_device, str)
                           else str(mobile_device)[:200])
                _logger.error(
                    "jamf_pro_mobile_device.build_from_string failed to parse XML (source_type=%s): %s | preview=%r",
                    source_type, e, preview,
                )
    ...
    def build_from_xml(self, mobile_device, source_type):
        ...
        try:
            ...
            # Malformed XML from Jamf was previously swallowed silently, leaving
            # self.mobile_record at the class-level empty default and producing
            # empty events with no indication that parsing failed. Log it so the
            # operator can correlate empty/missing devices with a parse error.
            preview = (mobile_device[:200] if isinstance(mobile_device, str) else str(mobile_device)[:200])
            _logger.error(
                "build_from_xml(mobile_device) failed to parse XML (source_type=%s): %s | preview=%r",
                source_type, e, preview,
            )
...
class RestrictedSoftware:
    ...
    def build_from_string(self, restricted_software, source_type):
        ...
        try:
            ...
            preview = (restricted_software[:200] if isinstance(restricted_software, str)
                       else str(restricted_software)[:200])
            _logger.error(
                "RestrictedSoftware.build_from_string failed to parse XML (source_type=%s): %s | preview=%r",
                source_type, e, preview,
            )
...
class MacApplication:
    ...
    def build_from_string(self, mac_applications: str, source_type: str, *args, **kwargs):
        ...
        try:
            ...
            preview = (mac_applications[:200] if isinstance(mac_applications, str)
                       else str(mac_applications)[:200])
            _logger.error(
                "MacApplication.build_from_string failed to parse XML (source_type=%s): %s | preview=%r",
                source_type, e, preview,
            )
            return
