...
MODULE_INFO_CACHE_TIME = datetime.timedelta(minutes=10)
...
def get_module_not_found_error(module_id, action):
    message = f"No module with id {module_id} could be found to {action}. Check your selection and try again."
    return message
...
def apply_module_add(view_element, post_dict, module_conf):
    ...
    if parent_module_id == "_top":
        if not post_dict.get("layoutPanel"):
        ...
        parent_module_type = parent_module.get("name", None)
        if not sv.module_allows_downstream_modules(module_conf, parent_module_type) :
            raise BadRequest( f"Error: {parent_module_type} modules can never have other modules downstream.")
...
def apply_module_reattach(view_element, post_dict, module_conf):
    ...
    if parent_module_id == "_top":
        ...
        parent_module_type = new_parent.get("name", None)
        if not sv.module_allows_downstream_modules(module_conf, parent_module_type) :
            raise BadRequest(f"Error: {parent_module_type} modules can never have other modules downstream.")
    ...
    for name in sv.VIEW_ATTRIBUTES:
        ...
        if name == "template" and posted_value and not sv.mako_template_exists(posted_value):
            ...
            if posted_value and posted_value.find("../") != -1:
                ...
                fail = f"No css file named {posted_value} could be found in the {app} app."
...
class BadRequest(Exception):
    """
    making our own error 400 class.
    """
...
class CanaryViewHandler(PersistentServerConnectionApplication):
    ...
    def __init__(self, command_line, command_arg):
        ...
        self.moduleconf_cache = simplecache.create_cache("moduleconf_cache", MODULE_INFO_CACHE_TIME)
    ...
    def add_entries_for_static_files(self, request, view_dict, root_endpoint):
        """
        busywork around the various static files and where they are on the filesystem
        """
        current_app_prefix = sv.get_static_url_prefix(request.session_key, request.app, request.locale, root_endpoint)
        canary_app_prefix = sv.get_static_url_prefix(request.session_key, "canary", request.locale, root_endpoint)
        ...
        module_html, module_css, module_js, class_names = sv.get_files_for_view(
            view_dict.get("modules"), self.module_conf, request.ui_theme)
        ...
        custom_app_css = sv.get_custom_css_for_view(view_dict, request.app, request.ui_theme)
        custom_app_css.extend(sv.get_application_css(request.app, request.ui_theme))
    ...
    def render_edit_view_form(self, request, view_element):
        """
        worker function to fill out the Editor's form when the user is
        going to edit the attributes of the view itself
        """
        ...
        # also add the actual attributes that it actually has atm.
    ...
    def render_view(self, request, splunk_config, view_dict):
        """
        ...
        view_dict = self.add_entries_for_static_files(request, view_dict, splunk_config["ROOT_ENDPOINT"])
        ...
        sv.inject_nav_html_if_needed(view_dict, request.session_key, request.app, request.locale, splunk_config.get("ROOT_ENDPOINT", ""))
        ...
        validation_errors = sv.get_validation_errors(view_dict, self.module_conf, used_panels)
        ...
        # next steps - we need to use get_static_url_prefix to make ourselves
        # actual reliable client-side paths to these files.
        #logger.error(splunk_config.get("IMPORT_FROM", None))
        #apps_exporting_js = splunk_config["IMPORT_FROM"]
        ...
        keys_to_add = {
            ...
            "jsonConfig": json.dumps(splunk_config),
    ...
    def handle(self, in_string):
        ...
        try:
            ...
            splunk_config = sv.get_config(request)

            # if the request was for a theme that isn't supported, we just give back "light" as the theme"
            if not sv.is_theme_supported(request.ui_theme, splunk_config):
                splunk_config["THEME"] = "light"
                request.ui_theme = "light"
                logger.error("requested theme is unsupported so we are returning the page in the light theme")


            request.csrf_token = sv.get_csrf_token(request)

            cache_key = "moduleconf - " + request.ui_theme
            self.module_conf = self.moduleconf_cache.get(cache_key)

            if not self.module_conf:
                self.module_conf = module_loader.get_modules(request.session_key, request.ui_theme)
                self.moduleconf_cache.put(cache_key, self.module_conf)


            self.add_time_point("getting modules")

            if request.method == "POST":
                return self.handle_post(request, splunk_config)

            if request.method == "GET":
                return self.handle_get(request, splunk_config)

            return sv.build_response(405, f"Method {request.method} not allowed")

        except (splunk.ResourceNotFound, sv.ResourceNotFound):
            try:
                sv.get_app_config(request.session_key, request.app)
            except (splunk.ResourceNotFound, sv.ResourceNotFound):
                logger.error("No app found named ", request.app)
                message = f"<h3>HTTP Error 404 : There is no app named '{request.app}' installed</h3> <p>(If you feel this app is installed though, check whether it's disabled, make sure splunkd has been restarted since install, or perhaps try logging in as a different splunk user.)</p>"
                return sv.build_html_response(404, message)
            message = f"<h3>HTTP Error 404 : There is no view named '{request.view}' within this app.</h3> Or if there is, it isn't readable by your user account."
            return sv.build_html_response(404, message)
    ...
    def handle_post(self, request, splunk_config):
        ...
        if posted_xml:
            view_element = sv.parse_view_element(posted_xml, request)
            try:
                view_dict = sv.make_view_dict(view_element, request.app, self.module_conf, True)
            except sv.PatternNotFoundError as e:
                return sv.build_response(500, str(e))
            sv.log_activity(request, "rendering a posted view")

            return self.render_view(request, splunk_config, view_dict)

        try:
            _view_type, view_element = sv.get_view(request, self.module_conf,)
            ...
            if request.action in ["edit_view_props", "create"]:
                ...
                apply_module_reattach(view_element, request.post_dict, self.module_conf)
            ...
            _used_panels = sv.fill_in_inherited_layout_panels(view_dict["modules"])
            validation_errors = sv.get_validation_errors(view_dict, self.module_conf)
            if validation_errors:
                raise BadRequest("\n".join(validation_errors))

            sv.log_activity(request, "posting changes to view")
        ...
        return sv.build_response(200, '{"success":true, "action":"' + request.action + '}')



    def handle_get(self, request, splunk_config):
        try:
            ...
            if not request.view:
                ...
                view_type, view_element = sv.get_view(request, self.module_conf)
                #logger.error("app hp request. We determined a view of %s which has type %s", request.view, view_type)

                # TODO - Someday this should probably be refactored into a method on Request.
            ...
            self.add_time_point("getting the actual view")


            if request.action in ["edit", "create"]:
                sv.log_activity(request, "loading edit view form")
            ...
            self.add_time_point("make_view_dict")
            ...
            if request.qs_dict.get("output_as", False) == "xml":

                xml_output = et.tostring(view_element)
                return sv.build_response(200, xml_output.decode("utf-8"), "application/xml")

            sv.log_activity(request, "loading view")
            try:
                return self.render_view(request, splunk_config, view_dict)
            except sv.IncludeFileNotFoundError as e:
                return sv.build_response(500, str(e))
