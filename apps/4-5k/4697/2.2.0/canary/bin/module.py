...
def get_base_url(session_key):
    """simpler way to make this common call"""
...
def handle_edit(request):
    """only ever called from one place in handle() """
    module_id = request.qs_dict.get("moduleId", None)

    sv.log_activity(request, "editor loading a form ")


    module_conf = module_loader.get_modules(request.session_key)

    _view_type, view_element = sv.get_view(request, module_conf)
    ...
    if request.action == "add":
        ...
        class_names = list(module_conf.keys())
        class_names.sort()
        ...
        if module_id == "_new":
            ...
            if parent_module_id != "_top":
                ...
                for att_name in module_attributes:
                    val, _inherited_val = sv.get_module_attribute(parent_module, att_name)
            ...
            for att_name in module_attributes:
                val, inherited_val = sv.get_module_attribute(module, att_name)

                attributes[att_name] = {
                    "value":val,
                    "inheritedValue":inherited_val
                }
            current_param_values = sv.get_module_nodes_params_as_dict(module)

    template_dict["currentParamValues"] = current_param_values
...
def handle_describe(session_key, module_class_name, param_name=None):
    ...
    if param_name in MODULE_ATTRIBUTES_WITH_HELPTEXT:
        ...
        if not param_name:
            ...
            template = "/editor/module_description.html"
            #logger.error()
            template_dict["params"] = module_class["params"]
        else:
    ...
    def handle(self, in_string):
        ...
        try:
            ...
            if request.action in ["edit", "add", "delete", "reattach_existing", "debug"]:
                ...
                return handle_describe(request.session_key, module_class_name, param_name)
