...
import modalert_geni_helper

class AlertActionWorkergeni(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkergeni, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_geni_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkergeni("TA-fancyalerts", "geni").run(sys.argv)
    sys.exit(exitcode)
