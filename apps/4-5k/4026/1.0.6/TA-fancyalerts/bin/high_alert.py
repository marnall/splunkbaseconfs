...
import modalert_high_alert_helper

class AlertActionWorkerhigh_alert(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerhigh_alert, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_high_alert_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerhigh_alert("TA-fancyalerts", "high_alert").run(sys.argv)
    sys.exit(exitcode)
