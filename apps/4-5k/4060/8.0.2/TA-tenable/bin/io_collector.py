...
class IOCollector(TenableCollector):
    ...
    def compliance_event_transformer(self,
        ...
        updates = {
            ...
            "ipv4s": asset.get("ipv4_addresses", None),
            "ipv6s": asset.get("ipv6_addresses", None),
            "fqdns": asset.get("fqdns", None),
            "agent_name": asset.get("agent_name", None),
            "agent_uuid": asset.get("agent_uuid", None),
            "state": event.get("state", "").lower(),
            "compliance_state": event.get('status'),
            "actual_value": self._check_fix_plugin_output(event.pop('actual_value', ''))
        }
