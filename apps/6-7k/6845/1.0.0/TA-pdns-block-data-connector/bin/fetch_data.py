...
class AWSDataFetcher:
    ...
    def __init__(self, bucket_name : str, region_name : str, client_prefix : str, key_id : str, secret_key : str, input_stanza: str):
        ...
        self.state_manager = StateManager(f"{splunk_home}/etc/apps/TA-pdns-block-data-connector/{input_stanza}-last-time.txt")
    ...
    def get_recent_file_keys_and_dates(self) -> List[Tuple[str, str]]:
        ...
        for datetime_to_hour in inclusive_datetime_range(utc_last_read, utc_now):
            ...
            if 'Contents' in response:
                keys_and_dates += [
                    (file_object['Key'], file_object['LastModified'])
