...
def collect_events(helper, ew):
    ...
    unique_id      = str(uuid4())
    start          = splunk_utils.get_order_ingestion_start_time(helper, data_input_name, start_time, version)
    ...
    helper.log_info(
        f'Starting Order ingestion data_input={data_input_name} id={unique_id} period_from={start} period_to={to}'
    )
    for orders in magento_orders.get_created_orders_within_period(
        start,
        to
    ):
        for order in orders:
            ...
            splunk_utils.write_to_index(helper, ew, modified_order, hostname, "orders")
    splunk_utils.set_order_ingestion_next_start_time(helper, data_input_name, start_time, version, to)
    helper.log_info(
        f'Finished Order ingestion data_input={data_input_name} id={unique_id}'
    )
