...
activation_key_path = os.path.join(os.environ['SPLUNK_HOME'], 'etc', 'apps', 'purge-master', 'bin')
...
logfile = os.sep.join([os.environ['SPLUNK_HOME'], 'var', 'log', 'splunk', 'purge-master.log'])
        ...
        try : 
            ...
            if key_validator:
                    ...
                    if increased_date<key_generated:
                        akmsg = "NA(This key is for Lifetime)"
                        days = "NA"
                        data = {"activation_key_generated_on":Key_Status, "activation_key_status":"Active"}
                    else:
