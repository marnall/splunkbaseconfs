...
def sendemail(recipient, sender, subject, bodyText, argvals, attachment, filename, authToken):


    #print("email sender: - recipient: "+recipient+" | subject: "+subject+" | body: "+body+" | attachment:"+attachment, file=sys.stderr)
    print("email sender: "+sender+" | recipient: "+recipient+" | subject: "+subject+" | body: "+bodyText, file=sys.stderr)
    ...
    username, password = getCredentials(authToken, "Spreadsheets")
    ...
    try:
        ...
        part2.add_header('Content-Disposition', 'attachment; filename="'+filename+'.xlsx"')
        msg.attach(part2)
    except Exception as e:
    ...
    try:
        ...
        smtp.quit()
        #os.remove(os.environ['SPLUNK_HOME'] + "/var/run/splunk/" + attachment) #delete file after sending
        return
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--execute" :
        ...
        logger.basicConfig(format='%(asctime)s %(levelname)s %(message)s', filename=os.path.join(os.environ['SPLUNK_HOME'],'var','log','splunk','sendspreadsheets.log'), filemode='a+', level=logger.INFO)
        ...
        title          = getarg(settings, "title", search_name)
        ...
        try:
            ...
            #report = reportFileName.split("/")[-1]
            #uri = "/services/Spreadsheet?dispatchfile="+report+"/results.csv.gz"

            url = server_uri+uri
            logger.info(url)

            #http://192.168.0.33:8000/en-GB/splunkd/__raw/services/Spreadsheet?savedsearch=spreadsheet%20examples%20-%20chart%3Dpie&chart=pie&title=Eventcount%20by%20index
            ...
            resp = splunk.entity.getEntity(['configs'],'conf-server', sessionKey=authToken, namespace="-", owner="-")
            print("one", file=sys.stderr)
            print(resp, file=sys.stderr)
            resp = splunk.entity.getEntity(['configs', 'conf-server'],'sslConfig', sessionKey=authToken, namespace="-", owner="-")
            ...
            """
            with closing(r), zipfile.ZipFile(io.BytesIO(r.content)) as file:
                print({member.filename: archive.read(member) for member in archive.infolist()})
            """

            #splunk.rest.simpleRequest(uri, method="get", getargs=dict(), sessionKey=authToken )
        ...
        try:
            ...
            print("try to send")
            print("email sender: "+sender+" | recipient: "+recipient+" | subject: "+subject+" | body: "+bodyText, file=sys.stderr)
            sendemail(recipient, sender, subject, bodyText, argvals, filecontent, filename, authToken)
