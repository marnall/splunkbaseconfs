...
from issues_collector import get_issues
...
def collect_events(helper, ew):
    ...
    try:
        ...
        latest_polling_timestamp_iso = latest_polling_time + 'Z' if latest_polling_time else False
        ...
        helper.log_info(f"Source name = {source}. Starting a new job! Last pulling timestamp = {latest_polling_timestamp_iso}. "
                         f"Last full sync = {latest_full_sync_time}. Trigger full sync = {trigger_full_sync}")
        issues_num = get_issues(helper, trigger_full_sync, latest_full_sync_time, ew, before_timestamp, latest_polling_timestamp_iso)
        helper.log_info(f"Source name = {source}. Found {issues_num} Issues!")
        ...
        if isinstance(e, gaierror):
            raise e
        helper.log_error(f"Source name = {source}. Got an error when trying to fetch issues events: {str(e)}")
