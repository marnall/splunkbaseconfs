...
class CLOCKIFY_WORKSPACE(smi.Script):

    def __init__(self):
        super(CLOCKIFY_WORKSPACE, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('clockify_workspace')
        scheme.description = 'Clockify Workspace'
    ...
    def stream_events(self, inputs, ew):
        ...
        status = response_workspace.status_code
        count = 0
        for workspace in result_workspaces:
            checkpoint = checkpointer.KVStoreCheckpointer(
                "TA-clockify-add-on-for-splunk_checkpoints",
                session_key,
                "TA-clockify-add-on-for-splunk"
            )
            state = checkpoint.get(str(workspace["id"]))
            if state is None:
                event = smi.Event(
                    data=json.dumps(workspace),
                    index=input_item.get("index"),
                    sourcetype="Clockify:Workspace",
                )
                ew.write_event(event)
                count = count + 1
                checkpoint.update(str(workspace["id"]), "Indexed")

        status = "API=" + str(url_workspace) + "| response_code =" + str(status) + "| number_of_events =" + str(count)

        logsummary.log_summary(activation_key=activation_key, meta=meta, url=url_workspace, status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CLOCKIFY_WORKSPACE().run(sys.argv)
    sys.exit(exit_code)
