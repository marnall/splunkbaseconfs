...
class CLOCKIFY_CLIENT(smi.Script):

    def __init__(self):
        super(CLOCKIFY_CLIENT, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('clockify_client')
        scheme.description = 'Clockify Client'
    ...
    def stream_events(self, inputs, ew):
        ...
        if key_validator:
            logsummary.activation_log_summary(meta, key_validator, "Inactive", source, host, version)
            sys.exit(2)
        ...
        for id in workspace_id:
            ...
            url_client = f'https://api.clockify.me/api/v1/workspaces/{id}/clients?page=1&page-size=500'
            if proxy_enabled == "1":
                response_client = requests.request("GET", url_client, headers=headers, proxies=proxy_dict)
            else:
                response_client = requests.request("GET", url_client, headers=headers)

            client_data = response_client.json()
            status = response_client.status_code
            for client in client_data:
                ...
                state = checkpoint.get(str(client["id"]))

                if state is None:
                    event = smi.Event(
                        data=json.dumps(client),
                        index=input_item.get("index"),
                        sourcetype="Clockify:Client",
                    )
                    ew.write_event(event)
                    count = count + 1
                    checkpoint.update(str(client["id"]), "Indexed")

            no = 2
            while client_data != []:
                client_url = f'https://api.clockify.me/api/v1/workspaces/{id}/clients?page={no}&page-size=500'
                if proxy_enabled == "1":
                    response_client = requests.request("GET", client_url, headers=headers, proxies=proxy_dict)
                else:
                    response_client = requests.request("GET", client_url, headers=headers)
        ...
        status = "API=" + str('https://api.clockify.me/api/v1/workspaces/{id}/clients?page={no}&page-size=500') + "| response_code=" + str(status) + "| number_of_events=" + str(count)

        logsummary.log_summary(activation_key=activation_key, meta=meta, url='https://api.clockify.me/api/v1/workspaces/{id}/clients?page=1&page-size=500', status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CLOCKIFY_CLIENT().run(sys.argv)
    sys.exit(exit_code)
