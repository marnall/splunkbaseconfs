...
class CLOCKIFY_EXPENSES(smi.Script):

    def __init__(self):
        super(CLOCKIFY_EXPENSES, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('clockify_expenses')
        scheme.description = 'Clockify Expenses'
    ...
    def stream_events(self, inputs, ew):
        ...
        for id in workspace_id:
            ...
            url_expenses = f"https://api.clockify.me/api/v1/workspaces/{id}/expenses?page=1&page-size=200"

            if proxy_enabled == "1":
                response_expenses = requests.request("GET", url_expenses, headers=headers, proxies=proxy_dict)
            else:
                response_expenses = requests.request("GET", url_expenses, headers=headers)

            expense_data = response_expenses.json()
            status = response_expenses.status_code
            try:
                print(expense_data["expenses"]["expenses"])
                if expense_data["expenses"]["expenses"]:
                    for expense in expense_data["expenses"]["expenses"]:
                        checkpoint = checkpointer.KVStoreCheckpointer(
                            "TA-clockify-add-on-for-splunk_checkpoints",
                            session_key,
                            "TA-clockify-add-on-for-splunk"
                        )
                        ch = str(expense).encode()
                        checkpoint_val = hashlib.md5(ch).hexdigest()

                        state = checkpoint.get(str(checkpoint_val))
                        if state is None:
                            event = smi.Event(
                                data=json.dumps(expense),
                                index=input_item.get("index"),
                                sourcetype="Clockify:Expenses",
                            )
                            ew.write_event(event)
                            count = count + 1
                            checkpoint.update(str(checkpoint_val), "Indexed")

                no = 2
                while expense_data["expenses"]["expenses"] != []:
                    expense_url = f'https://api.clockify.me/api/v1/workspaces/{id}/expenses?page={no}&page-size=200'
                    if proxy_enabled == "1":
                        response_expense = requests.request("GET", expense_url, headers=headers, proxies=proxy_dict)
                    else:
                        response_expense = requests.request("GET", expense_url, headers=headers)

                    expense_data = response_expense.json()
                    no += 1
                    if expense_data["expenses"]["expenses"]:
                        for expense in expense_data["expenses"]["expenses"]:
                            ch = str(expense).encode()
                ...
                event_dict = {"Message": f"This Expense Feature is Available for Premium Users Only, Pleae Upgrade your Clockify Account for Workspace ID - {id}"}
                event = json.dumps(event_dict)
                source = os.path.basename(sys.argv[0])
                sourcetype = ADDON_NAME
                host = self._input_definition.metadata['server_host']
                index = '_internal'
                logevent.log_event(self._input_definition.metadata, event, source, sourcetype, host, index)

        status = "API=" + str('https://api.clockify.me/api/v1/workspaces/{id}/expenses?page={no}&page-size=200') + "| response_code=" + str(status) + "| number_of_events=" + str(count)
        logsummary.log_summary(activation_key=activation_key, meta=meta, url=url_expenses, status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CLOCKIFY_EXPENSES().run(sys.argv)
    sys.exit(exit_code)
