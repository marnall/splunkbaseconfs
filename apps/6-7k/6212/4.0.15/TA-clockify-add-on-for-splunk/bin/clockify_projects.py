...
class CLOCKIFY_PROJECTS(smi.Script):

    def __init__(self):
        super(CLOCKIFY_PROJECTS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('clockify_projects')
        scheme.description = 'Clockify Projects'
    ...
    def stream_events(self, inputs, ew):
        ...
        for id in workspace_id:
            ...
            for project in result_projects:
                ...
                state = checkpoint.get(project["id"])
                # count+=1
                if state is None:
                    event = smi.Event(
                        data=json.dumps(project),
                        index=input_item.get("index"),
                        sourcetype="Clockify:Projects",
                    )
                    ew.write_event(event)
                    count = count + 1
                    checkpoint.update(str(project["id"]), "Indexed")
        ...
        status = "API=" + str('https://api.clockify.me/api/v1/workspaces/{id}/projects?page={no}&page-size=500') + "| response_code=" + str(status) + "| number_of_events=" + str(count)

        logsummary.log_summary(activation_key=activation_key, meta=meta, url='https://api.clockify.me/api/v1/workspaces/{id}/projects?page={no}&page-size=500', status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CLOCKIFY_PROJECTS().run(sys.argv)
    sys.exit(exit_code)
