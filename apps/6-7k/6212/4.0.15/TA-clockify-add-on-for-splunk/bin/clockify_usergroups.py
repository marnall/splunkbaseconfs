...
class CLOCKIFY_USERGROUPS(smi.Script):

    def __init__(self):
        super(CLOCKIFY_USERGROUPS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('clockify_usergroups')
        scheme.description = 'Clockify UserGroups'
    ...
    def stream_events(self, inputs, ew):
        ...
        if proxy_enabled == "1":
            response_workspace = requests.request("GET", url_workspace, headers=headers, proxies=proxy_dict, source=source, host=host)
        else:
        ...
        for id in workspace_id:
            url_ug = f'https://api.clockify.me/api/v1/workspaces/{id}/user-groups?page=1&page-size=500'

            if proxy_enabled == "1":
                response_ug = requests.request("GET", url_ug, headers=headers, proxies=proxy_dict)
            else:
                response_ug = requests.request("GET", url_ug, headers=headers)

            ug_data = response_ug.json()
            status = response_ug.status_code
            for ug in ug_data:
                ...
                state = checkpoint.get(str(ug["id"]))

                if state is None:
                    event = smi.Event(
                        data=json.dumps(ug),
                        index=input_item.get("index"),
                        sourcetype="Clockify:UserGroups",
                    )
                    ew.write_event(event)
                    count = count + 1
                    checkpoint.update(str(ug["id"]), "Indexed")

            no = 2
            while ug_data != []:
                ug_url = f'https://api.clockify.me/api/v1/workspaces/{id}/user-groups?page={no}&page-size=500'
                if proxy_enabled == "1":
                    response_ug = requests.request("GET", ug_url, headers=headers, proxies=proxy_dict)
                else:
                    response_ug = requests.request("GET", ug_url, headers=headers)
        ...
        status = "API=" + str('https://api.clockify.me/api/v1/workspaces/{id}/user-groups?page={no}&page-size=500') + "| response_code=" + str(status) + "| number_of_events=" + str(count)

        logsummary.log_summary(activation_key=activation_key, meta=meta, url='https://api.clockify.me/api/v1/workspaces/{id}/user-groups?page={no}&page-size=500', status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CLOCKIFY_USERGROUPS().run(sys.argv)
    sys.exit(exit_code)
