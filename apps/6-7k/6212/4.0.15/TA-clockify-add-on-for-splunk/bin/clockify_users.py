...
class CLOCKIFY_USERS(smi.Script):

    def __init__(self):
        super(CLOCKIFY_USERS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('clockify_users')
        scheme.description = 'Clockify Users'
    ...
    def stream_events(self, inputs, ew):
        ...
        for id in workspace_id:
            ...
            url_user = f'https://api.clockify.me/api/v1/workspaces/{id}/users?page=1&page-size=500'
            ...
            for user in user_data:
                ...
                state = checkpoint.get(str(user["id"]) + str(user["activeWorkspace"]))
                if state is None:
                    event = smi.Event(
                        data=json.dumps(user),
                        index=input_item.get("index"),
                        sourcetype="Clockify:Users",
                    )
                    ew.write_event(event)
                    count = count + 1
                    checkpoint.update(str(user["id"]) + str(user["activeWorkspace"]), "Indexed")

            no = 2
            while user_data != []:
                user_url = f'https://api.clockify.me/api/v1/workspaces/{id}/users?page={no}&page-size=500'
                if proxy_enabled == "1":
        ...
        status = "API=" + str('https://api.clockify.me/api/v1/workspaces/{id}/users?page={no}&page-size=500') + "| response_code=" + str(status) + "| number_of_events=" + str(count)

        logsummary.log_summary(activation_key=activation_key, meta=meta, url='https://api.clockify.me/api/v1/workspaces/{id}/users?page={no}&page-size=500', status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CLOCKIFY_USERS().run(sys.argv)
    sys.exit(exit_code)
