...
class CLOCKIFY_TAGS(smi.Script):

    def __init__(self):
        super(CLOCKIFY_TAGS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('clockify_tags')
        scheme.description = 'Clockify Tags'
    ...
    def stream_events(self, inputs, ew):
        ...
        for id in workspace_id:
            url_tags = f"https://api.clockify.me/api/v1/workspaces/{id}/tags?page=1&page-size=500"

            if proxy_enabled == "1":
                response_tag = requests.request("GET", url_tags, headers=headers, proxies=proxy_dict)
            else:
                response_tag = requests.request("GET", url_tags, headers=headers)

            tag_data = response_tag.json()
            status = response_tag.status_code
            for tag in tag_data:
                ...
                state = checkpoint.get(str(tag["id"]))
                if state is None:
                    event = smi.Event(
                        data=json.dumps(tag),
                        index=input_item.get("index"),
                        sourcetype="Clockify:Tags",
                    )
                    ew.write_event(event)
                    count = count + 1
                    checkpoint.update(str(tag["id"]), "Indexed")

            no = 2
            while tag_data != []:
                tag_url = f'https://api.clockify.me/api/v1/workspaces/{id}/tags?page={no}&page-size=500'
                if proxy_enabled == "1":
                    response_tag = requests.request("GET", tag_url, headers=headers, proxies=proxy_dict)
                else:
                    response_tag = requests.request("GET", tag_url, headers=headers)
        ...
        status = "API=" + str('https://api.clockify.me/api/v1/workspaces/{id}/tags?page={no}&page-size=500') + "| response_code=" + str(status) + "| number_of_events=" + str(count)
        logsummary.log_summary(activation_key=activation_key, meta=meta, url='https://api.clockify.me/api/v1/workspaces/{id}/tags?page={no}&page-size=500', status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CLOCKIFY_TAGS().run(sys.argv)
    sys.exit(exit_code)
