...
class CLOCKIFY_INVOICE(smi.Script):

    def __init__(self):
        super(CLOCKIFY_INVOICE, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('clockify_invoice')
        scheme.description = 'Clockify Invoice'
    ...
    def stream_events(self, inputs, ew):
        ...
        for id in workspace_id:
            ...
            url_invoice = f"https://api.clockify.me/api/v1/workspaces/{id}/invoices?page=1&page-size=500"

            if proxy_enabled == "1":
                response_invoice = requests.request("GET", url_invoice, headers=headers, proxies=proxy_dict)
            else:
                response_invoice = requests.request("GET", url_invoice, headers=headers)

            invoice_data = response_invoice.json()
            status = response_invoice.status_code
            try:
                print(invoice_data["invoices"])
                ...
                while invoice_data["invoices"] != []:
                    invoice_url = f'https://api.clockify.me/api/v1/workspaces/{id}/invoices?page={no}&page-size=500'
                    if proxy_enabled == "1":
                        response_invoice = requests.request("GET", invoice_url, headers=headers, proxies=proxy_dict)
                    else:
                        response_invoice = requests.request("GET", invoice_url, headers=headers)
                ...
                event_dict = {"Message": f"This Invoice Feature is Available for Premium Users Only, Pleae Upgrade your Clockify Account for Workspace ID - {id}"}
        ...
        status = "API=" + str('https://api.clockify.me/api/v1/workspaces/{id}/invoices?page={no}&page-size=500') + "| response_code=" + str(status) + "| number_of_events=" + str(count)

        logsummary.log_summary(activation_key=activation_key, meta=meta, url=url_invoice, status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CLOCKIFY_INVOICE().run(sys.argv)
    sys.exit(exit_code)
