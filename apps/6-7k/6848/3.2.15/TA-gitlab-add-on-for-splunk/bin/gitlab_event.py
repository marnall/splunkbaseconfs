...
class GITLAB_EVENT(smi.Script):

    def __init__(self):
        super(GITLAB_EVENT, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('gitlab_event')
    ...
    def stream_events(self, inputs, ew):
        ...
        if (start_date and end_date):
            x = project_id
            if x:
                url = f"https://{username}/api/v4/projects/{x}/events?per_page=100&after={start_date}&before={end_date}"
            else:
                url = f"https://{username}/api/v4/events?per_page=100&after={start_date}&before={end_date}"
...
if __name__ == '__main__':
    exit_code = GITLAB_EVENT().run(sys.argv)
    sys.exit(exit_code)
