...
class GITLAB_MERGE_REQUEST(smi.Script):

    def __init__(self):
        super(GITLAB_MERGE_REQUEST, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('gitlab_merge-request')
    ...
    def stream_events(self, inputs, ew):
        ...
        if (start_date and end_date):
            x = project_id
            if x:
                url = f"https://{username}/api/v4/projects/{x}/merge_requests?per_page=100&created_after={start_date}&created_before={end_date}"
            else:
                url = f"https://{username}/api/v4/merge_requests?per_page=100&created_after={start_date}&created_before={end_date}"
...
if __name__ == '__main__':
    exit_code = GITLAB_MERGE_REQUEST().run(sys.argv)
    sys.exit(exit_code)
