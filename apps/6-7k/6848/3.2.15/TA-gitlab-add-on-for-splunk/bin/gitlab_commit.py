...
class GITLAB_COMMIT(smi.Script):
    def __init__(self):
        super(GITLAB_COMMIT, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('gitlab_commit')
    ...
    def stream_events(self, inputs, ew):
        ...
        if (start_date and end_date):
            x = project_id
            url = f"https://{username}/api/v4/projects/{x}/repository/commits?per_page=100&since={start_date}&until={end_date}"
...
if __name__ == '__main__':
    exit_code = GITLAB_COMMIT().run(sys.argv)
    sys.exit(exit_code)
