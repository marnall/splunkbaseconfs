...
TA-apache_access_eventgen is a custom LOG generator by [Splunk EventGen](http://splunk.github.io/eventgen/).
This TA generates continuous event logs of general apache access combined.
It can be useful if we want a bunch of continuous sample logs.
The most of ip,url,useragent and referer is based on [Splunk Buttercup Games's event data](http://docs.splunk.com/images/Tutorial/tutorialdata.zip).
sourcetype = access_combined and using the splunk system timezone.
...
4. Create `eventgen_apache_access` event index on Splunk WebUI.
5. Restart Splunk
6. Search by `index="eventgen_apache_access"`
7. + Extract New Fields if you want additionals.
...
This TA generates it as apache access combined format like below.

```
127.0.0.1 - - [2020-10-01 13:55:36.555] "GET /apache_pb.gif HTTP/1.0" 200 2326 "http://www.referer.com/start.html" "Mozilla"
```

The most of ip,url,useragent and referer is based on [Splunk Buttercup Games's event data](http://docs.splunk.com/images/Tutorial/tutorialdata.zip).
This TA continue the Buttercup games events while adding 404 Not Found events.

TA-apache_access_eventgen will generates 500K events per 1 week.
You can adjust the log volume by modifying samples/apache_access_demo.csv or eventgen.conf.
...
[apache_access_demo.csv]
index = eventgen_apache_access
source = apache_access_demo.csv
...
# src_ip token
token.1.token = (127\.0\.0\.1)
token.1.replacementType = file
token.1.replacement = $SPLUNK_HOME/etc/apps/TA-apache_access_eventgen/samples/buttercupgame_clientip.sample

# method uri token
token.2.token = (GET /apache_pb.gif)
token.2.replacementType = file
token.2.replacement = $SPLUNK_HOME/etc/apps/TA-apache_access_eventgen/samples/buttercupgame_uris.sample

# status code token
token.3.token = (200)\s{1}
token.3.replacementType = file
token.3.replacement = $SPLUNK_HOME/etc/apps/TA-apache_access_eventgen/samples/status_code.sample

# bytes token
token.4.token = (2326)\s{1}
token.4.replacementType = random
token.4.replacement = integer[100:10000]

# referer token
token.5.token = (http://www.referer.com/start.html)
token.5.replacementType = file
token.5.replacement = $SPLUNK_HOME/etc/apps/TA-apache_access_eventgen/samples/buttercupgame_referer.sample

# user_agent token
token.6.token = (Mozilla)
token.6.replacementType = file
token.6.replacement = $SPLUNK_HOME/etc/apps/TA-apache_access_eventgen/samples/buttercupgame_useragent.sample

# 404 method uri token
token.7.token = (GET /apache_pb404.gif)
token.7.replacementType = file
token.7.replacement = $SPLUNK_HOME/etc/apps/TA-apache_access_eventgen/samples/404_uris.sample

# 404 referer token
token.8.token = (http://www.referer.com/notfound.html)
token.8.replacementType = file
token.8.replacement = $SPLUNK_HOME/etc/apps/TA-apache_access_eventgen/samples/404_referer_urls.sample
```
...
Special Thanks to [Splunk Buttercup Games's event data](http://docs.splunk.com/images/Tutorial/tutorialdata.zip), this tutorial data is very useful for splunk beginners.
