...
# Constants from resilient-lib/resilient_common.py for build_incident_url
CP4S_PREFIX = "cases-rest."
CP4S_RESOURCE_PREFIX = "app/respond"
INCIDENT_FRAGMENT = "#incidents"
CASE_FRAGMENT = "#cases"
WARN_BUILD_INCIDENT_ORG_ID = "Building an incident link without the ORG ID may produce a link that directs the user to a different incident resulting in unexpected results. It is recommended to provide the ORG ID." # pragma: no cover
...
class ResilientClient:
    ...
    def connect(self, config, verify, password=None, api_key_secret=None):
        ...
        if api_key_secret:
            self.logger.info("Connecting to QRadar SOAR server with API key ID and secret: " + str(args))
            self.resilient_client.set_api_key(self.configuration["user"], api_key_secret)
        elif password:
            self.logger.info("Connecting to QRadar SOAR server with username and password: " + str(args))
    ...
    def get_artifacts(configuration, limit):
        ...
        1. New pipe-delimited format: artifact1="type|value|description"
        2. Legacy separate fields: artifact1type, artifact1value, artifact1description
        """
        artifacts = []
        processed_artifacts = set()
        
        # First, check for new pipe-delimited format (artifact1, artifact2, etc.)
        for key, value in configuration.items():
            if key.startswith('artifact') and key[8:].isdigit():
                # Extract artifact number (e.g., "artifact1" -> "1")
                artifact_nbr = key[8:]
                
                # Skip if already processed or exceeds limit
                if artifact_nbr in processed_artifacts or int(artifact_nbr) > limit:
                    continue
                ...
                # Check if this is pipe-delimited format
                if '|' in value:
                    # New format: type|value|description
                    parts = value.split('|', 2)  # Split into max 3 parts
                    if len(parts) >= 2:
                        artifact_type = parts[0].strip()
                        artifact_value = parts[1].strip()
                        artifact_description = parts[2].strip() if len(parts) > 2 else ""
                        
                        if artifact_value:  # Only add if value is not empty
                            artifacts.append({
                                "value": artifact_value,
                                "type": artifact_type,
                                "description": artifact_description
                            })
                            processed_artifacts.add(artifact_nbr)
        
        # Second, check for legacy separate fields format (backward compatibility)
        for key, value in configuration.items():
            if key.startswith('artifact') and key.endswith('value'):
                ...
                # Skip if already processed by new format or exceeds limit
                if artifact_nbr in processed_artifacts or int(artifact_nbr) > limit:
                    continue
                ...
                # Check if value contains pipe character - could be pipe-delimited format in legacy field
                if '|' in value:
                    # Try to parse as pipe-delimited format
                    parts = value.split('|', 2)
                    if len(parts) >= 2:
                        artifact_type = parts[0].strip()
                        artifact_value = parts[1].strip()
                        ...
                        # Only add if value is not empty (skip if middle part is empty)
                        if artifact_value:
                            artifacts.append({
                                "value": artifact_value,
                                "type": artifact_type,
                ...
                # Legacy format: separate type, value, description fields
    ...
    def process_dt_value(self, value):
        ...
        # try to convert 3 supported input types, skip the remaining if one succeeds
        try:
            # parse a date string
        ...
        try:
            ...
            if sys.version_info.major < 3:
                ...
                try:
                    ...
                    # offset was not provided. set to utc
    ...
    def create_incident(self, mapping_config, artifacts, incident_file, update=False, existing_incident=None):
        ...
        if update:
            def update_fn(get_inc_dict):
                def merge_dict(dict1, dict2):
                    # Merge potentially nested dictionaries into one dict without removing dict keys.
                    # Use collections Abstract Base Class
                    for k, v in dict2.items():
                        if isinstance(dict1, collectionsABCMapping):
                            if isinstance(v, collectionsABCMapping):
                                dict1[k] = merge_dict(dict1.get(k, {}), v)
                            else:
                                dict1[k] = dict2[k]
                        else:
                            dict1 = {k: dict2[k]}
                    return dict1
                # Update the incident json from GET incident with incident_dict from mapping fields.
                # update_fn only has one parameter from the GET incidents requests call, so use the
                # recursive merge_dict function to merge nested dicts. 
                updated_inc = merge_dict(get_inc_dict, incident_dict)
                return updated_inc
            # GET incident, update it in update_fn and PUT it back.
            incident = self.resilient_client.get_put("/incidents/{}".format(existing_incident['id']), update_fn)
            self.logger.info("UPDATED CASE TO POST: " + repr(incident))
        ...
        if sys.version_info.major > 2:
            self.logger.info("CASE TO POST: " + repr(incident_dict))
        else:
            # python2 REPL can not correctly interpret unicode inside a data container
            self.logger.info(
                # pylint: disable=no-member
                "CASE TO POST: {" + "".join("u'%s': u'%s', " % (k, v) for k, v in incident_dict.iteritems()) + "}") 
    ...
    def build_case_url(self, config, case_id):
        """
        Build the case URL
        """
        ...
        case_url = self.build_incident_url(self.build_resilient_url(host, port),
                                           case_id,
                                           org_id)
        return case_url

    def build_incident_url(self, url, incidentId, orgId=None):
        """
        ...
        if not isinstance(url, str):
            self.logger.warning("Called 'build_incident_url' with a '{0}'  but was expecting a 'str' URL value. Returning original value.".format(type(url)))
            return url
        ...
        if orgId and isinstance(orgId, (str, int)):
            link += "?orgId={0}".format(url_encode(orgId))
        else:
            self.logger.warning(WARN_BUILD_INCIDENT_ORG_ID)

        return link

    def build_resilient_url(self, host, port):
        """
        ...
        if not isinstance(host, str):
            self.logger.warning("Called 'build_resilient_url' with a '{0}'  but was expecting a 'str' host value. Returning original value.".format(type(host)))
            return host
