...
# Change for Splunk Add-on!
# These constants are pulled in from resilient/constants.py
...
class BasicHTTPException(Exception):
    ...
    def raise_if_error(response):
        ...
        if response.status_code == 401:
            try:
                raise BasicHTTPException(response, err_reason=ERROR_MSG_CONNECTION_UNAUTHORIZED, err_text=ERROR_MSG_CONNECTION_INVALID_CREDS)
            except BasicHTTPException:
                traceback.print_exc()
                sys.exit(ERROR_CODE_CONNECTION_UNAUTHORIZED)
...
class BaseClient(object):
    ...
    def __init__(self, org_name=None, base_url=None, proxies=None, verify=None, certauth=None, custom_headers=None):
        ...
        base_headers = {
            ...
            HEADER_MODULE_VER_KEY: HEADER_MODULE_VER_VALUE
        }
        ...
        # Change for Splunk Add-on!
        # Replace commented out lines with code from helpers.py
        #if helpers.is_env_proxies_set():
        #    self.proxies = None
        if os.getenv(ENV_HTTPS_PROXY) or os.getenv(ENV_HTTP_PROXY):
            self.proxies = None
    ...
    #
    # Change for Splunk Add-on!
    # This method is not noramlly in BaseClient (comes from co3.py).
    # Added to allow us to access the ConstAPI from Splunk without extra dependencies
    # This method id is used to get the SOAR version and perform alternate logic for 
