...
import json
import os
from constants import APP_NAME, REALM, CONFIG_FILE, KV_WBATCH_SIZE, KV_RBATCH_SIZE
...
def get_credentials(session_key, conf_stanza, user_item, pw_item):

    # username from app_settings
    user = get_config_item(session_key, conf_stanza, user_item)
...
def get_crosslink_credentials(session_key):

    try:
        user, sec = get_credentials(session_key, 'pmi', 'client_id',
                                  'client_secret')

    except CredentialNotExistException:
        raise Exception('missing CrossLink API tokens')
...
class KVStore(object):
    ...
    def clean(self, query):
        ...
        r = self.collection.data.query(query=query)
        d = self.collection.data.delete(query=query)
