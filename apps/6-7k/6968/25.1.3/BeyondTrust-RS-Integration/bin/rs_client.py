...
########## Retrieves an authentication token from the RS REST API to be used for subsequent calls
...
########## Retrieves a batch of events from the RS REST API beginning with events ingested at the specified start date
def get_session_report(helper, auth_token: str, hostname: str, end_date: int):
    ...
    try:
        helper.log_debug(f'Preparing to attempt event ingestion from {hostname} starting at {end_date}')
