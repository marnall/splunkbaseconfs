...
class BOOKSTACK_CHAPTER(smi.Script):

    def __init__(self):
        super(BOOKSTACK_CHAPTER, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bookstack_chapter')
        scheme.description = 'bookstack:chapter'
    ...
    def stream_events(self, inputs, ew):
        ...
        off = 0
        count = 0
        url1 = f"https://bookstack.{domain}/api/chapters"
        ...
        while Tot >= off:
            url = f"https://bookstack.{domain}/api/chapters?offset={off}&count=500"
...
if __name__ == '__main__':
    exit_code = BOOKSTACK_CHAPTER().run(sys.argv)
    sys.exit(exit_code)
