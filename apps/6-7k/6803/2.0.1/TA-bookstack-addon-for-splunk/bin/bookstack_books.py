...
class BOOKSTACK_BOOKS(smi.Script):

    def __init__(self):
        super(BOOKSTACK_BOOKS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bookstack_books')
        scheme.description = 'bookstack:books'
    ...
    def stream_events(self, inputs, ew):
        ...
        off = 0
        count = 0
        url1 = f"https://bookstack.{domain}/api/books"
        ...
        while Tot >= off:
            url = f"https://bookstack.{domain}/api/books?offset={off}&count=500"
...
if __name__ == '__main__':
    exit_code = BOOKSTACK_BOOKS().run(sys.argv)
    sys.exit(exit_code)
