...
class BOOKSTACK_PAGES(smi.Script):

    def __init__(self):
        super(BOOKSTACK_PAGES, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bookstack_pages')
        scheme.description = 'bookstack:pages'
    ...
    def stream_events(self, inputs, ew):
        ...
        off = 0
        count = 0
        url1 = f"https://bookstack.{domain}/api/pages"
        ...
        Tot = r_json["total"]
        while Tot >= off:
            url = f"https://bookstack.{domain}/api/pages?offset={off}&count=500"
...
if __name__ == '__main__':
    exit_code = BOOKSTACK_PAGES().run(sys.argv)
    sys.exit(exit_code)
