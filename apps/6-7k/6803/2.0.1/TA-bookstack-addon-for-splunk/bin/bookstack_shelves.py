...
class BOOKSTACK_SHELVES(smi.Script):

    def __init__(self):
        super(BOOKSTACK_SHELVES, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bookstack_shelves')
        scheme.description = 'bookstack:shelves'
    ...
    def stream_events(self, inputs, ew):
        ...
        off = 0
        count = 0
        url1 = f"https://bookstack.{domain}/api/shelves"
        ...
        Tot = r_json["total"]
        while Tot >= off:
            url = f"https://bookstack.{domain}/api/shelves?offset={off}&count=500"
...
if __name__ == '__main__':
    exit_code = BOOKSTACK_SHELVES().run(sys.argv)
    sys.exit(exit_code)
