...
class BOOKSTACK_ATTACHMENT(smi.Script):

    def __init__(self):
        super(BOOKSTACK_ATTACHMENT, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('bookstack_attachment')
        scheme.description = 'bookstack:attachment'
    ...
    def stream_events(self, inputs, ew):
        ...
        url1 = f"https://bookstack.{domain}/api/attachments"
        ...
        Tot = r_json["total"]
        while Tot >= off:
            url = f"https://bookstack.{domain}/api/attachments?offset={off}&count=500"
            ...
            for book in r_json["data"]:
                ...
                if state is None:
                    event = smi.Event(
                        ...
                        sourcetype="bookstack_attachment",
                    )
...
if __name__ == '__main__':
    exit_code = BOOKSTACK_ATTACHMENT().run(sys.argv)
    sys.exit(exit_code)
