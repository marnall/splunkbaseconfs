...
def validate_input(helper, definition):
    ...
    pass
def print_current_eventid(helper, ew):
    event = helper.new_event(source=helper.get_input_type(), index=helper.get_output_index(), sourcetype=helper.get_sourcetype(), data=str(helper.get_check_point(str("previousId"))), time= datetime.datetime.now())

    ew.write_event(event)

def collect_events(helper, ew):
    #print_current_eventid(helper, ew)
    #event = helper.new_event(source=helper.get_input_type(), index=helper.get_output_index(), sourcetype=helper.get_sourcetype(), data="json.dumps(item)", time= datetime.datetime.now())
