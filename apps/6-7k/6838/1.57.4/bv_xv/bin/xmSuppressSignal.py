...
def usage(message):
    ...
    usageStatement = "xmSuppressSignal TYPE (default | model | actor | taxonomy) SIGNAL_TYPE signalType [MODEL_NAME modelName] [ACTOR_ID <actorId>] [TAXONOMY_ID [actorId~]action~process~field] [PARTIAL_TAXONOMY_ID taxonomyIdRegEx] [PROPERTY_NAME <propertyName> PROPERTY_VALUE <propertyValue>] [(START_DATE <startDateEpoch> END_DATE <endDateEpoch>) | (DAYS_OF_WEEK <0|1|...|6> HOURS_OF_DAY <0|1|...|23>)]\nSTART_DATE and END_DATE define the date range for signal suppression OR DAYS_OF_WEEK and HOURS_OF_DAY define days and hours the signal type should be suppressed";
...
if __name__ == '__main__':
    ...
    lastArg = ''
    logging.info("---------------------------------------------------------------------------------------")
    logging.info("xmSuppressSignal starting, args " + repr(sys.argv) + "]");
    ...
    try:
        ...
        for arg in sys.argv[1:]:
            if arg.lower() == "model_name":
            ...
            elif arg.lower() == "start_date":
                lastArg="startDate"
            elif arg.lower() == "end_date":
                lastArg="endDate"
            elif arg.lower() == "days_of_week":
                lastArg="daysOfWeek"
            elif arg.lower() == "hours_of_day":
                lastArg="hoursOfDay"
                ...
                endDate = arg
                lastArg=''
            elif lastArg == "daysOfWeek":
                daysOfWeek = arg
                lastArg=''
            elif lastArg == "hoursOfDay":
                hoursOfDay = arg
                lastArg=''
            else:
        ...
        if len(signalType) > 0:
            ...
            usage ("Missing argument SIGNAL_TYPE <signalType> ("+ type + ")");
        ...
        if type.lower() == "default" and len(signalType) == 0 and len(partialTaxonomyId) == 0:
            usage ("Missing argument SIGNAL_TYPE <signalType> and/or PARTIAL_TAXONOMY_ID <taxonomyRegEx>")
        ...
        if len(daysOfWeek) > 0:
            argList.append("-d"); 
            argList.append(daysOfWeek)

        if len(hoursOfDay) > 0:
            argList.append("-h"); 
            argList.append(hoursOfDay)

        logging.info("Calling xmSuppressSignal with arguments: [" + repr(argList) + "]")
        saUtils.runProcess(sys.argv[0], "xmSuppressSignal", argList, False)
        logging.info("---------------------------------------------------------------------------------------")
