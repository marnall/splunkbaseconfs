...
def usage(message):
    ...
    usageStatement = "Version 1: xmExportRules MODEL_NAME <modelName> [RULE_PACKAGE <rulePackageName>] [FILE <fileName>] [VERSION 1]\nVersion 2: xmExportRules [RULE_PACKAGE <rulePackageName>] APPLICATION (XVAW | XR | ABA) VERSION 2\nThis command exports the specified rule package to the specified file. If rulePackageName is omitted then all rules defined in the model will be exported. If FILE is omitted rules will be exported to $SPLUNK_HOME/etc/apps/bv_xv/scm/exports/<modelName>_rule-export.csv"
