#!/usr/bin/env python3
#
# File: command_elasticquerystats.py - Version 1.3.3
...
class elasticQueryStats(GeneratingCommand):
    ...
    def generate(self):
        ...
        try:
            ...
            if not (set(CONFIG["ELASTIC_COMMAND_SAVED_ROLES"] + CONFIG["ADMIN_ROLES"]) & set(current_user_roles)):
                ...
                raise Exception('Unauthorized - You are not allowed to run the "elasticquerystats" command')
            ...
            if query is None:
                ...
                raise ArgumentError("Saved search not found")

            if query.mode not in [SearchMode.DSL_STATS, SearchMode.ESQL]:
                logger.info(f'action="query_check",status="failure",user="{user}",reason="invalid_mode",mode="{query.mode}"')
                raise ArgumentError("Query is not in valid mode")
            ...
            # Get results
            if query.mode == SearchMode.DSL_STATS:
...
dispatch(elasticQueryStats, sys.argv, sys.stdin, sys.stdout, __name__)
