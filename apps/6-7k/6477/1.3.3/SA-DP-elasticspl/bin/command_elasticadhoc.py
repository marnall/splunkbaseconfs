#!/usr/bin/env python3
#
# File: command_elasticadhoc.py - Version 1.3.3
...
@Configuration()
class elasticAdhoc(GeneratingCommand):
    ...
    def generate(self):
        ...
        try:
            ...
            if not (set(CONFIG["ELASTIC_COMMAND_ADHOC_ROLES"] + CONFIG["ADMIN_ROLES"]) & set(current_user_roles)):
                ...
                raise Exception('Unauthorized - You are not allowed to run the "elasticadhoc" command')

            # Get the instance
            elastic_instance_service: ElasticInstanceService = ElasticInstanceService(uuid=self.uuid, client=client, session_key=session_key, user=user)
            ...
            for result in self.run(logger, query_runner_info):
                yield result
    ...
    def run(self, logger, query_runner_info):
        """
        Runs the query on unix systems using multiprocessing and queues
        """
        try:
            event_queue = Queue(maxsize=50)
            message_queue = Queue(maxsize=50)

            if self.mode == SearchMode.LUCENE:
                producers = [
                    Process(
    ...
    def format_result(self, res):
        ...
        if self.mode in [SearchMode.LUCENE, SearchMode.DSL]:
    ...
    def _validate_args(self):
        ...
        if self.mode is None:
            self.mode = SearchMode.DSL
        if self.mode not in [SearchMode.LUCENE, SearchMode.DSL, SearchMode.ESQL]:
            raise Exception(f"Mode must be { SearchMode.LUCENE }, { SearchMode.DSL } or { SearchMode.ESQL }")
        if self.scroll_size is None:
            self.scroll_size = 1000


if __name__ == "__main__":
    dispatch(elasticAdhoc, sys.argv, sys.stdin, sys.stdout, __name__)
