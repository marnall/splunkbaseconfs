...
def status_code_errors(response, api_call, log_label, helper):
    ...
    # Bytes responses come from download endpoints — cannot extract status code
    if isinstance(response, bytes):
        helper.log_debug(f'{log_label} Response from {api_call} was a binary download ({len(response)} bytes)')
        return
    ...
    if status_code == 429:
        rl_limit = headers.get('X-Ratelimit-Limit', 'unknown')
        rl_remaining = headers.get('X-Ratelimit-Remaining', 'unknown')
        helper.log_error(f'{log_label} Rate limit exhausted for {api_call} after retries — HTTP {status_code}, Limit: {rl_limit}, Remaining: {rl_remaining}, TraceID: {effective_traceid}')
