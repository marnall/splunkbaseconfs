...
logger = setup_logging(
    "eclecticiq_ic_app_create_sighting", log_level=logging.DEBUG)
...
class Send(PersistentServerConnectionApplication):  # type: ignore
    ...
    def format_data(sighting, meta):
        ...
        data["alert_source"] = meta["alert_source"] if meta.get("alert_source") else "splunk_workflow"
        data["src"] = meta["src"]
        data["dest"] = meta["dest"]
        ...
        data["sighting"] = "1"
        data["source_name_eiq"] = meta.get("source_name_eiq", "")
    ...
    def store_sighting(data, session_key):
        ...
        service = client.connect(
            token=session_key, owner="nobody", app="TA-eclecticiq")
    ...
    def handle(self, in_string):  # pylint: disable=R0915,R0201
        ...
        try:
            ...
            for item in sp_list.list():
                ...
                if account_name == conf_username.split("``")[0] and "splunk_cred_sep" not in item.content.get("clear_password"):
            ...
            if not url:
                ...
                Send.create_response(401, CREDS_NOT_FOUND)

            if not api_key:
                logger.error(
                    "No api_key found for user {}.".format(account_name))
                Send.create_response(401, CREDS_NOT_FOUND)
            ...
            logger.info("got index and host!")
            meta_data["observable_id"] = (
                payload.get("observable_id") if payload.get(
                    "observable_id") else ""
            )
            logger.info("preparing other meta fields!")
            ...
            logger.info("finished until event_hash!")
            meta_data["event_hash"] = (
                hashlib.sha512(
                    str(payload["_raw"]).encode("utf-8")).hexdigest()
                if payload.get("_raw")
                else payload.get("event_hash")
            )
            logger.info("Got event_hash!")
            meta_data["feed_id_eiq"] = payload["feed_id_eiq"]
            meta_data["meta_entity_url_eiq"] = payload["meta_entity_url_eiq"]
            meta_data["source_name_eiq"] = payload.get("source_name_eiq", "")
            meta_data["alert_source"] = payload.get("alert_source", "")
            logger.info("Got all meta fields!")
            ...
            if not Send.validate_type(payload[SIGHTING_TYPE], payload[SIGHTING_VALUE]):
                return Send.create_response(
                    501, "Sighting value {} does not match with selected sighting type {}.".format(payload[SIGHTING_VALUE], payload[SIGHTING_TYPE])
                )
            logger.info("value validated!")
            ...
            sighting[DATA_STR][META][TAGS] = list(
                map(str.strip, payload[SIGHTING_TAGS].split(",")))
            ...
            logger.info("request successful, got response!")
            content = json.loads(response.content)
            logger.info("sucessfully got response content!")
            hostname = url.split("/")
            hostname = "/".join(hostname[:-2])
            message = SIGHTING_CREATED.format(
            ...
            try:
                ...
                if not is_stored:
                    ...
                    logger.info(
                        "Sighting is stored. Key:{}.".format(is_stored))
            except Exception:
