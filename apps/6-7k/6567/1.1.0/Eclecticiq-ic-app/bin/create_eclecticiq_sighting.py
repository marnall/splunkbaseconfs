...
logger = setup_logging("eclecticiq_ic_app_create_sighting_alert_action", log_level=logging.DEBUG)
...
def format_data(configuration, result):
    ...
    record["sighting_value"] = (
        result.get("value_eiq") if result.get("value_eiq") else ""
    )
    ...
    record["index"] = result.get("event_index") if result.get("event_index") else ""
    record["observable_id"] = (
        result.get("observable_id") if result.get("observable_id") else ""
    )
    record["host"] = result.get("event_host") if result.get("event_host") else ""
    record["source"] = result.get("event_source") if result.get("event_source") else ""
    record["sourcetype"] = result.get("event_sourcetype") if result.get("event_sourcetype") else ""
    record["time"] = result.get("event_time") if result.get("event_time") else ""
    record["field"] = (
    ...
    record["alert_source"] = "Splunk Alert"
    record["event_hash"] = (
    ...
    record["source_name_eiq"] = (
        result.get("source_name_eiq") if result.get("source_name_eiq") else ""
    )
    logger.info(record)
    return record
...
def main(payload):
    ...
    body_to_send["acc_url"] = url_val
    body_to_send["creds"] = api_key
    body_to_send["proxy"] = json.dumps(proxy)
