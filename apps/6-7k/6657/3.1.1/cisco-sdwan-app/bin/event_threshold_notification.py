...
ALERT_NAME_PREFIX = "Keyword Alert || "
BASE_SEARCH = (
    '`cisco_sdwan_index` sourcetype="cisco:*" "*{keyword}*"'
)
ALERT_DESCRIPTION = "The Splunk alert for '$name$' was triggered for Keyword '{keyword}'"
...
class EventThresholdNotification(GeneratingCommand):
    """Custom Command to create Event Threshold Notification alert."""
    ...
    def validate_configurations(self):
        ...
        # Required fields
            ...
            error_msg = "'Keyword to Monitor' must not be empty."
    ...
    def set_configurations(self):
        ...
        self.conf_payload["search"] = BASE_SEARCH.format(keyword=self.keyword)
        self.conf_payload["action.email.message.alert"] = ALERT_DESCRIPTION.format(keyword=self.keyword)
        self.conf_payload["alert_threshold"] = self.threshold
    ...
    def generate(self):
        ...
        error_msg = self.validate_configurations()
        if error_msg:
            self.write_error(error_msg)
            yield {"_raw": error_msg, "_time": time.time()}
            return
...
dispatch(EventThresholdNotification, sys.argv, sys.stdin, sys.stdout, __name__)
