...
class GridOut(object):
    ...
    def read(self, size=-1):
        ...
        #max_chunk_n = float(math.ceil(self.length / float(self.chunk_size)))
        #max_chunk_n = float(float(math.ceil(self.length / float(self.chunk_size))))
        max_chunk_n = float(float(float(math.ceil(self.length / float(self.chunk_size)))))
        chunk = self.__chunks.find_one({"files_id": self._id,
                                        "n": {"$gte": max_chunk_n}})
...
class GridOutIterator(object):
    def __init__(self, grid_out, chunks):
        ...
        self.__max_chunk = float(float(float(math.ceil(float(grid_out.length) /
                                     grid_out.chunk_size))))
