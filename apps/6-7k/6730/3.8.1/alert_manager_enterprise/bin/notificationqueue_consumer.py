...
#
# File: notificationqueue_consumer.py - Version 3.8.1
...
class NotificationQueueConsumer(AmeModularInput):
    def get_scheme(self) -> Scheme:
        scheme = Scheme("AME Notification Queue Housekeeping")
        scheme.description = "This task processes the 'ame_notification_trigger_queue'"
    ...
    def handle_stream_events(
        ...
        try:
            notification_service = self.service_factory.notification_service
            notification_service.process_queue()
        except Exception as e:
            self.logger.exception(
                {"action": "handle_stream_events", "status": "failed", "exception": e}
            )


if __name__ == "__main__":
    sys.exit(NotificationQueueConsumer().run(sys.argv))
