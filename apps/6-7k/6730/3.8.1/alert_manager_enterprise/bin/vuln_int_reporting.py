...
#
# File: vuln_int_reporting.py - Version 3.8.1
...
class VulnIntReporting(AmeModularInput):
    def get_scheme(
        self,
    ) -> Scheme:
        scheme = Scheme("AME Reporting Scheduler")
        scheme.description = "This task creates and renders vulnerability reports."
    ...
    def handle_stream_events(
        ...
        try:
            self.vuln_int_reporting_service = self.service_factory.vuln_int_reporting_service
            self.vuln_int_reporting_service.send_reports()
        except Exception as exc:
            self.logger.exception(
                {
                    "action": "vuln_int_reporting",
                    "status": "failed",
                    "exception": exc,
                }
            )
...
if __name__ == "__main__":
    sys.exit(VulnIntReporting().run(sys.argv))
