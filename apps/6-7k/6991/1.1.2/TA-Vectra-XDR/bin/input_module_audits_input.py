...
def collect_events(helper, ew):
    ...
    try:
        ...
        import sys
        import os
        sys.path.insert(0, os.path.abspath(os.path.join(__file__, '..', 'modinputs', 'audits')))
        sys.path.insert(0, os.path.abspath(os.path.join(__file__, '..', 'common')))

        import collector

        input_name = helper.get_arg("name")
        logger = log.get_logger("audits_input_{}".format(input_name))

        asc = collector.EntityAuditsCollector(logger)
        asc.run(helper, ew)
