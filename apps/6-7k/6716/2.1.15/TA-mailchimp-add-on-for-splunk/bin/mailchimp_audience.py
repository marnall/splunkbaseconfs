...
class MAILCHIMP_AUDIENCE(smi.Script):

    def __init__(self):
        super(MAILCHIMP_AUDIENCE, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('mailchimp_audience')
        scheme.description = 'Audience'
    ...
    def stream_events(self, inputs, ew):
        ...
        url = f"https://{match_str}.api.mailchimp.com/3.0/lists/{audience_id}/members?offset={off}&count={num}"
...
if __name__ == '__main__':
    exit_code = MAILCHIMP_AUDIENCE().run(sys.argv)
    sys.exit(exit_code)
