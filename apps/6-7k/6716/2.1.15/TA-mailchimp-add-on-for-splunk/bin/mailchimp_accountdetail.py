...
class MAILCHIMP_ACCOUNTDETAIL(smi.Script):

    def __init__(self):
        super(MAILCHIMP_ACCOUNTDETAIL, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('mailchimp_accountdetail')
        scheme.description = 'Accountdetail'
    ...
    def stream_events(self, inputs, ew):
        ...
        url = f"https://{match_str}.api.mailchimp.com/3.0/"
        ...
        if proxy_enabled == "1":
            response = requests.get(url, params=params, auth=HTTPBasicAuth(name, api_key), proxies=proxy_dict)
        else:
            response = requests.get(url, params=params, auth=HTTPBasicAuth(name, api_key))
        ...
        state = checkpoint.get(str(account_id))
        ...
        if state is None:
            ...
            event = smi.Event(
                ...
                sourcetype='mailchimp:accountdetail',
            )

            ew.write_event(event)
            count = count + 1

            checkpoint.update(str(account_id), "Indexed")

        
        # checkpoint.delete(str(account_id))
...
if __name__ == '__main__':
    exit_code = MAILCHIMP_ACCOUNTDETAIL().run(sys.argv)
    sys.exit(exit_code)
