...
def get_events(helper, session: str, pra_hostname: str, xmlns: str):
    
    # array of string events per session
    return __get_events_from_session(helper, session, pra_hostname, xmlns)

########## -----------------------------------------------

def __get_events_from_session(helper, access_session: ET.Element, pra_hostname: str, xmlns):
    # returned events are dictionaries with 2 keys: 'end_time', and 'event_data'
    lsid = access_session.attrib['lsid']
    helper.log_info(f"Getting events for session lsid: '{lsid}'")

    session_data = {}
    session_data['lsid'] = lsid
    session_data['device_host'] = pra_hostname
    session_data['user_data'] = __get_user_data(helper, access_session.find('xmlns:rep_list', xmlns), xmlns)
    session_data['endpoint_data'] = __get_endpoint_data(helper, access_session.find('.//xmlns:customer', xmlns), xmlns)
    session_data['jumpoint_data'] = __get_jumpoint_data(helper, access_session, xmlns)

    events = {}
    events['end_time'] = access_session.find('.//xmlns:end_time', xmlns).attrib['timestamp']
    events['events'] = __parse_events_from_session(helper, session_data, access_session.find('xmlns:session_details', xmlns), xmlns)
...
def __siem_event_to_str(helper, siem_event):
    ...
    event += '|BeyondTrust|Privileged Remote Access|15.x+|'
