...
class Input(smi.Script):
    ...
    def get_scheme(self):
        scheme = smi.Scheme("bitwarden_groups")
        scheme.description = "bitwarden_groups input"
            ...
            try:
                ...
                for group in Request(
                    ...
                    group["members"] = list(
                        Request(
                            management_connection=api_manager,
                            endpoint=f"/public/groups/{group['id']}/member-ids",
                            http_request="GET",
                        ).execute()
                    )
                    event_writer.write_event(
                        smi.Event(
                            ...
                            source=f"{base_url}/public/groups",
                            host=host,
                        )
                    )
                    event_count += 1
