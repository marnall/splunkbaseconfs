...
class Input(smi.Script):
    ...
    def get_scheme(self):
        scheme = smi.Scheme("bitwarden_users")
        scheme.description = "bitwarden_users input"
            ...
            try:
                ...
                for user in Request(
                    ...
                    user["groups"] = list(
                        Request(
                            management_connection=api_manager,
                            endpoint=f"/public/members/{user['id']}/group-ids",
                            http_request="GET",
                        ).execute()
                    )
                    event_writer.write_event(
                        smi.Event(
                            ...
                            source=f"{base_url}/public/members",
                            host=host,
                        )
                    )
                    event_count += 1
