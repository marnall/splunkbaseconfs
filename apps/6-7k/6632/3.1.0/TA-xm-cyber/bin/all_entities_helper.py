"""Helper module for collecting and processing all entities data from XM Cyber."""
...
logger = setup_logging(f"{ta_prefix}_all_entities")
...
def validate_input(definition):
    ...
    """
    validate_oauth_authentication(definition, "All Entities", logger)


def stream_events(inputs, event_writer):
    """
    Stream events from all entities endpoint.
    ...
    if auth_type == "basic":
        ...
        try:
            logger.info(f"input_name={normalized_input_name} Initializing all entities data collection.")
            entities_collector = XMCyberCollector(inputs, event_writer)
            entities_collector.collect_events(entities_collector.xm_cyber_client.get_all_entities)
            logger.info(f"input_name={normalized_input_name} Exiting all entities data collection.")
        except Exception as e:
            logger.error(
                f"input_name={normalized_input_name} Error in all entities data collection. "
