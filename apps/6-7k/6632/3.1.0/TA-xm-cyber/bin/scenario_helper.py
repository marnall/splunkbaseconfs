"""Helper module for processing XM Cyber scenario data in the Splunk app."""
...
logger = setup_logging(f"{ta_prefix}_scenario")
...
def validate_input(definition):
    ...
    """
    validate_oauth_authentication(definition, "Scenario", logger)


def stream_events(inputs, event_writer):
    """
    Streams events from the scenario endpoint.
    ...
    if auth_type == "basic":
        ...
        try:
            logger.info(f"input_name={normalized_input_name} Initializing scenario data collection.")
            scenarios_collector = XMCyberCollector(inputs, event_writer)
            scenarios_collector.collect_events(scenarios_collector.xm_cyber_client.get_scenarios)
            logger.info(f"input_name={normalized_input_name} Exiting scenario data collection.")
        except Exception as e:
            logger.error(
                f"input_name={normalized_input_name} Error in scenario data collection. "
