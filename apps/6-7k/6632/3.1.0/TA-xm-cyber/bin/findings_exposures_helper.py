"""Helper module for collecting and processing findings and exposures data from XM Cyber."""
...
logger = setup_logging(f"{ta_prefix}_findings_exposures")
...
def validate_input(definition):
    ...
    """
    validate_oauth_authentication(definition, "Findings and Exposures", logger)


def stream_events(inputs, event_writer):
    """
    Streams events from the findings and exposures endpoint.
    ...
    if auth_type == "basic":
        service_name = "Findings and Exposures"
        ...
        try:
            logger.info(f"input_name={normalized_input_name} Initializing finding exposures data collection.")
            findings_exposures_collector = XMCyberCollector(inputs, event_writer)
            findings_exposures_collector.collect_events(
                findings_exposures_collector.xm_cyber_client.get_findings_exposures
            )
            logger.info(f"input_name={normalized_input_name} Exiting finding exposures data collection.")
        except Exception as e:
            logger.error(
                f"input_name={normalized_input_name} Error in finding exposures data collection. "
