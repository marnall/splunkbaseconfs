...
# NOTE: This file must be Python 3 compatible.
...
def exec_anaconda():
    ...
    libraries included in the conda environment.
    ...
    sa_scipy = PSC_PATH_PREFIX + "linux_x86_64"

    sa_path = os.path.join(get_apps_path(), sa_scipy)
    if not os.path.isdir(sa_path):
        raise Exception('Failed to find PSC (%s)' % sa_scipy)
    ...
    try:
        ...
        time.sleep(0.1)
        raise RuntimeError(
            'Error encountered while loading BLOOCK, see search.log. ' + 'PYTHON_PATH: ' + python_path
        )


def fix_sys_path():
    # After shelling into BLOOCK's Python interpreter, we no longer have access
