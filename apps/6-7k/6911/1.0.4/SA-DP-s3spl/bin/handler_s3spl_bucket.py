...
#
# File: handler_s3spl_bucket.py - Version 1.0.4
...
class BucketHandler(PersistentServerConnectionApplication, HandlerAbstract):
    ...
    def handle_get(self, data: dict):
        """
        Fetching Buckets from the configuration
        """
        try:
            bucket_service: BucketService = BucketService(uuid=self.uuid, client=client, session_key=self.session_key, privileged_key=self.privileged_key, user=self.user)
            buckets: list[dict] = bucket_service.get_buckets()

            return self.response(buckets, http.client.OK)
    ...
    def handle_post(self, data: dict):
        """
        Adding / updating / removing / ping Bucket
        """
    ...
    def handle_post_add(self, payload: dict):
        ...
        try:
            bucket_service: BucketService = BucketService(uuid=self.uuid, client=client, session_key=self.session_key, privileged_key=self.privileged_key, user=self.user)
            return self.response(bucket_service.add_bucket(payload["bucket"]), http.client.CREATED)
        except Exception as e:
            self.logger.error(traceback.format_exc())
            return self.__handle_error(str(e))
    ...
    def handle_post_update(self, payload: dict):
        ...
        try:
            bucket_service: BucketService = BucketService(uuid=self.uuid, client=client, session_key=self.session_key, privileged_key=self.privileged_key, user=self.user)
            return self.response(bucket_service.update_bucket(payload["bucket"]), http.client.OK)
        except Exception as e:
            self.logger.error(traceback.format_exc())
            return self.__handle_error(str(e))
    ...
    def handle_post_remove(self, payload: dict):
        ...
        try:
            bucket_service: BucketService = BucketService(uuid=self.uuid, client=client, session_key=self.session_key, privileged_key=self.privileged_key, user=self.user)
            return self.response(bucket_service.remove_bucket(payload["bucket"]), http.client.OK)
        except Exception as e:
            self.logger.error(traceback.format_exc())
            return self.__handle_error(str(e))
