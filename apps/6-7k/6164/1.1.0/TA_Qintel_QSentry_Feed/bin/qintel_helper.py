...
def _parse_http_error(response):
    sc = response.code
    resp_body = response.read()
    if resp_body:
        resp_body = resp_body.decode()
    
    msg = f'API connection error: {sc}'
    if resp_body:
        msg += f': {resp_body}'
...
def _search(**kwargs):
    ...
    raise_on_ratelimit = kwargs.get('raise_on_ratelimit', False)
    if raise_on_ratelimit in [True, 'true', '1', 1]:
        raise_on_ratelimit = True
    ...
    while request_attempts < max_retries:
        ...
        if response.code == 429 and raise_on_ratelimit is True:
            err_msg = _parse_http_error(response)
...
def qsentry_feed(query_type='anon', feed_date=datetime.today(), **kwargs):
    ...
    :param str query_type: Feed type [anon|mal_host]
    :param dict kwargs: extra client args [remote|token|params]
    ...
    if not kwargs.get('endpoint'):
        kwargs['endpoint'] = '/feed'

    kwargs['remote'] = _set_remote('qsentry', query_type, **kwargs)
    kwargs['token'] = kwargs.get('token', os.getenv('QSENTRY_TOKEN'))
    if kwargs.get('raise_on_ratelimit') is None:
        kwargs['raise_on_ratelimit'] = True

    if isinstance(feed_date, datetime):
        feed_date = str(feed_date.date())

    params = kwargs.get('params', {})
    params.update({'feed_type': query_type, 'date': feed_date})
    kwargs['params'] = params
