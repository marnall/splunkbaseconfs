...
def collect_events(helper, ew):
    ...
    if 'laggar' in sqs_queue:
        if region_regex == None:
            helper.log_error('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' +  stanza + ' AWS Region: The AWS Region could not be identified and is required for GovCloud. TA will now exit.')
    ...
    helper.log_info('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' +  stanza + ' AWS Region: ' + str(region))
    
    loglevel = helper.get_log_level()
    helper.log_info('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' +  stanza + 'logging level set to: ' + str(loglevel))
 
    # get proxy setting configuration and configure settings accordingly
    proxy = helper.get_proxy()

    if proxy:
        helper.log_info('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' + stanza + ': Proxy is Set')
        helper.log_debug('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' + stanza + ': Proxy Type: ' + str(proxy['proxy_type']) + ' Proxy URL: ' + str(proxy['proxy_url']) + ' Proxy Port: ' + str(proxy['proxy_port']))
    
        if proxy['proxy_username']:
            #proxy enabled with authentication - craft appropriate URL
            helper.log_info('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' + stanza + ': Proxy is configured with authentication.')
            ...
            helper.log_info('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' + stanza + ': Proxy is configured without authentication')
            proxy_string = (str(proxy['proxy_type']) + '://' + str(proxy['proxy_url']) + ':' + str(proxy['proxy_port'])) 
            proxy_settings = {'http':proxy_string, 'https':proxy_string}
    else:
        helper.log_info('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' + stanza + ': Proxy is Not Set') 
        proxy_settings = None
    ...
    def handle_file (**kwargs):
        ...
        helper.log_info('CROWDSTRIKE FDR TA - ' + fdr_title + '   ' +  stanza + ' preparing to send data to Splunk.')
        ...
        try:
            ...
            helper.log_info(fdr_title + ': Deleted SQS message')
        ...
        try:
            ...
            helper.log_debug(fdr_title + ' : File removal error - ' + str(e))
        ...
        try:
            ...
            helper.log_debug(fdr_title + ' : Folder removal error - ' + str(e))
    ...
    def download_message_files(**kwargs):
        ...
        while num_files > 0:
            ...
            helper.log_info('S3 output: ' + str(s3_output))
    ...
    def consume_data_replicator():
        ...
        while True:
            ...
            for msg in sqs_response: 
                ...
                counter += 1
                #tracking log entry
                helper.log_info(fdr_title + ': TA Message Counter = ' + str(msg_cnt) + ' Processed Count = ' + str(processed_cnt) + ' Reset Count = ' +str(reset_cnt) )
