...
class AUDITLOG_INPUT(smi.Script):

    def __init__(self):
        super(AUDITLOG_INPUT, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('auditlog_input')
        scheme.description = 'AuditLog Input'
    ...
    def stream_events(self, inputs, ew):
        ...
        logger = setup_logging("ta_googlescc_auditlog", input_name=input_name)
        ...
        audit_logs_subscription_name = input_items[1]['audit_logs_subscription_id']
        audit_logs_subscription_split = audit_logs_subscription_name.split("/")
        project_id = audit_logs_subscription_split[1]
        audit_logs_subscription_id = audit_logs_subscription_split[3]
        ...
        # Validating project id and auditlog subscription id
        scc_auditlog_sub_client = gsa.init_google_pubsub_client(
                project_id=project_ID,
                subscription_id=audit_logs_subscription_id,
        ...
        try:
            subscription = "projects/{0}/subscriptions/{1}".format(
                    project_ID, audit_logs_subscription_id
            )
            body = {"max_messages": 1, "return_immediately": True}
            validate = scc_auditlog_sub_client.service.projects().subscriptions().pull(
                    subscription=subscription,
                    body=body
            )
            data = validate.execute()
        except Exception:
            msg = "Please enter valid Project ID or Subscription ID."
        ...
        try:
            ...
            event_count = gsa.get_auditlog_data(logger, maximum_fetching, index_name, ew, org, scc_auditlog_sub_client)
...
if __name__ == '__main__':
    exit_code = AUDITLOG_INPUT().run(sys.argv)
    sys.exit(exit_code)