...
@Configuration()
class SnipeAssetCommand(StreamingCommand):
    search = Option()
    infield = Option()
    outfield = Option(default='assets')
    ...
    def stream(self, events):
        ...
        if not self.search and not self.infield:
            raise AttributeError('snipeasset: infield or search must be set')

        if self.search and self.infield:
            raise AttributeError('snipeasset: infield and search cannot both be set')
        ...
        for event in events:
            ...
            if query:
                if query in cache:
                    ...
                    results = list(snipeit.assetSearch(query, limit=self.limit))
                    cache[query] = results
...
dispatch(SnipeAssetCommand, sys.argv, sys.stdin, sys.stdout, __name__)
