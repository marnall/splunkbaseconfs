...
@Configuration()
class SnipeAssetSearchCommand(GeneratingCommand):
    search = Option()
    ...
    def generate(self):
        ...
        if query in cache:
            ...
            results = []
            for result in snipeit.assetSearch(query, limit=self.limit):
                # Unlikely but we don't want to clobber anything
                if '_time' in result:
                    result['result_time'] = result['_time']

                result['source'] = server
                result['sourcetype'] = 'snipeit_asset'
...
dispatch(SnipeAssetSearchCommand, sys.argv, sys.stdin, sys.stdout, __name__)
