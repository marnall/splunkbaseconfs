...
# Version : 1.0.0
# Date: Date: 01 Jun 2023

 === Splunk App to fetch NetWitness MetaData ===

 This Splunk app will connect to a NetWitness Concentrator via REST API.
 ...
 **NOTE: SSL access to the REST interface currently requires the use of a hack**
 ...
 To troubleshoot connections to your NetWitness device use:
   curl -u <user>:<password> "http://<serverip>:50105/sdk?msg=summary&id1=0&id2=0&size=2000&force-content-type=text/plain"
 ...
 Once the app is installed make sure to setup the app from the UI by providing the credentials to access Netwitness REST API

   # query = select * where service=53
 Nw Query to execute
 Currently no checks are performed for correct query syntax
 
 configure the interval (in seconds) at which the nwsdk.py script should run, in default/inputs.conf
 # [script://./bin/nwsdk.py]
 # interval = 60
