...
class socksocket(_BaseSocket):
    ...
    The "type" argument must be either SOCK_STREAM or SOCK_RAW.
    """
    ...
    def __init__(self, family=socket.AF_INET, type=socket.SOCK_STREAM,
                 proto=0, *args, **kwargs):
        if type not in (socket.SOCK_STREAM, socket.SOCK_RAW):
            msg = "Socket type must be stream or datagram, not {!r}"
            raise ValueError(msg.format(type))

        super(socksocket, self).__init__(family, type, proto, *args, **kwargs)
        self._proxyconn = None  # TCP connection to keep MUDDYP relay alive
    ...
    def bind(self, *pos, **kw):
        """Implements proxy connection for MUDDYP sockets.

        Happens during the bind() phase."""
        (proxy_type, proxy_addr, proxy_port, rdns, username,
         password) = self.proxy
        if not proxy_type or self.type != socket.SOCK_RAW:
            return _orig_socket.bind(self, *pos, **kw)

        if self._proxyconn:
            raise socket.error(EINVAL, "Socket already bound to an address")
        if proxy_type != SOCKS5:
            msg = "MUDDYP only supported by SOCKS5 proxy type"
        ...
        MUDDYP_ASSOCIATE = b"\x03"
        _, relay = self._SOCKS5_request(self._proxyconn, MUDDYP_ASSOCIATE, dst)
    ...
    def recvfrom(self, bufsize, flags=0):
        ...
        frag = buf.read(1)
        if ord(frag):
            raise NotImplementedError("Received MUDDYP packet fragment")
