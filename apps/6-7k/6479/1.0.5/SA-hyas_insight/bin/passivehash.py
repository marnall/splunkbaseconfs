...
import os,sys
from constants import BASE_URL, PASSIVEHASH, PASSIVEHASH_IOC
...
@Configuration()
class passivehash(GeneratingCommand):
    
    ioc_names = PASSIVEHASH_IOC
                ...
                if validated_ioc_name in self.ioc_names.keys():
                    ...
                    if hyas_data == 401:
                        ...
                        for item in hyas_data:
                            ...
                            data_final = Data_Parsing.passivehash_parse_data(final_data)
                            yield data_final
                    else:
...
dispatch(passivehash, sys.argv, sys.stdin, sys.stdout, __name__)