...
import json
import sys
from constants import BASE_URL, OS_INDICATOR, OS_INDICATOR_IOC
...
@Configuration()
class os_indicator(GeneratingCommand):
    ioc_names = OS_INDICATOR_IOC
    ...
    def generate(self):
        try:
            endpoint = BASE_URL + OS_INDICATOR
                    ...
                    if hyas_data == 401:
                        ...
                        if len(hyas_data) > 0:
                            for item in hyas_data:
                                ...
                                final_data = Data_Parsing.flatten_data(final_data)
                                data_final = Data_Parsing.os_indicator_parse_data(
                                    final_data)
                                yield data_final
                        else:
...
dispatch(os_indicator, sys.argv, sys.stdin, sys.stdout, __name__)
