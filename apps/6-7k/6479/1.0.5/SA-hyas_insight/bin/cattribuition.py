...
import os,sys
from constants import BASE_URL, C2ATTRIBUTION, C2ATTRIBUTION_IOC
...
@Configuration()
class cattribution(GeneratingCommand):
    
    ioc_names = C2ATTRIBUTION_IOC
    ...
    def generate(self):
        try:
            endpoint = BASE_URL + C2ATTRIBUTION
                ...
                if validated_ioc_name in self.ioc_names.keys():
                    ...
                    if hyas_data == 401:
                        ...
                        for item in hyas_data:
                            ...
                            data_final = Data_Parsing.cattribution_parse_data(final_data)
                            yield data_final
                    else:
...
dispatch(cattribution, sys.argv, sys.stdin, sys.stdout, __name__)