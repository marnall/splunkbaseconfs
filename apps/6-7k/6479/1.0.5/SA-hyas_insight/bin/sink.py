...
from constants import SINKHOLE, BASE_URL, SINKHOLE_IOC
...
@Configuration()
class sink(GeneratingCommand):
    
    ioc_names = SINKHOLE_IOC
                ...
                if validated_ioc_name in self.ioc_names.keys():
                    ...
                    if hyas_data == 401:
                        ...
                        for item in hyas_data:
                            ...
                            data_final = Data_Parsing.sinkhole_parse_data(final_data)
                            yield data_final
                    else:
...
dispatch(sink, sys.argv, sys.stdin, sys.stdout, __name__)