    ...
    try:
        ...
        if len(sys.argv) >1:
            for arg in sys.argv[1:]:
                ...
                # SERVICE
                elif arg.lower().startswith('servicename='):
                    eqsign = arg.find('=')
                    serviceName = arg[eqsign+1:len(arg)]
            ...
            raise Exception('xmSetActor-F-001: Usage: xmSetActor id=<string> model=<string> newid=<string> newmodel=<newModel> alternateids=<string> type=<string> subtype=<string> criticality=<string> [createdate=<string>] [updatedate=<string>] [imageid=<string>] [businessunit=<string>] [categorylist=<string>] [city=<string>] [country=<string>] [postalcode=<string>] [gmtOffset=<string>] [latitude=<string>] [longitude=<string>] [ownerlist=<string>] [region=<string>] [state=<string>] [tag=<string>] [watchlist=<string>] [enddate=<string>] [email=<string>] [name=<string>] [managedby=<string>] [phone1=<string>] [phone2=<string>] [startdate=<string>] [title=<string>] [cpuspeedghz=<string>] [dnslist=<string>] [hostname=<string>] [iplist=<string>] [mac=<string>] [memorysizemb=<string>] [numcpus=<string>] [ostype=<string>] [osversion=<string>] [pcidomain=<string>] [description=<string] [serviceName=<string>')

        logger.info ("Received arguments, id: [" + id + "], newId: [" + newId + "], model: [" + modelName + "], newModel: [" + newModel + "], alternateIds: [" + alternateIds + "], type: [" + type + "], subType: [" + subtype + "], createDate: [" + createDate + "], updateDate: [" + updateDate + "], criticality: [" + criticality + "], imageId: [" + imageId + "], serviceName: [" + serviceName + "]");

        if id == '' and newId == '':
            raise Exception("xmSetActor-F-001: parameter 'id' or 'newid' not found")
        if type == '':
            raise Exception("xmSetActor-F-001: parameter 'type' not found")
        if subtype == '':
            raise Exception("xmSetActor-F-001: parameter 'subtype' not found")
        if createDate == '':
            raise Exception("xmSetActor-F-001: parameter 'createdate' not found")
        if updateDate == '':
            raise Exception("xmSetActor-F-001: parameter 'updatedate' not found")
        if criticality == '':
            raise Exception("xmSetActor-F-001: parameter 'criticality' not found")
        # modelName is optional.

        if subtype == 'HUMAN':
            if name == '':
                raise Exception("xmSetActor-F-001: parameter 'name' not found")

        if subtype == 'MACHINE':
            if hostname == '':
                raise Exception("xmSetActor-F-001: parameter 'hostname' not found")

        if subtype == 'SERVICE':
            if serviceName == '':
                raise Exception("xmSetActor-F-001: parameter 'serviceName' not found")
        ...
        if id == '' and newId != '':
            ...
            logger.info ("1) CreateActor:  id == '' and newId != ''");
        elif id != '' and id != newId:
            checkForDuplicate = "true"
            logger.info ("2) UpdateActor: id != '' and id != newId");
        elif modelName != newModel:
            checkForDuplicate = "true"
            logger.info ("3) UpdateActor: modelName != newModelName");
        else:  # no need to check to see if the Id already exists, updating existing actor and not changing Id or modelName.
            logger.info ("No need to see if duplicate exists, updating existing actor's attributes")

        # Look for the actor in the database by either primaryId or alternateIds
        fromDB = collection.find_one ({"type" : "ACTOR", "modelName" : newModel, "$or" : [ { "primaryId" : newId}, { "alternateIds" : { "$in" : [newId] }} ]})
       ...
       # TODO: Need to validate alternateIds are not already in use by a DIFFERENT user... possibly do this below.
        ...
        if checkForDuplicate == "true":
            ...
            logger.info ("Looking up actor with id: [" + newId + "] and model: [" + newModel + "]")

            # Actor with the specified primary or alternateId not found in the database.
            if fromDB != None:
                logger.info("xmGetActor - Actor: [" + newId + "] model: [" + newModel + "] already exists!")
                alreadyExists = "true"
                print ("ALREADY_EXISTS")
            else:
               logger.info ("ACTOR [" + newId + "] DOES NOT EXIST!")

        # If creating new actor or updating existing (w/out changing Id)
        if alreadyExists != "true":

            logger.info ("alreadyExists != true") 

            #=======================================================================
            # Determine if another actor with specified alternateIds already exists.
            #=======================================================================
            for curAltId in alternateIdList:
                actorWithAltId = collection.find_one ({"type" : "ACTOR", "modelName" : newModel, "$or" : [ { "primaryId" : curAltId}, { "alternateIds" : { "$in" : [curAltId] }} ]})
                if actorWithAltId != None:
                    if fromDB != None:
                        if fromDB ['_id'] != actorWithAltId ['_id']:
                            logger.info ("Found actor that already uses alternateId: [" + curAltId + "], actor: [" + actorWithAltId['primaryId'] + "], modelName: [" + newModel + "]")
                            print ("Failure")
                            exit(1)
                    else:
                        logger.info ("Found actor that already uses alternateId: [" + curAltId + "], actor: [" + actorWithAltId ['primaryId'] + "], modelName: [" + curActorWithId['modelName'] + "]")
                        print ("Failure")
                        exit(1)

            # If the actor's identifiers (id and/or modelName) changed but existing actor not found, need to lookup the actor to update.
            if fromDB == None:
                fromDB = collection.find_one ({"type" : "ACTOR", "modelName" : modelName, "$or" : [ { "primaryId" : id}, { "alternateIds" : { "$in" : [id] }} ]})
                if fromDB == None:
                    logger.error ("ERROR - Expecting to perform an update but failed to find exsiting actor with id: [" + id + "], modelName: [" + modelName + "]");
                else:
                    logger.info ("Found actor to update, primaryId: [" + id + "], modelName: [" + modelName + "], _id: [" + repr (fromDB['_id']) + "]");

            properties = {}

            # Core properties which can be present on all actor types.
            ...
            if fromDB != None:
                ...
                logger.info ("Updating actor with primaryId: [" + id + "], alternateIds: [" + alternateIds + "], modelName: [" + modelName + "], newId: [" + newId + "], newModel: [" + newModel + ", type: [" + type + "], subType: [" + subtype + "], criticality: [" + criticality + "], createDate: [" + createDate + "], updateDate: [" + updateDate + "], imageId: [" + imageId + "], properties: [" + repr(properties) + "]");
                ...
                logger.info ("Creating new actor with primaryId: [" + newId + "], alternateIds: [" + alternateIds + "], modelName: [" + modelName + "], type: [" + type + "], subType: [" + subtype + "], criticality: [" + criticality + "], createDate: [" + createDate + "], updateDate: [" + updateDate + "], imageId: [" + imageId + "], properties: [" + repr(properties) + "]");

                collection.insert_one ({ 'primaryId' : newId, 'alternateIds' : alternateIdList, 'modelName' : newModel, 'type' : type, 'subType' : subtype, 'criticality' : int(criticality), 'createDate' : datetime.datetime.utcnow(), 'updateDate' : datetime.datetime.utcnow(), 'imageId' : imageId, 'properties' : properties});
