...
UTC = tz.gettz('UTC')
SECRET_REALM = "fireeye_iam_realm"

GLOBAL_API_KEY_NAME = "fireeye_api_key"
GLOBAL_SERVICE_REGION_NAME = "etp_service_region"

def setup_logging():
    logger = logging.getLogger('splunk.ta_fireeye_etp')    
    SPLUNK_HOME = os.environ['SPLUNK_HOME']
    
    LOGGING_DEFAULT_CONFIG_FILE = os.path.join(SPLUNK_HOME, 'etc', 'log.cfg')
    LOGGING_LOCAL_CONFIG_FILE = os.path.join(SPLUNK_HOME, 'etc', 'log-local.cfg')
    LOGGING_STANZA_NAME = 'python'
    LOGGING_FILE_NAME = "ta_fireeye_etp.log"
...
def get_etp_instance():
  ...
  if etp_instance is None:
    etp_instance = "etp.us.fireeye.com"
...
def get_secret_global(client, var_name):
  ...
  setup_util = Setup_Util(client._metadata.searchinfo.splunkd_uri, 
                          client._metadata.searchinfo.session_key, 
                          logger)
...
def get_requests_proxies(client):
  setup_util = Setup_Util(client._metadata.searchinfo.splunkd_uri, 
                        client._metadata.searchinfo.session_key, 
                        logger)
