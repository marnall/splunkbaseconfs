...
class AdvancedThreat(ETPClient):
  ...
  def __init__(self, **kwargs):
    ...
    self.etp_message_id = kwargs.get('etp_message_id')
    self.email_status = kwargs.get('email_status')
    
    # convert from_last_modified_on to a suitable format for ETP
    self.from_last_modified_on = kwargs.get('from_last_modified_on')
  ...
  def _create_attributes(self):
    attributes = {}
    if self.etp_message_id:
      attributes['etp_message_id'] = self.etp_message_id

    if self.legacy_id:
      attributes['legacy_id'] = self.legacy_id

    if self.email_status:
      attributes['email_status'] = [self.email_status]
  ...
  def alert_summary_api(self):
    ...
    if self.from_last_modified_on:
      body['fromLastModifiedOn'] = self.from_last_modified_on
    ...
    if self.from_last_modified_on and meta.get('fromLastModifiedOn').get('end'):
      self.from_last_modified_on = meta.get('fromLastModifiedOn').get('end')
