...
def collect_events(helper, ew):
    ...
    PRESET_FILE = splunk_app_url + "/etc/apps/TA-XProtect/xprotect_auditlogs.conf"
    ...
    if base_url.startswith('https'): 
        url = base_url + '/public/api/v1/'+ tenant + '/auditlogs?limit='+limit+'&startIndex='+presentIndex+'&gte='+gte+'&lte='+lte
    ...
    while done < total:
        if "pagination" in list(r.json().keys()):
            ...
            if "items" in list(r.json().keys()):
                    ...
                    if base_url.startswith('https'): 
                        url = base_url + '/public/api/v1/'+ tenant + '/auditlogs?limit='+limit+'&startIndex='+index+'&gte='+gte+'&lte='+lte
                        helper.log_debug(">>>> URL = " + url)
                        r = s.get(url = url, headers={'x-api-token': token}, verify=CA_CERT,cert=cert)
            else:
