...
def collect_events(helper, ew):
    ...
    PRESET_FILE = splunk_app_url + "/etc/apps/TA-XProtect/xprotect_events.conf"
    ...
    gte = str(_lte - int(60 * 20))
    f = open(PRESET_FILE, "w")
    PRESETS = {"gte": gte, "lte": lte, "startIndex": presentIndex }
    ...
    if base_url.startswith('https'): 
        url = base_url + '/public/api/v1/'+ tenant + '/alerts/detailedAlerts?limit='+limit+'&startIndex='+presentIndex+'&severity='+severity+'&gte='+gte+'&lte='+lte
    ...
    while done < total:
        if "pagination" in list(r.json().keys()):
            ...
            if "items" in list(r.json().keys()):
                    ...
                    if base_url.startswith('https'): 
                        url = base_url + '/public/api/v1/'+ tenant + '/alerts/detailedAlerts?limit='+limit+'&startIndex='+index+'&severity='+severity+'&gte='+gte+'&lte='+lte
                        helper.log_debug(">>>> URL = " + url)
                        r = s.get(url = url, headers={'x-api-token': token}, verify=CA_CERT,cert=cert)
            else:
