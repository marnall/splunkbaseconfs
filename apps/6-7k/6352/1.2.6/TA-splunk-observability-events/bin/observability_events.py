...
import modalert_observability_events_helper

class AlertActionWorkerobservability_events(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerobservability_events, self).__init__(ta_name, alert_name)
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_observability_events_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerobservability_events("TA-splunk-observability-events", "observability_events").run(sys.argv)
    sys.exit(exitcode)
