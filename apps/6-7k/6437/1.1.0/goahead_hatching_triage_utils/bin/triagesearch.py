...
COMMANDNAME = 'triagesearch'

logger = galib.setup_logging(APPNAME)

@Configuration()
class triagesearch(GeneratingCommand):
  ...
  query = Option(doc=''' search query with the spesific logic and search operators ''',require=True)   
  ...
  def generate(self):
    try:
      #logger.debug('triagesearch: %s', vars(self))
      try:
      ...
      # run query 
      try:
        from triagelib import search
        jsonResponse = search(baseUrl,apikey,self.query,self.limit,self.offset,logger)
      except Exception as e:
...
dispatch(triagesearch, sys.argv, sys.stdin, sys.stdout, __name__)
