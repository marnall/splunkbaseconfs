...
def setup_logger(level):
    ...
    file_handler = logging.handlers.RotatingFileHandler(
        os.path.join(
            ...
            "log",
            "splunk",
            "CybleThreatIntel_Configuration_Alerts.log",
        ),
        maxBytes=25_000_000,
        backupCount=5,
    )
...
def try_post(url: str, headers: Dict[str, str], payload: Dict[str, Any],
             proxies: Optional[Dict[str, str]], timeout: float, cert_path: Optional[str] = None) -> Tuple[bool, Optional[int], Optional[str], Optional[str]]:
    """
...
class validation(PersistentServerConnectionApplication):
    def __init__(self, command_line, command_arg):
        """
        Initializes the validation handler for the Cyble Alerts Configuration.
    ...
    def send_request(self, api_key: str, proxy_config: Optional[Dict[str, Any]], cert_path: Optional[str] = None):
        """
        ...
        :rtype: Dict[str, Any]
        """
        url = "https://bifrost.cyble.ai/engine/api/v1/y/tpi/splunk/alerts"
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            ok, status, body, error = try_post(url, headers, payload, proxies, timeout_secs,cert_path)
    ...
    def handle(self, in_string):
        ...
        try:
            ...
            hide_sensitive = str(payload.get("hide_sensitive", "")).lower() in ("true", "1", "on")
            ...
            if certificate_content:
                ...
                logger.info("No certificate provided — proceeding without custom certificate.")
