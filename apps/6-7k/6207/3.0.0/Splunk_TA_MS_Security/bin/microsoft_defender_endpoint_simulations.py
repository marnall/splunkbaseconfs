...
LOG_FILE_NAME = splunk_ta_ms_security_constants.SIMULATIONS_LOG_FILE_NAME
APP_NAME = import_declare_test.ta_name
SETTINGS_CONF_NAME = splunk_ta_ms_security_constants.SETTINGS_CONF_NAME


class MICROSOFT_DEFENDER_ENDPOINT_SIMULATIONS(smi.Script):
    def __init__(self):
        super().__init__()
    ...
    def get_scheme(self) -> Optional[smi.Scheme]:
        ...
        """
        scheme = smi.Scheme("microsoft_defender_endpoint_simulations")
        scheme.description = "Microsoft Defender Simulations"
    ...
    def stream_events(
        ...
        for input_name, input_item in inputs.inputs.items():
            ...
            check_point_key = f"simulation_lastUpdateTime_{input_name}"
            ...
            input_item["tenant_id"] = global_account.get("tenant_id")
            ...
            if access_token:
                ...
                today = datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
                filter_criteria = ApiSpecificContent.get_simulations_filter(
                    urls.api, query_date, today
                )
                simulation_url = urls.simulations + filter_criteria
                logger.debug(f"Simulation filter={simulation_url}")

                # Use chunked processing for simulations with additional report fetching
                try:
                    simulation_ingested = process_simulations_chunked(
                        logger=logger,
                        ...
                        simulation_report_url_template=urls.simulation_report,
                    ...
                    logger.info(
                        f"Simulations ingested using chunked processing - simulation_count={simulation_ingested}"
                    )
                except Exception as e:
                    logger.error(f"Chunked processing failed: {e}")
                    raise e

                # Note: Checkpoint handling is now done incrementally within process_simulations_chunked
            else:
                e = RuntimeError(
...
if __name__ == "__main__":
    exit_code = MICROSOFT_DEFENDER_ENDPOINT_SIMULATIONS().run(sys.argv)
    sys.exit(exit_code)
