    ...
    def stream_events(
        ...
        for input_name, input_item in inputs.inputs.items():
            ...
            if access_token:
                ...
                machines_url = urls.machines + f"?$filter=lastSeen+gt+{query_date}"
                logger.debug(f"Machines filter={machines_url}")

                try:
                    machines_ingested = process_events_chunked(
                        logger=logger,
                        ...
                        date_field_names=["lastSeen"],
                        request_timeout=int(input_item.get("request_timeout", 60)),
                    )

                    logger.info(
                        f"Total Machines ingested using chunked processing - machines_count={machines_ingested}"
                    )
                except Exception as e:
