...
class AccountValidator:
    ...
    def validate(self) -> None:
        ...
        try:
            logger.debug(
                f"Requesting connection using url '{urls.authorization}', client_id {client_id} and client secret starting with {client_secret[0:1]}"
            )
    ...
    def _validate_payload(payload: dict) -> None:
        ...
        if lookback is not None and int(lookback) >= int(interval):
            raise ValueError("Configuration error: lookback must be less than interval")
        required(
    ...
    def _raise_on_error_code(self, resp: dict) -> None:
        ...
        if error_code == 90038:
            logger.error(  # nosemgrep
                f"Account '{global_account}' is unauthorized for National Cloud"
            )
            raise RestError(
                400,
                f"Account '{global_account}' is unauthorized for National Cloud",
        ...
        if error_code == 500011:
            logger.error(  # nosemgrep
                f"Account '{global_account}' is unauthorized for GCC environment"
            )
            raise RestError(
                400,
                f"Account '{global_account}' is unauthorized for GCC environment",
