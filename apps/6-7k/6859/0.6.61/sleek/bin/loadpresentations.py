...
class LoadPresentations(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        data = []
        for (dirpath, dirnames, filenames) in walk(os.path.join(os.environ['SPLUNK_HOME'],'etc','apps','sleek','appserver','static','presentations')):
            for file in filenames:
                if not file.startswith(".DS_") :
                    f = open(os.path.join(os.environ['SPLUNK_HOME'],'etc','apps','sleek','appserver','static','presentations',file),"r+")
                    content = json.load(f)
                    data.append(content)
