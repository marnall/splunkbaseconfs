...
class CustomConfigMigrationHandler(ConfigMigrationHandler):
    ...
    def handleCreate(self, conf_info):
        ...
        try:
            ...
            for input, input_meta in all_inputs.items():
                if input.startswith("sophos_event_input://") and input_meta.get("index") == self.payload.get("index"):
                    raise RestError("500", f"Event input with index '{self.payload.get('index')}' already exists.")
    ...
    def handleRemove(self, conf_info):
        ...
        ck = checkpointer.KVStoreCheckpointer(
            "{}_checkpointer".format(app_name), session_key, app_name
        )
...
fields = [
    ...
    field.RestField(
        'endpoint_web_control_violation',
        required=False,
        encrypted=False,
        default=None,
        validator=None
    ...
    field.RestField(
        'endpoint_web_filtering_blocked',
        required=False,
        encrypted=False,
        default=None,
        validator=None
