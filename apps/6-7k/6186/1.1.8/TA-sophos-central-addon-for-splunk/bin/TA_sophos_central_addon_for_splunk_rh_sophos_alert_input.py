    ...
    def handleCreate(self, conf_info):
        ...
        try:
            ...
            for input, input_meta in all_inputs.items():
                if input.startswith("sophos_alert_input://") and input_meta.get("index") == self.payload.get("index"):
                    raise RestError("500", f"Alert input with index '{self.payload.get('index')}' already exists.")
