...
def get_credentials():
    ...
    try:
        result = subprocess.run(
            [str(splunk_bin), "show-decrypted", "--value", client_id_encrypted],
            capture_output=True,
            text=True,
            check=True
        )
        client_id_decrypted = result.stdout.strip()
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"Failed to decrypt client_id: {e}")

    try:
        result = subprocess.run(
            [str(splunk_bin), "show-decrypted", "--value", client_secret_encrypted],
            capture_output=True,
            text=True,
            check=True
        )
        client_secret_decrypted = result.stdout.strip()
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"Failed to decrypt client_secret: {e}")

    try:
        result = subprocess.run(
            [str(splunk_bin), "show-decrypted", "--value", urlbase_encrypted],
            capture_output=True,
            text=True,
            check=True
        )
        urlbase_decrypted = result.stdout.strip()
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"Failed to decrypt client_secret: {e}")
