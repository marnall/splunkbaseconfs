#!/usr/bin/env python
"""
Hunt.io C2 Feed Update Custom Command
This command retrieves the C2 feed data from Hunt.io and stores it in the c2_feed_collection KV store.
"""
import sys
import os
import json
...
# Import splunklib modules from the local lib directory instead of Splunk's Python SDK paths
...
try:
    ...
    logger = logging.getLogger('huntioc2feedupdate')
    logger.error(f"Import error: {str(e)}")
    raise


@Configuration()
class HuntIOC2FeedUpdateCommand(GeneratingCommand):
    """
    Custom search command that retrieves C2 feed data from Hunt.io and updates the KV store.
    
    This command:
    1. Calls the c2feed REST API endpoint
    2. Deletes all existing entries in the c2_feed_collection KV store
    ...
    def get_c2_feed(self, api_key):
        ...
        try:
            ...
            # Make internal call to splunk REST endpoint instead of direct API call
            # This leverages existing authentication and configuration
            service_args = {
            ...
            response = service.get('/servicesNS/nobody/SA-Hunt-io/c2feed', output_mode='json')
            ...
            logger.info(f"Successfully retrieved C2 feed with {len(content)} entries")
            return content
    ...
    def update_kvstore(self, data):
        ...
        try:
            logger.info("Updating KV store with new C2 feed data")
            
            # Get KV store collection
            collection_name = "c2_feed_collection"
            service_args = {
                'token': self._metadata.searchinfo.session_key,
                'app': 'SA-Hunt-io',
                'owner': 'nobody'
            ...
            for i in range(0, len(data), batch_size):
                ...
                logger.info(f"Added batch {i//batch_size + 1} to KV store ({i} to {min(i+batch_size, len(data))})")
    ...
    def generate(self):
        ...
        try:
            ...
            # Get C2 feed data
            c2_feed_data = self.get_c2_feed(api_key)
            
            if not c2_feed_data:
                yield {'error': 'Failed to retrieve C2 feed data'}
                return
            
            # Update KV store
            success = self.update_kvstore(c2_feed_data)
            
            if success:
                yield {'status': 'success', 'message': f'Updated KV store with {len(c2_feed_data)} C2 feed entries'}
            else:
...
dispatch(HuntIOC2FeedUpdateCommand, sys.argv, sys.stdin, sys.stdout, __name__) 