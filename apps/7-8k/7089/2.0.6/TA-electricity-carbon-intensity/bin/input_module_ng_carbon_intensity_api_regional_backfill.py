...
def collect_events(helper, ew):
    ...
    url = f"https://api.carbonintensity.org.uk/regional/intensity/{opt_start_date}/{opt_end_date}"
    ...
    for period in data:
        ...
        for region in regions:
            region["from"] = period.get("from")
            region["to"] = period.get("to")
            timestamp = ut.ng_convert_event_timestamp_to_total_seconds(period.get("from"), helper)
