...
class MyScript(Script):
    ...
    def get_scheme(self):
        # This will show in the list of inputs: Settgins > Data Inputs
        ...
        arg.description = "Caution! 'reset' command will delete all IoCs and reload from point zero. make sure can_delete permission is set to user role upon reseting" 
        arg.required_on_create = False
        scheme.add_argument(arg)

        arg = Argument("loading_type_kv")
        arg.title = "Load into kvsotre? If empty than the module will load IoCs as indexed events (licensed by volume), if checked IoCs will load into kvstore (license by amount of queries)"
        arg.data_type = Argument.data_type_boolean
        arg.description = "Default: leave empty. If empty the module will load IoCs as indexed events (licensed by volume), if checked IoCs will load into kvstore (license by amount of queries). Note: if you change after input creation, set command to 'reset'" # Descriptions of boolean does not show in splunk :(
        arg.required_on_create = False
        scheme.add_argument(arg)
    ...
    def perform_reset(self, chk: Checkpointer, input_name, start_date, logger):
        ...
        logger.info(f"Performing reset: Deleting iocs events...")
        splunk.delete_iocs_events()

        logger.info(f"Performing reset: Deleting iocs kv...")
        splunk.delete_iocs_kv()
    ...
    def stream_event_logic(self, inputs, event_writer):
        ...
        for input_name, input_item in input_items:
            ...
            if input_item.get("cmd") == "reset":
                # Note: from kela_iocs://{name} to {name}
                pure_input_name = input_name.replace("kela_iocs://","")
                self.perform_reset(chk, pure_input_name, start_date, logger)
            
            load_into_kv = bool(input_item.get("loading_type_kv") == "1")
            loading_type_desc = "kvstore" if  load_into_kv else "Indexed Evnets"
            logger.info(f"Loading type is set to {loading_type_desc}.")
            ...
            for ioc in iocs:
                ...
                if load_into_kv:
                    splunk.upsert_ioc_into_kv(ioc)
                else:
                    # Insert indexed event
                    event = Event()
                    event.stanza = input_name

                    # unique identification for all sourcetypes and all indexes from this modular input
                    ioc['kela_modular_input_uid'] = '11b9c229d6b1e02f579b920fb1c30637'

                    # Write ioc
                    event.data = json.dumps(ioc)
                    event.sourceType = "kela:iocs"
