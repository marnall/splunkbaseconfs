...
def validate_input(helper, definition):
    ...
    if global_account is None:
        raise ValueError(
            'An account is required. Please check your add-on configuration')
...
def collect_events(helper, ew):
    ...
    for t in type:
        file_context = read_config(STANZA, t, str(
            format_iso_time(delta_sec=360000)))
        ...
        while nextFlag:
            ...
            try:
                ...
                helper.log_error(
                    "[MVision EPO] Request timeout error: %s" % str(e))
                return 1
            except requests.exceptions.HTTPError as e:
                helper.log_error(
                    "[MVision EPO] Request error: %s %s" % (str(e), str(endpoint)))
                return 1
            except Exception as e:
                helper.log_error(
                    "[MVision EPO] Request exception: %s" % str(e))
                return 1

            if res.ok:
                if len(data['Events']) == 0:
                    helper.log_info(
                        f'[MVision EPO] No new MVision {t} identified')
                    nextFlag = False
                    ...
                    for raw_event in data['Events']:
                        ...
                        evt = helper.new_event(data=json.dumps(event), host='mvision:epo:api', index=helper.get_output_index(
                        ), source=helper.get_input_type(), sourcetype=helper.get_sourcetype(), done=True, unbroken=True)
                        ew.write_event(evt)
                    if 'Link' in res.headers and 'rel="next"' in res.headers['Link']:
                        ...
                        nextItem = json.loads(
                            base64.b64decode(nextItem))['since']
                        nextFlag = True
                    else:
                        nextFlag = False
                        event_ldTime = format_time(
                            data['Events'][-1]['receivedutc']['value'])
                        current_ldTime = format_time(nowTime)
                        if event_ldTime > current_ldTime:
                            helper.log_info(
                                f'{event_ldTime} - {current_ldTime}')
                            nowTime = data['Events'][-1]['receivedutc']['value']
                        update_config(STANZA, t, str(nowTime))

            else:
                helper.log_error(
                    f'[MVision EPO] Could not retrieve events: {res.status_code} - {res.reason}')
