...
class Input(smi.Script):
    ...
    def get_scheme(self):
        scheme = smi.Scheme("keycloak_audit")
        scheme.description = "keycloak_audit input"
    ...
    def stream_events(self, inputs: smi.InputDefinition, event_writer: smi.EventWriter):
        ...
        for input_name, input_item in inputs.inputs.items():
            ...
            last_fetch = checkpoint.get("last_fetch_audit")
            ...
            try:
                ...
                for realm in Request(
                    ...
                    admin_events = list(
                        Request(
                            management_connection=api_manager,
                            endpoint=f"realms/{realm['realm']}/admin-events",
                            input_params={"dateFrom": datetime.strftime(start_from, "%Y-%m-%d")},
                        ).execute()
                    )
                    for event in admin_events:
                        event_time = event["time"] / 1000
                        if event_time < int(start_from.timestamp()):
                            continue
                        event_writer.write_event(
                            smi.Event(
                                data=json.dumps(event),
                                source=f"{host}:{realm['realm']}:audit",
                ...
                checkpoint.update("last_fetch_audit", new_start_from)
