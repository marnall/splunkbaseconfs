...
class Jsm(CustomEventActionBase):
    def __init__(self, settings):
        self.logger = setup_logging(JSM_LOG_FILE, 'jsm.event.action')

        super(Jsm, self).__init__(settings, self.logger)

        config = self.get_config()

        self.result = self.settings.get('result')
        self.jsm_url = config[JSM_URL]
        self.priority = config['priority']

        self.logger.info(
            'jsm_url=%s priority=%s',
            'JSM_INIT',
            self.jsm_url,
            self.priority
        )
    ...
    def send_jsm_event(self):
        ...
        return requests.post(self.jsm_url, data=dumps(payload), headers=headers)
    ...
    def execute(self):
        ...
        try:
            response = self.send_jsm_event()
            if response.status_code < 200 or response.status_code > 299:
                self.logger.error('JSM responded with HTTP status=%d' % response.status_code)
...
if __name__ == '__main__':
    LOGGER = setup_logging(JSM_LOG_FILE, 'jsm.itsi')
    if len(sys.argv) > 1 and sys.argv[1] == '--execute':
        INPUT_PARAMS = sys.stdin.read()
        try:
            ITSI = Jsm(INPUT_PARAMS)
            ITSI.execute()
        except Exception, exception:
