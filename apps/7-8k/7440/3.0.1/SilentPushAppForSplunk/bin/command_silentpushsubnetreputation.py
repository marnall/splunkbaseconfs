...
logger = setup_logging("ta_silent_push_subnet_reputation_custom_command")


@Configuration()
class SilentPushSubnetReputation(GeneratingCommand):
    """Silent Push subnet reputation custom command."""

    explain = Option(name="explain", require=False)
    subnet = Option(name="subnet", require=True)
    mask = Option(name="mask", require=True)
    ...
    def generate(self):
        ...
        data = rest_helper_obj.get_subnet_reputation(self.subnet, self.mask, params)

        logger.info("message=command_info | Silent Push Info : Json Data Retrived.")
        if isinstance(data.get("error", {}), dict):
            raise AttributeError("Invalid Subnet")
        else:
            for event in data.get("response", {}).get("subnet_reputation", ""):
                yield {
                    "_raw": event,
                    "_time": time.time()
                }
...
dispatch(SilentPushSubnetReputation, sys.argv, sys.stdin, sys.stdout, __name__)
