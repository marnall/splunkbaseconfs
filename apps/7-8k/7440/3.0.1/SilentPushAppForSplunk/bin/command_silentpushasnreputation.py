...
logger = setup_logging("ta_silent_push_asn_reputation_custom_command")


@Configuration()
class SilentPushAsnReputation(GeneratingCommand):
    """Silent Push asn reputation custom command."""
    ...
    def generate(self):
        ...
        if self.explain:
            ...
            params = None
        data = rest_helper_obj.get_asn_reputation(self.asn, params)

        logger.info("message=command_info | Silent Push Info : Json Data Retrived.")
        if isinstance(data.get("response", {}).get("asn_reputation", ""), dict):
            yield {}
        else:
            for event in data.get("response", {}).get("asn_reputation", ""):
                yield {
                    "_raw": event,
                    "_time": time.time()
                }
...
dispatch(SilentPushAsnReputation, sys.argv, sys.stdin, sys.stdout, __name__)
