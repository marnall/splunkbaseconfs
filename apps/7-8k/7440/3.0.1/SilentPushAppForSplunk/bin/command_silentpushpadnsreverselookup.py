...
logger = setup_logging("ta_silent_push_reverse_lookup_custom_command")


@Configuration()
class SilentPushPadnsReverseLookupCommand(GeneratingCommand):
    """SilentPushPadnsReverseLookupCommand Class."""
    ...
    def generate(self):
        ...
        data = rest_helper_obj.get_padns_reverse_lookup(self.qtype, self.qname, params)

        error = data.get("response").get("error")
        logger.info(f'message=command_error | {error}')
...
dispatch(SilentPushPadnsReverseLookupCommand, sys.argv, sys.stdin, sys.stdout, __name__)
