...
logger = setup_logging("ta_silent_push_nameserver_reputation_custom_command")


@Configuration()
class SilentpushNameServerReputation(GeneratingCommand):
    """Silent Push nameserver reputation custom command."""

    explain = Option(name="explain", require=False)
    nameserver = Option(name="nameserver", require=True)
    ...
    def generate(self):
        ...
        data = rest_helper_obj.get_nameserver_reputation(self.nameserver, params)

        logger.info("message=command_info | Silent Push Info : Json Data Retrived.")
        if isinstance(data.get("error", {}), dict):
            raise AttributeError("Invalid NameServer")
        else:
            for event in data.get("response", {}).get("ns_server_reputation", ""):
                yield {
                    "_raw": event,
                    "_time": time.time()
                }
...
dispatch(SilentpushNameServerReputation, sys.argv, sys.stdin, sys.stdout, __name__)
