...
CHECKPOINT_KEY = "last_modified_datetime_outbound"
MAX_SIZE = 300

IN_FILTER = ('in', 'not in')
STATUS_LIST = ("accepted", "deleted", "delivered", "dropped", "dropped oob", "permanent failure", "processing", "quarantined", "rejected", "temporary failure", "scanned", "scan bypassed")
...
def validate_input(helper, definition):
    ...
    last_modified_datetime = definition.parameters.get('last_modified_datetime_outbound', None)
...
def collect_events(helper, ew):
    ...
    opt_has_attachment = helper.get_arg('has_attachment_outbound')
    opt_last_modified_datetime = helper.get_arg('last_modified_datetime_outbound')
    opt_from_email = helper.get_arg('from_email_outbound')
    opt_from_email_filter = helper.get_arg('from_email_filter_outbound')
    opt_status = helper.get_arg('status_outbound')
    opt_status_filter = helper.get_arg('status_filter_outbound')
    opt_time_lag_guard = helper.get_arg('email_time_lag_guard_outbound')
    ...
    helper.log_info(f'(Outbound) Retrieve email trace from: {state}')
    ...
    if utils.check_within_timedelta(helper, current_time, state_dt, opt_time_lag_guard):
        helper.log_info({
              'message': 'Skipped this interval because of time_lag_guard_outbound',
              'state': state 
            })
        return
    ...
    while loop_flag:
        ...
        for data in sorted_data:
            ...
            if utils.check_within_timedelta(helper, current_time, data_last_modified_datetime, opt_time_lag_guard):
              loop_flag = False
              helper.log_info({
                'message': 'Stopped importing email trace data in the middle of the process because of time_lag_guard_outbound.',
