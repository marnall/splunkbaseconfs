...
CHECK_POINT_KEY = "from_last_modified_on_outbound"
...
def validate_input(helper, definition):
    ...
    from_last_modified_on = definition.parameters.get('from_last_modified_on_outbound', None)
...
def collect_events(helper, ew):
    ...
    """
    opt_use_api_v1 = helper.get_arg('use_api_v1_outbound')
    opt_email_status = helper.get_arg('email_status_outbound')
    opt_from_last_modified_on = helper.get_arg('from_last_modified_on_outbound')
    opt_time_lag_guard = helper.get_arg('time_lag_guard_outbound')

    helper.log_info({'opt_use_api_v1': opt_use_api_v1})
    helper.log_info( { 'opt_time_lag_guard_outbound': opt_time_lag_guard } )
    ...
    helper.log_info(f'(Outbound) Retrieve alerts from: {state}')
    ...
    for data in sorted_data:
        ...
        if utils.check_within_timedelta(helper, current_time, data_last_modified_datetime, opt_time_lag_guard):
            helper.log_info({
              'message': 'Stopped importing alert data in the middle of the process because of time_lag_guard_outbound.',
