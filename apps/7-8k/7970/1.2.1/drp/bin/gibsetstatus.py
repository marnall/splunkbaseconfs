...
class gibsetstatus(GeneratingCommand):
    ...
    def generate(self):
        logger = Utils.get_logger(
            ...
            log_filename="modinput_set_status.log"
        )
        ...
        logger.info(f"Feed ID: {self.id_item}")
        logger.info(f"Status ID: {self.status}")
        try:
            poller = DRPPoller(
                username=USERNAME,
                api_key=API_KEY,
            ...
            status = "approve" if self.status else "reject"
            poller.change_status(feed_id=self.id_item, status=status)
            yield {"Message":"Status was changed"}
