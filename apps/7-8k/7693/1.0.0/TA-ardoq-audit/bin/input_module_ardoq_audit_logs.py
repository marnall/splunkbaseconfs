...
def get_start_date(helper, check_point_key):
    ...
    if (d not in [None,'']):
        return dateutil.parser.parse(d["end_date"]).strftime("%Y-%m-%d %H:%M:%S")
    else:
...
def collect_events(helper, ew):
    ...
    helper.log_debug("current_hour_utc={}".format(current_utc_hour))
    ...
    while(True):
        ...
        #start_date = dateutil.parser.parse(get_start_date(helper, check_point_key)).astimezone(datetime.timezone.utc)
        start_date = dateutil.parser.parse(get_start_date(helper, check_point_key))
        # Replace minutes, seconds, and microseconds with zero
        start_date = start_date.replace(minute=0, second=0, microsecond=0)

        # Check if start_date is less than the current UTC hour
        if int(start_date.timestamp()) < int(current_utc_hour.timestamp()):
            ...
            if response.ok:
                ...
                try:
                    if nextPageToken is not None:
                        ...
                        start_date_utc = start_date + datetime.timedelta(hours=1)
                        checkpoint_data["end_date"] = start_date_utc.strftime('%Y-%m-%d %H:%M:%S')
            ...
            helper.log_info("finished collecting logs until current time")
            sys.exit()
    ...
    #     # Get the current UTC time truncated to the hour
    #     current_utc_hour = datetime.datetime.utcnow().replace(minute=0, second=0, microsecond=0)
    #     helper.log_debug("current_hour={}".format(current_utc_hour))
