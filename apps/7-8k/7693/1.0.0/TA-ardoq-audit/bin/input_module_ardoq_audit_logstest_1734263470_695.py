            ...
            if response.ok:
                ...
                try:
                    if nextPageToken is not None:
                        ...
                        start_date_utc = dateutil.parser.parse(d["start_date"]).astimezone(datetime.timezone.utc)
                        checkpoint_data["end_date"] = (start_date_utc + datetime.timedelta(hours=1)).astimezone(datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
