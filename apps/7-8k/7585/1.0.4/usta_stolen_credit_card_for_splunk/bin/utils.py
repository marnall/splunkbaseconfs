...
# ---- Add-on constants ----
ADDON_NAME = "usta_stolen_credit_card_for_splunk"

CONF_SETTINGS = "usta_stolen_credit_card_for_splunk_settings"
CONF_ACCOUNT = "usta_stolen_credit_card_for_splunk_account"
CHECKPOINT_COLLECTION_NAME = "stolen_credit_cards"

API = {
    "stolen_credit_cards": "https://usta.prodaft.com/api/threat-stream/v4/fraud-intelligence/credit-card-tickets",
}
...
def validate_api_key(http: requests.Session) -> None:
    url = _merge_query(API["stolen_credit_cards"], {"size": "1"})
...
def build_stolen_credit_cards_url(created_after: Optional[str] = None) -> str:
    params = {
        "ordering": "created",
        "size": str(PAGINATION_SIZE),
    }
    if created_after:
        params["start"] = created_after
    return _merge_query(API["stolen_credit_cards"], params)
