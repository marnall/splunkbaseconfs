...
class FOLDERLESS_LIST(smi.Script):

    def __init__(self):
        super(FOLDERLESS_LIST, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('folderless_list')
        scheme.description = 'Folderless List'
    ...
    def stream_events(self, inputs: smi.InputDefinition, event_writer: smi.EventWriter):
        ...
        # FIX: Loop variable renamed from 'i' to 'team'
        ...
        # Folderless: space se directly list fetch (folder step nahi hai)
        for space_id in spaces_id:
            ...
            for lst in lists:
                checkpoint_key = str(lst["id"]) + str(lst["task_count"])
                state = checkpoint.get(checkpoint_key)
                if state is None:
                    event = smi.Event(data=json.dumps(lst, indent=2), index=input_item.get("index"), sourcetype="clickup:folderless_list")
...
if __name__ == '__main__':
    exit_code = FOLDERLESS_LIST().run(sys.argv)
    sys.exit(exit_code)