...
def read_splunk_config_ds_url(config_file):
    ...
    if ds_host and ds_mgmt_port and ds_token:
        logger.info("[success] configs parsable")
        return ds_host, ds_mgmt_port, ds_token
    else:
        logger.error("[fail] configs are missing")
...
if __name__ == "__main__":
    ...
    if if_conf_exists(config_file):
        logger.info("[success] configs exists")
        if test_ds_connection(config_file):
            logger.info("[success] SH DS connection - successful")
        else:
            logger.error("[fail] SH DS connection - Failed")
    else:
        logger.error("[fail] configs not readable")
