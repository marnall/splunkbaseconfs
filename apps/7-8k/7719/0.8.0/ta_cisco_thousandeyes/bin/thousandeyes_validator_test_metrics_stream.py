...
from thousandeyes_utils import get_hec_tokens, get_account_id, get_test_id
from thousandeyes_constant import THOUSANDEYES_STREAM_PAYLOAD, DEFAULT_ENDPOINT_DYNAMIC_TEST_TYPE


class StreamInputValidator:
    """ThousandEyes Stream Input Validator."""
    ...
    def init_payload(self):  # flake8: noqa: E271, E126
        """
        Return static payload for stream.

        :return: Payload Dictionary.
        """
        return THOUSANDEYES_STREAM_PAYLOAD
    ...
    def add_tests(self, type, tests, account_group):
        ...
        if tests.strip() == "All":
            ...
            if aid and type == "cea":
                all_tests = self.client.get_all_cea_tests_for_metrics(aid)
            elif aid and type == "endpoint":
                all_tests = self.client.get_all_endpoint_tests(aid)
            test_values = []
            for test in all_tests.get("tests", []):
                ...
                if endpoint_test_category:
                    test_values.append(
                        f"{test.get('testName')} ({test.get('testId')} | {test_type} | {endpoint_test_category})"
                    )
                else:
                    test_values.append(
    ...
    def add_tags(self, tags, account_group):
        """
        Add tags to the metric stream payload.
    ...
    def add_export_config(self, index, token):
        ...
        :param tests :  HEC token for payload.
        """
    ...
    def create_new_stream(self, aid):
        """
        Create a new Thousandeyes stream.
        ...
        self.logger.info(
            f"Creating new ThousandEyes stream using {self.payload.get('streamEndpointUrl')}."
        )
        ...
        self.logger.info(
            f"Successfully created new ThousandEyes stream using {self.payload.get('streamEndpointUrl')}."
        )
    ...
    def validate(self, data):
        ...
        if data.get("endpoint_tests", None) not in (None, ""):
            tests_endpoint = self.add_tests(
                "endpoint",
                data.get("endpoint_tests"),
        ...
        data["endpoint_tests"] = tests_endpoint
        ...
        if data.get("thousandeyes_stream_id", None) in (None, ""):
            ...
            del log_payload["exporterConfig"]["splunkHec"]["token"]
            self.logger.info(
                f"Updating ThousandEyes stream: {data.get('thousandeyes_stream_id')}."
            )
            self.logger.debug(f"Stream payload without HEC detail {log_payload}.")
            ...
            data["thousandeyes_stream_id"] = stream_response.get("id")
            self.logger.info(
                f"Successfully updated ThousandEyes stream {stream_response.get('id')}."
            )
