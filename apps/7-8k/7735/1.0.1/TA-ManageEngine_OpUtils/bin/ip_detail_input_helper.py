...
def get_ip_data(
    ...
    logger.info("Fetching subnet data from OpUtils API for IP input ...")
    ...
    while page <= total_pages:
        ...
        for subnet_info_data in subnet_data["rows"]:
            if int(subnet_info_data["usedCount"]) > 0:
                used_subnet.append(
                    {
                        "subnet-address": subnet_info_data["subnet-address"],
                        "subnet-address-cidr": subnet_info_data["subnet-address-cidr"],
                    }
                )
    ...
    logger.info(
        f"Fetched {len(used_subnet)} subnets. Collecting IP data for each subnet ..."
    )

    index_counter = 0

    for subnet in used_subnet:
        subnet_address = subnet["subnet-address"]
        subnet_address_cidr = subnet["subnet-address-cidr"]

        logger.debug(f"Fetching IP data for subnet {subnet_address} ...")

        try:
            sub_detail_response = session.get(
                url=f"https://{server_name_or_ip}:{port}/api/json/ipam/getSubnetDetail?subnetAddress={subnet_address}&apiKey={api_key}",
                verify=verify_certificate,
            )
            sub_detail_response.raise_for_status()
            sub_detail = sub_detail_response.json()
        except Exception as ex:
            log.log_exception(
                logger,
                ex,
        ...
        for data_list in sub_detail["rows"]:
            if data_list["status"] == "Used":
                data_list["subnet-address"] = subnet_address
                data_list["subnet-address-cidr"] = subnet_address_cidr

                # remove values that evaluate to False
                filtered_data_list = {k: v for k, v in data_list.items() if v}

                # write data to Splunk index
                event_writer.write_event(
                    smi.Event(
                        data=json.dumps(
                            filtered_data_list, ensure_ascii=False, default=str
                        ),
                        index=index,
                        source=source,
    ...
    logger.info(f"Fetched {index_counter} IP infos from API!")

    # remove empty or null fields from data
...
def stream_events(inputs: smi.InputDefinition, event_writer: smi.EventWriter):
    for input_name, input_item in inputs.inputs.items():
        ...
        try:
            ...
            sourcetype = "manageengine:ipam:ip:json"
            index_counter = get_ip_data(
                logger,
                api_key,
                server_name_or_ip,
                port,
            ...
            log.log_exception(
                ...
                e,
                "Ingesting Error",
                msg_before="Exception raised while ingesting data for IP input",
            )
