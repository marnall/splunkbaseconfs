"""sendbetterslides - a splunk email command to send reports as Powerpoint slidedeck with more formating options """
...
def sendemail(
    ...
    username, password = getCredentials(authToken, "betterslides")
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        ...
        logger.basicConfig(
            ...
            filename=os.path.join(
                os.environ["SPLUNK_HOME"], "var", "log", "splunk", "betterslides.log"
            ),
            filemode="a+",
            level=logger.INFO,
        )
        ...
        dashboards = getarg(settings, "dashboards", "")
        dashboards = six.moves.urllib.parse.quote( dashboards )
        print("dashboards we received:", file=sys.stderr)
        ...
        try:
            ...
            uri = "/services/betterslides?dashboards=" + dashboards
