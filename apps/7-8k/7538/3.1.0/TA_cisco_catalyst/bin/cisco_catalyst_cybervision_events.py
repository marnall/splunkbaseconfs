"""Modular input for Cyber Vision Events."""
...
CONNECTED_LOG = (
    ...
    "filter_value=cisco_catalyst_cybervision_events://{}, status=Connected,"
)
NOT_CONNECTED_LOG = (
    "instance={}, product=Cisco Cyber Vision, "
    "filter_value=cisco_catalyst_cybervision_events://{}, status=Not Connected,"
)


class CiscoCyberVisionEvents(smi.Script):
    """Get the Event details from Cisco CyberVision Server."""

    def __init__(self):
        """Initialise CiscoCyberVisionEvents class."""
        super(CiscoCyberVisionEvents, self).__init__()

    def get_scheme(self):
        """Load the arguments in the Configuration page."""
        scheme = smi.Scheme('cisco_catalyst_cybervision_events')
        scheme.description = (
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        try:
            ...
            logger = logger_manager.get_logger(
                f"cybervision_events_{input_name}", input_conf["logging_level"]
            )
            ...
            start_date_formated = datetime.datetime.fromtimestamp(
                start_date / 1000, tz=datetime.timezone.utc
            ).replace(tzinfo=None)
            end_date_formated = datetime.datetime.fromtimestamp(
                current_time / 1000, tz=datetime.timezone.utc
            ).replace(tzinfo=None)
            while True:
                params = {
                ...
                data = utils.cybervision_request_get("event", config_details, params)

                page += int(page_size)
                additional_fields = {"host": host_name, "time_field": "creation_time"}
                utils.cybervision_ingest_in_splunk(
                    input_conf, ew, data, sourcetype, additional_fields, source="Events"
                )
            ...
            logger.error(utils.DATA_COLLECTION_ERROR.format("Events", e))
...
if __name__ == '__main__':
    exit_code = CiscoCyberVisionEvents().run(sys.argv)
    sys.exit(exit_code)
