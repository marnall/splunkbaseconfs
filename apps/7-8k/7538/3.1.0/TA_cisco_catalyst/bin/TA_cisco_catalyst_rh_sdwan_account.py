"""TA-cisco_catalyst_sdwan_account."""
...
from sdwan.TA_cisco_catalyst_sdwan_server_validation import (
    ValidateAccount,
)
...
from consts import SDWAN_CERT_FILE_LOC
import logger_manager

logger = logger_manager.get_logger("sdwan_account_validation")
...
endpoint = SingleModel("ta_cisco_catalyst_sdwan_account", model, config_name="account")


class AccountHandler(AdminExternalHandler):
    """Handle the REST request to configure or remove a SDWAN account."""
    ...
    def handleRemove(self, confInfo):
        ...
        account_conf_file_name = "ta_cisco_catalyst_sdwan_account"
        ...
        try:
            ...
            input_list = []
            input_type_list = ["cisco_catalyst_sdwan_health", "cisco_catalyst_sdwan_site_and_tunnel_health"]

            for _input in created_inputs:
                cisco_sdwan_input = _input.split("://")
                if cisco_sdwan_input[0] in input_type_list:
                    configured_account = inputs_file.get(_input).get("sdwan_account")
                    if configured_account == account_stanza_name:
                        input_list.append(cisco_sdwan_input[1])
            if len(input_list) > 0:
            ...
            cert_file_loc = SDWAN_CERT_FILE_LOC.format(cert_name=cert_file_name)
