...
class SDWANSiteAndTunnelHealth(smi.Script):
    """Get the Health Details from Cisco SDWAN Server."""

    def __init__(self):
        """Initialise SDWANSiteAndTunnelHealth class."""
        super(SDWANSiteAndTunnelHealth, self).__init__()

    def get_scheme(self):
        """Load the arguments in the Configuration page."""
        scheme = smi.Scheme("cisco_catalyst_sdwan_site_and_tunnel_health")
    ...
    def stream_events(self, inputs: smi.InputDefinition, event_writer: smi.EventWriter):
        ...
        try:
            ...
            input_conf["input_stanza_name"] = "".join(
                ["cisco_catalyst_sdwan_site_and_tunnel_health://", input_name]
            )
            logger = logger_manager.get_logger(
                f"sdwan_site_and_tunnel_health_{input_name}", input_conf["logging_level"]
            )
            ...
            event_count = sdwan_utils.get_site_and_tunnel_health_data(health_data_collector, health_type_list)
            logger.info(
                f"Total site and tunnel health events ingested = {event_count} for health types = {health_type_list}\n"
...
if __name__ == "__main__":
    exit_code = SDWANSiteAndTunnelHealth().run(sys.argv)
    sys.exit(exit_code)
