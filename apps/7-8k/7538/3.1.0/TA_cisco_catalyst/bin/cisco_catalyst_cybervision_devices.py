"""Modular input for Cyber Vision Devices."""
...
CONNECTED_LOG = (
    ...
    "filter_value=cisco_catalyst_cybervision_devices://{}, status=Connected,"
)
NOT_CONNECTED_LOG = (
    "instance={}, product=Cisco Cyber Vision, "
    "filter_value=cisco_catalyst_cybervision_devices://{}, status=Not Connected,"
)


class CiscoCyberVisionDevices(smi.Script):
    """Get the Device details from Cisco CyberVision Server."""

    def __init__(self):
        """Initialise CiscoCyberVisionDevices class."""
        super(CiscoCyberVisionDevices, self).__init__()

    def get_scheme(self):
        """Load the arguments in the Configuration page."""
        scheme = smi.Scheme('cisco_catalyst_cybervision_devices')
        scheme.description = (
    ...
    def append_vulnerabilities(self, data, config_details):
        """Append vulnerabilities data."""
        for dictionary in data:
            device_id = dictionary["id"]
            dictionary["vulnerabilities"] = utils.cybervision_request_get(
                "devices/{}/vulnerabilities".format(device_id), config_details
            )
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        ...
        try:
            ...
            logger = logger_manager.get_logger(
                f"cybervision_devices_{input_name}", input_conf["logging_level"]
            )
            ...
            sourcetype = "cisco:cybervision:devices"
            endpoint = "/devices"
            ...
            while True:
                ...
                data = utils.cybervision_request_get("devices", config_details, params)
                self.append_vulnerabilities(data, config_details)
                ...
                utils.cybervision_ingest_in_splunk(
                    input_conf, ew, data, sourcetype, additional_fields, source="Devices"
                )
            ...
            logger.error(utils.DATA_COLLECTION_ERROR.format("Devices", e))
...
if __name__ == '__main__':
    exit_code = CiscoCyberVisionDevices().run(sys.argv)
    sys.exit(exit_code)
