"""Modular input for Cyber Vision Vulnerabilities."""
...
CONNECTED_LOG = (
    ...
    "filter_value=cisco_catalyst_cybervision_vulnerabilities://{}, status=Connected,"
)
NOT_CONNECTED_LOG = (
    "instance={}, product=Cisco Cyber Vision, "
    "filter_value=cisco_catalyst_cybervision_vulnerabilities://{}, status=Not Connected,"
)


class CiscoCyberVisionVulnerabilities(smi.Script):
    """Get the Vulnerability details from Cisco CyberVision Server."""

    def __init__(self):
        """Initialise CiscoCyberVisionVulnerabilities class."""
        super(CiscoCyberVisionVulnerabilities, self).__init__()

    def get_scheme(self):
        """Load the arguments in the Configuration page."""
        scheme = smi.Scheme('cisco_catalyst_cybervision_vulnerabilities')
        scheme.description = (
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter):
        """Collect the events from the Cisco CyberVision Server."""
        try:
            ...
            logger = logger_manager.get_logger(
                f"cybervision_vulnerabilities_{input_name}", input_conf["logging_level"]
            )
            ...
            while True:
                ...
                data = utils.cybervision_request_get("vulnerabilities", config_details, params)
            ...
            logger.error(utils.DATA_COLLECTION_ERROR.format("Vulnerabilities", e))
...
if __name__ == '__main__':
    exit_code = CiscoCyberVisionVulnerabilities().run(sys.argv)
    sys.exit(exit_code)
