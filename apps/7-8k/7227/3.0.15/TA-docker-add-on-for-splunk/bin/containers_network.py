...
class CONTAINERS_NETWORKS(smi.Script):

    def __init__(self):
        super(CONTAINERS_NETWORKS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('containers_networks')
        scheme.description = 'Containers Networks'
    ...
    def stream_events(self, inputs, ew):
        ...
        url_container = f'https://{docker_ip}:{docker_port}/v1.41/networks'
        ...
        for i in result_images:
            ...
            checkpoint = checkpointer.KVStoreCheckpointer(
                "TA-docker-add-on-for-splunk_checkpoints",
                session_key,
                "TA-docker-add-on-for-splunk"
            )
            ...
            if state is None:
                event = smi.Event(
                    ...
                    sourcetype="Docker_container_network",
                )
...
if __name__ == '__main__':
    exit_code = CONTAINERS_NETWORKS().run(sys.argv)
    sys.exit(exit_code)
