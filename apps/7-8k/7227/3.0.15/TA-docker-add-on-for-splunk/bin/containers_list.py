...
class CONTAINERS_LIST(smi.Script):

    def __init__(self):
        super(CONTAINERS_LIST, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('containers_list')
        scheme.description = 'Containers List'
    ...
    def stream_events(self, inputs, ew):
        ...
        url_container = f'https://{docker_ip}:{docker_port}/v1.41/containers/json?all=true'
        if proxy_enabled == "1":
        ...
        for i in result_images:
            ...
            if state is None:
                event = smi.Event(
                    ...
                    sourcetype="Docker_container_list",
                )
...
if __name__ == '__main__':
    exit_code = CONTAINERS_LIST().run(sys.argv)
    sys.exit(exit_code)
