...
class CONTAINERS_STATS(smi.Script):

    def __init__(self):
        super(CONTAINERS_STATS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('containers_stats')
        scheme.description = 'Containers Stats'
    ...
    def stream_events(self, inputs, ew):
        ...
        cfm = conf_manager.ConfManager(session_key, 'TA-docker-add-on-for-splunk', realm=f"__REST_CREDENTIAL__#{ADDON_NAME}#configs/conf-ta_docker_add_on_for_splunk_settings")
        ...
        try:
            ver = conf_manager.ConfManager(session_key, 'TA-docker-add-on-for-splunk', realm=f"__REST_CREDENTIAL__#{ADDON_NAME}#configs/conf-app")
        ...
        url_container = f'https://{docker_ip}:{docker_port}/v1.41/containers/json'

        if proxy_enabled == "1":
            response_stats = requests.request("GET", url_container, proxies=proxy_dict)
        else:
            response_stats = requests.request("GET", url_container)

        result_stats = response_stats.json()
        container_id = []
        count = 0
        status = ""
        for i in result_stats:
            container_id.append(str(i["Id"]))

        for j in container_id:
            url_stats = f'https://{docker_ip}:{docker_port}/v1.41/containers/{j}/stats?stream=false'
            response_stats = requests.request("GET", url_stats)
            r_stats = response_stats.json()
            status = response_stats.status_code

            event = smi.Event(
                data=json.dumps(r_stats, indent=2),
                index=input_item.get("index"),
                sourcetype="Docker_container_stats",
            )
            ew.write_event(event)
            count += 1
        status = status
        logsummary.log_summary(activation_key=activation_key, meta=meta, url=url_stats, status=status, count=count, source=source, host=host, version=version)


if __name__ == '__main__':
    exit_code = CONTAINERS_STATS().run(sys.argv)
    sys.exit(exit_code)
