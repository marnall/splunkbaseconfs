...
class CONTAINERS_IMAGES(smi.Script):
    def __init__(self):
        super(CONTAINERS_IMAGES, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('containers_images')
        scheme.description = 'Containers Images'
    ...
    def stream_events(self, inputs, ew):
        ...
        url_container = f'https://{docker_ip}:{docker_port}/v1.41/images/json?all=true'
        ...
        for i in result_images:
            ...
            if state is None:
                event = smi.Event(
                    ...
                    sourcetype="Docker_container_images",
                )
...
if __name__ == '__main__':
    exit_code = CONTAINERS_IMAGES().run(sys.argv)
    sys.exit(exit_code)
