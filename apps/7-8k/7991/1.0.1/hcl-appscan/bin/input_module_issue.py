...
def collect_events(helper, ew):
    try:
        helper.log_info("Enter in script")
        ...
        while True:
            applications = constants_definition.fetch_applications(keyID, secretID,appScanUrl,allowUntrustedConnection, skip_app, count_app, top_app)
            path = "/servicesNS/nobody/hcl-appscan/storage/collections/data/hcl_appscan_checkpointer/issues"
            try:
                data = rest.simpleRequest(
                        path,
                        sessionKey=helper.context_meta['session_key'],
            ...
            for app in applications:
                ...
                top = 100
                while True:
                    issues = constants_definition.fetch_issues(keyID, secretID,appScanUrl,allowUntrustedConnection, app['Id'], skip, count, top)
                    for issue in issues:
                        if state_list is None or "issues_" + str(issue["Id"]) not in state_list:
                            if state_list is None:
                                state_list = []
                            event = helper.new_event(index=helper.get_output_index(),
                                            sourcetype="AppScanIssues",
                                            source=source,
                                            data=json.dumps(issue, ensure_ascii=False))
                            ew.write_event(event)
                            if isinstance(state_list, str):
                                state_list = json.loads(state_list)
                            state_list.append("issues_" + str(issue["Id"]))
                            helper.save_check_point("issues", state_list)
                    skip += 100
                    if len(issues) < 100:
                        break
            skip_app += 10
