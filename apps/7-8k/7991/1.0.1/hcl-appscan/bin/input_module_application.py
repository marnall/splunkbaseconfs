...
def collect_events(helper, ew):
    try:
        ...
        try:
            ...
            pass
        path = "/servicesNS/nobody/hcl-appscan/storage/collections/data/hcl_appscan_checkpointer/applications"
        try:
            data = rest.simpleRequest(
                path,
                sessionKey=helper.context_meta['session_key'],
            ...
            state_list = None
        while True:
            helper.log_info(f"Fetching applications from AppScan with skip={skip} and count={count}, top={top}, appScanUrl={appScanUrl}, allowUntrustedConnection={allowUntrustedConnection}")
            applications = constants_definition.fetch_applications(keyID, secretID,appScanUrl,allowUntrustedConnection, skip, count, top)
            helper.log_info(f"Fetched {len(applications)} applications")
            for app in applications:
                if state_list is None or "applications_" + str(app["Id"]) not in state_list:
                    if state_list is None:
                        state_list = []
                    event = helper.new_event(index=helper.get_output_index(),
                                    sourcetype="AppScanApplications",
                                    source=source,
                                    data=json.dumps(app, ensure_ascii=False))
                    ew.write_event(event)
                    helper.log_info(app)
                    helper.log_info(app["Id"])
                    if isinstance(state_list, str):
                        state_list = json.loads(state_list)
                    state_list.append("applications_" + str(app["Id"]))
                    helper.save_check_point("applications", state_list)
            skip += 10
