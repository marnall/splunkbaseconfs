...
from nessus_checkpoint import NessusPluginCheckpoint


class VulnerabilityPluginDetailsInput(AddonInput):
    def collect(self, account_details, http_scheme, verify_ssl, checkpointer):
        ...
        ckpt = NessusPluginCheckpoint(self.logger, checkpointer, nessus_base_url, _start_date)

        if ckpt.contents.get(nessus_base_url,{}).get("start_date") != _start_date:
            ckpt.delete()
            ckpt = NessusPluginCheckpoint(self.logger, checkpointer, nessus_base_url, _start_date)

        nessus_pro_api.collect_plugin_data(ckpt)
...
def stream_events(input_script: smi.Script, inputs: smi.InputDefinition, event_writer: smi.EventWriter):
    ...
    for input_name, input_item in inputs.inputs.items():
        VulnerabilityPluginDetailsInput(session_key, input_name, input_item, event_writer)
