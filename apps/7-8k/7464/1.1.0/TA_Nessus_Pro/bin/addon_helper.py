...
def get_verify_ssl(session_key: str):
    try:
        settings_cfm = conf_manager.ConfManager(
            session_key,
            ADDON_NAME,
        ...
        api_details = settings_cfm.get_conf(
            f"{NORMALIZED_ADDON_NAME}_settings").get("api")

        verify_ssl = api_details.get('verify_ssl') if (
            api_details.get('verify_ssl')) else True

        return str_to_boolean_default_true(verify_ssl)
    except:
        return True


def get_http_scheme(session_key: str):
    try:
        settings_cfm = conf_manager.ConfManager(
            session_key,
            ADDON_NAME,
        ...
        return api_details.get('http_scheme') if (
            api_details.get('http_scheme')) else "https"
    except:
        return "https"
...
class AddonInput:
    ...
    def __init__(self, session_key, input_name, input_item, event_writer) -> None:
        ...
        try:
            ...
            self.collect(account_details, get_http_scheme(session_key), get_verify_ssl(session_key), input_checkpointer)
    ...
    def write_event(self, data, timestamp, sourcetype, source):
    ...
    def collect(self, account_details, http_scheme, verify_ssl, checkpointer: AddonInputCheckpointer):
