...
class GITLAB_SAAS_MERGE_REQUESTS(smi.Script):

    def __init__(self):
        super(GITLAB_SAAS_MERGE_REQUESTS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('gitlab_saas_merge_requests')
        scheme.description = 'gitlab:merge-requests'
    ...
    def stream_events(self, inputs, ew):
        ...
        try:
            if (start_date and end_date):
                ...
                id = ids_list
                for j in id:
                    base_url_merges = f"https://gitlab.com/api/v4/projects/{j}/merge_requests?per_page=100&since={start_date}&until={end_date}"
                    headers = {
                        "PRIVATE-TOKEN": access_token
                    }
                    if proxy_enabled == "1":
                        response = requests.get(base_url_merges, headers=headers, proxies=proxy_dict)
                    else:
                        response = requests.get(base_url_merges, headers=headers)
                    response2 = requests.head(base_url_merges, headers=headers)
            ...
            if (start_date and end_date):
                base_url = f"https://gitlab.com/api/v4/projects/{ID}/merge_requests?per_page=100&since={start_date}&until={end_date}"
                headers = {
                    "PRIVATE-TOKEN": access_token
                }
                if proxy_enabled == "1":
...
if __name__ == '__main__':
    exit_code = GITLAB_SAAS_MERGE_REQUESTS().run(sys.argv)
    sys.exit(exit_code)
