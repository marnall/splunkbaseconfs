...
class GITLAB_SAAS_EVENTS(smi.Script):

    def __init__(self):
        super(GITLAB_SAAS_EVENTS, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('gitlab_saas_events')
        scheme.description = 'gitlab:events'
    ...
    def stream_events(self, inputs, ew):
        ...
        try:
            if (start_date and end_date):
                ...
                count = 0
                for j in id:
                    base_url_events = f"https://gitlab.com/api/v4/projects/{j}/events?per_page=100&since={start_date}&until={end_date}"
                    headers = {
                        "PRIVATE-TOKEN": access_token
                    }
                    if proxy_enabled == "1":
                        response = requests.get(base_url_events, headers=headers, proxies=proxy_dict)
                    else:
                        response = requests.get(base_url_events, headers=headers)
                    response2 = requests.head(base_url_events, headers=headers)
            ...
            if (start_date and end_date):
                base_url = f"https://gitlab.com/api/v4/projects/{ID}/events?per_page=100&since={start_date}&until={end_date}"
                headers = {
                    "PRIVATE-TOKEN": access_token
                }
                if proxy_enabled == "1":
...
if __name__ == '__main__':
    exit_code = GITLAB_SAAS_EVENTS().run(sys.argv)
    sys.exit(exit_code)
