...
class Session(object):
    ...
    def _prep_x509_header(self, method, url, data=None):
        ...
        self.logger.debug(f"Authentication cookie {cookie}")
        return cookie
    ...
    def push_to_apic(self, url, data, timeout=None):
        ...
        if self.cert_auth and not (self.appcenter_user and self._subscription_enabled and self._logged_in):
            ...
            resp = self.session.post(
                post_url, data=data, verify=False, timeout=timeout, proxies=self._proxies, cookies=cookies
            )
    ...
    def get(self, url, timeout=None):
        ...
        resp = self.session.get(
            get_url, timeout=timeout, verify=False, proxies=self._proxies, cookies=cookies
        )
        if resp.status_code == 403:
            if self.cert_auth and not (self.appcenter_user and self._subscription_enabled):
                ...
                resp = self.session.get(get_url, timeout=timeout, verify=False, proxies=self._proxies)
