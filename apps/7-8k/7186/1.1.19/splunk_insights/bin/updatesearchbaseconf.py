...
S3_FILE_PATH = "is4s_searchbase/searchbase.conf"
...
def format_version_timestamp(version_int: int) -> str:
    ...
    if version_int == 0: return "Not Found"
    try:
...
def prune_old_backups():
    ...
    try:
        ...
        backup_files = [f for f in os.listdir(BACKUP_DIR) if f.startswith(DESTINATION_FILE_NAME) and f.endswith(".bak")]
        ...
        if len(backup_files) > MAX_BACKUPS:
            ...
            INFO(f"Found {len(backup_files)} backups. Deleting {len(files_to_delete)} old backups.")
...
class UpdateSearchbaseConf(GeneratingCommand):
    ...
    def reload_searchbase_conf(self, session_key: str, splunkd_uri: str) -> dict:
        ...
        try:
            # Using verify=False is common for internal Splunk connections that may use
            # self-signed certificates. This is a known security consideration.
    ...
    def generate(self) -> Generator[dict, None, None]:
        ...
        INFO(f"Starting command execution. Provided options: dryrun={self.dryrun}, force={self.force}")
        ...
        if self.dryrun is None and self.force is None:
            ...
            yield { "_time": time.time(), "status": "Error: Missing Option", "message": "Please specify an action. Use 'dryrun=true' to check for updates, or 'dryrun=false' to perform a safe update." }
            return
        ...
        try:
            ...
            yield {"_time": time.time(), "status": "Failed", "error": f"Could not download from S3: {e}"}
            return
        ...
        if self.dryrun is True:
            ...
            recommendation = "The local version is current or newer. No action is required."
            if s3_version > local_version:
                ...
                recommendation = "WARNING: Could not parse version from S3 file. DO NOT UPDATE."
            INFO(f"Dry run recommendation: {recommendation}")
            yield { "_time": time.time(), "status": "Dry Run", "local_version_timestamp": format_version_timestamp(local_version), "s3_version_timestamp": format_version_timestamp(s3_version), "recommendation": recommendation, }
            return

        if self.force:
            WARN(f"Action is 'force=true'. Bypassing version check and proceeding with update.")
        elif s3_version <= local_version:
            INFO("S3 version is not newer than local. Skipping update.")
            yield { "_time": time.time(), "status": "Skipped", "message": f"Local version ({local_version}) is current or newer than S3 version ({s3_version}).", "local_version_timestamp": format_version_timestamp(local_version), "s3_version_timestamp": format_version_timestamp(s3_version), }
            return

        # 4. Perform the update process: Backup -> Write -> Reload -> Prune.
        INFO(f"Proceeding with update. S3 version ({s3_version}) is newer than local ({local_version}) or force=true.")
...
if __name__ == "__main__":
    # Disable warnings for insecure requests, as verify=False is used intentionally.
