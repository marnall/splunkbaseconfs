# -----------------------------------------------
# DataExport Custom Splunk Command
# -----------------------------------------------
...
_LOG_PATH = os.path.join(_SPLUNK_HOME, "var", "log", "splunk", "dataexport.log")

# Initialize a logger for the DataExport command
...
_LOGGER.debug("[INIT] Logger initialized for the DataExport script.")
...
class DataExport(StreamingCommand):
    """
    Splunk Custom Streaming Command: DataExport

    This command performs the following steps:
    1. Collects incoming events from the Splunk pipeline.
    ...
    def __init__(self):
        ...
        _LOGGER.debug("[INIT] DataExport command instance created.")

        super(DataExport, self).__init__()
        self._events: List[Dict] = []  # acccumulate events for JSON serialisation

    # This method processes incoming records, retrieves credentials, compresses the events into a ZIP file,
    # and uploads the file to the specified REST API endpoint.
    def stream(self, records):

        _LOGGER.debug("[STREAM] Starting DataExport command.")
        ...
        try:
            ...
            _LOGGER.exception(
                "[DATAEXPORT ERROR] An error occurred in the stream function: %s",
                str(e),
            )
            raise
        ...
        try:
            ...
            try:
                ...
                _upload_file(
                    zip_bytes,
                    filename=f"{customer_name.replace(' ', '_')}-is4s-data-export.zip",
    ...
    def finish(self):
        _LOGGER.debug("[FINISH] DataExport execution completed.")


# Splunk entry‑point
if __name__ == "__main__":
    _LOGGER.debug("[MAIN] Starting DataExport command dispatch.")
    dispatch(DataExport, module_name=__name__)
