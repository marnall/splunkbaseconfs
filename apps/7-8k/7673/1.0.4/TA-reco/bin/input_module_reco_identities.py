...
UPDATED_AT_FIELD = "last_seen_time"
...
def collect_events(helper, ew):
    ...
    """Fetch all identities from Reco API and send to Splunk."""
    ...
    # Fetch all identities data with pagination
    try:
    ...
    if all_identities:
        helper.save_check_point("reco_identities_last_run", {"lastRun": datetime.now().strftime(OCCURRED_FORMAT)})
        helper.log_info("Checkpoint updated with last run time")

def fetch_all_identities(helper, tenant_url, api_key, page_size, after):
    """Retrieve all identities from Reco API with pagination."""
    headers = {"Authorization": f"Bearer {api_key}"}
    all_identities = []
    page_number = 0
    
    # First, get the total count
    count_params = create_identities_params(page_size, page_number, after)
    
    helper.log_info("Getting total count of identities from Reco API.")
    ...
    if count_response.status_code == 200:
        ...
        helper.log_info(f"Total number of identities: {total_count}")
    else:
    ...
    for page_number in range(total_pages):
        ...
        params = create_identities_params(page_size, page_number, after)
        ...
        identities = parse_response(response)
        
        # Enrich each identity with metadata
        for identity in identities:
            identity["total_identities_count"] = total_count
            identity["data_source"] = "enriched_identity_view"
            identity["page_number"] = page_number
            all_identities.append(identity)
        
        helper.log_info(f"Fetched {len(identities)} identities from page {page_number + 1}")
    
    return all_identities

def create_identities_params(page_size, page_number, after):
    """Create request parameters for identities."""
    params = {
        "getTableRequest": {
            "tableName": "enriched_identity_view",
            "pageSize": page_size,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python reco_collector_identities.py <tenant_url> <api_key>")
        sys.exit(1)
    ...
    class TestHelper:
        ...
        def get_input_type(self):
            return "reco_identities"
            
        def get_sourcetype(self):
            return "reco:identities"
    ...
    print("Starting identities collection test...")
