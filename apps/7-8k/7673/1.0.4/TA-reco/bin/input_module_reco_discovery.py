...
def collect_events(helper, ew):
    ...
    """Fetch app discovery data from Reco API and send to Splunk."""
    ...
    helper.log_info(f"Starting collection of app discovery events with page_size={page_size}")
    ...
    # Fetch all app discovery data with pagination
    try:
        all_apps = fetch_all_app_discovery(helper, tenant_url, api_key, page_size, after)
    ...
    if all_apps:
        helper.save_check_point("reco_discovery_last_run", {"lastRun": datetime.now().strftime(OCCURRED_FORMAT)})
        helper.log_info("Checkpoint updated with last run time")

def fetch_all_app_discovery(helper, tenant_url, api_key, page_size, after):
    """Retrieve all app discovery data from Reco API with pagination."""
    headers = {"Authorization": f"Bearer {api_key}"}
    all_apps = []
    page_number = 0
    
    # First, get the total count
    count_params = create_app_discovery_params(page_size, page_number, after)
    
    helper.log_info("Getting total count of apps from Reco API.")
    ...
    if count_response.status_code == 200:
        ...
        helper.log_info(f"Total number of apps: {total_count}")
    else:
    ...
    for page_number in range(total_pages):
        ...
        params = create_app_discovery_params(page_size, page_number, after)
        ...
        helper.log_info(f"Fetched {len(apps)} apps from page {page_number + 1}")
    
    return all_apps

def create_app_discovery_params(page_size, page_number, after):
    """Create request parameters for app discovery."""
    params = {
        "getTableRequest": {
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python reco_collector_app_discovery.py <tenant_url> <api_key>")
        sys.exit(1)
    ...
    class TestHelper:
        ...
        def get_input_type(self):
            return "reco_app_discovery"
            
        def get_sourcetype(self):
            return "reco:app:discovery"
    ...
    print("Starting app discovery collection test...")
