...
UPDATED_AT_FIELD = "last_activity_time_by_event"
...
def collect_events(helper, ew):
    ...
    """Fetch all enriched accounts from Reco API and send to Splunk."""
    ...
    helper.log_info(f"Starting collection of enriched accounts with page_size={page_size}")
    ...
    # Fetch all enriched accounts data with pagination
    try:
        all_accounts = fetch_all_enriched_accounts(helper, tenant_url, api_key, page_size, after)
        helper.log_info(f"Total enriched accounts fetched: {len(all_accounts)}")
        send_events(all_accounts, helper, ew)
    except Exception as e:
        helper.log_error(f"Error fetching enriched accounts: {e}")

    # Save the last fetched time to checkpoint
    if all_accounts:
        helper.save_check_point("reco_accounts_last_run", {"lastRun": datetime.now().strftime(OCCURRED_FORMAT)})
        helper.log_info("Checkpoint updated with last run time")

def fetch_all_enriched_accounts(helper, tenant_url, api_key, page_size, after):
    """Retrieve all enriched accounts from Reco API with pagination."""
    headers = {"Authorization": f"Bearer {api_key}"}
    all_accounts = []
    page_number = 0
    
    # First, get the total count
    count_params = create_enriched_accounts_params(page_size, page_number, after)
    
    helper.log_info("Getting total count of enriched accounts from Reco API.")
    ...
    if count_response.status_code == 200:
        ...
        helper.log_info(f"Total number of enriched accounts: {total_count}")
    else:
    ...
    for page_number in range(total_pages):
        ...
        params = create_enriched_accounts_params(page_size, page_number, after)
        ...
        accounts = parse_response(response)
        
        # Enrich each account with metadata
        for account in accounts:
            account["total_accounts_count"] = total_count
            account["data_source"] = "enriched_account_view"
            account["page_number"] = page_number
            all_accounts.append(account)
        
        helper.log_info(f"Fetched {len(accounts)} accounts from page {page_number + 1}")
    
    return all_accounts

def create_enriched_accounts_params(page_size, page_number, after):
    """Create request parameters for enriched accounts."""
    params = {
        "getTableRequest": {
            "tableName": "enriched_account_view",
            "pageSize": page_size,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python reco_collector_accounts.py <tenant_url> <api_key>")
        sys.exit(1)
    ...
    class TestHelper:
        ...
        def get_input_type(self):
            return "reco_enriched_accounts"
            
        def get_sourcetype(self):
            return "reco:enriched:accounts"
    ...
    print("Starting enriched accounts collection test...")
