...
from spurlib.secrets import _get_session_key

MMDB_REFRESH_ENDPOINT = "/spur_mmdb_refresh"
MMDB_MAX_AGE_SECONDS = 24 * 60 * 60
...
# Derive app directory from script location (two levels up from bin/spuriplocation.py)
APP_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
MMDB_PATH = os.path.join(APP_DIR, 'local', 'data', 'mmdb', 'ipgeo.mmdb')
...
def download_mmdb_if_needed(ctx, logger, mmdb_path):
    """
    Refresh the MMDB file if it is missing or older than one day (matches the
    Spur ipgeo feed's daily regeneration cadence). The refresh is performed by
    the privileged `/spur_mmdb_refresh` REST handler under system auth, so
    non-admin callers can trigger it without holding `list_storage_passwords`.
    Credentials never leave splunkd.
    """
    if os.path.exists(mmdb_path):
        file_age = time.time() - os.path.getmtime(mmdb_path)
        if file_age < MMDB_MAX_AGE_SECONDS:
            logger.debug("MMDB file exists and is recent, using existing file")
            return
        logger.debug("MMDB file exists but is stale (age=%ss), requesting refresh", int(file_age))

    session_key = _get_session_key(ctx)
    if not session_key:
        raise ValueError("No Splunk session key available; cannot request MMDB refresh.")

    logger.info("Requesting MMDB refresh from %s", MMDB_REFRESH_ENDPOINT)
    # raiseAllErrors=False so we can read the handler's JSON error body on
    # 4xx/5xx and surface it to the user instead of a generic HTTP status.
    try:
        response, content = rest.simpleRequest(
            MMDB_REFRESH_ENDPOINT,
            sessionKey=session_key,
            postargs={"feed_type": "ipgeo"},
            raiseAllErrors=False,
        )
    except Exception as e:
        raise ValueError(
            "Failed to refresh Spur MMDB file: %s. Please check your network connectivity." % e
        )
    ...
    if result is None:
        snippet = (content or "").strip()[:300] or "<empty>"
        raise ValueError(
            "Failed to refresh Spur MMDB file (HTTP %s): non-JSON response: %s" % (status, snippet)
        )

    if result.get("error"):
        raise ValueError("Failed to refresh Spur MMDB file: %s" % result["error"])

    if status and int(status) >= 400:
        raise ValueError("Failed to refresh Spur MMDB file: HTTP %s" % status)

    if not os.path.exists(mmdb_path):
        raise ValueError(
            "Spur MMDB refresh reported success but file is missing at %s" % mmdb_path
        )

    logger.info(
        "MMDB refresh complete (refreshed=%s) at %s",
