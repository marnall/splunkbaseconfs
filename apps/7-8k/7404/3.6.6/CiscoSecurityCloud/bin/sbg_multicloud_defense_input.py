...
from CiscoSecurityCloud.multicloud_defense.event_logger import MCDEventLogger
...
class CiscoMulticloudDefenseInput(smi.Script):
    """A base class that extends :class:`smi.Script`
    for implementing modular input Multicloud Defense.
    ...
    def get_scheme(self) -> smi.Scheme:
        ...
        """
        scheme = smi.Scheme("Cisco Multicloud Defense Input")
        scheme.description = "Cisco Multicloud Defense Input"
    ...
    def stream_events(
        ...
        for input_name, input_item in inputs.inputs.items():
            ...
            try:
                LOGGER.info(f"Start MCD input: {normalized_input_name}")
                ...
                LOGGER.info(f"Finish MCD input: {normalized_input_name}")
            except Exception as e:
                LOGGER.info(f"Exception during executing MCD input: {e}")
                pass


if __name__ == "__main__":
    exit_code = CiscoMulticloudDefenseInput().run(sys.argv)  # pragma: no cover
