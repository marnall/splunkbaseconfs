...
from CiscoSecurityCloud.ai_defense.event_logger import AIEventLogger
...
class SBGAIDefenseInput(smi.Script):
    """A base class that extends :class:`smi.Script`
    for implementing modular input AI Defense.
    ...
    def get_scheme(self) -> smi.Scheme:
        ...
        """
        scheme = smi.Scheme("Cisco AI Defense Input")
        scheme.description = "Cisco AI Defense Input"
        ...
        scheme.add_argument(
            smi.Argument(
                "promote_to_notables",
                title="Promote AI Defense Events to ES Notables",
                required_on_create=False,
            )
        )
        return scheme
    ...
    def stream_events(
        ...
        for input_name, input_item in inputs.inputs.items():
            ...
            try:
                LOGGER.info(f"Start AI Defense input: {normalized_input_name}")
                ...
                LOGGER.info(f"Finish AI Defense input: {normalized_input_name}")
            except Exception as e:
                LOGGER.info(f"Exception during executing AI Defense input: {e}")
                pass


if __name__ == "__main__":
    exit_code = SBGAIDefenseInput().run(sys.argv)  # pragma: no cover
