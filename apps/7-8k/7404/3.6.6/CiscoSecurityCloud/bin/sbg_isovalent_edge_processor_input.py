...
from isovalent.event_logger import IsovalentEventLogger
...
class SBGIsovalentEdgeProcessorInput(smi.Script):
    """A base class that extends :class:`smi.Script`
    for implementing modular input Isovalent Runtime Security.

    Methods ``get_scheme`` and ``stream_events``.
    """
    def __init__(self):
        super(SBGIsovalentEdgeProcessorInput, self).__init__()
    ...
    def get_scheme(self) -> smi.Scheme:
        ...
        """
        scheme = smi.Scheme("sbg_isovalent_edge_processor_input")
        scheme.description = "Isovalent Runtime Security"
    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter) -> None:
        ...
        for input_name, input_item in inputs.inputs.items():
            ...
            try:
                LOGGER.info(
                    f"Start Isovalent Runtime Security input: {normalized_input_name}"
                )
                session_key = self._input_definition.metadata["session_key"]

                IsovalentEventLogger(
                    normalized_input_name, input_item, session_key
                ).log_events()

                LOGGER.info(
                    f"Finish Isovalent Runtime Security input: {normalized_input_name}"
                )
            except Exception as e:
                LOGGER.info(
                    f"Exception during executing Isovalent Runtime Security input: {e}"
                )
                pass


if __name__ == "__main__":
    exit_code = SBGIsovalentEdgeProcessorInput().run(sys.argv)  # pragma: no cover
