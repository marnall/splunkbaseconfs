...
class SBGFWSyslogInput(smi.Script):
    def __init__(self):
        super().__init__()

    def get_scheme(self):
        scheme = smi.Scheme("SBG Secure Firewall Syslog Input")
        scheme.description = "Get log data from the Secure Firewall Syslog"
    ...
    def stream_events(self, inputs: smi.InputDefinition, event_writer: smi.EventWriter):
        for input_name, input_item in inputs.inputs.items():
            ...
            try:
                LOGGER.info(f"Start SFW_Syslog input: {normalized_input_name}")
                ...
                LOGGER.info(f"Finish SFW_Syslog input: {normalized_input_name}")

            except Exception as e:
                LOGGER.info(f"Exception during executing SFW_Syslog input: {e}")
                pass


if __name__ == "__main__":
    exit_code = SBGFWSyslogInput().run(sys.argv)  # pragma: no cover
