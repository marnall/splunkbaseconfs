...
from utils import (
    ...
    load_checkpoint,
    save_checkpoint,
    build_compromised_credentials_url,
)

# ---- Constants ----
COMPROMISED_CREDENTIALS_SOURCETYPE = "usta:account_takeover_prevention:compromised_credentials"
...
def save_to_splunk(
    ...
    for each in results:
        ...
        event_writer.write_event(
            smi.Event(
                ...
                sourcetype=COMPROMISED_CREDENTIALS_SOURCETYPE,
                source=COMPROMISED_CREDENTIALS_SOURCETYPE,
            )
        )
        ingested += 1
    ...
    log.events_ingested(
        logger,
        input_name,
        COMPROMISED_CREDENTIALS_SOURCETYPE,
        ingested,
        input_item.get("index"),
        account=input_item.get("account"),
    )
    logger.info(f"{COMPROMISED_CREDENTIALS_SOURCETYPE} - ingested: {ingested}")
    ...
    for input_name, input_item in inputs.inputs.items():
        ...
        try:
            ...
            if last_visited:
                ...
                logger.info(
                    f"No checkpoint found, fetching from last {MAX_HISTORICAL_DAYS} days ({created_at_after})"
                )

            url = build_compromised_credentials_url(created_at_after=created_at_after)

            while url:
                logger.info(f"Fetching compromised credentials from {url}")
                ...
                if not results:
                    logger.info("No new compromised credentials to ingest.")
                    break
