...
DEBUG_ENABLED = True
APPROVE_CAPABILITY = "falconer_allowlist_approve"
...
class AllowlistManage(PersistentServerConnectionApplication):
    def __init__(self, command_line, command_arg):
        super(AllowlistManage, self).__init__()
    ...
    def log(self, msg: str):
        ...
        try:
            ...
            with open(DEBUG_PATH, "a", encoding="utf-8") as f:
                f.write(f"[{ts}] [allowlist_manage] {msg}\n")
        except Exception:
            pass
    ...
    def _user_has_capability(self, session_key, capability):
        try:
            ...
            for entry in entries:
                caps = entry.get("content", {}).get("capabilities") or []
    ...
    def handle(self, in_string):
        try:
            ...
            if requested_status and requested_status in ("enabled", "disabled") and requested_status != current_status:
                if not self._user_has_capability(session_key, APPROVE_CAPABILITY):
                    raise ValueError("Approving suppressions is restricted")
