...
class PROXMOX_RESOURCES(smi.Script):

    def __init__(self):
        super(PROXMOX_RESOURCES, self).__init__()

    def get_scheme(self):
        scheme = smi.Scheme('proxmox_resources')
        scheme.description = 'Resources'
    ...
    def stream_events(self, inputs, ew):
        ...
        url = f'https://{domain_ip}/api2/json/cluster/resources'
        ...
        for i in r1["data"]:
            count += 1
            i.update({"eventNumber": count})
            Resources_list.append(i)
        if Resources_list != []:
            event = smi.Event(data=json.dumps(Resources_list, indent=2), index=input_item.get("index"), sourcetype="proxmox_resources")
...
if __name__ == '__main__':
    exit_code = PROXMOX_RESOURCES().run(sys.argv)
    sys.exit(exit_code)
