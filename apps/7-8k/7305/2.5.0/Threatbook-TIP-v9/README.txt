...
2、 Users need to configure the following information in the configuration:
    1. TIP related configurations
        ...
        1.3 Example of IP Reputation Check Filling
            ...
            Intelligence query interface: http://192.168.100.89:8090/tip_api/v4/ip
            APIKEY: 8b7c91d8c5c84202bcc94fbcc4e8d72b (TIP Platform Other ->Business Settings ->KEY)
        
        1.4 Example of filling out collapse detection
            Index name (input): test_in (indicating to read data from this index)
            Index name (output): testouts (indicating the output of collision intelligence data to this index)
            Intelligence query interface: http://192.168.100.89:8090/tip_api/v4/dns
            APIKEY: 8b7c91d8c5c84202bcc94fbcc4e8d72b (TIP Platform Other ->Business Settings ->KEY)
...
二、使用者需要在配置中，配置以下信息:
    1.TIP的相关配置
        ...
        1.3 IP信誉检测填写样例
            ...
            情报查询接口： http://192.168.100.89:8090/tip_api/v4/ip
            APIKEY：8b7c91d8c5c84202bcc94fbcc4e8d72b(TIP 平台 其他->业务设置->KEY)
        
         1.4 失陷检测填写样例
            索引名称（输入）：test_in （表示要从该索引中读取数据）
            索引名称（输出）：test_out（表示碰撞情报后的数据输出到该索引）
            情报查询接口： http://192.168.100.89:8090/tip_api/v4/dns
            APIKEY：8b7c91d8c5c84202bcc94fbcc4e8d72b(TIP 平台 其他->业务设置->KEY)
