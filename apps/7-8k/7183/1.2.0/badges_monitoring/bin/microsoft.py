...
def get_badges(user):
    try:
        url = 'https://learn.microsoft.com/api/profiles/transcript/share/{user}'.format(user=user.get('userid'))
        response = requests.get(url, headers={'Accept': 'application/json',})
        if response.status_code == 200:
            responseData = response.json()
            badgets = responseData.get('certificationData').get('activeCertifications')
            for e in badgets:
                e["user"] = user.get('user')
                e["userid"] = user.get('userid')
                e["username"] = responseData.get('userName')
                print(json.dumps(e))
        else:
...
if __name__ == "__main__":
    ...
    with open(lookup, 'r') as f:
        ...
        for user in users:
            if  user.get('platform').lower() == 'microsoft':
                #print(dict(user))
                get_badges(user)