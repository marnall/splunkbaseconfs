...
MOOG_STANZA_NAME = "AIOps_Incident_Management_Integration"
...
def setup_logging():
    ...
    logging_file_name = "AIOps_Incident_Management_Stream_Integration.log"
...
class MoogStreamingCommand(StreamingCommand):
    ...
    def handle_response(response):
        ...
        :return: None
        """
        # Handle request payload size limitation on AIOps Incident Management
        if response.status_code == 413:
            payload_size_error_msg = "Request rejected by AIOps Incident Management integration URL, " \
        ...
        # Handle other errors
            ...
            raise Exception("An error has occurred while executing HTTP request to AIOps Incident Management. Error code: "
                            "{}".format(str(response)))
    ...
    def decrypt_secrets(self, config):
        ...
        passwords = storage_passwords.list(owner="nobody", app="AIOps_Incident_Management", sharing="app")
        ...
        if self.is_https_url(url):
            if moog_cert:
                if not os.path.exists(moog_cert):
                    raise Exception("AIOps Incident Management certificate file: '{}' not found.".format(moog_cert))
    ...
    def send_req(self, url, api_key, events, verify=False, proxies=None):
        ...
        try:
            ...
            logger.info("Successfully sent {} events to AIOps Incident Management.".format(len(events["events"])))
            logger.info("Response received from AIOps Incident Management: {} with message: {}".format(str(res), str(res.text)))

        except requests.exceptions.SSLError:
            raise Exception(
                "An error has occurred while executing POST request with AIOps Incident Management. Error message: "
    ...
    def stream(self, records):
        """
        Custom command for sending events to AIOps Incident Management
        ...
        if not moog_configuration or moog_configuration['url'] == "":
            raise Exception("AIOps Incident Management configuration is missing. Please setup the AIOps Incident Management App.")
        ...
        if((is_moogsoft_on_prem_edition is None or is_moogsoft_on_prem_edition == '0') and (api_key is None or api_key == '')):
            raise Exception("AIOps Incident Management API Key is required if 'Configure AIOps Incident Management On_Prem Edition' is disabled")
        ...
        for record in records:
            ...
            if request_size_in_bytes >= batch_size_limit_in_bytes and len(events["events"]) > 0:
                ...
                logger.debug("Sending the following event data to AIOps Incident Management with total events : {}"
                ...
                # Update to total bytes sent to the AIOps Incident Management
                total_bytes_sent = total_bytes_sent + previous_request_size_in_bytes
                # create new empty batch
                events = {"events": []}
                # Increment number of batch of events send to the AIOps Incident Management
        ...
        if total_events_per_batch > 0:
            logger.debug(
                "Sending following event data to AIOps Incident Management with total events : {}, request payload size in "
        ...
        logger.info("AIOps Incident Management streaming command execution completed successfully")

    def execute(self, rest_url, api_key, events, moog_cert, proxies=None):
        """
        Send data to AIOps Incident Management
        :param rest_url: Endpoint configured in setup page
        :param events:  Events of Splunk
        :param moog_cert: AIOps Incident Management certificate configured in setup page
        :return: None (Instead send data to AIOps Incident Management)
        """
