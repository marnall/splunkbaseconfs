...
logger = setup_logger(
    logging.INFO,
    'sp6_poam_delete_attachment_rest_endpoint'
)



class AttachmentDelete(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        error = None
        error_message = 'An error occurred while attempting to delete the file.'
        logger.info(f'message="Deleting file from local evidence."')


        for attachment in attachments:
            dir_path = Path(attachment).parent
            ...
            try:
                ...
                error = str(e)
                logger.error(f'status="ERROR", message="An error occurred while attempting to delete the file: {error}"')

            else:
                logger.info(f'status="success", message="Successfully deleted file from local evidence."')

                poam_record['attachments'].remove(attachment)
