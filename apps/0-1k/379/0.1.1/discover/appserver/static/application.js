...
/**
 *   Updates the given lookup with the given field values. Executes the given callback when finished.
 */
function updateLookup(lookupName, fields, onSuccess) {
    var evalPart = [];
    var fieldsPart = ["fields"];
    var statsPart  = ["stats"];
    
    for (var i=0,len=fields.length; i<len; i++) {
        evalPart.push('eval ' + fields[i][0] + '="' + escapeQuotes(fields[i][1]) + '" | ' );
        fieldsPart.push(fields[i][0]);
        
    }
    statsPart.push(sprintf("last(*) as *", fields[fields.length-1][0], fields[fields.length-1][0]));
    statsPart.push("by " + fields[0][0] + " | ");
    
        
    var searchStr = ['| inputlookup ' + lookupName + ' | append [ stats count | ']

    searchStr.push(evalPart.join(' '));
    searchStr.push('fields - _* | ');
    searchStr.push(fieldsPart.join(' '));
    searchStr.push("] | ")
    searchStr.push(statsPart.join(' '));
    searchStr.push('outputlookup ' + lookupName);

    var search = new Splunk.Search(searchStr.join(""));

    if (!onSuccess) {
        onSuccess = function() {
            var logger = Splunk.Logger.getLogger("application.js");
            logger.debug("modified lookup");
        }
    }
    search.dispatchJob(onSuccess);
}

/**
 * Update the nav lookup to keep state of when anyone last used each view.
 */
$(document).ready(function() {
    // we dont care about the 'step 2' pages for any tuners or discoverers
    // because they each have their own state-management that is more fine-grained
    // than just 'when was it last viewed'
    if (Splunk.util.getCurrentView().indexOf("_2")==-1) {
        updateLookup(
            "nav_discover", 
            [
...
/**
 * binds an event handler such that when the given module's job completes, it will 
 * update the given lookup. Within that lookup it will modify the row for the 
 * current sourcetype (from querystring) and set the 'count' field in that row 
 * to the resultCount of that job.
 */
function updateStatusOnJobDone(lookup, moduleId) {
    // listening for the main job to finish so we update the lookup.
    $(document).bind('jobDone', function(event, doneJob) {
        var m = Splunk.Globals.ModuleLoader;
        var module = m.getModuleInstanceById(moduleId);
        if (!module.baseContext || !module.baseContext.has("search")) return false;

        // the simpler call to getContext probably works but there's an outside 
        // chance that cleaning it up would cause some regression somewhere. 
        //var ourJob = module.getContext().get("search").job;
        var ourJob = module.baseContext.get("search").job;

        if (ourJob.getSearchId() == doneJob.getSearchId()) {
            // appending to the lookup
            // #1 get the sourcetype
            var search = module.getContext().get("search");
            // if the url has a sourcetype in it, and the same sourcetype seems to be in the search
