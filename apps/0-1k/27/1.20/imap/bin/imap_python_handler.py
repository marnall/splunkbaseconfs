...
class IMAPHandler(admin.MConfigHandler):
  ...
  def setup(self):
    """
    set the reqd / optional arguments for the diff actions
    """ 
  ...
  def handleList(self, confInfo):
    """
    lists out all the configs in imap.config
    """
  ...
  def handleRemove(self, confInfo):
    """
    removes any config item from imap.config
    """
  ...
  def handleEdit(self, confInfo):
    """
    edits a config item from imap.config
    """
    # let's make sure this thing exists, first...
    existing = admin.ConfigInfo()
    self.handleList(existing)
