...
#							 Removed the hard-coded path separator '/', instead used os.path.join so it will work even if run on a non *nix platform
#							 Imported traceback module for improved error reporting
#							 Got rid of the splunk CLI interface to determine the last UID and used the splunk REST API instead
...
configOptions = [ 
  ...
  "splunkxpassword",   # splunk server encrypted password	
  "splunkHostPath",    # splunk server host path
  "deleteWhenDone",		# delete messages after indexing
  "body_separator",		# what if any text to put between header and body
  "timeout"			   # upper bound for the REST call to return a result (in seconds)
]
...
class IMAPProcessor(object):
	...
	"""
	Imapprocessor will download mail form an imap server and spit to sdtout.
	this is used by the bundle feature in Splunk to get mail into splunk
	"""
	...
	def __init__(self):
		...
		self.folders = ""
		self.fullHeaders = True
		self.includeBody = True
		self.noCache = False
		self.debug = False
		self.useSSL = False
		self.port = 143
		self.imapSearch = "(UNDELETED SMALLER 204800)"
		self.mimeTypes = "text/plain"
		self.splunkuser = 'admin'
		self.splunkpassword = 'changeme'
		#If supplied, splunkxpassword will take precedence over splunkpassword
	...
	def usage(self):
		...
		logging.debug("eg:")
		logging.debug("python getimap.py --server=<mail server name> --user=<user name> --password=<unencrypted password> OR")
		logging.debug("python getimap.py --server=<mail server name> --user=<user name> --xpassword=<encrypted password>")
		...
		try:
		  ...
		  if resCode == 'NO':
			...
			if self.debug:
				logging.debug(e)		 
				print traceback.print_exc(file=sys.stderr)		
			logging.error("ERROR - trying to select mailgox")
	...
	def getAllMail(self, M, box, endid):
		...
		try:
			...
			if self.debug:
				print traceback.print_exc(file=sys.stderr)		
			logging.error("ERROR - trying to search mailgox")
	...
	def fetchMessage( self, M, box, num , dstr):
		...
		try:
			...
			if self.includeBody:
				...
				if self.useBodySourceType:
				   # hardcoded the changing of sourcetype to mailbody. 
					# customers can change the procssing of mailbody's differntly in props.conf
					cstr.write("EndIMAPHeader" + '\n')
					cstr.write("***splunk*** sourcetype=" + self.bodySourceType + '\n')
			...
			logging.debug("1. Failed to get and print message iwth UID " + num )
			if self.debug:
				logging.debug(e)
				print traceback.print_exc(file=sys.stderr)
			logging.debug("2. Failed to get and print message iwth UID " + num )
...
def parseArgs():
	...
	optlist = None
	try:
	  optlist, args = getopt.getopt(sys.argv[1:], '?',['version', 'server=','user=', 'password=', 'xpassword=', 'port=', 'folders=', 'imapSearch=', 'fullHeaders=', 'includeBody=', 'mimeTypes=', 'splunkuser=', 'splunkpassword=', 'splunkxpassword=', 'splunkHostPath=', 'timeout=', 'noCache', 'debug'])
	  if 'version' in args:
		print "VEERSION =",  str(imapProc.version)
		return
