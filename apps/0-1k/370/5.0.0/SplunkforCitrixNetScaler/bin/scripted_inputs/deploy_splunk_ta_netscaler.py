...
INSTALLER_LOG_FILENAME = os.path.join(SPLUNK_HOME,'var','log','splunk','netscaler_installer.log')
STREAMFWD_LOG_FILENAME = os.path.join(SPLUNK_HOME,'var','log','splunk','netscalerfwd.log')
logger = logging.getLogger('netscaler_installer')
...
APP_NAME = 'SplunkforCitrixNetScaler'
...
STREAM_PATH = 'en-us/custom/SplunkforCitrixNetscaler/'
DEPENDENCIES = ['Splunk_TA_Citrix-NetScaler']
...
def create_inputs(appdir, disabled):
    ...
    fo.write( "# A separate IPFIX addon is needed in order for the following stanza to work.  http://apps.splunk.com/app/1801/\n")
    fo.write( "[ipfix://NetScaler_AppFlow]\n")
    fo.write( "sourcetype = appflow\n")
    fo.write( "index = netscaler\n")
    fo.write( "address = 0.0.0.0\n")
    fo.write( "port = 4739\n")
    fo.write( "buffer = 1048576\n")
    fo.write( "disabled = %d\n" % disabled)
...
def install_dependency(dep):
    ...
    try:
        ...
        if (dep == "Splunk_TA_Citrix-NetScaler"):
            create_inputs(dst, 0)
...
if __name__ == '__main__':
    ...
    logger.info("Splunk App for Citrix NetScaler Dependency Manager: Starting...")
    ...
    logger.info("Splunk App for Citrix NetScaler Dependency Manager: Exiting...") 
