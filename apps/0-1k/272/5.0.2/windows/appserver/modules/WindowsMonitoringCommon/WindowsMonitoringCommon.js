var WindowsMonitoring = {
		...
		*/
		...
		if (search) {
			if (token == "search") {
				...
				console.log("WARNING: WindowsMonitoring was asked to find value for reserved token that is unreserved, empty string will be sub'ed for token:" + token);
				return "";
			}
		}
		console.log("WARNING: WindowsMonitoring was asked to find value for reserved token with no search in context, it will be replaced with empty string, token:" + token);
		return "";
	},
	...
	*/
	...
	getJobProgress: function(key) {
		...
		*/
		if (this.jobRegistry.hasOwnProperty(key)) {
			...
			console.log("[WindowsMonitoring GLOBAL] ERROR: progress was requested on a key that is not registered with WindowsMonitoring, key:" + key);
			return 0.0;
		}
	},
	...
	getSIDForKey: function(key) {
		...
		*/
		if (this.jobRegistry.hasOwnProperty(key)) {
			...
			//Log Error return 0.0
			console.log("[WindowsMonitoring GLOBAL] ERROR: sid was requested on a key that is not registered with WindowsMonitoring, key:" + key);
			return false;
		}
	},
	...
	lookupSavedSearch: function(name, nameSpace, context) {
		...
		if ( ajaxResponse == "success" && targetArtifact.length > 0 ) {
			if (targetArtifact[0].hasOwnProperty('search')) {
				...
				searchString = WindowsMonitoring.replaceVariables(searchString, context);
			} else {
	...
	/**
	*hello I'm splunk applications here with WindowsMonitoringCommon!
	...
	*YOU NEED WindowsMonitoringCOMMON!
	*now with WindowsMonitoring.roundNumber you can get a number back rounded to any number of decimal places you want!
	*but wait I always want it to two places...
	*LOOK NO FURTHER, WindowsMonitoring.roundNumber will default to 2 places if you don't tell it how to round just type:
	*roundedNumber = WindowsMonitoring.roundNumber(num); 
	...
	*WindowsMonitoring.isNum! It always works in over 30 test cases of bad number coercion from
	*competitive brands!
	*/
	isNum: function(n) {
	...
	keyListener: function(event) {
		...
		{
			if (WindowsMonitoring.anubisWindow === "" || WindowsMonitoring.anubisWindow.closed || WindowsMonitoring.anubisWindow.name == undefined) {
				WindowsMonitoring.anubisWindow = window.open("/custom/SA-Utils/anubis_service/" + Splunk.util.getCurrentApp() + "/show?view=" + Splunk.util.getCurrentView(),"subWindow","HEIGHT=775,WIDTH=1400");
			}
			else {
				WindowsMonitoring.anubisWindow.focus();
			}
		}
	}
};
//Event Bindings for WindowsMonitoring Utilities
//Enable hotkeys
$(document).keydown( WindowsMonitoring.keyListener );

Splunk.Module.WindowsMonitoringCommon = $.klass(Splunk.Module, {
	initialize: function($super, container) {
		$super(container);
		console.log("loaded common tools into var WindowsMonitoring");
	...
	getModifiedContext: function (context) {
		...
		for (var name in this.qsVars) {
			WindowsMonitoring.storeVariable(name, null, this.qsVars[name], context);
		}
		return context;
	}
});
