...
logger = logging.getLogger('splunk.module.WindowsMonitoring_FTR')
...
from windowsmonitoring.models.windows import Windows 

class WindowsMonitoring_FTR(module.ModuleHandler):
    ''' 
    checks to see if app is configured or if user ignored last call to action 
    also handles setting the ignored bit in user's windows.conf
    '''
    ...
    def generateResults(self, **kwargs):
        ...
        legacy_handler = os.path.join(APPS_DIR, app_name, 'bin', 'setuphandler.py')
        legacy_setup = os.path.join(APPS_DIR, app_name, 'default', 'setup.xml')
                                    
        for legacy in [legacy_handler, legacy_setup]:
        ...
        try:
            a = Windows.get(Windows.build_id(user, app_name, user))
        except:
            a = Windows(app_name, user, user)
    ...
    def is_app_admin(self, app, user):
        ...
        user_roles = auth.getUser(name=user)['roles']

        if 'admin' in user_roles:
            return True

        for role in user_roles:
            for sub_role in auth.getRole(name=role)['imported_roles']:
                if 'admin' in sub_role:
                    return True
