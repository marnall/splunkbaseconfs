...
Splunk.Module.WindowsMonitoringDropdown = $.klass(Splunk.Module.DispatchingModule, {
	initialize: function($super, container) {
        ...
        this.useWildcards = Splunk.util.normalizeBoolean(this.getParam("useWildcards"));
        ...
        if (this.topLabel) {
            $(this.container).addClass('WindowsMonitoringDropdownTopLabel');
        }

		$(".WindowsMonitoringDropdownLabel",$(this.container)).html(this.label);
		//Handle soln namespace if defined and pull the previously used value
		if (this.solnns) {
			this.stickyValue = WindowsMonitoring.pullSelection(this.solnns);
		}
		else {
                        ...
                        .autocomplete({
                            ...
                            change: function( event, ui ) {
                                if ( !ui.item ) {
                                    ...
                                    if ( !valid && !$(this).attr('data-use-wildcards') ) {
	...
	renderResults: function(jsonRsp) {
		...
		for (var ii=0; ii<data.length; ii++) {
			var row = data[ii];
			var label = row[this.displayField];
			var values = [];
			//values.push(label); don't need the label value in the value array
			for (var jj=0; jj<this.valueFields.length; jj++) {
		...
		if (this.firstLoad && this.solnns && WindowsMonitoring.getVariableValue(this.solnns, context) && $("option[soln-ns=" + WindowsMonitoring.escapeStringForSelector(WindowsMonitoring.getVariableValue(this.solnns, context)) + "]",this.cbElement).length) {
			//Set to the value in the context if it exists and this is the first load
			$("input",this.cbElement.parent()).val($("option[soln-ns=" + WindowsMonitoring.escapeStringForSelector(WindowsMonitoring.getVariableValue(this.solnns, context)) + "]",this.cbElement).text());
			this.cbElement.val($("option[soln-ns=" + WindowsMonitoring.escapeStringForSelector(WindowsMonitoring.getVariableValue(this.solnns, context)) + "]",this.cbElement).val());
		}
		else if (this.stickyValue && $("option[soln-ns=" + WindowsMonitoring.escapeStringForSelector(this.stickyValue) + "]",this.cbElement).length) {
			//Set the value to the previously selected if it exists
			$("input",this.cbElement.parent()).val($("option[soln-ns=" + WindowsMonitoring.escapeStringForSelector(this.stickyValue) + "]",this.cbElement).text());
			this.cbElement.val($("option[soln-ns=" + WindowsMonitoring.escapeStringForSelector(this.stickyValue) + "]",this.cbElement).val());
		}
		else {
        ...
        if (this.useWildcards) {
            var myTextBox = $('input', this.cbElement.parent());
            myTextBox.attr('data-use-wildcards','1');
            myTextBox.keypress(function(e) {
                if (e.which === 13) {
                    that._enterKeyInitiated = true;
                    that.onChange();
                }
            });
        }
	...
	getModifiedContext: function() {
		...
		if (this.doneUpstream) {
            ...
            if (this.useWildcards && this._enterKeyInitiated) {
                var inputText = $('input', this.cbElement.parent()).val();
                values = [this.displayField, $.trim(inputText) === 'All' ? '*' : inputText]; // NB: this is hacky. Assumes "All" is a static item
                this._enterKeyInitiated = false;
            }
			for (ii=0; ii<values.length; ii+=2) {
				valueName = values[ii];
				value = values[ii+1];
				context = WindowsMonitoring.storeVariable(displayName,valueName,value, context);
			}
			//Handle the sticky value
			this.stickyValue = values[1];
			if (this.solnns) {
				//If a varname is defined place the selection in local storage
				WindowsMonitoring.stickSelection(this.solnns, this.stickyValue);
				context = WindowsMonitoring.storeVariable(this.solnns, null, this.stickyValue, context);
			}
			return context;
		}
		else {
